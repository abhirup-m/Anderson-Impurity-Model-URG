...
\begin{gather}
	...
	\ket{\Psi}_1 = c_\pm^s \frac{1}{\sqrt 2}\left(\ket{\uparrow, \downarrow} - \ket{\downarrow, \uparrow}\right) + c^c_\pm \frac{1}{\sqrt 2}\left(\ket{\uparrow\downarrow, 0} + \ket{0, \uparrow\downarrow}\right), \quad E_1 =  -V^*\sqrt{\gamma^2 + 4} -\frac{1}{4}U^* - \frac{3}{8}{J^*}
\end{gather}
...
We now look at each \(\ket{\Phi_i}\) separately. For \(\ket{\Phi_i} = \ket{\Psi}_1 \otimes \ket{0,0}\), we can write
\begin{equation}\begin{aligned}
...
\end{aligned}\end{equation}
The first relation gives \(v_{in} = \bra{\Phi}_i v \ket{\Psi}_n = \left(\bra{\Psi}_n v^\dagger \ket{\Phi}_i\right)^* = 0\). The set of states that give non-zero \(v^\dagger_{in}\) is
\begin{equation}\begin{aligned}
	\ket{\Psi}_n = \ket{\Psi}_\pm^\sigma = -\frac{1}{\mathcal{N}_\pm}\left[4V\ket{\sigma,0} + 2\left(\frac{U}{2} \pm \Delta\right) \ket{0,\sigma}\right]\otimes\ket{\overline\sigma},~\sigma = \uparrow,\downarrow, ~ E_\pm^\sigma = -\frac{U}{4} \pm \frac{1}{2}\Delta~,
\end{aligned}\end{equation}
such that  \(v_{\pm\sigma,i} = \braket{\Psi^\sigma_\pm | v | \Phi_i} = \frac{\sigma t}{\mathcal{N}_\pm\sqrt 2} \left[4Vc_s + \left(U \pm 2\Delta\right) c_c\right] \). \(\mathcal{N}_\pm\) is the normalisation factor and \(\Delta = \sqrt{\left(\frac{U}{2}\right)^2 + 4V^2}\). The renormalisation for this value of \(i\) then becomes
\begin{equation}\begin{aligned}
	\ket{\Phi}_0 \bra{\Phi}_0 \sum_{n}\frac{|v_{ni}|^2}{-\delta E_{n}} = \ket{\Phi}_0 \bra{\Phi}_0 \sum_{\sigma,\pm}\frac{|v_{\pm\sigma,i}|^2}{-\delta E_{\pm}^\sigma} = \ket{\Phi}_0 \bra{\Phi}_0 t^2\sum_{\pm}\frac{1}{\mathcal{N}_\pm^2}\frac{\left[4Vc_s + \left(U \pm 2\Delta\right) c_c\right]^2}{\delta E_{\pm}^\sigma}
\end{aligned}\end{equation}
where \(\delta E_{\pm}^\sigma = E_\text{gs} - E^\sigma_{\pm}\). For \(i=1\), we get
\begin{equation}\begin{aligned}
\ket{\Phi}_1 \bra{\Phi}_1 t^2\sum_{\pm}\frac{1}{\mathcal{N}_\pm^2}\frac{\left(4Vc_s\right)^2 + \left(U \pm 2\Delta\right)^2 c_c^2}{\delta E_{\pm}^\sigma}
\end{aligned}\end{equation}

The total Hamiltonian \(H_0^* + H_X\) is invariant under the transformations \(c^\dagger_\sigma \to c_\sigma, t \to -t\). Since the renormalisations only involve even powers of \(t\), we can conclude that the renormalisation for \(i=2\)  and \(i=3\) will be the same as that of \(i=1\) and \(i=0\) respectively. The total renormalisation at second order is
\begin{equation}\begin{aligned}
	\Delta H = \sum_{i=0}^3\ket{\Phi}_i \bra{\Phi}_i t^2\sum_{\pm}\frac{1}{\mathcal{N}_\pm^2}\frac{\left(4Vc_s\right)^2 + \left(U \pm 2\Delta\right)^2 c_c^2}{\delta E_{\pm}^\sigma} + \left(\ket{\Phi}_0 \bra{\Phi}_0 + \ket{\Phi}_3 \bra{\Phi}_3\right)\sum_{\pm}\frac{t^2}{\mathcal{N}_\pm^2}\frac{8V c_s \left(U \pm 2\Delta\right) c_c}{\delta E_{\pm}^\sigma}
\end{aligned}\end{equation}
