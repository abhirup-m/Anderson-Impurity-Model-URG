...
\chapter{Preliminaries}\label{section1}
...
\end{aligned}\end{equation}
Since we are working with a spin eigenstate, \(\hat n_{k\sigma}\hat n_{k^\prime\sigma}\) will be 0. The quasiparticle energy then simplifies to
\begin{equation}\begin{aligned}
	\epsilon_{k\sigma} = \epsilon_{k}^0 + \sum_{k}f_{kk^\prime}\hat n_{k^\prime\overline\sigma}
\end{aligned}\end{equation}
Due to the absence of a spin-parallel term, we can write
\begin{equation}\begin{aligned}
	\label{rel_landau}
...
\begin{figure}[htpb]
	...
	\label{Fkkqq}
\end{figure}
\section{Calculation of Wilson ratio from effective Fermi liquid Hamiltonian}
At \(T=0\), we can ignore the two-particle scattering interaction and focus on the Fermi liquid part of the effective Hamiltonian. The Wilson ratio for a general Fermi liquid is given, up to leading order in the Landau parameter \(F_0^a\), as
\begin{equation}\begin{aligned}
	R = \frac{1}{1 + F_0^a} \approx 1 - F_0^a
\end{aligned}\end{equation}
while the charge susceptibility is 
\begin{equation}\begin{aligned}
	\chi_c = \frac{\rho(\epsilon_F)}{1 + F_0^s} \approx \rho\left(\epsilon_F\right) \left(1 - F_0^s\right)
\end{aligned}\end{equation}
These Landau parameters are defined as follows:
\begin{equation}\begin{aligned}
