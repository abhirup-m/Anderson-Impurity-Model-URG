...
\begin{figure}[tbh!]
\begin{center}
	\includegraphics[scale=0.54]{../stargraph_num/notzero1.png}
	\hspace*{\fill}
	\includegraphics[scale=0.54]{../stargraph_num/notzero2.png}
...
\begin{figure}[tbh!]
\begin{center}
	\includegraphics[scale=0.54]{../stargraph_num/zero2.png}
	\hspace*{\fill}
	\includegraphics[scale=0.54]{../stargraph_num/zero1.png}
...
\begin{equation}\begin{aligned}
	\label{kondosym}
	\Delta J = - J^2 \frac{1}{\omega - \frac{1}{2}\epsilon_q+ \frac{1}{2}J}
\end{aligned}\end{equation}
...
\begin{center}
	\includegraphics[scale=0.55]{../kondo_num/rel.png}
	\hspace*{\fill}
	\includegraphics[scale=0.55]{../kondo_num/rel2.png}
...
\begin{equation}\begin{aligned}
\label{newh}
\left[c^\dagger_{q\beta}T, \eta\right]
\end{aligned}\end{equation}
We will be decoupling an electron \(q\beta\) at the energy shell \(\epsilon_q\). The diagonal part (that comes down in the denominator) is
\begin{equation}\begin{aligned}
	\label{term1}
...
\comm{
	\pb In order to evaluate the remaining operator in the denominator (\(d\ol\beta\)), we will need to split the off-diagonal term into two parts:
\begin{equation}\begin{aligned}
V_q^* c^\dagger_{d\beta}c_{q\beta} = V_q^* c^\dagger_{d\beta}c_{q\beta}\left[ \hat n_{d\ol\beta} + 1 - \hat n_{d\ol\beta} \right] = V_q^* c^\dagger_{d\beta}c_{q\beta}\hat n_{d\ol\beta} + V_q^* c^\dagger_{d\beta}c_{q\beta}\left(1 - \hat n_{d\ol\beta}\right)
\end{aligned}\end{equation}
This gives
\begin{equation}\begin{aligned}
	\eta &= \frac{1}{\omega + \frac{1}{2}\epsilon_q - \epsilon_d - 2\left(\epsilon_d + U\right) \tau_{d\ol\beta}}\left[V_q^* c^\dagger_{d\beta}c_{q\beta}\hat n_{d\ol\beta} + V_q^* c^\dagger_{d\beta}c_{q\beta}\left(1 - \hat n_{d\ol\beta}\right)\right]\\
     &= \frac{1}{\omega_1 + \frac{1}{2}\epsilon_q - 2\epsilon_d - U }V_q^* c^\dagger_{d\beta}c_{q\beta}\hat n_{d\ol\beta} + \frac{1}{\omega_0 + \frac{1}{2}\epsilon_q + U}V_q^* c^\dagger_{d\beta}c_{q\beta}\left(1 - \hat n_{d\ol\beta}\right)
\end{aligned}\end{equation}
We have allowed for multiple \(\omega\) in the denominator because the two scattering terms act on different states.
}
The renormalization is
\begin{equation}\begin{aligned}
\Delta \ham = c^\dagger_{q\beta} T \eta - \eta c^\dagger_{q\beta}T &= \frac{|V_q|^2}{\omega + \frac{1}{2}\epsilon_q - \epsilon_d - \left(\epsilon_d + U \right)\hat n_{d\ol\beta}} \left[ \left(1 - \hat n_{d\beta}\right)\hat n_{q\beta} - \left(1 - \hat n_{q\beta}\right)\hat n_{d\beta}\right] 
\end{aligned}\end{equation}
\comm{
	To relate \(\omega_1\) and \(\omega_0\), we can use a symmetry of the model. Looking at eq.~\ref{model:siam}, we can say that the Hamiltonian is invariant under the following transformation:
\begin{equation}\begin{aligned}
	\epsilon_k \to -\epsilon_k, && c_k \to c^\dagger_k, && c_d \to -c^\dagger_d, && \epsilon_d \to -\epsilon_d
\end{aligned}\end{equation}
We should then expect the renormalization to also be invariant under such a transformation. The renormalization transformations as follows under the above-mentioned operation:
\begin{equation}
	c^\dagger_{q\beta} T \eta - \eta c^\dagger_{q\beta}T \to \left[\frac{|V_q|^2 \left(1 - \hat n_{d\ol\beta}\right)}{\omega_1 - \frac{1}{2}\epsilon_q + \epsilon_d - U } + \frac{|V_q|^2 \hat n_{d\ol\beta}}{\omega_0 - \frac{1}{2}\epsilon_q  + \epsilon_d + U}\right]\left[\left(1 - \hat n_{q\beta}\right)\hat n_{d\beta} - \left(1 - \hat n_{d\beta}\right)\hat n_{q\beta}\right] 
\end{equation}
Comparing with the original form, we get the following constraint:
\begin{equation}
	\omega_1 + \frac{1}{2}\epsilon_q  - \epsilon_d - U = -\omega_0 + \frac{1}{2}\epsilon_q - \epsilon_d - U \implies \omega_0 = -\omega_1
\end{equation}
Using this, the renormalization at an energy \(\epsilon_q\) becomes
\begin{equation}
\Delta \ham = \left[\frac{|V_q|^2 \hat n_{d\ol\beta}}{\omega_1 + \frac{1}{2}\epsilon_q - \epsilon_d - U } + \frac{|V_q|^2 \left(1 - \hat n_{d\ol\beta}\right)}{-\omega_1 + \frac{1}{2}\epsilon_q  - \epsilon_d + U}\right]\left[ \left(1 - \hat n_{d\beta}\right)\hat n_{q\beta} - \left(1 - \hat n_{q\beta}\right)\hat n_{d\beta}\right] 
\end{equation}
}
Similarly, the renormalization at an energy \(-\epsilon_q\) is obtained simply by replacing \(\epsilon_q \to -\epsilon_q\):
\begin{equation}
\Delta \ham = \frac{|V_q|^2}{\omega^\prime - \frac{1}{2}\epsilon_q - \epsilon_d - \left(\epsilon_d + U \right)\hat n_{d\ol\beta}}\left[ \left(1 - \hat n_{d\beta}\right)\hat n_{q\beta} - \left(1 - \hat n_{q\beta}\right)\hat n_{d\beta}\right]
\end{equation}
...
\begin{equation}\begin{aligned}
	\Delta \ham = -\frac{|V_q|^2  \hat n_{d\beta}}{\omega + \frac{1}{2}\epsilon_q - \epsilon_d - \left(\epsilon_d + U \right)\hat n_{d\ol\beta}} - \frac{|V_q|^2 \left(1 - \hat n_{d\beta}\right)}{-\omega^\prime + \frac{1}{2}\epsilon_q + \epsilon_d + \left(\epsilon_d + U \right)\hat n_{d\ol\beta}}
\end{aligned}\end{equation}
\textit{As an ansatz}, we assume \(\omega = -\omega^\prime\).
\begin{equation}\begin{aligned}
	\label{vanilla:siam:ren}
	\Delta \ham = -\frac{|V_q|^2  \hat n_{d\beta}}{\omega + \frac{1}{2}\epsilon_q - \epsilon_d - \left(\epsilon_d + U \right)\hat n_{d\ol\beta}} - \frac{|V_q|^2 \left(1 - \hat n_{d\beta}\right)}{\omega + \frac{1}{2}\epsilon_q + \epsilon_d + \left(\epsilon_d + U \right)\hat n_{d\ol\beta}}
\end{aligned}\end{equation}
\subsection{Scaling equations}
...
\begin{equation}\begin{aligned}
	\label{urg-siam}
	\Delta E_2 &= -2\sum_{q}\frac{|V_q|^2}{\omega + \frac{1}{2}\epsilon_q - 2\epsilon_d - U }\\
	\Delta E_1 &= -\sum_{q}\frac{|V_q|^2}{\omega - \frac{1}{2}\epsilon_q - \epsilon_d} - \sum_{q}\frac{|V_q|^2}{\omega - \frac{1}{2}\epsilon_q + \epsilon_d}\\
	\Delta E_0 &= -2\sum_{q}\frac{|V_q|^2}{\omega + \frac{1}{2}\epsilon_q}
\end{aligned}\end{equation}
Using the relations \(\epsilon_d = E_1 - E_0\) and \(U = E_2 + E_0 - 2E_1\), we can write
\begin{equation}\begin{aligned}
	\Delta \epsilon_d &= -\sum_{q}\frac{|V_q|^2}{\omega - \frac{1}{2}\epsilon_q - \epsilon_d} - \sum_{q}\frac{|V_q|^2}{\omega - \frac{1}{2}\epsilon_q + \epsilon_d} + 2\sum_{q}\frac{|V_q|^2}{\omega + \frac{1}{2}\epsilon_q}\\
	\Delta U &= -2\sum_{q}\frac{|V_q|^2}{\omega + \frac{1}{2}\epsilon_q - 2\epsilon_d - U } -2\sum_{q}\frac{|V_q|^2}{\omega + \frac{1}{2}\epsilon_q} + 2\sum_{q}\frac{|V_q|^2}{\omega - \frac{1}{2}\epsilon_q - \epsilon_d} + 2\sum_{q}\frac{|V_q|^2}{\omega - \frac{1}{2}\epsilon_q + \epsilon_d}\\
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	...
	E^0_2 &= \frac{\epsilon_q}{2} + \epsilon_d - \frac{J_z}{2}\\
	E^0_3 = E^0_4 &= \frac{\epsilon_q}{2} + \epsilon_d\\
	E^0_5 &= \frac{\epsilon_q}{2} + \epsilon_d - \frac{J_z}{2}\\
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\omega_1 &= \omega_3 = \omega_4 = -\frac{1}{2}\epsilon_q + \epsilon_d \\
	\omega_2 &= -\frac{1}{2}\epsilon_q\\
	\omega_5 &= -\frac{1}{2}\epsilon_q + \epsilon_d - \frac{J_z}{2}\\
\end{aligned}\end{equation}
We will \textit{assume} that the relations between these values of \(\omega_i\) will hold for the URG \(\omega_i\) along the flow as well. We want to write all the \(\omega_i\) in terms of a single \(\omega = -\frac{\epsilon_q}{2} - \frac{1}{2}J_z\).
\begin{equation}\begin{aligned}
	\omega_1 &= \omega_3 = \omega_4 = \omega + \epsilon_d + \frac{J_z}{2}\\
	\omega_2 &= \omega + \frac{J_z}{2}\\
	\omega_5 &= \omega + \epsilon_d
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	...
	\omega^\prime_2 = \omega^\prime_3 &= -\frac{1}{2}\epsilon_q + \epsilon_d + \frac{J_z}{2}\\
	\omega^\prime_4 = \omega^\prime_5 &= -\frac{1}{2}\epsilon_q + \epsilon_d - \frac{J_z}{2}\\
\end{aligned}\end{equation}
Writing these in terms of \(\omega = -\frac{\epsilon_q}{2} - \frac{J_z}{2}\) gives
\begin{equation}\begin{aligned}
	\omega^\prime_1 &= \omega + 2\epsilon_d + U\\
	\omega^\prime_2 &= \omega^\prime_3 = \omega + \epsilon_d + J_z\\
...
\begin{equation}\begin{aligned}
	...
	\xi_2^\prime &= \omega - \frac{\epsilon_q}{2} + \epsilon_d + J_z\\
...
\begin{equation}\begin{aligned}
	\Delta \epsilon_d &= \frac{|V_1|^2}{\xi_1} + \frac{|V_0|^2}{\xi_2^\prime} - \frac{2|V_0|^2}{\xi_2} + \sum_k \left(\frac{1}{\xi_3}J_t^2  + \frac{1}{2\xi_3}J_z^2\right)\\
	\Delta U &= \frac{2|V_1|^2}{\xi_1^\prime} + \frac{2|V_0|^2}{\xi_2} - 2\frac{|V_1|^2}{\xi_1} - 2\frac{|V_0|^2}{\xi_2^\prime} - \sum_k \left[\frac{2}{\xi_3}J_t^2 + \frac{1}{\xi_3}J_z^2\right]\\
...
\begin{equation*}\begin{aligned}
	\xi_1 &= \omega - \frac{\epsilon_q}{2} - \epsilon_d - U + \frac{J_z}{2}\\[5pt]
	\xi_2 &= \omega - \frac{\epsilon_q}{2} - \epsilon_d + J_z\\[5pt]
\xi_3 &= \xi_4 = \xi_5 = \omega - \frac{\epsilon_q}{2} + \frac{J_z}{2}\\[5pt]
	\xi_1^\prime &= \omega - \frac{\epsilon_q}{2} + \epsilon_d + U + J_z\\[5pt]
\xi_2^\prime &= \omega - \frac{\epsilon_q}{2} + \epsilon_d + J_z\\[5pt]
\xi_3^\prime &= \xi_4^\prime = \xi_5^\prime = \omega - \frac{\epsilon_q}{2} + \frac{J_z}{2} \\[5pt]
		     &= \xi_3 = \xi_4 = \xi_5\\[5pt]
\end{aligned}\end{equation*}
\end{minipage}
...
\Delta V &= -\rr{\hf J_z + J_t}\left(\sum_{\epsilon_q=D} + \sum_{\epsilon_q=-D}\right)V_q\fr{1}{\omega - \hf \epsilon_q - \epsilon_d + \hf J_z}\\
\Delta J_t &= -J_z J_t\left(\sum_{\epsilon_q=D} + \sum_{\epsilon_q=-D}\right)\fr{1}{\omega - \hf \epsilon_q - \epsilon_d + \hf J_z}\\
\Delta J_z &= -J_t^2 \left(\sum_{\epsilon_q=D} + \sum_{\epsilon_q=-D}\right)\fr{1}{\omega - \hf \epsilon_q - \epsilon_d + \hf J_z}\\
\eeq
}
\subsection{Particle-Hole symmetry}
...
\beq
\vec C = \frac{1}{2}\sum_{kk^\prime\alpha\beta} {\psi^k_\alpha}^\dagger \vec \sigma_{\alpha\beta} \psi^{k^\prime}_\beta
\eeq
The various components of the isospin are
\begin{flalign}
	C^z_{kk^\prime} 
	&= \frac{1}{2}\sum_{\alpha} {\psi^k_\alpha}^\dagger \sigma^z_{\alpha\alpha} \psi^{k^\prime}_\alpha = \frac{1}{2}\rr{c^\dagger_{k\ua}c_{k^\prime \ua} - c_{k^\prime\da}c^\dagger_{k \da}} = \frac{1}{2}\left(c^\dagger_{k\ua}c_{k^\prime \ua} + c^\dagger_{k\da}c_{k^\prime \da} - \delta_{kk^\prime}\right)\\
	&= \frac{1}{2}\sum_\sigma\left(c^\dagger_{k\sigma}c_{k^\prime \sigma} - \frac{1}{2}\delta_{kk^\prime}\right)\label{diagonalCz}\\
	C^x_{kk^\prime} 
	&= \frac{1}{2}\sum_{\alpha} {\psi^k_\alpha}^\dagger \sigma^z_{\alpha\ol\alpha} \psi^{k^\prime}_{\ol\alpha} = \frac{1}{2}\left( c^\dagger_{k\ua}c^\dagger_{k^\prime \da} + c_{k\ua}c_{k^\prime \da} \right) \\
	C^y_{kk^\prime} 
	&= \frac{1}{2}\sum_{\alpha} {\psi^k_\alpha}^\dagger \sigma^z_{\alpha\ol\alpha} \psi^{k^\prime}_{\ol\alpha} = -i \frac{1}{2}\left( c^\dagger_{k\ua}c^\dagger_{k^\prime \da} - c_{k\ua}c_{k^\prime \da} \right) \\
\end{flalign}
...
C^x_d &= \frac{1}{2}\left(  c^\dagger_{d\ua}c^\dagger_{d \da} + c_{d\ua}c_{d \da} \right)\\
C^y_d &= -i \frac{1}{2}\left(  c^\dagger_{d\ua}c^\dagger_{d \da} - c_{d\ua}c_{d \da} \right)\\
\end{aligned}\end{equation}
The full charge Kondo interaction can now be written down in terms of these isospins:
\begin{equation}\begin{aligned}
4K_z \sum_{kk^\prime}C_d^z C^z_{kk^\prime} + K_t \sum_{kk^\prime}\rr{C_d^+ C^-_{kk^\prime}+ C_d^- C^+_{kk^\prime}}
\end{aligned}\end{equation}
where \(C^\pm \equiv C^x \pm iC^y\). For \(4K_z = 2K_t=K\), we get an \(SU(2)\)-charge symmetric model:
\beq
...
\begin{equation}\begin{aligned}
	...
	\ham_X &= V_qc^\dagger_{q\beta}c_{d\beta} + \text{h.c.} + K_z \rr{\hat n_d -1}\sum_k \rr{c^\dagger_{q\beta}c_{k\beta} + \text{h.c.}} + K_t \sum_k \rr{c^\dagger_{d\beta}c^\dagger_{d\ol\beta}c_{q\beta}c_{k\ol\beta} + \text{h.c.}} \\
\end{aligned}\end{equation}
As usual, we have considered only one mobile electron on the shell we are decoupling, and we only keep the energy of that electron and the impurity in the diagonal part which comes down in the denominator. Note that the factors of half in \(C^z\) are cancelled by the factor of \(4\) in \(4K_z\). The last term in \(\ham_d\) is obtained by setting \(k=k^\prime=q\) and \(\sigma=\beta\) in eq.~\ref{diagonalCz}, and then recognizing that \(\hat n_{q\beta} - \frac{1}{2} = \tau_{q\beta}\). The \(K_z\) part of \(\ham_X\) is obtained by noting that
\begin{equation}\begin{aligned}
...
\end{aligned}\end{equation}
The calculation of renormalization will proceed similar to the spin-Kondo Anderson URG. We will again separate the off-diagonal piece \(\ham_d^X\) into separate parts \(T_i\), calculate the renormalization in particle and hole sectors, and then finally relate the \(\omega_i\) using their bare values.
\pb The off-diagonal parts for this problem are
\begin{equation}\begin{aligned}
	T_1 &= V_1 c_{d\beta}\hat n_{d\ol\beta}\\
	T_2 &= V_0 c_{d\beta}\left(1 - \hat n_{d\ol\beta}\right)\\
	...
	T_5 &= K_t c^\dagger_{k\ol\beta}c_{d\beta}c_{d\ol\beta}\\
\end{aligned}\end{equation}
\subsection{Particle sector}
...
\begin{equation}\begin{aligned}
	c^\dagger_{q\beta} T \eta
\end{aligned}\end{equation}
This is at energy \(-\epsilon_q\). Using the expressions of the \(T_i\), this becomes
\begin{equation}\begin{aligned}
	&\left(1 - \hat n_{d\beta}\right)\left[\frac{|V_1|^2}{\xi_1} + \frac{|V_0|^2}{\xi_2}\right] + \left[\frac{{K_z^1}^2}{\xi_3}\hat n_{d\beta}\hat n_{d\ol\beta} + \frac{{K_z^0}^2}{\xi_4}\left(1 - \hat n_{d\beta}\right)\left(1 - \hat n_{d\ol\beta}\right)\right]c_{k^\prime\beta}c^\dagger_{k\beta} \\
	...
	&- \frac{1}{2}\left(\frac{1}{\xi_1} + \frac{1}{\xi_5} \right)V_1 K_t \left(1 - \hat n_{d\beta}\right)\left(c^\dagger_{d\ol\beta}c_{k\ol\beta} + \text{h.c.}\right)- \frac{1}{2}\left(\frac{1}{\xi_3} + \frac{1}{\xi_5} \right)K_z^1 K_t \left(c^\dagger_{d\beta}c^\dagger_{d\ol\beta}c_{k\beta}c_{k^\prime\ol\beta} + \text{h.c.}\right)
\end{aligned}\end{equation}
The indices \(k,k^\prime\) are summed over. \(\xi_i\) is defined exactly as before, \(\omega_i - E_i^0\). 
\pb The intermediate energies, \(E_i^0\), are
\begin{flalign*}
	E^0_1 &= E^0_3 = E^0_5 = \frac{\epsilon_q}{2} + 2\epsilon_d + U - \frac{K_z}{2}\\
	E^0_2 &= \frac{\epsilon_q}{2} + \epsilon_d \\
	E^0_4 &= \frac{\epsilon_q}{2} + \frac{K_z}{2}\\
\end{flalign*}
\subsection{Hole sector}
...
&- \frac{1}{2}\left(\frac{1}{\xi^\prime_2} + \frac{1}{\xi^\prime_5}\right)V_0 K_t\hat n_{d\beta}\left(c^\dagger_{d\ol\beta}c_{k\ol\beta} + \text{h.c.}\right) - \frac{1}{2}\left(\frac{1}{\xi^\prime_4} + \frac{1}{\xi^\prime_5}\right)K_z^0 K_t\left(c^\dagger_{d\beta}c^\dagger_{d\beta}c_{k^\prime\beta}c_{k\ol\beta} + \text{h.c.}\right) 
\end{aligned}\end{equation}
where \(\xi^\prime = \omega^\prime = E_i^1\). The intermediate energies in this sector are
\begin{equation}\begin{aligned}
	E^1_1 &= \frac{\epsilon_q}{2} + \epsilon_d\\
	E^1_2 = E_4^1 = E_5^1 &= \frac{\epsilon_q}{2} - \frac{K_z}{2}\\
	E^1_3 &= \frac{\epsilon_q}{2} + 2\epsilon_d + U + \frac{K_z}{2}
\end{aligned}\end{equation}
...
Just as in the previous subsection, we relate the \(\omega_i\) using their diagonal values. First the particle sector \(\omega_i\):
\begin{equation}\begin{aligned}
	\omega_1 &= -\frac{\epsilon_q}{2} + \epsilon_d\\
	\omega_2 &= \omega_4 = \omega_5 = -\frac{\epsilon_q}{2} - \frac{K_z}{2}\\
	\omega_3 &= -\frac{\epsilon_q}{2} + 2\epsilon_d + U + \frac{K_z}{2}\\
\end{aligned}\end{equation}
Rewriting everything in terms of \(\omega = -\frac{\epsilon_q}{2} - \frac{K_z}{2}\) gives
\begin{equation}\begin{aligned}
	\omega_1 &= \omega + \epsilon_d + \frac{K_z}{2}\\
	\omega_2 &= \omega_4 = \omega_5 = \omega\\
	\omega_3 &= \omega + 2\epsilon_d + U + K_z
\end{aligned}\end{equation}
For the hole sector, we get
\begin{equation}\begin{aligned}
	\omega^\prime_1 &= \omega^\prime_3 = \omega^\prime_5 = -\frac{\epsilon_q}{2} + 2\epsilon_d + U - \frac{K_z}{2}\\
	\omega^\prime_2 &= -\frac{\epsilon_q}{2} + \epsilon_d - \frac{K_z}{2}\\
	\omega^\prime_4 &= -\frac{\epsilon_q}{2} + \frac{K_z}{2}\\
\end{aligned}\end{equation}
and, in terms of \(\omega\),
\begin{equation}\begin{aligned}
	\omega^\prime_1 &= \omega^\prime_3 = \omega^\prime_5 = \omega + 2\epsilon_d + U\\
	\omega^\prime_2 &= \omega + \epsilon_d\\
	\omega^\prime_4 &= \omega
\end{aligned}\end{equation}
The denominators can thus be written as
\begin{equation}\begin{aligned}
	\xi_1 &= \omega - \frac{\epsilon_q}{2} - \epsilon_d - U + K_z\\
	\xi_2 &= \omega - \frac{\epsilon_q}{2} - \epsilon_d\\
	\xi_3 &= \omega - \frac{\epsilon_q}{2} + \frac{3K_z}{2}\\
	\xi_4 &= \omega - \frac{\epsilon_q}{2} - K_z\\
	\xi_5 &= \omega - \frac{\epsilon_q}{2} - 2\epsilon_d - U + \frac{K_z}{2}\\
	\xi_1^\prime &= \omega - \frac{\epsilon_q}{2} + \epsilon_d + U\\
	\xi_2^\prime &= \omega - \frac{\epsilon_q}{2} + \epsilon_d + \frac{K_z}{2}\\
	\xi_3^\prime &= \omega - \frac{\epsilon_q}{2} - \frac{K_z}{2}\\
	\xi_4^\prime &= \omega - \frac{\epsilon_q}{2} + \frac{K_z}{2}\\
	\xi_5^\prime &= \omega - \frac{\epsilon_q}{2} + 2\epsilon_d + U + \frac{K_z}{2}\\
\end{aligned}\end{equation}
...
\subsection{Scaling Equations}
To get the scaling equations, we perform similar steps as in the spin-Kondo URG. First of all, we substitute \(\hat n_{q\beta}\) with \(1\) and \(0\) in the particle and hole sectors respectively. Secondly, in the RG equations that involve  \(\hat n_{d\ol\beta}\) in the denominator, we replace the quantum fluctuations with \(E_j + \omega\), where \(E_j\) is the impurity part of the energy of the initial state and \(\omega\) is a redefined quantum fluctuation scale. Thirdly, by using the constraint \(\eta = \left( \eta^\dagger \right) ^\dagger\), we can relate the denominators of the RG equations of the \(V\)s and \(K\)s. The term with \(K_z^2\) in either sector mostly cancel each other, leaving a renormalization of \(\epsilon_d\) and \(U\) at \(K^2\) order.
\begin{equation}\begin{aligned}
	\sum_{kk^\prime q\beta}\frac{K_z^2 \rr{1 - \hat n_d}^2\left[c^\dagger_{k\beta}c_{k^\prime\beta} + c_{k^\prime\beta}c^\dagger_{k\beta}\right]}{\omega_0 - \hf\epsilon_q - H_{imp} - \frac{1}{2}K_z\rr{\hat n_d - 1}} = \sum_{kk^\prime q\beta}\frac{K_z^2 \rr{1 - \hat n_d}^2\delta_{kk^\prime}}{\omega_0 - \hf\epsilon_q - H_{imp} - \frac{1}{2}K_z\rr{\hat n_d - 1}}
\end{aligned}\end{equation}
Since \(\left(1 - \hat n_d\right)^2 = \left(1 - \hat n_{d\ua}\right)\left(1 - \hat n_{d\da}\right) + \hat n_{d\ua}\hat n_{d\da}\), we can write down the renormalization in \(\Delta E_0\) and \(\Delta E_2\) and hence in \(\epsilon_d\) and \(U\).
\begin{equation}\begin{aligned}
\Delta E_0 = \sum_{k q\beta}\frac{K_z^2}{\omega_0 - \hf\epsilon_q + \frac{1}{2}K_z}\\
\Delta E_2 = \sum_{kk^\prime q\beta}\frac{K_z^2}{\omega_0 - \hf\epsilon_q - 2\epsilon_d - U - \frac{1}{2}K_z}\\
\end{aligned}\end{equation}
Making the substitution \(\omega_0 = \omega \)
The penultimate term in the hole sector, gets canceled by a piece from the final term in particle sector, and the remaining piece gives the renormalization in \(K_z\).
\begin{flalign}
	\Delta \epsilon_d &= \sum_q \frac{|V_q|^2}{\omega - \frac{1}{2}\epsilon_q + \epsilon_d + \frac{1}{2}K_z} + \frac{|V_q|^2}{\omega - \frac{1}{2}\epsilon_q - \epsilon_d - U + \frac{1}{2}K_z} - \frac{|V_q|^2}{\omega - \frac{1}{2}\epsilon_q - \epsilon_d - \frac{1}{2}K_z}\\
			  &+ \frac{8K_z^2}{\omega - \frac{1}{2}\epsilon_q }\\
	\Delta U &= \frac{|V|^2}{\omega - \frac{1}{2}\epsilon_q + \epsilon_d + U -\frac{1}{2}K_z} + \frac{|V|^2}{\omega - \frac{1}{2}\epsilon_q - \epsilon_d - \frac{1}{2}K_z} - \frac{|V|^2}{\omega - \frac{1}{2}\epsilon_q - \epsilon_d - U + \frac{1}{2}K_z} \\
		 &- \frac{|V|^2}{\omega - \frac{1}{2}\epsilon_q + \epsilon_d +  \frac{1}{2}K_z} - \frac{16 K_z^2}{\omega - \frac{1}{2}\epsilon_q }\\
	\Delta V &= \frac{-V_q^* K_t + V_q K_z}{\omega - \frac{1}{2}\epsilon_q + \frac{1}{2}K_z}\\
	\Delta K_z &= \frac{-K_t^2}{\omega - \hf \epsilon_q + \frac{1}{2}K_z}\\
	\Delta K_t &= \frac{-2K_zK_t}{\omega - \hf \epsilon_q + \frac{1}{2}K_z}\\
\end{flalign}
