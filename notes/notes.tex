...
\begin{equation}\begin{aligned}
	T(\omega) = \sum_l T_l(\omega)
\end{aligned}\end{equation}
the determinant of \(T(\omega)\) will be the product of the eigenvalues \(T_l(\omega)\), so we can write
\begin{equation}\begin{aligned}
	\text{Det}\left[ T(\omega) \right] = \prod_l T_l = \exp\left[ i\sum_l \delta_l(\omega) \right] \prod_l\left( -\frac{1}{\pi g(\omega)}\right)\sin \delta_l(\omega)
\end{aligned}\end{equation}
and the argument of this function will be the total scattering phase shift:
\begin{equation}\begin{aligned}
	\label{phase_in_T}
	arg\left[\text{Det}\left[ T(\omega) \right]\right] = \sum_l \delta_l(\omega) \equiv \delta(\omega)
\end{aligned}\end{equation}
...
\end{aligned}\end{equation}
This is the generalized Friedel sum rule and it tells us that the change in the number of available states below the Fermi surface is related to the scattering \(S-\)matrix at the Fermi energy. For a spherically symmetric scatteror, we can use \(S(\epsilon_F) = \sum_l e^{2i\delta_l(\epsilon_F)}\), and get
\begin{equation}\begin{aligned}
	\Delta N = \frac{1}{\pi}\text{Im}\ln \exp\left[2i \sum_l g_l \delta_l(\epsilon_F)\right] = \frac{1}{\pi}\text{Im}\left[2i \sum_l  g_l\delta_l(\epsilon_F)\right] = \frac{2}{\pi}\delta(\epsilon_F)
\end{aligned}\end{equation}
\(g(l)\) is the degeneracy of \(l\) state and \(\delta = \sum_l  g_l\delta_l\) is the total change in phase due to the scattering.
...
\begin{center}
	\includegraphics[scale=0.45]{../figures/Vc_D_q11.png}
	\includegraphics[scale=0.45]{../figures/Vc_D_q12.png}
