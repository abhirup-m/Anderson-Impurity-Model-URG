...
\end{aligned}\end{equation}
Note that these scaling equations are for decoupling a pair of electrons \(q,\ua\da\). To decouple a shell of electrons, we need to sum over \(q\). If we set \(J_z = J_t\), we end up with an SU(2)-symmetric model \(J \vec{S_d}\cdot\vec{s}\).
...
\end{aligned}\end{equation}
This gives us the same condition as in the \(J=0\) cases.
