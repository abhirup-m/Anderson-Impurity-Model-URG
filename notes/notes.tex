...
\begin{equation}\begin{aligned}
	...
	\eta &= \frac{1}{\omega - \ham_d} \left[ J_z\sum_k \beta S_d^z c^\dagger_{k\beta}c_{q\beta} + J_t \sum_k c^\dagger_{d\beta}c_{d\ol\beta} c^\dagger_{k\ol\beta}c_{q\beta} \right]\\
	     &=\sum_k \left[\frac{1}{\omega_1 + \frac{1}{2}\epsilon_q+ \frac{1}{2}J_z \left( \hat n_{d\beta} - \hat n_{d\ol\beta} \right) \hat n_{q\ol\beta}} J_z S_d^z \beta c^\dagger_{k\beta}c_{q\beta} + \frac{1}{\omega_2 + \frac{1}{2}\epsilon_q+ \frac{1}{2}J_z \hat n_{q\ol\beta}}J_t  c^\dagger_{d\beta}c_{d\ol\beta} c^\dagger_{k\ol\beta}c_{q\beta}\right] 
\end{aligned}\end{equation}
To relate \(\omega_1\) and \(\omega_2\), we will use their diagonal values. By replacing them with the initial state energies, we can write
\begin{equation}\begin{aligned}
	\omega_1 = 
\end{aligned}\end{equation}
...
\end{aligned}\end{equation}
The total renormalization will have three types of terms: \(J_z^2, J_t^2\) and \(J_z J_t\).
\begin{equation}\begin{aligned}
	\sum_{kk^\prime} \hat n_{q\beta}\left[\frac{\frac{1}{4} J_z^2  c_{k^\prime\beta} c^\dagger_{k\beta}}{\omega + \frac{1}{2}\epsilon_q+ \frac{1}{2}J_z \left( \hat n_{d\beta} - \hat n_{d\ol\beta} \right) \hat n_{q\ol\beta}} + \frac{ \frac{1}{2} J_z J_t  \left( c^\dagger_{d\ol\beta}c_{d\beta}c_{k^\prime\ol\beta}c^\dagger_{k\beta} + \text{h.c.}\right)}{\omega + \frac{1}{2}\epsilon_q+ \frac{1}{2}J_z \hat n_{q\ol\beta}} + \frac{J_t^2 \hat n_{d\ol\beta}\left(1 - \hat n_{d\beta}\right) c_{k^\prime\ol\beta}c^\dagger_{k\ol\beta}}{\omega + \frac{1}{2}\epsilon_q+ \frac{1}{2}J_z \hat n_{q\ol\beta}}\right]
\end{aligned}\end{equation}
