...
\end{center}
To make progress, we will try to figure out how the system will behave at very large $D$ (that is, a thermodynamic ally large system). We will see that we will be able to make concrete statements in such a limit.

\subsection{First quadrant}
From the first quadrant of fig.~\ref{Veffect}, we can see that the there is a shift in the dominance of the fixed points at some value of $V$. That is, for $V$ less than a certain value, the more dominant fixed points are the ones on the blue line, with $U^* > U_0$. For $V$ larger than this value, the fixed points on the red line are more dominant. More specifically, the critical coupling \(V_c\) is that value of \(V\) at which the number of irrelevant \((U^* = 0)\) and relevant \((U^* > U_0)\) fixed points are equal. From our numerical studies, we conclude that \(V_c\) depends on the bare values of \(D\), \(J\) and \(K\). Changing \(J\) and \(K\) does not introduce any qualitatively new behaviour, so we focus on the effect of $D$.
\pb The dependence of $V_c$ on $D$ is shown in fig.~\ref{Vc_vs_D}. It reaches a minima and then increases. The large $D$ behaviour is an increase in $V_c$. We can thus conclude that for very large $D$, this critical $V$ will also be very large, and the flip from the blue line to the red line will be very unfavorable, and the fixed point phase will be one with \(U^* > U_0\).
\begin{center}
	\includegraphics[scale=0.39]{../figures/Vc_q11.pdf}
	\includegraphics[scale=0.39]{../figures/Vc_q12.pdf}
	\captionof{figure}{Variation of the critical value \(V_c\) with bandwidth. The left panel shows the lowering of $V_c$ just up to the minimum. The right panel shows the subsequent increase. They have been shown separately for clarity.}
	\label{Vc_vs_D}
\end{center}
One also needs to check how the fractions of the number of relevant and irrelevant fixed points compare to each other. To that end, we have plotted the difference between the number of relevant and irrelevant fixed points at a particular value of $V$, as a function of $D$, in the left pane of fig.~\ref{frac_D}. The almost linear increase suggests that at large $D$, only the relevant fixed points will survive.
\pb The variation of the fixed point value of the Kondo coupling \(J\) with the bare bandwidth $D$ is shown in the right pane of fig.~\ref{frac_D}. For a thermodynamically large system (where $D \to \infty$), the fixed point value \(J^*\) should go to infinity.
\begin{figure}[htpb]
\centering
\includegraphics[scale=0.65]{../figures/frac_vs_D_quad1.pdf}
\includegraphics[scale=0.65]{../figures/J_vs_D.pdf}
...
\caption{\textit{Left}:Variation of the difference between number of fixed points with increase in bandwidth, for the first quadrant. \textit{Right}: Variation of the maximum $J^*$ with bare value of bandwidth, for the first quadrant.}
\label{frac_D}
\end{figure}
...
\begin{equation}\begin{aligned}
	c_{2\sigma} \equiv \sum_{k} c_{k\sigma}
\end{aligned}\end{equation}
This operator essentially creates a conduction electron at the zeroth lattice site. For our two-site problem, we will imagine this to be the annihilation operator for the site 2, for the spin sigma. The corresponding operator for the first site is of course just the impurity electron annihilation operator:
\begin{equation}\begin{aligned}
	c_{1\sigma} \equiv c_{d\sigma}
\end{aligned}\end{equation}
The various terms of the Hamiltonian can now be written in terms of these operators. We now make a simplifying assumption. We write the kinetic energy part of the Hamiltonian as
\begin{equation}\begin{aligned}
	\epsilon_2\left[\ket{\hat n_2 = 0}\bra{\hat n_2 = 0} + \ket{\hat n_2 = 2}\bra{\hat n_2 = 2}\right]
\end{aligned}\end{equation}
Here, \(\hat n_2\) is the number of electrons on the first site. This way of writing the kinetic energy piece implies that we take the singly-occupied configuration (\(\hat n_2=1\)) as the zero-energy configuration (\(\mu=0\)), and the states with 0 or 2 electrons are excitations on top of this ground state. \(\hat n_2 = 2\) is a 1-particle excitation and \(\hat n_2 = 0\) is a 1-hole excitation. Both these excitations have an energy \(\epsilon_2 - \mu = \epsilon_2\). A succint way of writing this term is
\begin{equation}\begin{aligned}
	\epsilon_2 \tau_2^2
\end{aligned}\end{equation}
where 
\begin{equation}\begin{aligned}
	\tau_2 &= \sum_\sigma \tau_{1\sigma} = \sum_\sigma \left( \hat n_{1\sigma} - \frac{1}{2} \right) = \hat n_2  - 1\\
	\implies \tau_2^2 &= \begin{cases}
		1 & \text{ when }\hat n_2 = 0,2\\
		0 & \text{ when }\hat n_2 = 1\\
	\end{cases}
\end{aligned}\end{equation}
The impurity diagonal part of \(\ham_{IR}\) will survive only when \(\hat n_d = \hat n_1 = 1\). So we write it as
\begin{equation}\begin{aligned}
	- \frac{U^*}{2}\hat n_d + U^* \hat n_{d\ua}\hat n_{d\da} = - \frac{U^*}{2}\left(\hat n_{d\uparrow} + \hat n_{d \downarrow} - 2\hat n_{d\ua}\hat n_{d\da}\right) = - \frac{U^*}{2} \left(\hat n_{1 \uparrow} - \hat n_{1 \downarrow}\right)^2 \equiv \epsilon_1 \left(\hat n_{1 \uparrow} - \hat n_{1 \downarrow}\right)^2
\end{aligned}\end{equation}
where \(\epsilon_2 = - \frac{U^*}{2}\) is the energy of site 2.  The off-diagonal terms can also be similarly transformed into a two-site problem:
\begin{equation}\begin{aligned}
	\vec S_d &\equiv \vec S_1\\
	\vec s &= \sum_{kk^\prime\atop{\alpha\beta}}c^\dagger_{k\alpha}\vec \sigma_{\alpha\beta}c_{k^\prime\beta} =\sum_{\alpha\beta}c^\dagger_{2\alpha}\vec \sigma_{\alpha\beta}c_{2\beta} \equiv \vec S_2\\
\end{aligned}\end{equation}
The charge isospins can be written down similarly. Combining these, the interaction part can be written as
\begin{equation}\begin{aligned}
-t\sum_{\sigma}\left( c^\dagger_{1\sigma}c_{2\sigma} + \text{h.c.} \right) + j \vec{S_1}\cdot\vec{S_2} + k \vec{C_1}\cdot\vec{C_2}
\end{aligned}\end{equation}
where we can identity \(-t = V^*, j=J^*, k=K^*\). The total Hamiltonian for the two-site problem is
\begin{equation}\begin{aligned}
	\ham_{IR} = \epsilon_2 \tau_2^2 + \epsilon_1 \left(\hat n_{1 \uparrow} - \hat n_{1 \downarrow}\right)^2 +  -t\sum_{\sigma}\left(c^\dagger_{1\sigma}c_{2\sigma} + \text{h.c.} \right) + j\vec{S_1}\cdot\vec{S_2} + k \vec{C_1}\cdot\vec{C_2}
\end{aligned}\end{equation}
...
\pb First lets get the trivial cases of \(\hat n = 0, 4\) out of the way. The only possible states are \(\ket{0000}\) and \(\ket{1111}\) respectively. Both these states are eigenstates because the first one has no electron to scatter, and the second one has no vacant state to scatter into. Both the states have an energy of \(\epsilon_2\).
...
\begin{equation}\begin{aligned}
	4t\ket{\uparrow,0} + \left( 2\epsilon_1 \mp \Delta\left( \epsilon_1, t \right)  \right) \ket{0, \uparrow}, && E = \frac{\epsilon_1}{2} \pm \frac{\Delta\left( \epsilon_1, t \right) }{4}
\end{aligned}\end{equation}
where \(\Delta\left( \epsilon_1, t \right)  = \sqrt{4\epsilon_1^2 + 16t^2}\). The other two eigenstates (corresponding to magnetization \(- \frac{1}{2}\) need not be calculated separately; since the Hamiltonian is invariant under the transformation \( \uparrow \leftrightarrow \downarrow\), we can do a similar transformation on the eigenkets to get the eigenkets for the other subspace.
\begin{equation}\begin{aligned}
	4t\ket{\downarrow,0} + \left( 2\epsilon_1 \mp \Delta \right) \ket{0, \downarrow}, && E = \frac{\epsilon_1}{2} \pm \frac{\Delta}{4}
\end{aligned}\end{equation}
...
\end{aligned}\end{equation}
Since a double impurity has the same energy as a vacant impurity (because of p-h symmetry, both are zero), the energy of the first site will not change. However, a doubly-occupied second site will have energy of \(\epsilon_2\), so the diagonal element \(\epsilon_1\) in eq.~\ref{n1mat} will be replaced by \(\epsilon_1 + \epsilon_2\). We can then write down the eigenstates and eigenvalues directly from those of \(\hat n=1\), by changing the basis states and by replacing \(\epsilon_1 \to \epsilon_1 + \epsilon_2\).
\begin{align}
	\left.\begin{array}{l}
	4t\ket{\uparrow, \uparrow \downarrow} + \left[ 2\left(\epsilon_1 + \epsilon_2\right) \mp \Delta\left(\epsilon_1 + \epsilon_2, t\right)\right] \ket{\uparrow \downarrow, \uparrow}\\
4t\ket{\downarrow, \uparrow \downarrow} + \left[ 2\left(\epsilon_1 + \epsilon_2\right) \mp \Delta\left(\epsilon_1 + \epsilon_2, t\right)\right] \ket{\uparrow \downarrow, \downarrow}\end{array}\right\} E = \frac{1}{2}\left(\epsilon_1 + \epsilon_2\right) \pm \frac{\Delta\left(\epsilon_1 + \epsilon_2, t\right)}{4}
\end{align}
