...
\begin{equation}\begin{aligned}
\Delta \ham = c^\dagger_{q\beta} T \eta= \frac{|V_q|^2}{\omega - \frac{1}{2}\epsilon_q - \epsilon_d - \left(\epsilon_d + U \right)\hat n_{d\ol\beta}} \left(1 - \hat n_{d\beta}\right)\hat n_{q\beta} 
\end{aligned}\end{equation}
Similarly, the renormalization in the hole sector (\(\hat n_{q\beta} = \hat n_{q\ol\beta} = 0\)), at an energy \(\epsilon_q\) is:
\begin{equation}
	\Delta \ham = T^\dagger c \eta^\dagger = \frac{|V_q|^2}{\omega^\prime - \frac{1}{2}\epsilon_q - \epsilon_d\hat n_{d\ol\beta}}\left(1 - \hat n_{q\beta}\right)\hat n_{d\beta}
\end{equation}
