...
\end{aligned}\end{equation}
where \(\Sigma(z)\) is in general complex and is zero at the free orbital fixed point. Following Seki and Yunoki, we can define a winding number for this Green's function, \textit{at the Fermi surface}:
\begin{equation}\begin{aligned}
	n_{\text{Det }G_d^{-1}(\Gamma_0)} = \oint_{\Gamma_0} \frac{dz}{2\pi i} \frac{\partial{\ln\text{Det }G_d^{-1}(z)}}{\partial{z}} = \oint_{\text{Det }G_d^{-1}(\Gamma_0)} \frac{\mathrm{d} \text{Det }G_d^{-1}}{\text{Det }G_d^{-1}}
\end{aligned}\end{equation}
Since \(n_{\text{Det }G_d^{-1}(\Gamma_0)}\) counts the number of times the curve \(\text{Det }G_d^{-1}(\Gamma_0)\) winds around the origin, it is integer-valued and topological. Seki and Yunoki also show that the this number is given by
\begin{equation}\begin{aligned}
