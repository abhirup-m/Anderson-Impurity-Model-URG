...
V\ket{\Psi} = T\ket{\Phi}
\eeq
where \il{\ket{\Psi}} is the total scattered wavefunction and \il{\ket{\Phi}} is the incoming (non-interacting) wavefunction. They satisfy the Schrodinger equations
\begin{gather}
...
\begin{align}
\ket{\Psi} &= \ket{\Phi} + G_0 T \ket{\Phi} \\
\label{lsmod} \implies \ket{\Psi} &=(1+G_0T) \ket{\Phi}
\end{align}
Eq.~\ref{lse} can also be written as 
\beq
\ket{\Psi} = (1- G_0V)^{-1} \ket{\Phi}
\eeq
...
\end{aligned}\end{equation}
By comparing with eq.~\ref{tmatexp}, we can recognize the term piece inside the brackets as the \(T-\)matrix, and write
\begin{equation}\begin{aligned}
	G = G_0 + G_0 T G_0
	\label{G_and_G0}
\end{aligned}\end{equation}
...
\beq
\ket{\Psi} = \rr{2\pi}^{-\fr{3}{2}}\qq{e^{ikx} + f \fr{e^{ikr}}{r}}
\eeq
where the wave amplitude \il{f(k^\prime,k) \sim \bra{k^\prime} V \ket{\ket{\Psi}}}.
Using the definition of \il{T}, we get
\beq
f(k^\prime,k) \sim \bra{k^\prime} T \ket{k}
\eeq
By definition, the \il{S-}matrix is the unitary operator that relates the non-interacting eigenstate to the interacting one:
\beq
\ket{\Psi} = S\ket{\Phi}
\eeq
We can derive a relation between \(S\) and \(T\) starting from eq.~\ref{lsmod}:
\beq
S\ket{\Phi} =\ket{\Psi} = \left(1 + G_0 T\right)\ket{\Phi} \implies  S = 1 + G_0 T
\eeq
This also leads to relation between the scattering matrix and the Greens functions. Continuing with the last equation, we can write
\begin{equation}\begin{aligned}
	S G_0 &= G_0 + G_0 T G_0 = G  && \left[ \text{using eq.~\ref{G_and_G0}} \right]\\
...
\end{aligned}\end{equation}
\textit{The quantity \(N\) counts the total number of states in the system below the Fermi surface.} The change in the density of states induced by the interaction term \(V\) is
\begin{equation}\begin{aligned}
	\label{doschange}
...
\begin{equation}\begin{aligned}
	-\text{Tr}\left[G(\omega) - G_0(\omega)\right] &= \frac{\partial{}}{\partial{\omega}} \ln \left\{\text{Det}\;\left[G\right]\left(\text{Det}\;\left[G_0\right]\right)^{-1}\right\}\\
						       ...
						       &= \frac{\partial{}}{\partial{\omega}} \ln \text{Det} \left[S\right] && \left[\text{using eq.~\ref{SinG}}\right] 
\end{aligned}\end{equation}
This is the main result, and we can substitute this in eq.~\ref{doschange}, to get
\begin{equation}\begin{aligned}
	\Delta \rho\left( \omega \right) &= \frac{1}{\pi}\text{Im}\frac{\partial{}}{\partial{\omega}} \ln \text{Det} \left[S\right] \\
	\implies \Delta N &=  \frac{1}{\pi}\text{Im}\int_{-\infty}^{\epsilon_F} d\omega\frac{\partial{}}{\partial{\omega}} \ln \text{Det} \left[S\right] = \frac{1}{\pi}\text{Im}\ln \text{Det} \left[S\left( \epsilon_F \right)\left\{S\left( -\infty \right)\right\}^{-1}\right]
\end{aligned}\end{equation}
For \(\omega \to -\infty\), we can write
\begin{equation}\begin{aligned}
	\omega - H_0 - V = \omega - H_0 \implies G(-\infty) = G_0(-\infty)
\end{aligned}\end{equation}
such that \(S\left( -\infty \right) = \mathbb{I}\), and we get
\begin{equation}\begin{aligned}
	\Delta N = \frac{1}{\pi}\text{Im}\ln \text{Det} \left[S\left( \epsilon_F \right)\right]
\end{aligned}\end{equation}
This tells us that the change in the number of available states below the Fermi surface is related to the scattering \(S-\)matrix at the Fermi energy.
...
\begin{center}
    ...
    \centering
    \includegraphics[scale=0.5]{../figures/quad2.png}
\end{minipage}
%\hspace{0.01\textwidth}
  \begin{minipage}{.49\textwidth}
    \centering
    \includegraphics[scale=0.5]{../figures/quad1.png}
\end{minipage}
  ...
  \begin{minipage}{.49\textwidth}
    \centering
    \includegraphics[scale=0.5]{../figures/quad3.png}
\end{minipage}
%\hspace{0.02\textwidth}
  \begin{minipage}{.49\textwidth}
    \centering
    \includegraphics[scale=0.5]{../figures/quad4.png}
\end{minipage}
\label{Veffect}
...
\pb The dependence of $V_c$ on $D$ is shown in fig.~\ref{Vc_vs_D}. It reaches a minima and then increases. The large $D$ behaviour is an increase in $V_c$. We can thus conclude that for very large $D$, this critical $V$ will also be very large, and the flip from the blue line to the red line will be very unfavourable, and the fixed point phase will be one with \(U^* > U_0\).
\begin{center}
	\includegraphics[scale=0.5]{../figures/frac_vs_D_quad1.png}
	\includegraphics[scale=0.5]{../figures/frac_vs_D_quad1.png}
	\captionof{figure}{Variation of the critical value \(V_c\) with bandwidth. The left panel zooms into a smaller range and clearly shows the minima. The right panel is for a larger range and shows the ultimate increase.}
	\label{Vc_vs_D}
\end{center}
One also needs to check how the fractions of the number of relevant and irrelevant fixed points compare to each other. To that end, we have plotted the difference between the number of relevant and irrelevant fixed points at a particular value of $V$, as a function of $D$. The almost linear increase suggests that at large $D$, only the relevant fixed points will survive.
\begin{center}
	\includegraphics[scale=0.5]{../figures/frac_vs_D_quad1.png}
	\captionof{figure}{}
\end{center}
