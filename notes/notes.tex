...
\end{aligned}\end{equation}
There we used \(\sinh x \approx \cosh x \approx \frac{1}{2}e^x \text{ for }x\to \infty\). If we are in the first quadrant, then the fixed point values are such that \(\frac{3k}{4} + \epsilon_d - \frac{3j}{4} < 0\), so the exponential will go to 0 in that limit, and we can write
\begin{equation}\begin{aligned}
	\chi\vert_{\beta \to \infty} = \frac{1}{2j}
\end{aligned}\end{equation}
On the other hand, in the third quadrant, we have \(\frac{3k}{4} + \epsilon_d - \frac{3j}{4} > 0\), and the exponential in the denominator will diverge.
\begin{equation}\begin{aligned}
