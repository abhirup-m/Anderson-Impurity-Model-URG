...
\end{aligned}\end{equation}
We now make a simplifying assumption: We keep only the \(\vec r=0\) mode of the decomposition:
\begin{equation}\begin{aligned}
...
\end{aligned}\end{equation}
\(\epsilon_F\) is the Fermi energy, which we henceforth set to 0. The second site thus does not have any energy cost. The impurity diagonal part of \(\ham_{IR}\) will survive only when \(\hat n_d = \hat n_1 = 1\). So we write it as
\begin{equation}\begin{aligned}
...
\begin{equation}\begin{aligned}
	\ham_{IR} = \epsilon_1 \left(\hat n_{1 \uparrow} - \hat n_{1 \downarrow}\right)^2 +  -t\sum_{\sigma}\left(c^\dagger_{1\sigma}c_{2\sigma} + \text{h.c.} \right) + j\vec{S_1}\cdot\vec{S_2} + k \vec{C_1}\cdot\vec{C_2}
\end{aligned}\end{equation}
...
\pb First lets get the trivial cases of \(\hat n = 0, 4\) out of the way. The only possible states are \(\ket{0000}\) and \(\ket{1111}\) respectively. Both these states are eigenstates because the first one has no electron to scatter, and the second one has no vacant state to scatter into. Both the states have 0 energy eigenvalue.
...
\end{aligned}\end{equation}
Since a double impurity has the same energy as a vacant impurity (because of p-h symmetry, both are zero), the diagonal energyies of the sites will not change. We can then write down the eigenstates and eigenvalues directly from those of \(\hat n=1\).
\begin{align}
	\left.\begin{array}{l}
	4t\ket{\uparrow, \uparrow \downarrow} + \left[ 2\epsilon_1 \mp \Delta\left(\epsilon_1, t\right)\right] \ket{\uparrow \downarrow, \uparrow}\\
4t\ket{\downarrow, \uparrow \downarrow} + \left[ 2\epsilon_1 \mp \Delta\left(\epsilon_1, t\right)\right] \ket{\uparrow \downarrow, \downarrow}\end{array}\right\} E = \frac{1}{2}\epsilon_1 \pm \frac{\Delta\left(\epsilon_1, t\right)}{4}
\end{align}
...
\begin{equation}\begin{aligned}
	\ket{\uparrow, \uparrow} &\mapsto \left(\epsilon_1 + \frac{j}{4}\right)\ket{\uparrow, \uparrow} \\
	\ket{\downarrow, \downarrow} &\mapsto \left(\epsilon_1 + \frac{j}{4}\right)\ket{\downarrow, \downarrow}\\
	\frac{1}{\sqrt 2}\left(\ket{\uparrow, \downarrow} + \ket{\downarrow, \uparrow}\right) &\mapsto \left( \epsilon_1 + \frac{1}{4}j \right) \frac{1}{\sqrt 2}\left(\ket{\uparrow, \downarrow} + \ket{\downarrow, \uparrow}\right)\\
	...
	\frac{1}{\sqrt 2}\left(\ket{\uparrow, \downarrow} - \ket{\downarrow, \uparrow}\right) &\mapsto \left( \epsilon_1 - \frac{3}{4}j \right) \frac{1}{\sqrt 2}\left(\ket{\uparrow, \downarrow} - \ket{\downarrow, \uparrow}\right) + 2t\frac{1}{\sqrt 2}\left(\ket{\uparrow\downarrow, 0} - \ket{0, \uparrow\downarrow}\right)\\
	\frac{1}{\sqrt 2}\left(\ket{\uparrow\downarrow, 0} - \ket{0, \uparrow\downarrow}\right) &\mapsto - \frac{3}{4}k \frac{1}{\sqrt 2}\left(\ket{\uparrow\downarrow, 0} - \ket{0, \uparrow\downarrow}\right) + 2t\frac{1}{\sqrt 2}\left(\ket{\uparrow, \downarrow} - \ket{\downarrow, \uparrow}\right)\\
\end{aligned}\end{equation}
