...
\begin{equation}\begin{aligned}
	\sum_\beta \Delta \ham = - \frac{1}{\xi} \left[2J_z J_t \left( S_d^+ s^- + S_d^- s^+ \right) + 2J_t^2 S_d^z s^z\right] + \hat O
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\sum_\beta \frac{1}{\xi} J_t^2 \hat n_{d\ol\beta}\left(1 - \hat n_{d\beta}\right) \sum_{k} = \frac{1}{\xi} J_t^2 \left(\hat n_d - \hat n_{d \uparrow} \hat n_{d \downarrow}\right) \sum_k = \frac{1}{\xi} J_t^2 \sum_k
\end{aligned}\end{equation}
...
\pb If we set \(J_z = J_t = \frac{J}{2}\), we end up with an SU(2)-symmetric model \(J \vec{S_d}\cdot\vec{s}\).
\begin{equation}\begin{aligned}
	\label{kondosym}
...
\begin{center}
	\includegraphics[scale=0.3]{../siam-symmetrical/edvsD.png}
	\captionof{figure}{Change in fixed point value of \(|\epsilon_d|\) with $D$.}
	\label{edvsD}
\end{center}
\end{minipage}
...
\begin{center}
	...
	\captionof{figure}{Fixed point phases in the plane of bare couplings, at \(V=0\).}
	\label{siamphases}
\end{center}
\subsection{Effect of \(V\) on the fixed points}
The introduction of \(V\) into the RG equations has drastic effects on the first and third quadrants of the phase diagram shown just above. The minimal change is the presence of fixed points with \(U^* = 0\) in the first and third quadrants, and the presence of fixed points with \(U^* \neq 0\) in the second and fourth quadrants. The changes in the second and fourth quadrants die off for larger bandwidths, so the results in fig.~\ref{siamphases}, for the second and fourth quadrants, would survive for a thermodynamically large system.
\pb However, for even a modest bare value of \(V\), the dominant behaviour flips in the first and third quadrants. Fixed points with \(U*=0\) form the majority of these phases, the \(U* \neq 0\) forming a much smaller subset. This difference becomes amplified in the first quadrant as the bandwidth increases, and gets stunted in the third quadrant. 
\pb The number of relevant and irrelevant fixed points as a function of the bare \(V\) are shown in fig.~\ref{Veffect}. These plots show that for \(V\) above some critical value, the dominant fixed point in all quadrants is \(U^*=0\).
\begin{center}
	\begin{minipage}{.48\textwidth}
    \centering
    \includegraphics[scale=0.5]{../figures/0q2.png}
\end{minipage}
\hspace{0.02\textwidth}
  \begin{minipage}{.48\textwidth}
    \centering
    \includegraphics[scale=0.5]{../figures/0q1.png}
\end{minipage}
  ...
  \begin{minipage}{.48\textwidth}
    \centering
    \includegraphics[scale=0.5]{../figures/0q3.png}
\end{minipage}
\hspace{0.02\textwidth}
  \begin{minipage}{.48\textwidth}
    \centering
    \includegraphics[scale=0.4]{../figures/0q4.png}
\end{minipage}
\label{Veffect}
\captionof{figure}{Change in number of fixed points as a function of \(V\). Each quadrant represents the corresponding quadrant in the phase diagram. First and third quadrants show the transition. Bare values are mentioned in plot titles. Calculations were performed for \(\omega \in \left[ - \frac{D_0}{2}, \frac{D_0}{2} \right] \) and \(U \in \left[ 0, 5 \right] \).}
\end{center}
To show that for a thermodynamically large system, the only fixed point is the \(U^*=0\) one, we plotted the ratio of the number of fixed points for \(U^*=0\) and \(U^* \neq 0\) as a function of the bare bandwidth. The ratio increases almost linearly with the bandwidth, implying that for large enough bandwidths, the fraction of fixed points with \(U^* \neq 0\) is effectively zero.
\begin{center}
	\includegraphics[scale=0.4]{../figures/Dvscount_q1.png}
	\includegraphics[scale=0.4]{../figures/Dvscount_q3.png}
	\captionof{figure}{Increase in the ratio (in the stable region, \(V \sim D_0/5\)) of irrelevant fixed points to relevant fixed points with the bare bandwidth, for the first quadrant (left pane) and the third quadrant (right pane).}
\end{center}
From the figs.~\ref{Veffect}, we can see that the transition to \(U^*=0\) happens at a critical \(V\) which depends on the values of \(J\) and \(K\). By defining the critical coupling \(V_c\) as that value of \(V\) at which the number of irrelevant and relevant fixed points are equal,we can see how this \(V_c\) varies with the bandwidth.
\begin{center}
	\includegraphics[scale=0.5]{../figures/Vc_q1.png}
	\includegraphics[scale=0.5]{../figures/Vc_q32.png}
	\captionof{figure}{Variation of the critical value \(V_c\) with bandwidth. In first quadrant (left), it vanishes as the bandwidth increases. For the third quadrant (right) however, it increases.}
\end{center}
For the fixed point in the first quadrant, it is clear that the critical \(V\) shrinks as the bandwidth increases, implying that for a large system, the only pertinent phase is the one at \(U^*=0\), because any infinitesimal \(V\) will be able to make \(U\) irrelevant. However, in the third quadrant, it becomes harder for the \(V\) to make \(U\) irrelevant as the bandwidth increases. 
\pb We can hence conclude that the only \(V\) has on the fixed point values, in the thermodynamic limit, is that it changes the value of \(U^*\) in the first quadrant of fig.~\ref{phases} to zero. The rest of the values of \(U^*\) remain unchanged.
...
\end{aligned}\end{equation}
\(S_N\) is the set of all permutations of the set \(\left\{1,2, ..., N \right\} \). \(p\) is a string of numbers \(p_1 p_2 ... p_N\), and runs through all such permutations. \(p(i)\) then refers to the \(i^{th}\) element of such a permutation. The sign factor \(\left( -1 \right) ^p\) will be \(+1\) for all even permutations and \(-1\) for the odd ones. For brevity, we can drop the index \(i\) that labels the particle and understand that the first ket in the product denotes the state of the first particle, the second that of the second particle and so on. For three particles, it looks like
\begin{equation}\begin{aligned}
...
\subsection{von-Neumann entropy of total state}
Since the total state is a pure state, it can be represented by a density matrix \(\rho = \ket{\Psi}\bra{\Psi}\), so it will have a single non-zero eigenvalue \(1\), and the VNE will be 0.

\subsection{von-Neumann entropy of intermediate reduced state}
If we trace out \(m\) particles from the total density matrix \(\rho^N\), the von-Neumann entropy \(S_m\) of the remaining density matrix \(\rho^{N-m}\) is
\begin{equation}\begin{aligned}
	S_m = \ln \frac{N(N-1)...(N-m+1)}{m!} = \ln {}^N C_m
\end{aligned}\end{equation}
This has the property \(S_{N-m} = \ln {}^N C_{N-m} = \ln {}^N C_{m} = S_m\)
\subsection{von-Neumann entropy of final reduced state}
If we trace out \(N-1\) particles from the total density matrix \(\rho^N\), the von-Neumann entropy \(S_{N-1}\) of the remaining density matrix \(\rho^{N-1}\) is
\begin{equation}\begin{aligned}
	S_m = \ln N
\end{aligned}\end{equation}

\subsection{Fermi gases and Fermi liquids}
\begin{equation}\begin{aligned}
	N_0 &= \text{number of particles on Fermi surface}\\
	N_< &= \text{number of particles inside Fermi surface}\\
	N &= N_0 + N_<
\end{aligned}\end{equation}
Trace out all particles \textit{on and above} Fermi surface.
\begin{equation}\begin{aligned}
	S_{FL} = \ln \left[{}^{N}C_{N_0}\right] = \ln \left[{}^{N}C_{N_<}\right]
\end{aligned}\end{equation}

\subsection{Marginal Fermi liquid}
\begin{equation}\begin{aligned}
	N_> &= \text{number of particles outside Fermi surface}\\
	N_0 &= \text{number of particles on Fermi surface}\\
	N_< &= \text{number of particles inside Fermi surface}\\
	N &= N_0 + N_< + N_>
\end{aligned}\end{equation}
Trace out all particles \textit{on and above} Fermi surface.
\begin{equation}\begin{aligned}
	S_{MFL} = \ln \left[{}^{N}C_{N_0 + N_>}\right] = \ln \left[{}^{N}C_{N_<}\right]
\end{aligned}\end{equation}
The change from FL is
\begin{equation}\begin{aligned}
	S_{FL} - S_{MFL} = \ln \frac{{}^{N}C_{N^{FL}_<}}{{}^{N}C_{N^{MFL}_<}} \approx \ln \frac{ N^{MFL}_<!}{N^{FL}_<!} &&\left[ \text{ using }N - m \approx N \right] 
\end{aligned}\end{equation}

\subsection{modified von-Neumann entropy of final reduced state}
If we define a modified von-Neumann entropy
\begin{equation}\begin{aligned}
	S_> = \text{Tr}_{k\geq k_F}\left[ \rho \ln \rho \right] = \sum_{k \geq k_F}\bra{k}\rho \ln \rho\ket{k}
\end{aligned}\end{equation}
which traces over only the states \text{on and above} the Fermi surface, the final reduced state will give
\begin{equation}\begin{aligned}
	\rho^{1} = \frac{1}{N}\sum_{j=1}^N \ket{k_j}\bra{k_j} \implies S^1_> = \left( N_0 + N_> \right) \times \left( -\frac{1}{N}\ln \frac{1}{N} \right) = \frac{N_0 + N_>}{N}\ln N
\end{aligned}\end{equation}



\subsection{Derivations-I}
...
\end{aligned}\end{equation}
\(S_N^{p_1}\) is the set of permutations of the set \(\left\{1, 2, ..., \hat p_1, ..., N \right\} \), where the hat on \(p_1\) means it is absent. The index which runs over the set \(S_N^{p_1}\) is \(p^1\). The factor of \(\left(-1\right)^p\) is equal to \(\left(-1\right)^{p^1}\), for the following reason. If, for some value of \(p\) with a given \(p_1\), the signature of the permutation \(p\) is negative, we can reorder the set \(S_N^{p_1}\) such that the signature of \(p^1\) is also negative. With this observation, we can write
\begin{equation}\begin{aligned}
	\label{sumofless}
...
\end{aligned}\end{equation}
is the antisymmetrised state of \(N-1\) particles, with momenta ranging from \(1\) to \(N\) excluding \(k_{p_1}\). When expanded, the equation \ref{sumofless} looks like
\begin{equation}\begin{aligned}
...
\end{aligned}\end{equation}
Continuing this procedure, if we trace out exactly \(N-1\) particles, we will get
\begin{equation}\begin{aligned}
	\rho^{1} = \frac{1}{N(N-1)...3.2}\sum_{p_1=1}^N\sum_{p_2=1\atop{p_2 \neq p_1}}^N... \sum_{p_{N-1}=1\atop{p_{N-1} \neq p_1, p_2,...,p_{N-2}}}^N\ket{\Psi^{1}(\hat p_1 \hat p_2...\hat p_{N-1})}\bra{\Psi^{1}(\hat p_1 \hat p_2...\hat p_{N-1})}
\end{aligned}\end{equation}
Note that since, out of \(N\) momenta, \(N-1\) are being excluded, the get \(\ket{\Psi^{1}(\hat p_1 \hat p_2...\hat p_{N-1})}\) has just one momentum state; it will be of the form
\begin{equation}\begin{aligned}
	\ket{\Psi^{1}(\hat p_1 \hat p_2...\hat p_{N-1})} = \ket{k}, \text{ such that }k \neq k_{p_i},  \forall \; i \in [1,N-1]
\end{aligned}\end{equation}
\(p_1\) can take \(N\) values, so \(p_2\) can take \(N-2\). We can thus replace the complicated nested summations with a single sum over all ordered strings in which we pick out \(N-1\) numbers from the list \(\left[ 1, N \right] \). We can label such a string as \(C(p_1,p_2,...,p_{N-1})\).
\begin{equation}\begin{aligned}
	\sum_{p_1=1}^N\sum_{p_2=1\atop{p_2 \neq p_1}}^N... \sum_{p_{N-1}=1\atop{p_{N-1} \neq p_1, p_2,...,p_{N-2}}}^N
\end{aligned}\end{equation}
Now, we can also label the string by the index it is missing. For example, if the string \(p_1,p_2,...,p_{N-1}\) is excluding the number \(j\), then we can label the string as \(C_j\), and a sum over \(C_j\) would then mean a sum over all the permutations which exclude \(j\). We would then have to finally sum over \(j\).
\begin{equation}\begin{aligned}
	\sum_{p_1=1}^N\sum_{p_2=1\atop{p_2 \neq p_1}}^N... \sum_{p_{N-1}=1\atop{p_{N-1} \neq p_1, p_2,...,p_{N-2}}}^N = \sum_{j=1}^N \sum_{C_j}
\end{aligned}\end{equation}
Since the ket \(\ket{\Psi^1}\) is determined completely by \(j\) (it does not depend on \(C_j\)), we can write
\begin{equation}\begin{aligned}
	\rho^{1} = \frac{1}{N!}\sum_{j=1}^N \ket{\Psi^{1}(k_j)}\bra{\Psi^{1}(k_j)}\sum_{C_j}
\end{aligned}\end{equation}
The number of \(C_j\) for a particular \(j\) is the number of possible permutations of the \(N-1\) \(p_i\), hence
\begin{equation}\begin{aligned}
	\sum_{C_j} = (N-1)!
\end{aligned}\end{equation}
so
\begin{equation}\begin{aligned}
	\rho^{1} = \frac{1}{N}\sum_{j=1}^N \ket{\Psi^{1}(k_j)}\bra{\Psi^{1}(k_j)} = \frac{1}{N}\sum_{j=1}^N \ket{k_j}\bra{k_j}
\end{aligned}\end{equation}
The von-Neumann entropy of this density matrix is
\begin{equation}\begin{aligned}
	S = \text{Tr}\left( \rho \ln \rho \right) = \sum_{i=1}^N \bra{k_i} \rho \ln \rho  \ket{k_i} = \ln N
\end{aligned}\end{equation}

\subsection{Derivation-II}
...
\end{aligned}\end{equation}
The kets \(\ket{\Psi^{N-m}(s)}\) and \(\ket{\Psi^{N-m}(s^\prime)}\) for \(s \neq s^\prime\) are orthogonal, because they will involve atleast one uncommon momentum. There are \({}^NC_m\) such states in the summation over \(s\). This means we can write \(\rho\) in the basis of these states. We can therefore write the density matrix \(\rho^{N-m}\) in this basis, and it will be diagonal, with all eigenvalues equal to
\begin{equation}\begin{aligned}
	\rho^{N-m}_s = \frac{m!}{N(N-1)...(N-m+1)}
\end{aligned}\end{equation}
The von-Neumann entropy of \(\rho^{N-m}\) is thus
\begin{equation}\begin{aligned}
	-\sum_s \rho_s \ln \rho_s &= -{}^N C_m \frac{m!}{N(N-1)...(N-m+1)} \ln \frac{m!}{N(N-1)...(N-m+1)}\\
				 &= \ln \frac{N(N-1)...(N-m+1)}{m!}\\
				 &= \ln {}^N C_m
\end{aligned}\end{equation}
\subsection{Derivation-III}
Each of the product states in eq.~\ref{slater},
\begin{equation}\begin{aligned}
	 \prod_{i=1}^N \ket{k_{p_i}}_i
\end{aligned}\end{equation}
are orthogonal to each other. That is, if \(p\) and \(q\), both members of \(S_N\) are two distinct permutations, then
\begin{equation}\begin{aligned}
	\left(\left( -1 \right)^q\prod_{j=1}^N \bra{k_{q_j}}_j\right)\left(\left( -1 \right)^p\prod_{i=1}^N \ket{k_{p_i}}_i\right) = 0, \text{ and }\left(\left( -1 \right)^p\prod_{j=1}^N \bra{k_{p_j}}_j\right)\left(\left( -1 \right)^p\prod_{i=1}^N \ket{k_{p_i}}_i\right)=1
\end{aligned}\end{equation}
This is because the two products have atleast one momentum state different. There will be \(N!\) such product states and they form a part of a particular basis of the Hilbert space.  If we label such product states as
\begin{equation}\begin{aligned}
	\ket{\Phi_p} = \left( -1 \right) ^p \prod_{i=1}^N \ket{k_{p_i}}_i
\end{aligned}\end{equation}
then
\begin{equation}\begin{aligned}
	\rho^N = \ket{\Psi^N}\ket{\Psi^N} &= \frac{1}{N!}\sum_{p,q \in S_N} \ket{\Phi_p} \bra{\Phi_q}
\end{aligned}\end{equation}
To find the von-Neumann entropy of this state, first note that the density matrix has an eigenstate
\begin{equation}\begin{aligned}
	\ket{\chi} = \sum_{r\in S_N}\ket{\Phi_r}
\end{aligned}\end{equation}
because
\begin{equation}\begin{aligned}
	\rho\ket{\chi} = \frac{1}{N!}\sum_{r\in S_N}\sum_{p,q \in S_N} \ket{\Phi_p} \bra{\Phi_q} \ket{\Phi_r}= \frac{1}{N!}\sum_{r\in S_N}\sum_{p,q \in S_N} \ket{\Phi_p} \delta_{qr} = \ket{\chi}
\end{aligned}\end{equation}
Thus, \(\rho\) has an eigenvalue \(1\), which implies that the other eigenvalues must be 0 because the trace of \(\rho\) must be 1, and all the eigenvalues must be positve-semi-definite. Therefore, the von-Neumann entropy is zero:
\begin{equation}\begin{aligned}
	S = \sum_i \rho_i \ln \rho_i = 0 + 1\times\ln 1 = 0
\end{aligned}\end{equation}
