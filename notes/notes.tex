...
\end{aligned}\end{equation}
This Green's function has only a real pole, so the state will be infinitely long-lived, signalled by a dirac-delta density of states:
\begin{equation}\begin{aligned}
...
\end{aligned}\end{equation}
\textbf{Since this impurity state is delocalised over long time scales compared to \(\tau \sim \frac{1}{\Delta}\), it gets added to the Fermi volume. Exactly one state gets added because there is only one such excitation corresponding to \(\omega = \Sigma_0\).} The Luttinger sum becomes
\begin{equation}\begin{aligned}
...
\begin{equation}\begin{aligned}
	\frac{2}{\pi}\delta(\epsilon_F) = \Delta N_v
\end{aligned}\end{equation}
where \(\Delta N_v\) is the change in Luttinger's volume. For this case, it is 1. The phase shift is thus
%
%\(n_d\) is the impurity occupancy which we have already argued to be 1, but which can also be calculated directly from the d.o.s.
%\begin{equation}\begin{aligned}
%	n_d = 2\int_{-\infty}^0 \frac{1}{\pi}\frac{\Delta}{\left(\omega - \Sigma_0 \right)^2 + \Delta^2} = \frac{2}{\pi} \cot^{-1} \left(\frac{\Sigma_0}{\Delta} \right)
%\end{aligned}\end{equation}
%The factor of 2 is the spin-degeneracy. At the strong-coupling fixed point, we have \(V \gg 1 \implies \Delta \gg 1 \implies n_d \approx 1\). Substituting this into the Friedel sum rule gives
\begin{equation}\begin{aligned}
...
\begin{equation}\begin{aligned}
	\ket{\text{cloud}} = \frac{1}{4}\ket{\uparrow^d}\frac{1}{\sqrt 2}\left(\ket{k_1 \uparrow} + \ket{k_{-1} \uparrow}\right) - \frac{1}{4}\ket{\downarrow^d}\frac{1}{\sqrt 2}\left(\ket{k_1 \downarrow} + \ket{k_{-1} \downarrow}\right) + \frac{1}{4}\ket{2^d}\ket{0} \\
	+ \frac{1}{4}\ket{0^d}\ket{k_1 \uparrow, k_1 \downarrow,k_{-1} \uparrow,k_{-1} \downarrow}
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\ket{\text{IR}} = \frac{1}{4}\bigg[\ket{\uparrow^d}\frac{1}{\sqrt 2}\left(\ket{k_1 \uparrow} + \ket{k_{-1} \uparrow}\right) - \ket{\downarrow^d}\frac{1}{\sqrt 2}\left(\ket{k_1 \downarrow} + \ket{k_{-1} \downarrow}\right) + \ket{2^d}\ket{0} \\
	+ \ket{0^d}\ket{k_1 \uparrow, k_1 \downarrow,k_{-1} \uparrow,k_{-1} \downarrow}\bigg]\ket{k_{-2} \uparrow} \ket{k_{-2} \downarrow} \ket{k_{-3} \uparrow} \ket{k_{-3} \downarrow} 
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	U_0 = \frac{1}{2}\left[ 1 + \eta_0^\dagger \right] \\
	U_1 = \frac{1}{2}\left[ 1 + \eta_1 \right] 
\end{aligned}\end{equation}
where the subscripts \(0\) and \(1\) indicate it decouples an electron from the hole and particle sectors respectively. The inverse transformations for re-entangling \(q\beta\), therefore, are
\begin{equation}\begin{aligned}
	U_0^\dagger = \frac{1}{2}\left[ 1 + \eta_0 \right] \\
	U_1^\dagger = \frac{1}{2}\left[ 1 + \eta_1^\dagger \right] 
\end{aligned}\end{equation}
The \(\eta\)-s are
\begin{equation}\begin{aligned}
	\eta_1^\dagger = V \left[\lambda_1 \hat n_{d\ol\beta} + \lambda_2 \left( 1 - \hat n_{d\ol\beta} \right)  \right]c^\dagger_{q\beta}c_{d\beta} + \lambda_3 \sum_{k} \left\{ \frac{J}{2}\left( S_d^z \beta c^\dagger_{q\beta}c_{k\beta} + c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k\ol\beta}\right) \right.\\
...
\end{aligned}\end{equation}
\begin{equation}\begin{aligned}
	\eta_1^\dagger = V^* \left[\lambda_2 \hat n_{d\ol\beta} + \lambda_1 \left( 1 - \hat n_{d\ol\beta} \right)  \right]c^\dagger_{d\beta}c_{q\beta} + \lambda_3 \sum_{k} \left\{ \frac{J}{2}\left( S_d^z \beta c^\dagger_{k\beta}c_{q\beta} + c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\right) \right.\\
