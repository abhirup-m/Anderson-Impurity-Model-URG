...
\begin{equation}\begin{aligned}
	\epsilon_1\left[\ket{\hat n_1 = 0}\bra{\hat n_1 = 0} + \ket{\hat n_1 = 2}\bra{\hat n_1 = 2}\right]
\end{aligned}\end{equation}
Here, \(\hat n_1\) is the number of electrons on the first site. This way of writing the kinetic energy piece implies that we take the singly-occupied configuration (\(\hat n_1=1\)) as the zero-energy configuration (\(\mu=0\)), and the states with 0 or 2 electrons are excitations on top of this ground state. \(\hat n_1 = 2\) is a 1-particle excitation and \(\hat n_1 = 0\) is a 1-hole excitation. Both these excitations have an energy \(\epsilon_1 - \mu = \epsilon_1\). A succint way of writing this term is
\begin{equation}\begin{aligned}
	\epsilon_1 \tau_1^2
\end{aligned}\end{equation}
where 
\begin{equation}\begin{aligned}
	\tau_1 &= \sum_\sigma \tau_{1\sigma} = \sum_\sigma \left( \hat n_{1\sigma} - \frac{1}{2} \right) = \hat n_1  - 1\\
	\implies \tau_1^2 &= \begin{cases}
		1 & \text{ when }\hat n_1 = 0,2\\
		0 & \text{ when }\hat n_1 = 1\\
	\end{cases}
\end{aligned}\end{equation}
The impurity diagonal part of \(\ham_{IR}\) will survive only when \(\hat n_2 = 1\). So we write it as
\begin{equation}\begin{aligned}
	- \frac{U^*}{2}\hat n_d + U^* \hat n_{d\ua}\hat n_{d\da} = - \frac{U^*}{2}\left(\hat n_{d\uparrow} + \hat n_{d \downarrow} - 2\hat n_{d\ua}\hat n_{d\da}\right) = - \frac{U^*}{2} \left(\hat n_{2 \uparrow} - \hat n_{2 \downarrow}\right)^2 \equiv \epsilon_2 \left(\hat n_{2 \uparrow} - \hat n_{2 \downarrow}\right)^2
\end{aligned}\end{equation}
...
\begin{equation}\begin{aligned}
	\vec S_d &\equiv \vec S_2\\
	\vec s &= \sum_{kk^\prime\atop{\alpha\beta}}c^\dagger_{k\alpha}\vec \sigma_{\alpha\beta}c_{k^\prime\beta} =\sum_{\alpha\beta}c^\dagger_{1\alpha}\vec \sigma_{\alpha\beta}c_{1\beta} \equiv \vec S_1\\
\end{aligned}\end{equation}
...
\end{aligned}\end{equation}
The total Hamiltonian for the two-site problem is
\begin{equation}\begin{aligned}
	\ham_{IR} = \epsilon_1 \tau_1^2 + \epsilon_2 \left(\hat n_{2 \uparrow} - \hat n_{2 \downarrow}\right)^2 +  V^* \sum_{\sigma}\left(c^\dagger_{1\sigma}c_{2\sigma} + \text{h.c.} \right) + J^* \vec{S_1}\cdot\vec{S_2} + K^* \vec{C_1}\cdot\vec{C_2}
\end{aligned}\end{equation}
This Hamiltonian conserves the total number operator \(\hat n_1 + \hat n_2\), so we can analyse the various subspaces corresponding to particular values of \(\hat n_1 + \hat n_2\) separately. 
