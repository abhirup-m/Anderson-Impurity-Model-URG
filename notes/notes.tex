...
\end{aligned}\end{equation}
Cyclic permutations of the labels \(x,y,z\) produce the other equations. If we set \(J^x = J^y = J^t\), we specialize to the more symmetric Kondo model with equal couplings \(J^t\) along the transverse directions: \(J^z S_d^z \sigma^z + J^t\left(S_d^+\sigma^- + S_d^- \sigma^+\right)\). Substituting this in the coupling RG equatons recovers the standard hyperbolic form of the Kondo scaling equations:
\begin{equation}\begin{aligned}
	\frac{\delta J^t}{\delta J^z} &= 2J^z J^t \frac{\rho(0)|\delta D|}{D}\\
	\delta J^z &= 2{J^t}^2 \frac{\rho(0)|\delta D|}{D}\\
	\frac{\delta J^t}{\delta J^z} &= \frac{J_z}{J_t}
\end{aligned}\end{equation}

We now consider the third order term. From here on, we will assume \(J^a = J\) for simplicity. The virtual hole three vertex term, \(V^\dagger G_0(E)XG_0(E)V\), is of the form
\begin{equation}\begin{aligned}
