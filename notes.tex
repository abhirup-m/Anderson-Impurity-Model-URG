...
\eeq
This means that the operator \il{U_q} for going from \il{\ket{,\Psi_1}} to \il{\ket{\Psi}} is described by 
\beq[inv]
U_q^{-1} = \mc{C}\rr{1+ \eta_q + \mc{O}}
\eeq
where \il{\mc{O}} is some operator function of \il{\eta_q} and \il{\eta_q^\dagger} such that 
\beq
\rr{1 + \eta_q + \mc{O}}\ket{1,\Psi_1} = \rr{1+\eta_q}\ket{1,\Psi_1} \implies \mc{O}\ket{1,\Psi_1} = 0
\eeq
and \il{\mc{C}} is a real scalar.
To find \il{\mc{O}}, note that we can write \il{U_q^{-1}} as 
\beq
U_q^{-1} = \mc{C} \ex{\eta_q + \mc{O}}
\eeq
This is because \il{\eta_q^2 = 0} and since \il{\mc{O}} is made up of \il{\eta_q} and \il{\eta_q^\dagger} is has the same property.
This exponential form allows us to write
\beq[u1]
U_q = \mc{C} \ex{-\eta_q - \mc{O}} = \mc{C}\rr{1 - \eta_q - \mc{O}}
\eeq
But, since \il{U_q} is required to be unitary, we can write, from eq.~\ref{inv},
\beq[u2]
U_q^\dagger = U_q^{-1} = \mc{C}\rr{1+\eta_q+\mc{O}} \implies U_q = \mc{C}\rr{1+\eta_q^\dagger + \mc{O}^\dagger}
\eeq
Comparing the two expression of \il{U_q} in eqs.~\ref{u1} and \ref{u2},
\beq
1 - \eta_q - \mc{O} = 1+\eta_q^\dagger + \mc{O}^\dagger \implies -\eta_q = \mc{O}^\dagger \implies \mc{O} = -\eta_q^\dagger
\eeq
This improves eq.~\ref{u1}:
\beq
U_q = \mc{C}\rr{1 + \eta_q^\dagger - \eta_q}
\eeq
To find \il{\mc{C}}, we can use the unitarity of \il{U_q}:
\beq
1 = U_q^\dagger U_q = \mc{C}^2\rr{1 + \cc{\eta_q^\dagger,\eta_q}} = \mc{C}^2\times 2 \implies \mc{C} = \fr{1}{\sqrt 2}
\eeq
...
\Delta^- \ham &= \sum_{q\beta} c_{q\beta}^\dagger \text{Tr}\rr{\ham c_{q\beta}}\eta_{q\beta}\\
	      &= \sum_{q\beta} c_{q\beta}^\dagger \text{Tr}\rr{\ham c_{q\beta}}\fr{1}{\omega_h - H_h}\text{Tr}\rr{c^\dagger_{q\beta}\ham }c_{q\beta}\\
\eeq
where we have replaced \(\hat \omega\) by its eigenvalue \(\omega\) and \(H_h = \text{Tr}\rr{\ham \rr{1- \hat n_{q\beta}}}\). The other term in the commutator gives the particle sector contribution:
\beq
\Delta^+ \ham &= \sum_{q\beta} \eta_{q\beta}c_{q\beta}^\dagger \text{Tr}\rr{\ham c_{q\beta}}\\
	      &= \sum_{q\beta} \text{Tr}\rr{c^\dagger_{q\beta}\ham }c_{q\beta}\fr{1}{\omega_e - H_e}c_{q\beta}^\dagger \text{Tr}\rr{\ham c_{q\beta}}\\
\eeq
...
\eeq
This is the piece that comes in the denominator. The off-diagonal part involving the electron on the shell is
\beq
...
\eeq
Notice that all the operators on the left of the propagator have a \(c_{q\ua}\) and those on the right have a \(c^\dagger_{q\ua}\). This combination produces a particle in the intermediate state. In this intermediate state, since we have \(\hat n_{q\ua} = 1\), we can evaluate the diagonal part:
\beq
\ham^D = \epsilon_q + \hf J_z S_d^z
\eeq
There are two terms in each bracket, so in total there are four terms. The first term has two \(S^z\).
\beq
\Delta^+_1 \ham &= \sum_{kk^\prime q} \fr{1}{4} J_z^2 S_d^z c^\dagger_{k\ua}c_{q\ua}\fr{1}{\omega - \epsilon_q - \hf J_z S_d^z} S_d^zc^\dagger_{q\ua}c_{k^\prime\ua}\\
	&= \sum_{kk^\prime q} \fr{1}{4} J_z^2 c^\dagger_{k\ua}c_{q\ua}\fr{1}{\omega - \epsilon_q - \hf J_z S_d^z} S_d^z S_d^z c^\dagger_{q\ua}c_{k^\prime\ua}\\
	&= \sum_{kk^\prime q} \fr{1}{16} J_z^2 c^\dagger_{k\ua}c_{q\ua}\fr{1}{\omega - \epsilon_q - \hf J_z S_d^z} c^\dagger_{q\ua}c_{k^\prime\ua}\\
&= \fr{1}{16} J_z^2 \sum_{kk^\prime q} c^\dagger_{k\ua}c_{q\ua}c^\dagger_{q\ua}c_{k^\prime\ua} \qq{\fr{\hf + S^z_d }{\omega - \epsilon_q - \fr{1}{4} J_z } + \fr{\hf - S^z_d}{\omega - \epsilon_q + \fr{1}{4} J_z}} \\
&= \fr{1}{32} J_z^2 \sum_{kk^\prime q} c^\dagger_{k\ua}c_{k^\prime\ua} \qq{\fr{1}{\omega - \epsilon_q - \fr{1}{4} J_z } + \fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z}} \\
	   &+ \fr{1}{16} J_z^2 \sum_{kk^\prime q} S_d^z c^\dagger_{k\ua}c_{k^\prime\ua} \qq{\fr{1}{\omega - \epsilon_q - \fr{1}{4} J_z } - \fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z}} \\
\eeq
...
\beq
\Delta^+_2 \ham &= \sum_{kk^\prime q} \fr{1}{4} J_z J_t S_d^z c^\dagger_{k\ua}c_{q\ua}\fr{1}{\omega - \epsilon_q - \hf J_z S_d^z} S_d^-c^\dagger_{q\ua}c_{k^\prime\da}
\eeq
The \(S_d^-\) in the numerator on the right means we must have \(S^z_d = -\hf\) in the denominator. 
\beq
\Delta^+_2 \ham &=  \fr{1}{4} J_z J_t\sum_{kk^\prime q} S_d^z c^\dagger_{k\ua}c_{q\ua}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} S_d^-c^\dagger_{q\ua}c_{k^\prime\da}\\
&=  \fr{1}{4} J_z J_t\sum_{kk^\prime q} S_d^z S_d^-c^\dagger_{k\ua}c_{q\ua}c^\dagger_{q\ua}c_{k^\prime\da}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} \\
&=  -\fr{1}{8} J_z J_t\sum_{kk^\prime q} S_d^-c^\dagger_{k\ua}c_{k^\prime\da}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} \\
\eeq
There we used \(S_d^z S_d^- = -\hf S_d^-\). This renormalizes \(J_t\).
\pb The third term gives
\beq
\Delta^+_3 \ham &= \sum_{kk^\prime q} \fr{1}{4} J_z J_t S_d^+ c^\dagger_{k\da}c_{q\ua}\fr{1}{\omega - \epsilon_q - \hf J_z S_d^z} S_d^z c^\dagger_{q\ua}c_{k^\prime\ua}
\eeq
The \(S_d^+\) in the numerator on the left means we must have \(S^z_d = -\hf\) in the denominator. 
\beq
\Delta^+_3 \ham &=  \fr{1}{4} J_z J_t\sum_{kk^\prime q} S_d^+ c^\dagger_{k\da}c_{q\ua}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} S_d^z c^\dagger_{q\ua}c_{k^\prime\ua}\\
&=  \fr{1}{4} J_z J_t\sum_{kk^\prime q} S_d^+ S_d^z c^\dagger_{k\da}c_{q\ua}c^\dagger_{q\ua}c_{k^\prime\ua}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} \\
&=  -\fr{1}{8} J_z J_t\sum_{kk^\prime q} S_d^+ c^\dagger_{k\da}c_{k^\prime\ua}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} \\
\eeq
There we used \(S_d^+ S_d^z = -\hf S_d^+\). This renormalizes \(J_t\).
\pb The fourth term gives
\beq
\Delta^+_4 \ham &= \sum_{kk^\prime q} \fr{1}{4} J_t^2 S_d^+ c^\dagger_{k\da}c_{q\ua}\fr{1}{\omega - \epsilon_q - \hf J_z S_d^z} S_d^- c^\dagger_{q\ua}c_{k^\prime\da}
\eeq
The \(S_d^+\) in the numerator on the left means we must have \(S^z_d = -\hf\) in the denominator. 
\beq
\Delta^+_4 \ham &=  \fr{1}{4} J_t^2 \sum_{kk^\prime q} S_d^+ c^\dagger_{k\da}c_{q\ua}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} S_d^- c^\dagger_{q\ua}c_{k^\prime\da}\\
&=  \fr{1}{4} J_t^2 \sum_{kk^\prime q} S_d^+ S_d^- c^\dagger_{k\da}c_{q\ua}c^\dagger_{q\ua}c_{k^\prime\da}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} \\
&=  \fr{1}{4} J_t^2\sum_{kk^\prime q} \rr{\hf + S_d^z} c^\dagger_{k\da}c_{k^\prime\da}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} \\
\eeq
...
\eeq
The diagonal part will be 0, because \(\hat n_{q\ua} =0\) in intermediate state. However, we will need to subtract an initial energy from \(\omega\), and that initial energy is \(E_0 = -\epsilon_q + \hf J_z S^z_d\)
The first term has two \(S^z\).
\beq
\Delta^-_1 \ham &= \sum_{kk^\prime q} S_d^zc^\dagger_{q\ua}c_{k^\prime\ua}\fr{1}{4} J_z^2 \fr{1}{\omega - E_0 + E_0} S_d^z c^\dagger_{k\ua}c_{q\ua}\\
	&= \fr{1}{16} J_z^2\sum_{kk^\prime q} c^\dagger_{k\ua}c_{k^\prime\ua} \fr{1}{\omega - \epsilon_q + \hf J_z S_d^z} \\
	&= \fr{1}{16} J_z^2\sum_{kk^\prime q} c_{k^\prime\ua}c^\dagger_{k\ua} \qq{\fr{\hf + S^z_d}{\omega - \epsilon_q + \fr{1}{4} J_z} + \fr{\hf - S^z_d}{\omega - \epsilon_q - \fr{1}{4} J_z}} \\
	&= -\fr{1}{32} J_z^2\sum_{kk^\prime q} c^\dagger_{k\ua}c_{k^\prime\ua} \qq{\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} + \fr{1}{\omega - \epsilon_q - \fr{1}{4} J_z}} \\
	&- \fr{1}{16} J_z^2\sum_{kk^\prime q} c^\dagger_{k\ua}c_{k^\prime\ua} S_d^z\qq{\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} - \fr{1}{\omega - \epsilon_q - \fr{1}{4} J_z}}\\
\eeq
...
\pb The second term gives
\beq
\Delta^-_2 \ham &= \sum_{kk^\prime q} \fr{1}{4} J_z J_t S_d^-c^\dagger_{q\ua}c_{k^\prime\da}\fr{1}{\omega - \epsilon_q + \hf J_z S_d^z} S_d^z c^\dagger_{k\ua}c_{q\ua}
\eeq
The \(S_d^-\) in the numerator on the left means we must have \(S^z_d = \hf\) in the denominator. 
\beq
\Delta^-_2 \ham &=  \fr{1}{4} J_z J_t\sum_{kk^\prime q} S_d^- S_d^z c^\dagger_{q\ua}c_{k^\prime\da}c^\dagger_{k\ua}c_{q\ua}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} \\
&=  \fr{1}{8} J_z J_t\sum_{kk^\prime q} S_d^-c_{k^\prime\da}c^\dagger_{k\ua}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} \\
\eeq
There we used \(S_d^- S_d^z = \hf S_d^-\). This renormalizes \(J_t\).
\pb The third term gives
\beq
\Delta^-_3 \ham &= \sum_{kk^\prime q} \fr{1}{4} J_z J_t S_d^z c^\dagger_{q\ua}c_{k^\prime\ua}\fr{1}{\omega - \epsilon_q + \hf J_z S_d^z} S_d^+ c^\dagger_{k\da}c_{q\ua}
\eeq
The \(S_d^+\) in the numerator on the right means we must have \(S^z_d = \hf\) in the denominator. 
\beq
\Delta^-_3 \ham &=  \fr{1}{4} J_z J_t\sum_{kk^\prime q} S_d^z S_d^+ c^\dagger_{q\ua}c_{k^\prime\ua}c^\dagger_{k\da}c_{q\ua}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} \\
&=  \fr{1}{4} J_z J_t\sum_{kk^\prime q} S_d^+ c^\dagger_{k\da}c_{k^\prime\ua}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} \\
&=  -\fr{1}{8} J_z J_t\sum_{kk^\prime q} S_d^+ c^\dagger_{k\da}c_{k^\prime\ua}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} \\
\eeq
There we used \(S_d^z S_d^+ = \hf S_d^+\). This renormalizes \(J_t\).
\pb The fourth term gives
\beq
\Delta^-_4 \ham &= \sum_{kk^\prime q} \fr{1}{4} J_t^2 S_d^- c^\dagger_{q\ua}c_{k^\prime\da}\fr{1}{\omega - \epsilon_q + \hf J_z S_d^z} S_d^+ c^\dagger_{k\da}c_{q\ua}
\eeq
The \(S_d^+\) in the numerator on the right means we must have \(S^z_d = \hf\) in the denominator. 
\beq
\Delta^-_4 \ham &=  \fr{1}{4} J_t^2 \sum_{kk^\prime q} S_d^- S_d^+ c^\dagger_{q\ua}c_{k^\prime\da} c^\dagger_{k\da}c_{q\ua}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} \\
&=  \fr{1}{4} J_t^2 \sum_{kk^\prime q} \rr{\hf - S_d^z} c_{k^\prime\da}c^\dagger_{k\da}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} \\
&=  -\fr{1}{4} J_t^2 \sum_{kk^\prime q} \rr{\hf - S_d^z} c^\dagger_{k\da}c_{k^\prime\da}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z} \\
\eeq
...
\eeq
The diagonal part (the part that appears in the denominator of the URG equations) is
\beq
\ham_D = \sum_{k<\Lambda_N,\sigma}\rr{\epsilon_k  + \sigma J_z S^z_d}\hat n_{k\sigma} + \epsilon_q \hat n_{q\beta} + \beta J_z S^z_d\hat n_{q\beta} + H_{imp}\\
\eeq
As a simplification, we will ignore the terms that pertain to the lower electrons (\(k<q\)) in \(\ham_D\).
\beq[diag]
\ham_D^+ = \epsilon_q \hat n_{q\beta} + \beta J_z S^z_d\hat n_{q\beta} + H_{imp}\\
\eeq
...
J_z S_d^z\sum_{k}\rr{\hat n_{k\ua} - \hat n_{k\da}} &=  J_z S_d^z\sum_{k}\rr{\hat \tau_{k\ua} - \hat \tau_{k\da}}
\eeq
...
\beq
\ham_D^+ = \epsilon_q \hat \tau_{q\beta} + \beta J_z S^z_d\hat \tau_{q\beta} + H_{imp}\\
\eeq
...
\eeq
The entire renormalization expression has nine terms- one of order \il{|V_q|^2}, four of order \il{V_q J} and four of order \il{J^2}. Since this state is characterized by a particle in the intermediate state, the diagonal part can be evaluated by substituting \(\tau_{q\beta}=\hf\) in eq.~\ref{diag}:
\beq
\ham_D^+ = \epsilon_q \hf + \hf \beta J_z S^z_d + H_{imp}\\
\eeq
 \paragraph{1.}
\beq
...
\beq
H_1 = \ham_D^+ 	&= \epsilon_q \hf + \hf \beta J_z S^z_d + H_{imp}\\
		&= \epsilon_q \hf - \fr{1}{4} J_z \hat n_{d\ol\beta} + \epsilon_d \hat n_{d\ol\beta}\\
\eeq
...
\Delta_1^+ \ham_N &= \sum_{q\beta}|V_q|^2 c^\dagger_{d\beta}c_{q\beta}\fr{1}{\hat \omega^+ - H_1}c^\dagger_{q\beta}c_{d\beta}\\
		  &= \sum_{q\beta}|V_q|^2 c^\dagger_{d\beta}c_{q\beta}c^\dagger_{q\beta}c_{d\beta}\fr{1}{\omega^+ - \hf \epsilon_q - \epsilon_d \hat n_{d\ol\beta} + \fr{1}{4} J_z \hat n_{d\ol\beta}}
\eeq
...
\Delta_1^+ \ham_N &= \sum_{q\beta}|V_q|^2 \hat n_{d\beta}\fr{1}{\omega^+ - \hf \epsilon_q + \rr{\fr{J_z}{2} - \epsilon_d}\hat n_{d\ol\beta}}\\
		  &= \sum_{q\beta}|V(q)|^2\hat n_{d\beta}\qq{\fr{ \hat n_{d\ol\beta}}{\omega^+ - \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z} + \fr{\rr{1 - \hat n_{d\ol\beta}}}{\omega^+ - \hf \epsilon_q}}\\
		  &= \sum_{q\beta}|V(q)|^2\hat n_{d\beta}\qq{\fr{1}{\omega^+ - \hf \epsilon_q} + \hat n_{d\ol\beta}\rr{\fr{1}{\omega^+ - \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z} - \fr{1}{\omega^+ - \hf \epsilon_q}}}\\
\eeq
\paragraph{2.}
\beq
...
\beq
H_1 = \ham_D^+ = \hf \epsilon_q + \fr{1}{4} J_z \beta S^z_d + \epsilon_d = \hf \epsilon_q - \fr{1}{4} J_z + \epsilon_d
\eeq
The first term \(\hf \epsilon_q + \fr{1}{4} J_z \beta S^z_d\) is the total dispersion of the electron \(q\beta\). The \(\epsilon_d\) is the impurity energy and the third term is the total background energy.
\beq
\Delta^+_2 \ham_N &= -\hf J_z\sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{q\beta}\hat n_{d\ol\beta}c_{q\beta}^\dagger c_{k\beta}\fr{1}{\omega^+ - H_1}\\
		  &= -\hf J_z\sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{k\beta}\fr{\hat n_{d\ol\beta}}{\omega^+ - \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z}
\eeq
\paragraph{3.}
\beq
...
\beq
H_1 = \hf \epsilon_q - \fr{1}{4} J_z + \epsilon_d 
\eeq
\beq
\Delta^+_3 \ham_N &= \sum_{q\beta k}J_t V_q^*c^\dagger_{d\beta}c_{q\beta}c^\dagger_{d\ol\beta} c_{d\beta}c_{q\beta}^\dagger c_{k\ol\beta}\fr{1}{\omega^+ - H_1}\\
		  ...
		  &= -J_t \sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{k\beta}\fr{\hat n_{d\ol\beta}}{\omega^+  - \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z}
\eeq
\paragraph{4.}
\beq
...
\Delta^+_4 \ham_N &= -\hf J_z \sum_{q\beta k}V_q \hat n_{d\ol\beta}c^\dagger_{k\beta}c_{q\beta}c_{q\beta}^\dagger c_{d\beta}\fr{1}{\omega^+ - H_1}\\
		  &= \sum_{q\beta k}-\hf J_z V_q \hat n_{d\ol\beta}\rr{1 - \hat n_{q\beta}}c^\dagger_{k\beta} c_{d\beta}\fr{1}{\omega^+ - \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z}\\
		  &= -\hf J_z\sum_{q\beta k}V_q c^\dagger_{k\beta} c_{d\beta}\fr{\hat n_{d\ol\beta}}{\omega^+ - \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z}
\eeq
\paragraph{5.}
\beq
...
\Delta^+_5 \ham_N &= \sum_{q\beta k}J_t V_q c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}c_{q\beta}^\dagger c_{d\beta}\fr{1}{\omega^+ - H_1}\\
		  &= -\sum_{q\beta k}J_t V_q \rr{1 - \hat n_{q\beta}}\hat n_{d\beta}c^\dagger_{k\ol\beta} c_{d\ol\beta}\fr{1}{\omega^+ - \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z}\\
		  &=-J_t  \sum_{q\beta k}V_q c^\dagger_{k\beta} c_{d\beta}\fr{\hat n_{d\ol\beta}}{\omega^+ - \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z}\\
\eeq
%}
\paragraph{6.}
\beq
...
\beq
H_1 &= \hf \epsilon_q + \hf \beta J_z S^z_d + H_{imp}
\eeq
...
\Delta^+_6 \ham_N &=\fr{1}{4}J_z^2 \sum_{k^\prime q\beta k}  \rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}c^\dagger_{k\beta}c_{q\beta} c^\dagger_{q\beta}c_{k^\prime\beta}\fr{1}{\omega^+ - H_1}\\
		  &=\fr{1}{4}J_z^2 \sum_{k^\prime q\beta k}  \rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}\rr{1  - \hat n_{q\beta}}c^\dagger_{k\beta}c_{k^\prime\beta} \fr{1}{\omega^+ - \hf \epsilon_q  - H_{imp}- \hf \beta J_z S^z_d}\\
		  &=\fr{1}{4}J_z^2 \sum_{k^\prime q\beta k} c^\dagger_{k\beta}c_{k^\prime\beta} \fr{\rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}}{\omega^+ - \hf \epsilon_q - \epsilon_d - \hf \beta J_z S^z_d}\\
		  &=\fr{1}{4}J_z^2\sum_{k^\prime q\beta k} c^\dagger_{k\beta}c_{k^\prime\beta}\qq{ \fr{\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}} {\omega^+ - \hf \epsilon_q - \epsilon_d - \fr{1}{4} J_z} + \fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}} {\omega^+ - \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z}}
\eeq
...
Intermediate (\il{\hat n_{d\beta}=0,\hat n_{q\beta}=\hat n_{d\ol\beta}=1}) energy is
\beq
H_1 = \hf \epsilon_q + \beta J_z S^z_d + \epsilon_d = \hf \epsilon_q - \fr{1}{4} J_z + \epsilon_d
\eeq
\beq
\Delta^+_7 \ham_N &= \sum_{q\beta kk^\prime}\hf J_zJ_t c^\dagger_{k\beta}c_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}\fr{-1}{\omega^+ - H_1}\\
		  &= -\hf J_z J_t \sum_{q\beta kk^\prime}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^+ - \hf\epsilon_{q} - \epsilon_d + \fr{1}{4} J_z}\\
		  &= -\hf J_z J_t \sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^+ - \hf\epsilon_{q} - \epsilon_d + \fr{1}{4} J_z}\\
\eeq
\paragraph{8.}
\beq
...
\Delta^+_8 \ham_N &=- \sum_{q\beta kk^\prime}\hf J_z J_t c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}c_{q\beta}^\dagger c_{k^\prime\beta}\fr{1}{\omega^+ - H_1}\\
		  &=-\hf J_z J_t \sum_{q\beta kk^\prime}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^+- \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z}\\
		  &= -\hf J_z J_t\sum_{q\beta kk^\prime}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^+ - \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z}\\
\eeq
\paragraph{9.}
\beq
...
\Delta^+_9 \ham_N &= \sum_{q\beta k k\prime}J_t^2 c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta} c_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c_{q\beta}^\dagger c_{k^\prime\ol\beta}\fr{1}{\omega^+  - H_1}\\
		  &= J_t^2\sum_{q\beta k k\prime}\rr{1 - \hat n_{q\beta}} \hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}c^\dagger_{k\ol\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^+  - \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z}\\
		  &= J_t^2\sum_{q\beta k k\prime}c^\dagger_{k\beta}c_{k^\prime\beta}\fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^+ - \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z}
\eeq
...
\Delta^+ U &=  \sum_{q}2|V(q)|^2\rr{\fr{1}{\omega^+ - \hf \epsilon_q - \epsilon_d + \fr{1}{4} {J_z}} - \fr{1}{\omega^+ - \hf \epsilon_q}}\\
\Delta^+ V &= -\rr{\hf J_z + J_t}\sum_{q}V_q\fr{1}{\omega^+ - \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z}\\
\Delta^+ J_t &= -J_z J_t\sum_{q}\fr{1}{\omega^+ - \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z}\\
\Delta^+ J_z &= -J_t^2 \sum_{q}\fr{1}{\omega^+ - \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z}
\end{flalign}
\subsection{Hole sector}
...
\eeq
\(\ham_D^-\) is the energy of the hole state. Since the hole state consists of a missing electron, the diagonal part can be evaluated by substituting \(\tau_{q\beta}=-\hf\) in eq.~\ref{diag}.
\beq
\ham_D^- = -\hf \epsilon_q -\hf \beta J_zS^z_d + H_\text{imp}
\eeq
\paragraph{1.}
\beq
...
\beq
H_1 = \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta} -\hf\epsilon_q - \hf\beta J_zS^z_d = -\hf\epsilon_q - \fr{1}{4} J_z \rr{1 - \hat n_{d\ol\beta}} + \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta}
\eeq
\beq
...
\beq
\Delta_1^- \ham_N &= \sum_{q\beta}|V_q|^2 \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}\fr{1}{\omega^- + \hf\epsilon_q - \fr{1}{4}J_z\rr{1 - \hat n_{d\ol\beta}} - \epsilon_d - \rr{\epsilon_d + U}\hat n_{d\ol\beta}} \\
		  &= \sum_{q\beta}\rr{1 - \hat n_{d\beta}}\qq{\fr{|V_q^1|^2 \hat n_{d\ol\beta}}{\omega^- + \hf\epsilon_q - 2\epsilon_d - U } + \fr{|V_q^0|^2\rr{1 - \hat n_{d\ol\beta}}}{\omega^- + \hf\epsilon_q - \epsilon_d  + \fr{1}{4} J_z}}\\
		  &= \sum_{q\beta}|V(q)|^2\left[ \hat n_{d\ol\beta}\rr{\fr{1}{\omega^- + \hf\epsilon_q - 2\epsilon_d - U} - \fr{2}{\omega^- + \hf\epsilon_q - \epsilon_d  + \fr{1}{4} J_z}} \right.\\
		  &\quad\quad\left.+ \hat n_{d\ua}\hat n_{d\da}\rr{\fr{1}{\omega^- + \hf\epsilon_q - \epsilon_d  + \fr{1}{4} J_z} - \fr{1}{\omega^- + \hf\epsilon_q - 2\epsilon_d - U}}\right]
\eeq
\paragraph{2.}
\beq
...
The intermediate (\il{\hat n_{q\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = -\hf\epsilon_q + \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta} - \fr{1}{4} J_z\rr{1 - \hat n_{d\ol\beta}}
\eeq
\beq
\Delta_2^- \ham_N &= \sum_{q\beta k}\fr{1}{2} J_z\rr{1 - \hat n_{d\ol\beta}} V_q c^\dagger_{q\beta}c_{d\beta}\rr{1 - \hat n_{d\ol\beta}}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^- - H_1}\\
		  &= -\sum_{q\beta k}\hat n_{q\beta}c^\dagger_{k\beta}c_{d\beta}\fr{V_q\fr{1}{2} J_z\rr{1 - \hat n_{d\ol\beta}}}{\omega^- + \hf\epsilon_q - \epsilon_d - \rr{\epsilon_d + U}\hat n_{d\ol\beta}  + \fr{1}{4} J_z\rr{1 - \hat n_{d\ol\beta}}}\\
		  &= -\fr{1}{2} J_z\sum_{q\beta k} V_q \hat n_{q\beta}c^\dagger_{k\beta}c_{d\beta}\fr{ \rr{1 - \hat n_{d\ol\beta}}}{\omega^- + \hf\epsilon_q - \epsilon_d  + \fr{1}{4} J_z}\\
\eeq
\paragraph{3.}
\beq
...
\beq
H_1 = \epsilon_d - \hf\epsilon_q - \hf J_z \beta S^z_d = \epsilon_d - \hf\epsilon_q - \fr{1}{4} J_z
\eeq
\beq
\Delta_3^- \ham_N &= \sum_{q\beta k}J_t V_q c^\dagger_{q\beta}c_{d\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^- - H_1}\\
		  &= \sum_{q\beta k}J_t V_q \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}c^\dagger_{k\ol\beta}c_{d\ol\beta}\fr{-1}{\omega^- + \hf\epsilon_q - \epsilon_d  + \fr{1}{4} J_z}\\
		  &= -J_t \sum_{q\beta k}V_q c^\dagger_{k\beta}c_{d\beta}\fr{1 - \hat n_{d\ol\beta}}{\omega^- + \hf\epsilon_q  - \epsilon_d  + \fr{1}{4} J_z}\\
\eeq
\paragraph{4.}
\beq
...
\Delta_4^- \ham_N &= \sum_{q\beta k}V^*_q c^\dagger_{q\beta}c_{k\beta}c^\dagger_{d\beta}c_{q\beta}\fr{\fr{1}{2} J_z\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - H_1} \\
		  &= \sum_{q\beta k}\hat n_{q\beta}V^*_q c_{k\beta}c^\dagger_{d\beta}\fr{\fr{1}{2} J_z\rr{1 - \hat n_{d\ol\beta}}}{\omega^- + \hf\epsilon_{q} - \epsilon_d - \rr{\epsilon_d + U}\hat n_{d\ol\beta}  + \fr{1}{4} J_z\rr{1 - \hat n_{d\ol\beta}}} \\
		  &= -\fr{1}{2} J_z\sum_{q\beta k}V_q^* c^\dagger_{d\beta}c_{k\beta} \fr{1 - \hat n_{d\ol\beta}}{\omega^- + \hf\epsilon_{q} - \epsilon_d  + \fr{1}{4} J_z}\\
\eeq
\paragraph{5.}
\beq
...
The intermediate (\il{\hat n_{q\beta}=\hat n_{d\ol\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = -\hf\epsilon_q + \epsilon_d - \fr{1}{4} J_z
\eeq
\beq
\Delta_5^- \ham_N &= \sum_{q\beta k}J_t V_q^* c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k\ol\beta}c^\dagger_{d\beta}c_{q\beta}\fr{1}{\hat \omega^- - H_1}\\
		  &= -J_t \sum_{q\beta k}V_q^* \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}c^\dagger_{d\ol\beta}c_{k\ol\beta}\fr{1}{\omega^- + \hf\epsilon_q - \epsilon_d + \fr{1}{4} J_z}\\
		  &= -J_t \sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{k\beta}\fr{1 - \hat n_{d\ol\beta}}{\omega^- + \hf\epsilon_q - \epsilon_d + \fr{1}{4} J_z}\\
\eeq
\paragraph{6.}
\beq
...
\beq
H_1 &= H_\text{imp} - \hf\epsilon_q - \hf \beta J_z S^z_d
\eeq
\beq
\Delta_6^- \ham_N &= \sum_{q\beta kk^\prime}\fr{J_z^2}{4}\rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}c^\dagger_{q\beta}c_{k^\prime\beta}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^- - H_1}\\
		  &=\fr{J_z^2}{4} \sum_{q\beta kk^\prime} \hat n_{q\beta}\rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}c_{k^\prime\beta}c^\dagger_{k\beta}\fr{1}{\omega^- + \hf\epsilon_q - H_\text{imp} + \hf \beta J_z S^z_d}\\
		  &= -\fr{J_z^2}{4}\sum_{q\beta kk^\prime}c^\dagger_{k\beta}c_{k^\prime\beta}\qq{\fr{\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega^- + \hf\epsilon_q - \epsilon_d + \fr{1}{4}J_z} + \fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^- + \hf\epsilon_q  - \epsilon_d - \fr{1}{4}J_z}} \\
		  &\quad+ \fr{J_z^2}{4}\sum_{q\beta k}\qq{\fr{\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega^- + \hf\epsilon_q  - \epsilon_d + \fr{1}{4}J_z} + \fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^- + \hf\epsilon_q  - \epsilon_d - \fr{1}{4}J_z}}\\
\eeq
\paragraph{7.}
\beq
...
The intermediate (\il{\hat n_{q\beta}=\hat n_{d\ol\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = \epsilon_d - \hf\epsilon_q - \fr{1}{4} J_z
\eeq
\beq
\Delta_7^- \ham_N &= \sum_{q\beta kk^\prime}\hf  J_z J_t c^\dagger_{q\beta}c_{k^\prime\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^- - H_1} \\
		  &= \sum_{q\beta kk^\prime}\hf  J_z J_t  \hat n_{q\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{-1}{\omega^- + \hf\epsilon_q - \epsilon_d  + \fr{1}{4} J_z} \\
		  &= -\hf  J_z J_t \sum_{q\beta kk^\prime}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^- + \hf\epsilon_q - \epsilon_d  + \fr{1}{4} J_z} \\
		  &= -\hf  J_z J_t \sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^- + \hf\epsilon_q - \epsilon_d  + \fr{1}{4} J_z}
\eeq
\paragraph{8.}
\beq
...
\Delta_8^- \ham_N &= \sum_{q\beta kk^\prime}\hf  J_z J_t c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^- - H_1}\\
		  &= \sum_{q\beta kk^\prime}\hf  J_z J_t \hat n_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{-1}{\omega^- + \hf\epsilon_q - \epsilon_d - \fr{1}{4}J_z}\\
		  &= -\hf  J_z J_t \sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^- + \hf\epsilon_q - \epsilon_d  + \fr{1}{4} J_z}\\
\eeq
\paragraph{9.}
\beq
...
\Delta_9^- \ham_N &= \sum_{q\beta kk^\prime}J_t^2c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}c^\dagger_{d\beta} c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^- - H_1}\\
		  ...
		  &= \sum_{q\beta kk^\prime}J_t^2\hat n_{q\beta}\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}c_{k^\prime\ol\beta}c^\dagger_{k\ol\beta}\fr{1}{\omega^- + \hf\epsilon_q - \epsilon_d  + \fr{1}{4} J_z}\\
		  &=-J_t^2 \sum_{q\beta kk^\prime}c^\dagger_{k\beta}c_{k^\prime\beta}\fr{\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega^- + \hf\epsilon_q - \epsilon_d + \fr{1}{4} J_z} + J_t^2\sum_{q k \beta}\fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^- + \hf\epsilon_q - \epsilon_d + \fr{1}{4} J_z}\\
\eeq
...
\begin{flalign*}
	\Delta^- \epsilon_d &=  \sum_{q}|V(q)|^2\rr{\fr{1}{\omega^- + \hf\epsilon_q - 2\epsilon_d - U} - \fr{2}{\omega^- + \hf\epsilon_q - \epsilon_d + \fr{1}{4} J_z}} + \\
		    &\quad\sum_{q k}\qq{\fr{\fr{1}{4}J_z^2 + J_t^2}{\omega^- + \hf\epsilon_q - \epsilon_d + \fr{1}{4}J_z} + \fr{\fr{1}{4}J_z^2}{\omega^- + \hf\epsilon_q - \epsilon_d - \fr{1}{4}J_z}}\\
	\Delta^- U &=  2\sum_{q}|V(q)|^2\rr{\fr{1}{\omega^- + \hf\epsilon_q - \epsilon_d + \fr{1}{4} J_z} - \fr{1}{\omega^- + \hf\epsilon_q - 2\epsilon_d - U}} -\\
		    &\quad2\sum_{q k}\qq{\fr{\fr{1}{4}J_z^2 + J_t^2}{\omega^- + \hf\epsilon_q - \epsilon_d + \fr{1}{4}J_z} + \fr{\fr{1}{4}J_z^2}{\omega^- + \hf\epsilon_q - \epsilon_d - \fr{1}{4}J_z}}\\
\Delta^- V &= -\rr{\fr{1}{4} J_z + J_t}\sum_{q}V_q\fr{1}{\omega^- + \hf\epsilon_q - \epsilon_d + \fr{1}{4} J_z}\\
\Delta^- J_t &= -J_z J_t\sum_{q}\fr{1}{\omega^- + \hf\epsilon_q - \epsilon_d + \fr{1}{4} J_z}\\
\Delta^- J_z &= -J_t^2 \sum_{q}\fr{1}{\omega^- + \hf\epsilon_q - \epsilon_d + \fr{1}{4} J_z}
\end{flalign*}
...
\beq
\Delta^+\gamma\equiv\Delta^+\rr{\epsilon_d + \hf U} &= \sum_q |V(q)|^2\fr{1}{\omega^+ - \hf \epsilon_q - \epsilon_d + \fr{1}{4} J_z}\\
\Delta^-\gamma\equiv\Delta^-\rr{\epsilon_d + \hf U} &= -\sum_q |V(q)|^2\fr{1}{\omega^- + \hf\epsilon_q - \epsilon_d + \fr{1}{4} J_z}
\eeq
...
\begin{flalign*}
\Delta U &= \fr{|V|^2\rr{U + \hf J}n(D)}{\rr{\omega^- - D}\rr{\omega^- - D - \hf U - \fr{1}{4}J}} - \fr{1}{8} \fr{J^2n(D)\mc{V}(D)\qq{6\rr{\omega^- - D} - 3U - J}}{\rr{\omega^- - \epsilon_q  - \hf U}^2 - \fr{1}{16}J^2}\\
\Delta V &= -\fr{3}{4}JV n(D)\fr{1}{\omega^- - D - \hf U - \fr{1}{4}J}\\
\Delta J &= -\hf J^2 n(D)\fr{1}{\omega^- - D - \hf U - \fr{1}{4}J}
\end{flalign*}
...
\beq
\Delta U &= \fr{|V|^2 U n(D)}{\rr{\omega^- - D}\rr{\omega^- - D - \hf U}}
\eeq
...
where 
\beq
P(D-\delta D) = \sum_{k_1,k_2<D-\delta D}\ket{n_{k_1}}\bra{n_{k_2}}
\eeq
...
\beq[deltaV]
\Delta \hat V = \hat V\fr{1}{E - \hat\ham_0 - \hat V}\hat V
\eeq
...
\beq[pms]
\Delta \hat V_{P} = \hat V\fr{1}{E - \hat\ham_0}\hat V
\eeq
...
\beq
\Delta \hat V_{U} = \hat V\fr{1}{\hat\ham^\prime - \hat\ham_0 - \hat V}\hat V
\eeq
...
\beq
\Delta \hat V_{U} = \hat V\fr{1}{\hat\ham^\prime - \hat V_i - \hat\ham_0 - V^D}\hat V
\eeq
We now define a new quantity \(\hat \omega = \hat\ham^\prime - \hat V_i\):
\beq[urg]
\Delta \hat V_{U} = \hat V\fr{1}{\hat\omega - \hat\ham_0 - V^D}\hat V
\eeq
...
\beq
\Delta V_P &=  c^\dagger_{d\ua}c_{q\ua}\fr{1}{E - \hat \ham_0}c^\dagger_{q\ua}c_{d\ua}\\
\eeq
...
which gives
\beq
\Delta V_P &=  c^\dagger_{d\ua}c_{q\ua}\fr{1}{E - \epsilon_q - \epsilon_d \hat n_{d\ol\beta}}c^\dagger_{q\ua}c_{d\ua}\\
&=  \fr{1}{E - \epsilon_q - \epsilon_d \hat n_{d\ol\beta}}c^\dagger_{d\ua}c_{q\ua}c^\dagger_{q\ua}c_{d\ua}\\
&= \rr{\fr{\hat n_{d\ol\beta}}{E^a - \epsilon_q - \epsilon_d } + \fr{1 - \hat n_{d\ol\beta}}{E^b - \epsilon_q }}c^\dagger_{d\ua}c_{q\ua}c^\dagger_{q\ua}c_{d\ua}\\
\eeq
