...
\beq
\wl \ham = U_q \ham U_q^\dagger = \hf\text{Tr}\qq{\ham}+  2\tau_q\text{Tr}\qq{\ham\tau_q} + 2\tau_q \cc{c_q^\dagger \text{Tr}\rr{\ham c_q},\eta_q}
\eeq
...
\begin{itemize}
	\item The scalar \(E\) in PMS refers to the energy eigenvalue of the many-body eigenstate \(\ket{\Psi}\), while the operator \(\hat \omega\) in URG contains both the renormalized Hamiltonian as well as the current purely scattering term \(V^i\). The difference is that while PMS ignores the scattering in the denominator, URG accounts for it, atleast in principle, inside the \(\hat \omega\).
	\item URG has an extra term in the denominator, the diagonal term \(V^D\) that arises from the scattering term. It is this term that makes the URG equations non-perturbative. This term is not there in PMS because the PMS result is the lowest order term in an expansion.
	\item If the scattering term \(\hat V\) is such that such a term does not appear, then the URG expression becomes more or less the same as the PMS expression and URG loses its charm in some sense.
\end{itemize}
...
\eeq
The quantity \(\xi\) now represents the energy of the many-body eigenstate above the non-interacting state. We typically take that to be very small and ignore it w.r.t the bandwidth \(\epsilon_q\).
