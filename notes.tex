...
s^-_{k,q} = c^\dagger_{k\da}c_{q\ua}\\
...
\beq
\fr{\delta_{k\sigma}}{R}\pd{E_k}{k} = \fr{J^*}{2}\sum_{q=\Lambda^*}\qq{n_{q\ol\sigma}- n_{q\sigma} + \delta n_{q\ol\sigma} - \delta n_{q\sigma} }\\
\eeq
...
\eeq
where \il{B^\prime = B+h} is the total field on the impurity spin. We can write the expression for susceptibility in terms of this total field:
\beq
