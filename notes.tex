...
\beq
H_0 \ket{0,\phi_0} + V_-\ket{1,\phi_1} &= E \ket{0,\phi_0}\\
H_0 \ket{1,\phi_1} + V_+\ket{0,\phi_0} &= E \ket{1,\phi_1}\\
\eeq
Eliminating \(\ket{1,\phi_1}\) gives
\beq
H_0 \ket{0,\phi_0} + V_-\frac{1}{E - H_0}V_+\ket{0,\phi_0} &= E \ket{0,\phi_0}\\
\eeq
...
\beq
\tilde \ham = H_0 + \underbrace{V_-\frac{1}{E - H_0}V_+ +  V_+\frac{1}{E - H_0}V_-}_\text{renormalization}
\eeq
...
\beq
\Delta \ham = \sum_{ijk}\frac{1}{E^k_0 - E^j_1}\rr{\ket{0_i}\bra{0_k}V_{ij}V^\dagger_{kj} - \ket{1_i}\bra{1_j}V^\dagger_{ki}V_{kj}}
\eeq
This is a general expression that would work irrespective of whether you are decoupling multiple electrons or a single electron. To make a better connection with URG, we next show how the PMS formalism works out for a single-electron decoupling.
...
\subsubsection{Renormalization from SWT and Comparison with SWT}
...
\eeq
Comparing with the PMS result eq.~\ref{pmsren}, it is clear that this is not the same result. This expression appears to be more general because of the presence of four terms instead of the two in PMS. If we assume that all the energies in a particular subspace are equal (\(E_0^i = E_0^k\) and \( E_1^j= E_1^k\)), this expression becomes
\beq
\Delta \ham &= \hf \sum_{ijk}\left[\frac{2}{E_0^k - E_1^j}V_{ij}V^\dagger_{kj}\ket{0_i}\bra{0_k} - \frac{2}{E_0^i - E_1^k}V^\dagger_{ij}V_{ik}\ket{1_j}\bra{1_k}\right] \\
	    &=\sum_{ijk}\frac{1}{E_0^k - E_1^j}\left[V_{ij}V^\dagger_{kj}\ket{0_i}\bra{0_k} - V^\dagger_{ki}V_{kj}\ket{1_i}\bra{1_j}\right] \\
\eeq
