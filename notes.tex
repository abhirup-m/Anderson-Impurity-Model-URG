...
\eeq
Since the only requirement on \(\ket{\psi_i}\) is that it diagonalize the Hamiltonian in the subspace of \(q\beta\), there is freedom in the choice of this state. We can exploit this freedom and choose the \(\ket{\psi_i}\) to be an eigenstate of \(\hat \omega_i\) corresponding to an eigenvalue \(\omega_i\):
\beq
\qq{\ham^d + c^\dagger_{q\beta}T + T^\dagger c_{q\beta}}\ket{\psi_i(\omega_i)} = \rr{\omega_i - \ham^d}\ket{\psi_i(\omega_i)}
\eeq
If we now substitue the expansion \ref{matpsi} and gather the terms that result in \(\hat n_{q\beta}=1\), we get
\beq[changea]
c^\dagger_{q\beta}T\ket{0}\ket{\phi_0^i} = \rr{\omega_i - \ham^d_e}\ket{1}\ket{\phi_1^i}
\eeq
Similarly, gathering the terms that result in \(\hat n_{q\beta}=0\) gives
\beq[changeb]
T^\dagger c_{q\beta}\ket{1}\ket{\phi_1^i} = \rr{\omega_i - \ham^d_h}\ket{0}\ket{\phi_0^i}
\eeq
We now define two many-particle transition operators:
\beq[etadagdef]
{\eta_e}^\dagger(\omega_i) = \fr{1}{\omega_i- \ham^d_e}c^\dagger_{q\beta}T \\
\eta_h(\omega_i) = \fr{1}{\omega_i- \ham^d_h}T^\dagger c_{q\beta}
\eeq
In terms of these operators, eqs.~\ref{changeb} become
\beq
...
\beq
U_1 \ham U_1^\dagger &= \hf\rr{1+\eta_h^\dagger - \eta_h}\ham\rr{1 + \eta_h - \eta^\dagger_h}\\
				&= \hf\rr{1+\eta_h^\dagger - \eta_h}\rr{\ham + \ham\eta_h - \ham\eta_h^\dagger}\\
				&=\hf\rr{\ham+ \ham\eta_h - \ham\eta_h^\dagger + \eta_h^\dagger \ham + \eta_h^\dagger\ham\eta_h - \eta_h^\dagger \ham\eta_h^\dagger - \eta_h\ham - \eta_h \ham \eta_h + \eta_h \ham \eta_h^\dagger}\\
&=\hf\rr{\ham^D + \ham^i + \ham^I + \ham\eta_h - \ham\eta_h^\dagger + \eta_h^\dagger \ham + \eta_h^\dagger\ham\eta_h - \eta_h^\dagger \ham\eta_h^\dagger - \eta_h\ham - \eta_h \ham \eta_h + \eta_h \ham \eta_h^\dagger}\\
&=\hf\rr{\ham^D + \ham^i + \ham^I + \qq{\eta^\dagger_h - \eta_h,\ham} + \eta_h^\dagger\ham\eta_h - \eta_h^\dagger \ham\eta_h^\dagger - \eta_h \ham \eta_h + \eta_h \ham \eta_h^\dagger}\\
\eeq
...
\beq
 \tilde \ham &= \hf\rr{\underbrace{\ham^D + \ham^i + \qq{\eta^\dagger_h - \eta_h,\ham} + \eta_h^\dagger\ham\eta_h + \eta_h \ham \eta_h^\dagger}_\text{group 1} + \overbrace{\ham^I - \eta_h^\dagger \ham\eta_h^\dagger - \eta_h \ham \eta_h}^\text{group 2}}\\
\eeq
Group 2 consists of purely off-diagonal terms; they amount to 0. To see how, note that terms that have two \il{\eta_h} or two \il{\eta_h^\dagger} can only be nonzero if the intervening \il{\ham} has an odd number of creation or destruction operators.
\beq[beats]
\eta_h \ham \eta_h = \eta_h c_q^\dagger  T \eta_h
\eeq
and
\beq[tora]
\eta_h^\dagger \ham\eta_h^\dagger = \eta_h^\dagger T^\dagger c_q \eta_h^\dagger
\eeq
Group 2 becomes
\beq
\text{group 2} = \ham^I - \eta_q^\dagger T^\dagger c_q \eta_q^\dagger - \eta_q c_q^\dagger  T \eta_q = c^\dagger_q T + T^\dagger c_q - \eta_q^\dagger T^\dagger c_q \eta_q^\dagger - \eta_q c_q^\dagger  T \eta_q
\eeq
To simplify this, we use the relation
\beq
\eta_h c_q^\dagger  T \eta_h &= T^\dagger c_q \fr{1}{\hat \omega - H_e \hat n_q}c_q^\dagger  T \eta_h\\
			      &= T^\dagger c_q \rr{\fr{1}{\hat \omega - H_e \hat n_q}c_q^\dagger  T} \eta_h\\
			      &= T^\dagger c_q \eta_h^\dagger\eta_h &&\qq{\text{eq.~\ref{etadagdef}}}\\
