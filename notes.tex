...
Intermediate (\il{\hat n_{q\beta}=1, \hat n_{d\ol\beta}=1,\hat n_{d\beta}=0}) energy is
\beq
H_1 = \epsilon_q + J_z \beta S^z_d + \epsilon_d = \epsilon_q - \hf J_z + \epsilon_d
\eeq
...
\eeq
\beq
\Delta^+_2 \ham_N &= -\hf J_z\sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{q\beta}\hat n_{d\ol\beta}c_{q\beta}^\dagger c_{k\beta}\fr{1}{\omega^+ - H_1 - \epsilon_q + \hf J_z}\\
		  &= -\hf J_z\sum_{q\beta k}c^\dagger_{d\beta}c_{k\beta}\fr{\hat n_{d\ol\beta}{V_q^1}^*}{\omega^+ - \epsilon_q + \hf J_z}
\eeq
\paragraph{3.}
\beq
\Delta^+_3 \ham_N = \sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+}J_t c^\dagger_{d\ol\beta}c_{d\beta} c_{q\beta}^\dagger c_{k\ol\beta}
...
\eeq
\beq
\Delta^+_3 \ham_N &= \sum_{q\beta k}J_t V_q^*c^\dagger_{d\beta}c_{q\beta}c^\dagger_{d\ol\beta} c_{d\beta}c_{q\beta}^\dagger c_{k\ol\beta}\fr{1}{\omega^+ - H_1  - \epsilon_q + \hf J_z}\\
		  &= \sum_{q\beta k}-J_t V_q^*\hat n_{d\beta}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\ol\beta}c_{k\ol\beta}\fr{1}{\omega^+ - H_1  - \epsilon_q + \hf J_z}\\
		  &= -J_t \sum_{q\beta k}c^\dagger_{d\beta}c_{k\beta}\fr{{V^1_q}^*\hat n_{d\ol\beta}}{\omega^+  - \epsilon_q + \hf J_z}
\eeq
\paragraph{4.}
\beq
\Delta^+_4 \ham_N = \sum_{q\beta k\sigma} J_z \beta S^z_dc^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+}V_qc_{q\beta}^\dagger c_{d\beta}
...
The initial (\il{\hat n_{q\beta}=0,\hat n_{d\beta}=\hat n_{d\ol\beta}=1}) energy is
\beq
H_0 = 2\epsilon_d + U = H_1 + \epsilon_d + U - \epsilon_q + \hf J_z
\eeq
\beq
\Delta^+_4 \ham_N &= \sum_{q\beta k}-\hf J_z V_q \hat n_{d\ol\beta}c^\dagger_{k\beta}c_{q\beta}c_{q\beta}^\dagger c_{d\beta}\fr{1}{\omega^+ - H_0 + \epsilon_d + U - \epsilon_q + \hf J_z}\\
		  &= \sum_{q\beta k}-\hf J_z V_q \hat n_{d\ol\beta}\rr{1 - \hat n_{q\beta}}c^\dagger_{k\beta} c_{d\beta}\fr{1}{\omega^+ + \epsilon_d + U - \epsilon_q + \hf J_z}\\
		  &= -\hf J_z\sum_{q\beta k}c^\dagger_{k\beta} c_{d\beta}\fr{V_q^1 \hat n_{d\ol\beta}}{\omega^+ - \epsilon_q + \epsilon_d + U + \hf J_z}
\eeq
\paragraph{5.}
\beq
\Delta^+_5 \ham_N = \sum_{q\beta k\sigma}J_t c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+}V_q c_{q\beta}^\dagger c_{d\beta}
...
Intermediate (\il{\hat n_{d\beta}=0,\hat n_{q\beta}=\hat n_{d\ol\beta}=1}) energy is
\beq
H_1 = \epsilon_q - \hf J_z + \epsilon_d + \mc{E}_0
\eeq
The initial (\il{\hat n_{q\beta}=0,\hat n_{d\beta}=\hat n_{d\ol\beta}=1}) energy is
\beq
H_0 = 2\epsilon_d + U = H_1 + \epsilon_d + U - \epsilon_q + \hf J_z
\eeq
\beq
\Delta^+_5 \ham_N &= \sum_{q\beta k}J_t V_q c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}c_{q\beta}^\dagger c_{d\beta}\fr{1}{\omega^+ - H_0 + \epsilon_d + U - \epsilon_q + \hf J_z}\\
		  &= -\sum_{q\beta k}J_t V_q \rr{1 - \hat n_{q\beta}}\hat n_{d\beta}c^\dagger_{k\ol\beta} c_{d\ol\beta}\fr{1}{\omega^+ + \epsilon_d + U - \epsilon_q + \hf J_z}\\
		  &=-J_t  \sum_{q\beta k}c^\dagger_{k\beta} c_{d\beta}\fr{V_q^1 \hat n_{d\ol\beta}}{\omega^+ - \epsilon_q + \epsilon_d + U + \hf J_z}\\
\eeq
%}
\paragraph{6.}
\beq
...
&= \sum_{q\beta}|V_q|^2 \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}\fr{1}{\omega^- - \epsilon_q + \epsilon_d + U\hat n_{d\ol\beta} - \fr{J_z}{2}\rr{1 - \hat n_{d\ol\beta}}} \\
 &= \sum_{q\beta}\rr{1 - \hat n_{d\beta}}\qq{\fr{|V_q^1|^2 \hat n_{d\ol\beta}}{\omega^- - \epsilon_q + \epsilon_d + U } + \fr{|V_q^0|^2\rr{1 - \hat n_{d\ol\beta}}}{\omega - \epsilon_q + \epsilon_d - \fr{J_z}{2}}}\\
 &= \sum_{q\beta}\left[ \hat n_{d\ol\beta}\rr{\fr{|V_q^1|^2}{\omega - \epsilon_q + \epsilon_d + U} - \fr{2|V_q^0|^2}{\omega - \epsilon_q + \epsilon_d - \fr{J_z}{2}}} \right.\\
 &\left.+ \hat n_{d\ua}\hat n_{d\da}\rr{\fr{|V_q^0|^2}{\omega - \epsilon_q + \epsilon_d - \fr{J_z}{2}} - \fr{|V_q^1|^2}{\omega - \epsilon_q + \epsilon_d + U}}\right]
\eeq
\paragraph{2.}
\beq
\Delta_2^- \ham_N &= \sum_{q\beta k} V_q c^\dagger_{q\beta}c_{d\beta}\fr{1}{\hat \omega^- - \ham_D^-} J_z \beta S^z_d c^\dagger_{k\beta}c_{q\beta}
...
\beq
H_0 &= \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta}\\
    &= H_1 + \epsilon_q + \hf J_z\rr{1 - \hat n_{d\ol\beta}}
\eeq
\beq
\Delta_2^- \ham_N &= \sum_{q\beta k}\hf J_z\rr{1 - \hat n_{d\ol\beta}} V_q c^\dagger_{q\beta}c_{d\beta}\rr{1 - \hat n_{d\ol\beta}}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^- + H_1}\\
&= -\sum_{q\beta k}\hat n_{q\beta}c^\dagger_{k\beta}c_{d\beta}\fr{V_q\hf J_z\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q - \hf J_z\rr{1 - \hat n_{d\ol\beta}}}\\
&= -\sum_{q\beta k}\hat n_{q\beta}c^\dagger_{k\beta}c_{d\beta}\fr{\hf J_z V_q^0 \rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q - \hf J_z}\\
\eeq
\paragraph{3.}
\beq
\Delta_3^- \ham_N &= \sum_{q\beta k}V_q c^\dagger_{q\beta}c_{d\beta}\fr{1}{\hat \omega^- - \ham_D^-}J_t c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}
...
The initial state (\il{\hat n_{q\beta}=\hat n_{d\ol\beta}=1,\hat n_{d\beta}=0}) energy is
\beq
H_0 &= \epsilon_{q} + \epsilon_d = H_1 + \epsilon_q + \hf J_z
\eeq
\beq
\Delta_3^- \ham_N &= \sum_{q\beta k}J_t V_q c^\dagger_{q\beta}c_{d\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^- + H_1}\\
		  &= \sum_{q\beta k}J_t V_q \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}c^\dagger_{k\ol\beta}c_{d\ol\beta}\fr{-1}{\omega^- - \epsilon_q - \hf J_z}\\
		  &= -J_t \sum_{q\beta k}c^\dagger_{k\beta}c_{d\beta}\fr{V_q^0 \rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q - \hf J_z}\\
\eeq
\paragraph{4.}
\beq
\Delta_4^- \ham_N &= \sum_{q\beta k}\hf J_z \beta S^z_d c^\dagger_{q\beta}c_{k\beta}\fr{1}{\hat \omega^- - \ham_D^-}V^*_q c^\dagger_{d\beta}c_{q\beta}
...
\beq
H_0 &= \epsilon_d \hat n_{d\ol\beta}\\
    &= H_1 + \epsilon_{q} + \hf J_z\rr{1 - \hat n_{d\ol\beta}} - \epsilon_d - U\hat n_{d\ol\beta}\\
\eeq
\beq
\Delta_4^- \ham_N &= \sum_{q\beta k}V^*_q c^\dagger_{q\beta}c_{k\beta}c^\dagger_{d\beta}c_{q\beta}\fr{\hf J_z\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - H_0 - \epsilon_{q} - \hf J_z\rr{1 - \hat n_{d\ol\beta}} + \epsilon_d + U\hat n_{d\ol\beta}} \\
		  &= \sum_{q\beta k}\hat n_{q\beta}V^*_q c_{k\beta}c^\dagger_{d\beta}\fr{\hf J_z\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_{q} - \hf J_z\rr{1 - \hat n_{d\ol\beta}} + \epsilon_d + U\hat n_{d\ol\beta}} \\
		  &= -\sum_{q\beta k} c^\dagger_{d\beta}c_{k\beta} \fr{{V^0_q}^* \hf J_z\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_{q} - \hf J_z+ \epsilon_d}\\
\eeq
\paragraph{5.}
\beq
\Delta_5^- \ham_N &= \sum_{q\beta k}J_t  c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k\ol\beta}\fr{1}{\hat \omega^- - \ham_D^-}V_q^* c^\dagger_{d\beta}c_{q\beta}
...
\eeq
\beq
\Delta_5^- \ham_N &= \sum_{q\beta k}J_t V_q^* c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k\ol\beta}c^\dagger_{d\beta}c_{q\beta}\fr{1}{\hat \omega^- - \epsilon_q + \epsilon_d - \hf J_z}\\
		  ...
		  &= -J_t \sum_{q\beta k}c^\dagger_{d\beta}c_{k\beta}\fr{{V_q^0}^* \rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q + \epsilon_d- \hf J_z}\\
\eeq
\paragraph{6.}
\beq
\Delta_6^- \ham_N &= \sum_{q\beta kk^\prime}J_z \beta S^z_d c^\dagger_{q\beta}c_{k^\prime\beta}\fr{1}{\hat \omega^- - \ham_D^-}J_z \beta S^z_d c^\dagger_{k\beta}c_{q\beta}
...
\Delta J_t &= - J_z J_t \sum_q \rr{\fr{1}{\omega^+ - \epsilon_q  + \hf J_z} + \fr{1}{\omega^- - \epsilon_q  - \hf J_z}}
\end{align*}
\subsubsection{SU(2) invariance and Kondo model one-loop form}
Setting \(J_z = J_t = \hf J\) makes the interaction \(SU(2)\) symmetric; the last two RG equations can then be written in the common form:
\beq
2\Delta J_z = 2 \Delta J_t = \Delta J &= - \hf J^2 \sum_q \rr{\fr{1}{\omega^+ - \epsilon_q  + \fr{1}{4}J} + \fr{1}{\omega^- - \epsilon_q  - \fr{1}{4}J}}
\eeq
...
\Delta J_t &= -  J_z J_t \sum_q \rr{\fr{1}{\omega - \epsilon_q  + \hf J_z} + \fr{1}{\omega - \epsilon_q  - \hf J_z}}
\end{align*}
\subsubsection{Scaling equations for p-h and SU(2) symmetry and \(\omega = 0\)}
\begin{align*}
	\Delta \epsilon_d &= -\sum_q \left[\frac{|V_q^0|^2}{\epsilon_q - \epsilon_d } + \fr{|V_q^1|^2}{\epsilon_q - \epsilon_d} - \fr{2|V_q^0|^2}{\epsilon_q + \epsilon_d - \fr{1}{4}J} + \fr{J^2}{16}\sum_{qk}\rr{\fr{5}{\epsilon_q - \fr{1}{4}J} + \fr{1}{\epsilon_q + \fr{1}{4}J}}\right]\\
	\Delta U &=- \sum_q 2\left[\fr{|V_q^1|^2 + |V_q^0|^2}{\epsilon_q + \epsilon_d - \fr{1}{4}J} - \fr{|V_q^1|^2 + |V_q^0|^2}{\epsilon_q - \epsilon_d} - \fr{J^2}{8}\sum_{qk}\rr{\fr{5}{\epsilon_q - \fr{1}{4}J} + \fr{1}{\epsilon_q + \fr{1}{4}J}}\right]\\
	\Delta V_1 &= \sum_q\fr{3}{4}J V_1(q)\fr{1}{ \epsilon_q + \epsilon_d - \fr{1}{4}J}\\
	\Delta V_0 &= \sum_q\fr{3}{4}J V_0(q)\fr{1}{ \epsilon_q - \fr{1}{4}J}\\
	\Delta J &= \fr{1}{2}J^2 \sum_q \fr{\epsilon_q}{\epsilon_q^2 - \fr{1}{16}J^2}\\
\end{align*}
\end{document}
