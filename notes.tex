...
In 1975, Kenneth Wilson solved the problem by using his numerical renormalization group method in which he iteratively diagonalized chains of@thesis{ID,
	author = {author},
	title = {title},
	type = {type},
 ...
 increasing length to go to the low energy physics.
...
\beq
\hf \qq{S,\ham_X} = -\hat n_{d\beta}|V|^2\qq{\frac{1 - \hat n_{d\ol\beta}}{\epsilon_q - \epsilon_d} + \frac{\hat n_{d\ol\beta}}{\epsilon_q - \epsilon_d - U}}
\eeq
This is \textit{identical to the result obtained from PMS} of the SIAM (see \cite{hewson}). This shows that the extra terms that are generated in SWT (the s-d term) appear \textit{only because the connection between the impurity subspaces are integrated out in one shot.} This relation is further substantiated in the next subsection.
...
\eeq
The differences between the perturbative and non-perturbative ways are summarized in the table below.
\begin{center}
...
\begin{tabular}{|c|c|c|}
	...
	\hline
		...
		URG&\(- \frac{4}{\pi^2}S\ham_0 S  + \frac{2}{\pi}\qq{S,\ham_X}\)&\(\qq{S,\ham_0} = -\frac{\pi}{4}\ham_X + \frac{4}{\pi}S \ham_X S\)\\
	\hline
%\captionof{table}{Comparison of perturbative and non-perturbative canonical transformations}
\end{tabular}
\end{center}
...
\beq
\ham_{ch} = \sum_{kq}\qq{V_q V_kc^\dagger_{q\ua}c^\dagger_{k\da}c_{d\da}c_{d\ua}\rr{\frac{1}{\epsilon_d + U - \epsilon_k} + \frac{1}{\epsilon_d + U - \epsilon_q}} + V_q^*V_k^* \rr{\frac{1}{\epsilon_k - \epsilon_d} + \frac{1}{\epsilon_q - \epsilon_d}}c^\dagger_{d\ua}c^\dagger_{d\da}c_{k\da}c_{q\ua}}
\eeq
