...
Later calculations with Bethe ansatz in 1980 by Andrei and Weigmann \cite{Andrei},\cite{weigmann} corroborated Wilson's findings.
...
\beq
{\eta}^\dagger(\omega_i^1) = \fr{1}{\omega_i^1- \ham^d}c^\dagger_{q\beta}T \\
\eta(\omega_i^0) = \fr{1}{\omega_i^0- \ham^d}T^\dagger c_{q\beta}
\eeq
We can write this compactly as
\beq[etadagdef]
\eta(\hat \omega) = \fr{1}{\hat \omega_i- \ham^d}T^\dagger c_{q\beta}
\eeq
	where \(\hat \omega_i = \omega_i^0\rr{1 - \hat n_{q\beta}} + \omega_i^1\hat n_{q\beta} = \begin{pmatrix} \omega_i^1 & \\ & \omega_i^0 \end{pmatrix}\) is a 2x2 matrix and \(\ham^d = \ham^d_0\rr{1 - \hat n_{q\beta}} + \ham^d_1\hat n_{q\beta}\). It is easy to check that this reproduces the previous forms of \(\eta_0\) and \(\eta_1^\dagger\).
