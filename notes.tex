...
\eeq
The remaining integral gives \il{-\fr{\pi^2}{3}}. Using \il{\rho_\ua = \rho_\da = \rho_d}, we get
\beq
...
\beq
m &= \delta n_\ua - \delta n_\da\\
  &= \sum_\sigma \delta n_\sigma\\
  &= \sum_{k\sigma} \pd{n_\sigma}{\epsilon_{k\sigma}}\delta \epsilon_{k\sigma}\\
  &= \sum_{k\sigma} \rho_{k\sigma}\rr{\sigma h + U\delta n_{- \sigma}}\\
\eeq
...
Substituting this in the magnetisation gives
\beq
m &= \sigma h\sum_{k\sigma} \rho_{k\sigma}\rr{1 + U\rho_{-\sigma}(0)}\\
  &= \sigma h\rho_{\sigma}(0)\qq{1 + U\rho_{-\sigma}(0)
}
\eeq
The susceptibility is
\beq
\chi_\text{imp} = \lim_{h \to 0} \fr{m}{h} = \rho_d(0)\qq{1 + U\rho_d(0)}
\eeq
There I used the fact that in the absence of any field, \il{\rho_\ua = \rho_\da = \rho_d}.
The Wilson ratio is
\beq
R = 1+U\rho_d(0)
\eeq
...
\beq
G_d(\omega) = \fr{1}{\omega - \epsilon_d - i\Delta - \Sigma_I(\omega)}
\eeq
Expanding the self energy up to first order in energy gives
\beq
\Sigma_I = \Sigma_I(0) + \omega \Sigma_I^\prime \equiv \Sigma_I(0) + \rr{1 - Z^{-1}}\omega
\eeq
where \il{Z = \rr{1 - \Sigma_I^\prime}^{-1}}. Substituting this in \il{G_d(\omega)} gives
\beq[impg]
G_d(\omega) &= \fr{1}{\omega - \epsilon_d - i\Delta - \Sigma_I(0) - \rr{1 - Z^{-1}}\omega}\\
	  &= \fr{Z}{Z\omega - Z\epsilon_d - iZ\Delta - Z\Sigma_I(0) - Z\omega + \omega}\\
	  &= \fr{Z}{\omega - Z\rr{\epsilon_d + \Sigma_I(0)} - iZ\Delta }\\
...
\eeq
In the particle-hole symmetric case (\il{\epsilon_d = 0}), the density of states reduces to
\beq
\rho_d = \fr{1}{\pi\Delta}
\eeq
So the condition eq.~\ref{nosusc} gives \il{U = \pi \Delta}. This can be substituted in the Wilson ratio to give
\beq
R = 1 + \sin^2 \delta(0)
\eeq
From the Friedel sum rule, we can relate the phase shift \il{\delta(0)} to the number of electrons bound within the singlet, \il{\Delta N\equiv N - N^{(0)}}.
\beq
\Delta N = \fr{1}{\pi} \sum_\sigma \delta_\sigma
\eeq
The incoming electrons can have \il{\sigma = \ua,\da}. Since the impurity singlet ground state is rotationally invariant, we have \il{\delta_\ua = \delta_\da = \delta(0)}.\beq
\Delta N = \fr{1}{\pi}2\delta(0) \implies \delta(0) = \fr{\pi}{2}\Delta N
\eeq
\beq
R = 1 + \sin^2 \rr{\fr{\pi}{2}\Delta N}= 1 + \sin^2 \rr{\fr{\pi}{2}n_D(\Gamma)}
\eeq
\pagebreak
...
\begin{gather}
	...
	\implies T_K = D\sqrt{\fr{\Delta}{\epsilon_d}}\ex{\fr{\epsilon_d}{2\Delta}}\label{jean}
\end{gather}
...
\eeq
The effective Hamiltonian obtained after disentangling momentum+spin states on the shell \il{\Lambda_j} is
\beq[dec]
\ham_{j-1} = 2^{-N}\text{Tr}_{1,2,...N}\ham_j + \sum_{l=1}^{N}\tau_l\cc{c^\dagger_{l}\text{Tr}_l\qq{\ham_j c_l},\eta_l}
\eeq
...
\beq
\text{Tr}_{1,2,...N}\ham = \sum_{n_1=0,1;n_2=0,1;...;n_N=0,1} \bra{n_1}\bra{n_2}...\bra{n_N} \ham \ket{n_N}...\ket{n_1}
\eeq
...
\eeq
\il{\sum^<} means the sum is over all states below the shell \il{\Lambda_j} while \il{\sum^=} means the sum is over all states pn the shell \il{\Lambda_j}. Now I need to compute the second term. That term involves another trace, \il{\text{Tr}_l\qq{\ham c_l}}. The kinetic energy term will not survive this. The only terms that will survive are those that have exactly one \il{c_{q\beta}^\dagger}. 
\beq
...
\beq
\ham = \sum_{k,\sigma} \epsilon_k \hat n_{k\sigma} + \fr{J^*}{2}\sum_{k_1,k_2 < \Lambda^*\atop{\alpha,\beta}} \mb{S}\cdot \mb{\sigma}_{\alpha\beta} c^\dagger_{k_1,\alpha} c_{k_2,\beta} + \sum_{k>\Lambda^*} J_k S^z s^z_k
\eeq
The zero-mode Hamiltonian is
\beq
\ham_\text{coll} &= \fr{J^*}{2}\sum_{k_1,k_2 < \Lambda^*\atop{\alpha,\beta}} \mb{S}\cdot \mb{\sigma}_{\alpha\beta} c^\dagger_{k_1,\alpha} c_{k_2,\beta} + \sum_{k>\Lambda^*} J_k S^z s^z_k + \sum_{k > \Lambda^*,\sigma}\epsilon_k\hat n_{k\sigma}\\
		 &= J^* \mb{S}\cdot \mb{s} + S^z \sum_{k>\Lambda^*} J_k s^z_k+ \sum_{k > \Lambda^*,\sigma}\epsilon_k\hat n_{k\sigma}
\eeq
where
\beq
...
The ground state is
\beq
\ket{\Psi^*} = \sum_{|\mb{k}|<\Lambda^*\atop{\mb{q}\neq \mb{k}}}\rr{\ket{\ua_d}\ket{\da_\mb{k}} \otimes \ket{s^z_\mb{q}=m}- \ket{\da_d}\ket{\ua_\mb{k}}\otimes \ket{s^z_\mb{q}=-m}}
\eeq
which is verified easily:
\beq
S^z \sum_{k>\Lambda^*} J_k s^z_k \ket{\Psi^*} &= \sum_{k>\Lambda^*} J_k \sum_{|\mb{k}|<\Lambda^*\atop{\mb{q}\neq \mb{k}}}\rr{S^z\ket{\ua_d}\ket{\da_\mb{k}} \otimes s^z_k\ket{m}- S^z\ket{\da_d}\ket{\ua_\mb{k}}\otimes s^z_k\ket{-m}}\\
					      &=-J_qm \sum_{|\mb{k}|<\Lambda^*\atop{\mb{q}\neq \mb{k}}}\rr{\ket{\ua_d}\ket{\da_\mb{k}} \otimes \ket{m}- \ket{\da_d}\ket{\ua_\mb{k}}\otimes \ket{-m}}\\
					      &=-J_q \ket{\Psi^*}
\eeq
\beq
S^z s^z\ket{\Psi^*} &= \sum_{|\mb{k}|<\Lambda^*\atop{\mb{q}\neq \mb{k}}}\rr{S^z \ket{\ua_d}s^z\ket{\da_\mb{k}} \otimes \ket{m}- S^z\ket{\da_d}s^z\ket{\ua_\mb{k}}\otimes \ket{-m}}\\
		    &= -N_\text{shell}\ket{\Psi^*}
\eeq
\beq
\rr{S^+ s^- + S^- s^+}\ket{\Psi^*} &= \sum_{|\mb{k}|<\Lambda^*\atop{\mb{q}\neq \mb{k}}}\rr{S^- \ket{\ua_d}s^+\ket{\da_\mb{k}} \otimes \ket{m}- S^+\ket{\da_d}s^-\ket{\ua_\mb{k}}\otimes \ket{-m}}\\
		    &= -N_\text{shell}\ket{\Psi^*}
\eeq
Therefore,
\beq
\ham_\text{coll} \ket{\Psi^*} &= -\rr{J^* N_\text{shell} + J_q}\ket{\Psi^*}
\eeq
...
\beq
\ket{\Psi} = a_\da \ket{\ua_\text{imp}}\ket{\da} + a_\ua\ket{\da_\text{imp}}\ket{\ua} 
\eeq
Note that \il{\ket{\ua},\ket{\da}} are many-electron states. The eigenvalue equation for this state is
\beq
	\fr{1}{J^*}\rr{E_0 - \ham^*_0}\ket{\Psi} &= \rr{S^z s^z + \fr{1}{2}S^+s^- + \fr{1}{2}S^- s^+}\rr{a_\da \ket{\ua_\text{imp}}\ket{\da} + a_\ua\ket{\da_\text{imp}}\ket{\ua}}\\
						 &=\fr{s^z}{2}\rr{a_\da \ket{\ua_\text{imp}}\ket{\da} - a_\ua\ket{\da_\text{imp}} \ket{\ua}} + a_\da \ket{\da_\text{imp}}\fr{s^+}{2}\ket{\da} + a_\ua \ket{\ua_\text{imp}}\fr{s^-}{2} \ket{\ua}
\eeq
Comparing the coefficients of \il{\ket{\ua_\text{imp}}} and \il{\ket{\da_\text{imp}}}, we get
\beq
	\fr{1}{J^*}\rr{E_0 - \ham^*_0}a_\da\ket{\da} &= \fr{s^z}{2}a_\da \ket{\da} + a_\ua \fr{s^-}{2} \ket{\ua}\\
	\fr{1}{J^*}\rr{E_0 - \ham^*_0}a_\ua\ket{\ua} &= -\fr{s^z}{2}a_\ua \ket{\ua} + a_\da \fr{s^+}{2} \ket{\da}
\eeq
From the first equation, we can isolate \il{\ket{\da}}:
\beq
\ket{\da} = \qq{\fr{1}{J^*}\rr{E_0 - \ham^*_0} + 1}^{-1}\fr{a_\ua}{a_\da} s^-\ket{\ua}
\eeq
Substituting this in the second equation gives
\beq
\fr{1}{J^*}\rr{E_0 - \ham^*_0}\ket{\ua} &= - \ket{\ua} + s^+ \qq{\fr{1}{J^*}\rr{E_0 - \ham^*_0} + 1}^{-1} s^-\ket{\ua}\\
\rr{E_0 - \ham^*_0}\ket{\ua} &= \qq{-\fr{J^*}{2}s^z + \rr{\fr{J^*}{2}}^2s^+ \fr{1}{\rr{E_0 - \ham^*_0} - \fr{J^*}{2}s^z}s^-}\ket{\ua}\\
			     &\equiv \ham_\text{eff}\ket{\ua}
\eeq
Now we can simplify the effective Hamiltonian for the Kondo cloud. First put \il{E_0=0} and \il{s^z=\fr{1}{2}}. Then,
\beq
...
\eeq
\beq
4s^+\qq{\ham^*_0,s^-} = 4\sum_{k_1 k_2 k_3 k_4}c^\dagger_{k_3c^\dagger_{k_1\da}c_{k_2\ua}\rr{\epsilon_{k_1} - \epsilon_{k_2}}}
\eeq
\beq
\cc{\ham_0^*,\qq{\ham^*_0, s^-}} &= \sum_{k,\sigma,k_1 k_2}\rr{\epsilon_{k_1} - \epsilon_{k_2}}\cc{n_{k\sigma},c^\dagger_{k_1\ua}c_{k_2\da}}
\eeq
To simplify this anticommutator, note
\beq
c^\dagger_{k_1\ua}c_{k_2\da}n_{k\sigma} &= c^\dagger_{k_1\ua}c_{k_2\da}c^\dagger_{k\sigma}c_{k\sigma}\\
					&= c^\dagger_{k_1\ua}\rr{\delta_{kk_2}\delta_{\sigma\da} - c^\dagger_{k\sigma}c_{k_2\da}}c_{k\sigma}\\
&= \delta_{kk_2}\delta_{\sigma\da}c^\dagger_{k_1\ua}c_{k\sigma}+ c^\dagger_{k\sigma}c^\dagger_{k_1\ua}c_{k_2\da}c_{k\sigma}\\
&= \delta_{kk_2}\delta_{\sigma\da}c^\dagger_{k_1\ua}c_{k_2\da}- c^\dagger_{k\sigma}c^\dagger_{k_1\ua}c_{k\sigma}c_{k_2\da}\\
&= \delta_{kk_2}\delta_{\sigma\da}c^\dagger_{k_1\ua}c_{k_2\da}- c^\dagger_{k\sigma}\rr{\delta_{kk_1}\delta_{\sigma\ua} -c_{k\sigma}c^\dagger_{k_1\ua}}c_{k_2\da}\\
&= \rr{\delta_{kk_2}\delta_{\sigma\da} - \delta_{kk_1}\delta_{\sigma\ua}}c^\dagger_{k_1\ua}c_{k_2\da} + n_{k\sigma}c^\dagger_{k_1\ua}c_{k_2\da}\\
\implies \cc{n_{k\sigma},c^\dagger_{k_1\ua}c_{k_2\da}} &= \qq{\rr{\delta_{kk_2}\delta_{\sigma\da} - \delta_{kk_1}\delta_{\sigma\ua}} + 2n_{k\sigma}}c^\dagger_{k_1\ua}c_{k_2\da}
\eeq
Therefore,
\beq
\cc{\ham_0^*,\qq{\ham^*_0, s^-}} &= \sum_{k,\sigma,k_1 k_2}\rr{\epsilon_{k_1} - \epsilon_{k_2}} \qq{\rr{\delta_{kk_2}\delta_{\sigma\da} - \delta_{kk_1}\delta_{\sigma\ua}}c^\dagger_{k_1\ua}c_{k_2\da} + 2n_{k\sigma}c^\dagger_{k_1\ua}c_{k_2\da}}\\
				 &=2n(0)\sum_{k_1 k_2}\rr{\epsilon_{k_1} - \epsilon_{k_2}} c^\dagger_{k_1\ua}c_{k_2\da}
\eeq
This gives
\beq
-\fr{16}{{J^*}}s^+{\ham^*_0}^2s^- &= -\fr{16}{J^*}\qq{}
\eeq
