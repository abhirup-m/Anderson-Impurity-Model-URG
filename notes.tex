...
where
\beq
\eta^\dagger = \fr{1}{E - \ham^d_1 - V_0}V_+
\eeq
Substituting this in the second equation gives
\beq[reneq]
...
\beq[deltaV]
\Delta V_0\big\vert_{PMS} = V_- \eta^\dagger
\eeq
...
\beq
\Delta \ham = V_+ \eta + V_- \eta^\dagger
\eeq
...
\beq
\eta &= \fr{1}{\tilde\ham - \ham^d_0 - V_0}V_-\\
...
\beq
\eta^\dagger &= \fr{1}{\hat \omega - \ham^d_1}V_+\\
\eeq
...
\end{center}
To make matters more clear, we can look at a specific term. For the SIAM,
\beq
\ham = \sum_{k\sigma}\rr{\epsilon_k \tau_{k\sigma} + V c^\dagger_{k\sigma}c_{d\sigma} + \text{h.c.}}
\eeq
we will consider the scattering \(V=c^\dagger_{q\ua}c_{d\ua}\). \(q\ua\) is a state in the high energy subspace. The model
\beq
\Delta V_0 &=  c^\dagger_{d\ua}c_{q\ua}\fr{1}{E - \hat \ham_0}c^\dagger_{q\ua}c_{d\ua}\\
\eeq
The intermediate energy (at the propagator) is
\beq
\hat \ham_0 = \sum_{k<q,\sigma}\epsilon_k \tau_{k\sigma} + \epsilon_d \hat n_{d\ol\beta} + \hf\epsilon_q
\eeq
which gives
\beq
\Delta V_0 &=  c^\dagger_{d\ua}c_{q\ua}\fr{1}{E - \sum_{k<q,\sigma}\epsilon_k \tau_{k\sigma} - \hf \epsilon_q - \epsilon_d \hat n_{d\ol\beta}}c^\dagger_{q\ua}c_{d\ua}\\
&=  \fr{1}{E - \sum_{k<q,\sigma}\epsilon_k \tau_{k\sigma} - \hf \epsilon_q - \epsilon_d \hat n_{d\ol\beta}}c^\dagger_{d\ua}c_{q\ua}c^\dagger_{q\ua}c_{d\ua}\\
\eeq
Note that \(E\) is the exact eigenvalue corresponding to the initial state \(\ket{\Psi_0}\). Since we do not know that energy, we will approximate it by the bare energy \(E = \sum_{k<q,\sigma}\epsilon_k \tau_{k\sigma} + \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta}\). 
\beq
\Delta V_0 &=  \fr{1}{\epsilon_d + U\hat n_{d\ol\beta}  - \hf \epsilon_q}c^\dagger_{d\ua}c_{q\ua}c^\dagger_{q\ua}c_{d\ua}\\
\eeq
The URG result is
\beq
\Delta V_0 &=  \fr{1}{\omega - \hf \epsilon_q - \epsilon_d \hat n_{d\ol\beta}}c^\dagger_{d\ua}c_{q\ua}c^\dagger_{q\ua}c_{d\ua}\\
\eeq
The difference from PMS is that in URG, \(\omega\) is obtained numerically by searching for fixed points.
\comm{
