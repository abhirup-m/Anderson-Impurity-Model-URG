...
\ham = \ham_0 + \ham_X
\eeq
\(\ham^D\) is diagonal w.r.t a particular degree of freedom. \(V\) is off-diagonal w.r.t that same degree of freedom. Let \(S\) be an \textit{anti-Hermitian} and \textit{off-diagonal} operator. \(U = e^S\) is then a unitary transformation.
\beq
...
\qq{S,\ham_0} = -\ham_X
\eeq
Avoiding the perturbative route, we can take \(S = \frac{\pi}{4}\rr{\eta^\dagger - \eta}\). The \(\eta\) and its conjugate satisfy the properties laid out in the formalism of URG. As a result, 
\beq
\cosh S = \frac{1}{\sqrt 2},\text{ and } \sinh S = \frac{1}{\sqrt 2}\rr{\eta^\dagger - \eta}
\eeq
The off-diagonal part now becomes
\beq
H_2 = \hf\rr{\ham_X - \eta^\dagger \ham_X \eta^\dagger - \eta \ham_X \eta + \qq{\eta^\dagger - \eta, \ham^D}}
\eeq
The vanishing of this quantity is now the decoupling condition, and is also given in eq 16 of ref.~\cite{holography1}.
\comm{
In this section, we show that the generator for Schrieffer-Wolff transformation of the Anderson model can be derived as a first order approximation of a more general decoupling condition which arises from the URG.
\pb The function of the Schrieffer-Wolff transformation is to decouple a set of states. In the following, we wish to decouple the electronic state \(q\beta\). Following ref.~\cite{piers}, we assume an operator 
\beq
\eta^\dagger = \theta c^\dagger_{q\beta}c_{d\beta}
\eeq
where \(\theta \equiv A + B\hat n_{d\ol\beta}\) and the coefficients \(A,B\) are in general a function of all orders of \(V\). From the formalism of URG, we know that the unitary transformation is
\beq
U = \ex{\frac{\pi}{4}\rr{\eta^\dagger - \eta}} = \frac{1}{\sqrt 2}\rr{1 + \eta^\dagger - \eta}
\eeq
We will now find the transformed Hamiltonian \(\tilde \ham = U \ham U^\dagger\) where \(\ham\) is the SIAM Hamiltonian eq.~\ref{model}, and we will determine \(\theta\) by requiring the off-diagonal terms to vanish. We will first determine how each of the operators transform under the operation.
\beq
\tilde c_{k\neq q,\sigma} &= U c_{k\neq q,\sigma} U^\dagger\\
			  &= \rr{1 + \theta c^\dagger_{q\beta}c_{d\beta} - \theta c^\dagger_{d\beta}c_{q\beta}} c_{k\neq q,\sigma} \rr{1 + \theta c^\dagger_{d\beta}c_{q\beta} - \theta c^\dagger_{q\beta}c_{d\beta}}\\
			  &= \qq{1 + \theta^2\rr{ \hat n_{q\beta} + \hat n_{d\beta} - 2\hat n_{q\beta}}} c_{k\neq q,\sigma}
\eeq
\beq
\tilde c_{q,\beta}   &= \rr{1 + \theta c^\dagger_{q\beta}c_{d\beta} - \theta c^\dagger_{d\beta}c_{q\beta}} c_{q\beta} \rr{1 + \theta c^\dagger_{d\beta}c_{q\beta} - \theta c^\dagger_{q\beta}c_{d\beta}}\\
&= \rr{1 + \theta c^\dagger_{q\beta}c_{d\beta}} c_{q\beta} \rr{1 - \theta c^\dagger_{q\beta}c_{d\beta}}\\
&= c_{q\beta} - \theta c_{d\beta}
\eeq
\beq
\tilde c_{d,\beta}   &= \rr{1 + \theta c^\dagger_{q\beta}c_{d\beta} - \theta c^\dagger_{d\beta}c_{q\beta}} c_{d\beta} \rr{1 + \theta c^\dagger_{d\beta}c_{q\beta} - \theta c^\dagger_{q\beta}c_{d\beta}}\\
&= \rr{1 - \theta c^\dagger_{d\beta}c_{q\beta}} c_{d\beta} \rr{1 + \theta c^\dagger_{d\beta}c_{q\beta}}\\
&= c_{d\beta} + \theta c_{q\beta}
\eeq
\beq
\tilde c_{d,\ol\beta}   &= \rr{1 + A c^\dagger_{q\beta}c_{d\beta} - A c^\dagger_{d\beta}c_{q\beta}} c_{d\ol\beta} \rr{1 + \rr{A+B} c^\dagger_{d\beta}c_{q\beta} - \rr{A+B} c^\dagger_{q\beta}c_{d\beta}}\\
			&= \rr{1 + Bc^\dagger_{d\beta}c_{q\beta} - Bc^\dagger_{q\beta}c_{d\beta} + A(A+B) \rr{\hat n_{q\beta} + \hat n_{d\beta} - 2\hat n_{q\beta}\hat n_{d\beta}}} c_{d\ol\beta}
\eeq
}
