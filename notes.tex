...
\end{minipage}
\end{center}
\paragraph \centering \includegraphics[scale=0.29]{empty.png}
...
Its conjugate is
\beq
\text{Tr}\qq{c_{q\beta}^\dagger\ham } = \fr{J}{2}\sum_{k\alpha \neq q\beta} \mathbf{S}\cdot \mathbf{\sigma}_{\beta\alpha}c^\dagger_{k\alpha}
\eeq
We also compute another trace: \il{\text{Tr}\rr{\ham^D \hat n_{q\beta}}}, where
\beq
\ham^D = \sum_{k\alpha}\epsilon_k n_{k\alpha} + \fr{J}{2}\sum_{k\alpha}{S^z}{\sigma^z}_{\alpha\alpha}n_{k\alpha} = \sum_{k\alpha}\xi_{k\beta}n_{k\alpha}
\eeq
where \il{\xi_{k\alpha} =\rr{\epsilon_k + \fr{J}{2}{S^z}{\sigma^z}_{\alpha\alpha}}}. Therefore,
\beq
 \text{Tr}\rr{\ham^D n_{q\beta}} &= \sum_{k\alpha \neq q\beta}\xi_{k\alpha}n_{k\alpha} + \xi_{q\beta}\\
				 &= \ol \ham^D_l  + \xi_{q\beta}\\
 \implies  \text{Tr}\rr{\ham^D \hat n_{q\beta}} \hat n_{q\beta} &= \ol \ham^D_l  \hat n_{q\beta}+ \xi_{q\beta} \hat n_{q\beta}
\eeq
where \il{\ol \ham^D_l = \ham^D - \xi_{q\beta} \hat n_{q\beta}}. This allows us to write the \il{\eta_{q\beta}}:
\beq
\eta_{q\beta} = \qq{\eta^\dagger_{q\beta}}^\dagger = \qq{\fr{1}{\hat\omega - \text{Tr}\rr{\ham^D \hat n_{q\beta}}\hat n_{q\beta}}c^\dagger_{q\beta}T}^\dagger = \text{Tr}\qq{c^\dagger_{q\beta}\ham}c_{q\beta}\fr{1}{\hat\omega - \text{Tr}\rr{\ham^D \hat n_{q\beta}}\hat n_{q\beta}}\\
\eeq
Note that \il{\hat\omega} itself has an \il{\ol \ham^D_l n_l} in its definition. This cancels the \il{\ham^D} in the above trace. The total expression is
\beq
\eta_{q\beta} = \rr{\fr{J}{2}\sum_{k\alpha \neq q\beta} \mathbf{S}\cdot \mathbf{\sigma}_{\beta\alpha}c^\dagger_{k\alpha}} c_{q\beta}\fr{1}{\rr{\hat\omega - \fr{\xi{q\beta}}{2} - \ol \ham^D_l  \hat n_{q\beta}} - \xi_{q\beta}\tau_{q\beta}}
\eeq
Define \il{\hat{\ol  \omega} = \hat\omega -  \fr{\xi{q\beta}}{2} - \ol \ham^D_l  \hat n_{q\beta}}. The second term in the effective Hamiltonian \il{\ham_{j-1}} can then be written as
\beq
\sum_{l=1}^{N}\tau_l\left\{c^\dagger_{l}\text{Tr}_l\right.&\left.\qq{\ham_j  c_l},\eta_l\right\} = \\
							  & \sum_{|q|=\Lambda_j,\atop{\beta=\ua,\da}} \tau_{q\beta}\left\{\fr{J}{2}\sum_{k\alpha \neq q\beta} \mathbf{S}\cdot \mathbf{\sigma}_{\beta\alpha}c^\dagger_{q\beta} c_{k\alpha},\fr{J}{2}\sum_{k^\prime\alpha^\prime \neq q\beta} \mathbf{S}\cdot \mathbf{\sigma}_{\alpha^\prime\beta}c^\dagger_{k^\prime\alpha^\prime} c_{q\beta}\fr{1}{\hat{\ol\omega} - \xi_{q\beta}\tau_{q\beta}}\right\}\\
							  &=\fr{J^2}{4}\sum_{|q|=\Lambda_j,\atop{\beta=\ua,\da}} \tau_{q\beta}\left\{\sum_{k\alpha \neq q\beta} \mathbf{S}\cdot \mathbf{\sigma}_{\beta\alpha}c^\dagger_{q\beta} c_{k\alpha},\sum_{k^\prime\alpha^\prime \neq q\beta} \mathbf{S}\cdot \mathbf{\sigma}_{\alpha^\prime\beta}c^\dagger_{k^\prime\alpha^\prime} c_{q\beta}\fr{1}{\hat{\ol\omega} - \xi_{q\beta}\tau_{q\beta}}\right\}\\
\eeq
...
\beq
\ham_\text{coll} \ket{\Psi^*} &= \fr{1}{\sqrt 2}\sum_{|\mb{k}|<\Lambda^*}\qq{\rr{-\fr{J^*}{4} - \fr{J^q}{4}} \ket{\ua_d}\ket{\da_\mb{k}}\ket{\da_{\mb{q}}}- \rr{-\fr{J^*}{4} - \fr{J^q}{4}} \ket{\da_d}\ket{\ua_\mb{k}} \ket{\ua_{\mb{q}}}}\\
			      &= -\fr{1}{4}\rr{J^* + J^q}\ket{\Psi^*}
\eeq
The lowest value of the energy occurs when \il{\mb{q}} is placed on the shell \il{\Lambda^*}, because \il{J^*} \il{J^q} falls rapidly with distance from the Kondo cloud. Therefore,
\beq
E_g = -\fr{J^*}{2}
\eeq
...
\rho_\text{all} &= \ket{\Psi^*}\bra{\Psi^*}\\
		&= \fr{1}{2}\qq{\ket{2}\bra{2}\ket{\da}\bra{\da} - \ket{2}\bra{3}\ket{\da}\bra{\ua} - \ket{3}\bra{2}\ket{\ua}\bra{\da} + \ket{3}\bra{3}\ket{\ua}\bra{\ua}}
\eeq
...
\beq
\ket{\Downarrow} = \qq{\fr{1}{J^*}\rr{E_0 - \ham^*_0} + 1}^{-1}\fr{a_\ua}{a_\da} s^-\ket{\Uparrow}
\eeq
Substituting this in the second equation gives
\beq[effeig]
\fr{1}{J^*}\rr{E_0 - \ham^*_0}\ket{\Uparrow} &= - \ket{\Uparrow} + s^+ \qq{\fr{1}{J^*}\rr{E_0 - \ham^*_0} + 1}^{-1} s^-\ket{\Uparrow}\\
...
\beq
s^+ s^- &= \fr{1}{2} + s^z = 1\\
\implies \fr{J^*}{4} - J^*s^+ s^- &= \fr{J^*}{4} -J^* = -\fr{3J^*}{4}\\
...
\beq
H_\text{eff} = - \fr{3J^*}{4} +  4\ham_0^* -\fr{16}{J^*}{\ham_0^*}^2 + \fr{16}{J^*}\sum_{k_1 k_2 k_3 k_4}\rr{\epsilon_{k_1} - \epsilon_{k_2}}^2c^\dagger_{k_3\ua}c_{k_4\da}c^\dagger_{k_1\da}c_{k_2\ua}
\eeq
...
\pb
The requirement that the total energy of the composite system be zero constrains the kinetic energy of the Kondo cloud. This is understood by noting that (eq.~\ref{effeig}) the eigenvalue of \il{H_\text{eff}} is \il{E_0 - \ham_0^*}. Equating that to the final form of \il{H_\text{eff}} and dropping the terms with \il{\fr{1}{J^*}}, we get
\beq
\rr{E - \ham_0^*}\ket{\Uparrow} =\rr{ - \fr{3J^*}{4} +  4\ham_0^*}\ket{\Uparrow}
\eeq
Putting \il{E_0 = 0} gives
\beq
\ham_0^* \ket{\Uparrow} = \fr{3J^*}{20}\ket{\Uparrow}
\eeq
This is the minimum value of the kinetic energy for the Kondo cloud Hamiltonian; anything lower than this would lead to a total energy \il{E_0} less than 0 and the destabilization of the Fermi surface. This tells us that just as one electron comes below the Fermi surface to form the singlet with the impurity spin, other electrons have to excite suitably above the Fermi surface (that is, become more energetic) in order to constrain the ground state energy at 0. Excitations of the cloud then involve higher values of \il{\ham_0^* > \fr{3J^*}{20}} and lead to excitations of the composite system as a whole (\il{E_0 > 0}). The spectrum of \il{\ham^*_0} can then be thought to be gapped; the kinetic energy must start from a non-zero value. It is not possible to have a kinetic energy infinitesimally close to the Fermi surface.
