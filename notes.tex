...
\beq[etadagdef]
\eta(\hat \omega) = \fr{1}{\hat \omega_i- \ham^d}T^\dagger c_{q\beta}
\eeq
where \(\hat \omega_i = \omega_i^0\rr{1 - \hat n_{q\beta}} + \omega_i^1\hat n_{q\beta}\) is a 2x2 matrix and \(\ham^d = \ham^d_0\rr{1 - \hat n_{q\beta}} + \ham^d_1\hat n_{q\beta}\). It is easy to check that this reproduces the previous forms of \(\eta_0\) and \(\eta_1^\dagger\).
...
\eeq
Henceforth we will assume that this constraint has been imposed and drop the subscripts \(0\) and \(1\) on the \(\eta\). 
