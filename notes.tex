...
\begin{flalign}
	C^z_{kk^\prime} &= \frac{1}{2}\sum_{\alpha} {\psi^k_\alpha}^\dagger \sigma^z_{\alpha\alpha} \psi^{k^\prime}_\alpha = \frac{1}{2}\rr{c^\dagger_{k\ua}c_{k^\prime \ua} - c_{k^\prime\da}c^\dagger_{k \da}} = \frac{1}{2}\left(c^\dagger_{k\ua}c_{k^\prime \ua} + c^\dagger_{k\da}c_{k^\prime \da} - \delta_{kk^\prime}\right)\\
	C^x_{kk^\prime} &= \frac{1}{2}\sum_{\alpha} {\psi^k_\alpha}^\dagger \sigma^z_{\alpha\ol\alpha} \psi^{k^\prime}_{\ol\alpha} = \frac{1}{2}\left( c^\dagger_{k\ua}c^\dagger_{k^\prime \da} + c_{k\ua}c_{k^\prime \da} \right) \\
	C^y_{kk^\prime} &= \frac{1}{2}\sum_{\alpha} {\psi^k_\alpha}^\dagger \sigma^z_{\alpha\ol\alpha} \psi^{k^\prime}_{\ol\alpha} = -i \frac{1}{2}\left( c^\dagger_{k\ua}c^\dagger_{k^\prime \da} - c_{k\ua}c_{k^\prime \da} \right) \\
\end{flalign}
...
\begin{flalign*}
&Gc^\dagger_{q\beta}T\\
&= \frac{1}{\omega_0 - \ham_d}\sum_{q\beta}\qq{V_qc^\dagger_{q\beta}c_{d\beta} + 2 K_z \rr{\hat n_d -1}\sum_k c^\dagger_{q\beta}c_{k\beta} + K_t \sum_k c^\dagger_{q\beta}c^\dagger_{k\ol\beta}c_{d\beta}c_{d\ol\beta} }\\
 	     &=\sum_{q\beta}\left[\frac{V_q}{\omega_0 - \hf\epsilon_q - \epsilon_d \hat n_{d\ol\beta} - \frac{1}{2}K_z\rr{\hat n_{d\ol\beta}-1}}c^\dagger_{q\beta}c_{d\beta} + \frac{2 K_z \rr{\hat n_d -1}}{\omega_0 - \hf\epsilon_q - H_{imp} - \frac{1}{2}K_z\rr{\hat n_{d}-1}}\sum_k c^\dagger_{q\beta}c_{k\beta} \right.\\
...
\begin{flalign*}
	     &\sum_{q\beta}\qq{V_q^*c^\dagger_{d\beta}c_{q\beta} + 2 K_z \rr{\hat n_d -1}\sum_k c^\dagger_{k\beta}c_{q\beta} + K_t \sum_k c^\dagger_{d\beta}c^\dagger_{d\ol\beta}c_{q\beta}c_{k\ol\beta}}Gc^\dagger_{q\beta}T\\
&=\sum_{q\beta}\left[\frac{|V_q|^2 \hat n_{d\beta}\rr{1 - \hat n_{q\beta}}}{\omega_0 - \hf\epsilon_q - \epsilon_d \hat n_{d\ol\beta} - \frac{1}{2}K_z\rr{\hat n_{d\ol\beta}-1}} - \sum_k\frac{2V_q^*K_z c^\dagger_{d\beta}c_{k\beta}\rr{1 - \hat n_{d\ol\beta}}\rr{1 - \hat n_{q\beta}}}{\omega_0 - \hf\epsilon_q + \frac{1}{2}K_z} \right.\\
&- \left.\sum_k\frac{V_q^* K_t\hat n_{d\beta}\rr{1 - \hat n_{q\beta}}c^\dagger_{k\ol\beta}c_{d\ol\beta}}{\omega_0 - \hf\epsilon_q + K_z} - \sum_k\frac{2K_z V_q\rr{1 - \hat n_{d\ol\beta}}\rr{1 - \hat n_{q\beta}}c^\dagger_{k\beta}c_{d\beta}}{\omega_0 - \hf\epsilon_q + \frac{1}{2}K_z}\right. \\
&+\left. \sum_{kk^\prime}\frac{4K_z^2 \rr{1 - \hat n_d}^2\rr{1 - \hat n_{q\beta}}c^\dagger_{k\beta}c_{k^\prime\beta}}{\omega_0 - \hf\epsilon_q - H_{imp} - \frac{1}{2}K_z\rr{\hat n_d - 1}} - \sum_{kk^\prime}\frac{2K_z K_t \rr{1 - \hat n_{q\beta}}c^\dagger_{k\beta}c^\dagger_{k^\prime\ol\beta}c_{d\beta}c_{d\ol\beta}}{\omega_0 - \hf\epsilon_q + \frac{1}{2}K_z}\right.\\
&- \left.\sum_k\frac{K_t V_q \rr{1 - \hat n_{q\beta}}\hat n_{d\beta}c^\dagger_{d\ol\beta}c_{k\ol\beta}}{\omega_0 - \hf\epsilon_q + \frac{1}{2}K_z} - \sum_{kk^\prime}\frac{2K_z K_t \rr{1 - \hat n_{q\beta}}c^\dagger_{d\beta}c^\dagger_{d\ol\beta}c_{k\beta}c_{k^\prime\ol\beta}}{\omega_0 - \hf\epsilon_q + \frac{1}{2}K_z}\right.\\
...
\beq
	G T^\dagger c_{q\beta} &= \frac{1}{\omega - \ham_d}\qq{V_q^*c^\dagger_{d\beta}c_{q\beta} + 2 K_z \rr{\hat n_d -1}\sum_k c^\dagger_{k\beta}c_{q\beta} + K_t \sum_k c^\dagger_{d\beta}c^\dagger_{d\ol\beta}c_{q\beta}c_{k\ol\beta} }\\
			       &=\frac{V^*_q c^\dagger_{d\beta}c_{q\beta}}{\omega_1 - \hf\epsilon_q - \epsilon_d - \rr{\epsilon_d + U}\hat n_{d\ol\beta} + \frac{1}{2}K_z\hat n_{d\ol\beta}} + \frac{2 K_z \rr{\hat n_d -1}\sum_k c^\dagger_{k\beta}c_{q\beta}}{\omega_1 - \hf\epsilon_q - H_{imp} + \frac{1}{2}K_z\rr{\hat n_{d}-1}} \\
			     &+\frac{K_t \sum_k c^\dagger_{d\beta}c^\dagger_{d\ol\beta}c_{q\beta}c_{k\ol\beta}}{\omega_1 - \hf\epsilon_q -2\epsilon_d - U + \frac{1}{2}K_z}\\
\eeq
The renormalization is, therefore,
\begin{flalign*}
...
&=\frac{|V_q|^2 \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}}{\omega_1 - \hf\epsilon_q - \epsilon_d - \left( \epsilon_d + U - \frac{1}{2}K_z \right) \hat n_{d\overline\beta}} - \frac{2V_qK_z c^\dagger_{k\beta}c_{d\beta}\hat n_{d\ol\beta}\hat n_{q\beta}}{\omega_1 - \hf\epsilon_q - 2\epsilon_d - U + \frac{1}{2}K_z}\\
&- \frac{V_q K_t\hat n_{q\beta}\rr{1 - \hat n_{d\beta}}c^\dagger_{d\ol\beta}c_{k\ol\beta}}{\omega_1 - \hf\epsilon_q - 2\epsilon_d - U + \frac{1}{2}K_z} - \frac{2K_z V_q^*\hat n_{d\ol\beta}\hat n_{q\beta}c^\dagger_{d\beta}c_{k\beta}}{\omega_1 - \hf\epsilon_q -2\epsilon_d - U + \frac{1}{2}K_z} \\
&+ \frac{4K_z^2 \rr{\hat n_d - 1}^2\hat n_{q\beta}c_{k\beta}c^\dagger_{k^\prime\beta}}{\omega_1 - \hf\epsilon_q - H_{imp} + \frac{1}{2}K_z\rr{\hat n_d - 1}} - \frac{2K_z K_t \hat n_{q\beta}c^\dagger_{d\beta}c^\dagger_{d\ol\beta}c_{k\beta}c_{k^\prime\ol\beta}}{\omega_1 - \hf\epsilon_q -2\epsilon_d - U + \frac{1}{2}K_z}\\
&- \frac{K_t V^*_q \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}c^\dagger_{k\ol\beta}c_{d\ol\beta}}{\omega_1 - \hf\epsilon_q -2\epsilon_d - U + \frac{1}{2}K_z} - \frac{2K_z K_t \hat n_{q\beta}c^\dagger_{k\beta}c^\dagger_{k^\prime\ol\beta}c_{d\beta}c_{d\ol\beta}}{\omega_1 - \frac{1}{2}\epsilon_q - 2\epsilon_q - U + \frac{1}{2}K_z}\\
& + \frac{K_t^2 \left(1 - \hat n_{d\beta}\right) \left(1 - \hat n_{d\ol\beta}\right) \hat n_{q\beta}c^\dagger_{k\ol\beta}c_{k^\prime\ol\beta}}{\omega_1 - \hf\epsilon_q - 2\epsilon_d - U + \frac{1}{2}K_z}
\end{flalign*}
\subsection{Scaling Equations}
To get the scaling equations, we perform similar steps as in the spin-Kondo URG. First of all, in the RG equations that involve  \(\hat n_{d\ol\beta}\) in the denominator, we replace the quantum fluctuations will \(E_j + \omega\), where \(E_j\) is the impurity part of the energy of the initial state and \(\omega\) is a redefined quantum fluctuation scale. Secondly, by using the constraint \(\eta = \left( \eta^\dagger \right) ^\dagger\), we can relate the denominators of the RG equations of the \(V\)s and \(K\)s. The term with \(K_z^2\) in either sector mostly cancel each other, leaving a renormalization of \(\epsilon_d\) and \(U\) at \(K^2\) order. The penultimate term in the hole sector, gets canceled by a piece from the final term in particle sector, and the remaining piece gives the renormalization in \(K_z\).
\begin{equation}\begin{aligned}
	...
	\Delta V &= \frac{-V_q^* K_t + 2 V_q K_z}{\omega - \frac{1}{2}\epsilon_q + \frac{1}{2}K_z}\\
