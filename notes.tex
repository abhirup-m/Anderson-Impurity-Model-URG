...
\eeq
This is the same as the PMS result eq.~\ref{pmsren}
...
where 
\beq
\eta &= \frac{1}{\omega_0 - \ham^d}\sum_{ij}V_{ij}\ket{\phi_0^i}\bra{\phi_1^j}\\
\eta^\dagger &= \frac{1}{\omega_1 - \ham^d}\sum_{ij}V_{ij}\ket{\phi_0^i}\bra{\phi_1^j} = \sum_{ij}\frac{1}{\omega - \ham^d}V_{ij}\ket{\phi_0^i}\bra{\phi_1^j}\frac{1}{\omega_0 - \ham^d}\\
\implies \eta^\dagger - \eta &= \sum_{ij}\frac{1}{\omega - E_0^i}\rr{V^\dagger_{ij}\ket{\phi_1^j}\bra{\phi_0^i} - V_{ij}\ket{\phi_0^i}\bra{\phi_1^j}}\\
&= \sum_{ij}\frac{1}{E_0^i - \omega}\rr{V_{ij}\ket{\phi_0^i}\bra{\phi_1^j} - V^\dagger_{ij}\ket{\phi_1^j}\bra{\phi_0^i}}\\
\eeq
Comparing with the \(S\) of eq.~\ref{swtgen}, the prescription to go from URG to SWT is to replace \(\omega \to E_1^j\).
...
>>>>>>> 1508599bc7f0293cd4571ac79aa1d27b70350e02
\beq
...
\beq
\sum_{kq}g_{kq}\rr{b^\dagger_q + b_q}c^\dagger_{k+q,\sigma}c_{k\sigma}
\eeq
and \(\ham_d = \sum_{k\sigma}\epsilon_k \hat n_{k\sigma} + \sum_q \hbar\omega_q \hat n_q\) is the kinetic energy of the electron and phonons. We choose
\beq
