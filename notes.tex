...
\Delta_1^- \ham_N &= \sum_{q\beta}|V_q|^2 \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}\fr{1}{\omega^- - \hf\epsilon_q + \fr{1}{4}J_z\rr{1 - \hat n_{d\ol\beta}} - \epsilon_d - \rr{\epsilon_d + U}\hat n_{d\ol\beta}} \\
		  &= \sum_{q\beta}|V_q|^2\rr{1 - \hat n_{d\beta}}\qq{\fr{\hat n_{d\ol\beta}}{\omega^- - \hf\epsilon_q - 2\epsilon_d - U } + \fr{1 - \hat n_{d\ol\beta}}{\omega^- - \hf\epsilon_q - \epsilon_d  + \fr{1}{4} J_z}}\\
...
\beq
\eta &= \fr{1}{\omega^- - \hf\epsilon_q - \epsilon_d + \hf \beta J_z S^z_d}J_z \beta S^z_d c^\dagger_{k^\prime\beta}c_{q\beta}\\
\eta^\dagger &= \fr{1}{\omega^+ - \hf\epsilon_q - H_\text{imp} - \hf \beta J_z S^z_d}J_z \beta S^z_d c^\dagger_{q\beta}c_{k^\prime\beta}
\eeq
From the expression of \(\eta\), we can take its Hermitian conjugate to get another expression for \(\eta^\dagger\):
\beq
\eta^\dagger &= J_z \beta S^z_d c^\dagger_{q\beta}c_{k^\prime\beta}\fr{1}{\omega^- - \hf\epsilon_q - \epsilon_d + \hf \beta J_z S^z_d}\\
\eeq
Comparing the two expressions gives
\beq
\fr{1}{\omega^- - \hf\epsilon_q - \epsilon_d + \hf \beta J_z S^z_d} = \fr{1}{\omega^+ - \hf\epsilon_q - H_\text{imp} - \hf \beta J_z S^z_d}
\eeq
...
\beq
2\Delta J_z = 2 \Delta J_t = \Delta J &= - J^2 \sum_{q}\fr{1}{\omega^+ - \hf\epsilon_q - \epsilon_d + \fr{1}{16}J}
\eeq
...
\beq[and2kondo]
2\Delta J_z = 2 \Delta J_t = \Delta J &= - J^2 \sum_{q}\fr{1}{\tilde \omega^+ - \hf\epsilon_q + \fr{1}{16}J}
\eeq
...
\beq
\Delta J = - J^2 \sum_{q}\fr{1}{\tilde \omega^+ - \hf\epsilon_q + \fr{1}{16}J}
\eeq
For \(\tilde \omega^+ = 0\), we get
\beq
\Delta J = 2J^2 \sum_{q}\fr{1}{D - \fr{1}{8} J}
\eeq
...
\begin{flalign*}
	\Delta^- U &=  2\sum_{q}|V(q)|^2\rr{\fr{1}{\omega^+ - \hf\epsilon_q - \epsilon_d + \fr{1}{16} J} - \fr{1}{\omega^+ - \hf\epsilon_q}} -\\
		   &\quad\fr{1}{8}J^2\sum_{q k}\qq{\fr{5}{\omega^+ - \hf\epsilon_q - \epsilon_d + \fr{1}{16}J} + \fr{1}{\omega^+ - \hf\epsilon_q - \epsilon_d - \fr{1}{16}J}}\\
\Delta^- V &= -\rr{\fr{1}{16} J + J_t}\sum_{q}V_q\fr{1}{\omega^+ - \hf\epsilon_q - \epsilon_d + \fr{1}{16} J}\\
\Delta^- J_t &= -J_z J_t\sum_{q}\fr{1}{\omega^+ - \hf\epsilon_q - \epsilon_d + \fr{1}{16} J}\\
\Delta^- J_z &= -J_t^2 \sum_{q}\fr{1}{\omega^+ - \hf\epsilon_q - \epsilon_d + \fr{1}{16} J}
\end{flalign*}
...
Eliminating \(\ket{\Psi_1}\) from the two equations gives
\beq[reneq]
\rr{\ham^d_0 + V_0 + V_- \fr{1}{E - \ham^d_1 - V_0}V_+}\ket{\Psi_0} = E\ket{\Psi_0}
\eeq
This new Hamiltonian,
\beq
\tilde \ham_0 = \ham^d_0 + V_0 + V_- \fr{1}{E - \ham^d_1 - V_0}V_+
\eeq
...
\beq[deltaV]
\Delta V_0\big\vert_{PMS} = V_- \fr{1}{E - \ham^d_1 - V_0}V_+
\eeq
...
The final expression for the renormalization can be written directly from the PMS expression simply by replacing \(E\) with \(\ham^\prime\).
\beq
\Delta V_0\big\vert_{URG} =  V_-\fr{1}{\ham^\prime_1 - \ham^d_1 -  V_0} V_+
\eeq
This allows us to write down a unitary operator that decouples the entangled state:
\beq
U \ket{\Phi} = \ket{\Phi_0} \text{ and } \qq{\hat n_q, \tilde \ham} = 0
\eeq
...
Doing that returns a rotated Hamiltonian which is diagonal in \(q\), and is a function of the chosen state, same as URG.
The conclusion is that depending on which state we choose as our diagonal non-Fock basis, URG and PMS will cause flows along different lines in general.
\pb In the context of URG, we define a quantum fluctuation energy scale \(\hat \omega\):
\beq
\hat \omega = \ham^\prime_1 -  V_0
\eeq
This allows us to write
\beq
\Delta V_0\big\vert_{URG} =  V_-\fr{1}{\hat\omega - \ham^d_1} V_+
\eeq
...
\eeq
Here \(V\) destroys a high energy electron and \(V^\dagger\) creates one.\cite{Kondo}
