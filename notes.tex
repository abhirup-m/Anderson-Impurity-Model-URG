...
\ham = \bordermatrix{~ & \ket{1} & \ket{0} \cr
              ...
              & T^\dagger & H_0 \cr}
\eeq
The basis in which this matrix is written is \(\cc{\ket{1},\ket{0}}\) where \(\ket{i}\) is the set of all states where \(\hat n_{q\beta}=i\). The aim of one step of the URG is to find a unitary operator \(U\) such that the new Hamiltonian \(U \ham U^\dagger\) is diagonal in this already-chosen basis.
\beq
...
\eeq
Since \(\ket{\psi_i}\) is the set of eigenstates of \(\ham\) and \(\ket{i}\) is the set of eigenstates in which \(U\ham U^\dagger\), we can relate \(\ket{\psi_i}\) and \(\ket{i}\) by the same transformation : \(\ket{\psi_i} = U^\dagger\ket{i}\). We can expand the state \(\ket{\psi_i}\) in the subspace of \(q\beta\):
\beq[matpsi]
\ket{\psi_i} = \sum_{j=0,1} \ket{j}\bra{j}\ket{\psi_i} \equiv \ket{1}\ket{\phi_1^i} + \ket{0}\ket{\phi_0^i}\eeq
where \(\ket{\phi_j^i} = \bra{j}\ket{\psi_i}\). If we substitute the expansions \ref{matpsi} and \ref{matham} into the eigenvalue equation \ref{diageq}, we get
\beq
\qq{H_e \hat n_{q\beta} + H_h \rr{1 - \hat n_{q\beta}} + c^\dagger_{q\beta}T + T^\dagger c_{q\beta}}\qq{\ket{1}\ket{\phi_1^i} + \ket{0}\ket{\phi_0^i}} = \tilde H_i\qq{\ket{1}\ket{\phi_1^i} + \ket{0}\ket{\phi_0^i}}
\eeq
Gathering the terms that result in \(\hat n_{q\beta}=1\) gives
\beq[changea]
H_e\ket{1}\ket{\phi_1^i} + c^\dagger_{q\beta}T\ket{0}\ket{\phi_0^i} = \tilde H_i\ket{1}\ket{\phi_1^i} \implies \rr{\tilde H_i - H_e}\ket{1}\ket{\phi_1^i} = c^\dagger_{q\beta}T\ket{0}\ket{\phi_0^i}
\eeq
Similarly, gathering the terms that result in \(\hat n_{q\beta}=0\) gives
\beq[changeb]
H_h\ket{0}\ket{\phi_0^i} + T^\dagger c_{q\beta}\ket{1}\ket{\phi_1^i} = \tilde H_i\ket{0}\ket{\phi_0^i} \implies \rr{\tilde H_i - H_h}\ket{0}\ket{\phi_0^i} = T^\dagger c_{q\beta}\ket{1}\ket{\phi_1^i}
\eeq
...
H_e = \text{tr}\qq{\ham \hat n_{q\beta}} = \text{tr}\qq{\rr{\ham^d + \ham^i}\hat n_{q\beta}} = \ham^d_e + \ham^i_e\\
H_h = \text{tr}\qq{\ham \rr{1 - \hat n_{q\beta}}} = \text{tr}\qq{\rr{\ham^d + \ham^i}\rr{1 - \hat n_{q\beta}}} = \ham^d_h + \ham^i_h
\end{gathered}
\end{equation*}
Using this separation, we can write eqs.~\ref{changea} and \ref{changeb} as
\beq[change]
\rr{\rr{\tilde H_i - \ham^i_e}- \ham^d_e}\ket{1}\ket{\phi_1^i} = c^\dagger_{q\beta}T\ket{0}\ket{\phi_0^i}\\
\rr{\rr{\tilde H_i - \ham^i_h} - \ham^d_h}\ket{0}\ket{\phi_0^i} = T^\dagger c_{q\beta}\ket{1}\ket{\phi_1^i}
\eeq
We now define a new operator \(\hat \omega_i = \tilde H_i - \ham^i\), such that 
\begin{equation*}
	\begin{gathered}
\tilde H_i - \ham^i_e = \text{tr}\qq{\rr{\tilde H_i - \ham^i}\hat n_{q\beta}} = \text{tr}\qq{\hat \omega \hat n_{q\beta}} \equiv \hat \omega_e\\
\tilde H_i - \ham^i_h = \hat \omega_h
\end{gathered}
\end{equation*}
We can now write eq.~\ref{change} in terms of \(\hat\omega\):
\beq[changec]
\rr{\hat \omega_e- \ham^d_e}\ket{1}\ket{\phi_1^i} = c^\dagger_{q\beta}T\ket{0}\ket{\phi_0^i}\\
\rr{\hat \omega_h - \ham^d_h}\ket{0}\ket{\phi_0^i} = T^\dagger c_{q\beta}\ket{1}\ket{\phi_1^i}
\eeq
We now choose the \(\ket{\phi_j^i}\) such that they are eigenstates of \(\hat \omega_{e,h}\):
\begin{flalign*}
\hat \omega_e\ket{\phi_1^i} = \omega_e\ket{\phi_1^i}\\
\hat \omega_h\ket{\phi_0^i} = \omega_h\ket{\phi_0^i}
\end{flalign*}
where \(\omega_{e,h}\) are numbers, not operators. The states \(\ket{\phi_j^i}\) are therefore characterized by eigenvalues \(\omega_{e,h}\). Eqs.~\ref{changec} simplify to
\beq[changed]
\ket{1}\ket{\phi_1^i(\omega_e)} = \fr{1}{\omega_e- \ham^d_e}c^\dagger_{q\beta}T\ket{0}\ket{\phi_0^i(\omega_h)}\\
\ket{0}\ket{\phi_0^i(\omega_h)} = \fr{1}{\omega_h - \ham^d_h}T^\dagger c_{q\beta}\ket{1}\ket{\phi_1^i(\omega_e)}
\eeq
We now define two many-particle transition operators:
\beq[etadagdef]
{\eta_e}^\dagger(\omega_e) = \fr{1}{\omega_e- \ham^d_e}c^\dagger_{q\beta}T \\
\eta_h(\omega_h) = \fr{1}{\omega_h- \ham^d_h}T^\dagger c_{q\beta}
\eeq
In terms of these operators, eqs.~\ref{changed} become
\beq
	\ket{1}\ket{\phi_1^i} &= {\eta_e}^\dagger\ket{0}\ket{\phi_0^i}\\
	\ket{0}\ket{\phi_0^i} &= \eta_h\ket{1}\ket{\phi_1^i}
\eeq
These allow us to write
\beq
	\ket{\psi_1} &= \ket{1}\ket{\phi_1^i} + \ket{0}\ket{\phi_0^i} = \rr{1 + \eta_h}\ket{1}\ket{\phi_1^i}\\
	\ket{\psi_0} &= \rr{1 + {\eta_e}^\dagger}\ket{0}\ket{\phi_0^i}
\eeq
Recalling that \(\ket{\psi_i} = U^\dagger \ket{i}\), we can read off the required transformations:
\beq[Ui]
	U_1 &= 1 + \eta_h\\
	U_0 &= 1 + \eta_e^\dagger
\eeq
...
\beq
	\eta_h^2 = {\eta_e^\dagger}^2 = 0 &&\qq{{c^\dagger}^2 = c^2 = 0}\\
\eeq
Second is:
\beq[antic]
	\ket{1}\ket{\phi_1^i} &= {\eta_e}^\dagger\ket{0}\ket{\phi_0^i} = \eta_e^\dagger \eta_h \ket{1}\ket{\phi_1^i} \implies \eta_e^\dagger \eta_h = \hat n_{q\beta}\\
	\ket{0}\ket{\phi_0^i} &= \eta_h\ket{1}\ket{\phi_1^i} = \eta_h \eta_e^\dagger\ket{\phi_0^i}\implies \eta_h  \eta_e^\dagger= 1 - \hat n_{q\beta}\\
	\implies \cc{\eta_h,\eta_e^\dagger} &= 1
\eeq
...
\begin{equation*}
	\begin{gathered}
	\eta_e^\dagger \eta_h \ket{\psi_i} = \ket{1}\ket{\phi_1^i} = \hat n_{q\beta}\ket{\psi_i}\\
	\eta_h\eta_e^\dagger \ket{\psi_i} = \ket{0}\ket{\phi_0^i} = \rr{1 - \hat n_{q\beta}}\ket{\psi_i}\\
	\cc{\eta_e^\dagger,\eta_h}\ket{\psi_i} = \ket{\psi_i}
\end{gathered}
\end{equation*}
...
U_1 = e^{\eta_h}
\eeq
using the fact that \(\eta_h^2 = 0\). It is shown in ref.~\cite{suzuki} that corresponding to a similarity transformation \(e^\omega\),there exists a unitary transformation \(e^G\) where
\beq
G = \tanh^{-1}\rr{\omega - \omega^\dagger}
\eeq
Applying that to the problem at hand gives
\beq
 U_1^\dagger &= \ex{\tanh^{-1}\rr{\eta_h - \eta_h^\dagger}}\\
	   &= \frac{1 + \eta_h - \eta_h^\dagger}{1 + \cc{\eta_h,\eta_h^\dagger}}\\
	   &= \fr{1}{\sqrt 2}\rr{1 + \eta_h - \eta_h^\dagger}
\eeq
The \textit{unitary} operator that transforms the entangled eigenstate \il{\ket{\psi_1}} to the state \il{\ket{1}} is thus
\beq
U_1 = \fr{1}{\sqrt 2}\rr{1 + \eta_h^\dagger - \eta_h}
\eeq
It can also be written as \(\ex{\frac{\pi}{4}\rr{\eta^\dagger_h - \eta_h}}\) because
\beq
 \ex{\frac{\pi}{4}\rr{\eta^\dagger_h - \eta_h}} &= 1 + \rr{\eta^\dagger_h - \eta_h}\fr{\pi}{4} + \frac{1}{2!}\rr{\eta^\dagger_h - \eta_h}^2\rr{\fr{\pi}{4}}^2 + \frac{1}{3!}\rr{\eta^\dagger_h - \eta_h}^3\rr{\fr{\pi}{4}}^3 + ...\\
&= 1 + \rr{\eta^\dagger_h - \eta_h}\fr{\pi}{4} - \frac{1}{2!}\rr{\fr{\pi}{4}}^2 - \frac{1}{3!}\rr{\eta^\dagger_h - \eta_h}\rr{\fr{\pi}{4}}^3 + \frac{1}{4!}\rr{\fr{\pi}{4}}^4 + ...\\
&= \cos \frac{\pi}{4} + \rr{\eta^\dagger_h - \eta_h}\sin\fr{\pi}{4}\\
&= 1 + \eta_h^\dagger - \eta_h
\eeq
There we used
\beq
\rr{\eta^\dagger_h - \eta_h}^2 = {\eta^\dagger_h}^2 + \eta_h^2 - \cc{\eta^\dagger_h,\eta_h} = -1 &&\qq{\because\eta^2 = {\eta^\dagger}^2=0}
\eeq
and hence
\beq
\rr{\eta^\dagger_h - \eta_h}^3 = -1\rr{\eta^\dagger_h - \eta_h}
\eeq
and so on. Since \(U_0\) is obtained from \(U_1\) simply by replacing \(\eta_h\) with \(\eta_e^\dagger\), we can easily surmise that the unitary transformation that rotates an eigenstate \(\ket{\psi_0}\) to the state \(\ket{0}\) is
\beq
U_0 = \fr{1}{\sqrt 2}\rr{1 + \eta_e - \eta_e^\dagger} = \ex{\frac{\pi}{4}\rr{\eta_e - \eta_e^\dagger}}
\eeq
\comm{
...
\end{itemize}
\section{Schrieffer-Wolff Transformation in the context of URG and PMS}
\subsection{Schrieffer-Wolff transformation as a limit of URG}
...
\beq
\hf \qq{S,\ham_X} &= |V|^2\qq{\frac{1 - \hat n_{d\ol\beta}}{\epsilon_q - \epsilon_d} + \frac{\hat n_{d\ol\beta}}{\epsilon_q - \epsilon_d - U}}\qq{\hat n_{q\beta}\rr{1 - \hat n_{d\beta}} - \hat n_{d\beta}\rr{1 - \hat n_{q\beta}}}\\
		  &=-\hat n_{d\beta}|V|^2\qq{\frac{1 - \hat n_{d\ol\beta}}{\epsilon_q - \epsilon_d} + \frac{\hat n_{d\ol\beta}}{\epsilon_q - \epsilon_d - U}}
\eeq
In the last step, we put \(\hat n_{q\beta}=0\) because previously we assumed \(\epsilon_q>0\) and high energy virtual excitations above the Fermi surface must necessarily be vacant in the initial state. We can obtain the renormalization from decoupling a high energy \textit{hole} state directly from this expression, just by choosing \(\hat n_{q\beta}=1\) and setting \(\epsilon_q \to -\epsilon_q\).
\beq
\hf \qq{S,\ham_X} &=-\rr{1 - \hat n_{d\beta}}|V|^2\qq{\frac{1 - \hat n_{d\ol\beta}}{\epsilon_q + \epsilon_d} + \frac{\hat n_{d\ol\beta}}{\epsilon_q + \epsilon_d + U}}
\eeq
These two results - the renormalization in the particle and hole sectors - is \textit{identical to the result obtained from PMS} of the SIAM (see \cite{hewson}). 
...
\beq[cossin]
\cosh S = \frac{1}{\sqrt 2},\text{ and } \sinh S = \frac{1}{\sqrt 2}\rr{\eta^\dagger - \eta}
\eeq
...
That gives
\beq
H_2 = \hf\qq{\frac{4}{\pi}\qq{S,\ham_0} + \ham_X - \frac{16}{\pi^2}S \ham_X S}
\eeq
...
\subsection{Kondo model from PMS-type approach}\label{SWT from PMS}
