...
\eeq
\(\ham\) is the total Hamiltonian and \(\ket{\Psi}\) and \(E\) are the exact eigenstate and eigenvalue of \(\ham\). We imagine a separation of the total Hilbert space into two set of states, and we call these two states \(\ket{0}\) and \(\ket{1}\). This separation depends on which scattering term we want to kill by this transformation. For example, in the URG, we typically select a particular electron \(q\beta\) and then kill the  scattering terms that change the number of this state. In that case, \(\ket{0}\) will refer to the set of states \(\ket{\hat n_{q\beta}=0}\otimes\ket{\phi_0}\) and \(\ket{1}\) will refer to the set of states \(\ket{\hat n_{q\beta}=1}\otimes\ket{\phi_1}\). \(\ket{\phi_{0,1}}\) refer to the states of all the other electrons. As another example, if we wanted to separate the charge-Kondo and the spin-Kondo from the SIAM, we would want to kill the terms that scatter between the spin-full subspace \(\hat n_d=1\) to the spin-less subspace \(\hat{n_d}=0,2\). These two will then be the \(\ket{0}\) and \(\ket{1}\) sets. 
\pb Keeping this separation in mind, the exact eigenstate \(\ket{\Psi}\) can be split as \(\ket{\Psi} = \ket{0,\phi_0} + \ket{1,\phi_1}\). The Hamiltonian can also be split as 
\beq
\ham = H_0 + V_+ + V_-
\eeq
\(H_0\) does not scatter between \(\ket{0}\) and \(\ket{1}\). It contains the diagonal parts as well as scatterings inside the subspaces. \(V_\pm\) scatter between the subspaces:
\beq
V_+ \ket{0} \to \ket{1},&& V_+ \ket{1} \to 0\\
V_- \ket{1} \to \ket{0},&& V_- \ket{0} \to 0
\eeq
The Schrodinger equation can thus be split into
\beq
H_0 \ket{0,\phi_0} + V_-\ket{1,\phi_1} &= E_0 \ket{0,\phi_0}\\
H_0 \ket{1,\phi_1} + V_+\ket{0,\phi_0} &= E_1 \ket{1,\phi_1}\\
\eeq
Eliminating \(\ket{1,\phi_1}\) gives
\beq
H_0 \ket{0,\phi_0} + V_-\frac{1}{E_1 - H_0}V_+\ket{0,\phi_0} &= E_0 \ket{0,\phi_0}\\
\eeq
The effective Hamiltonian in this subspace is therefore
\beq
\tilde \ham_0 = H_0 + V_-\frac{1}{E_1 - H_0}V_+
\eeq
Similarly, eliminating \(\ket{0,\phi_0}\) gives the effective Hamiltonian in the other subspace,
\beq
\tilde \ham_1 = H_0 + V_+\frac{1}{E_0 - H_0}V_-
\eeq
The total effective Hamiltonian that does not scatter between the two subspaces is
\beq
\tilde \ham = H_0 + \underbrace{V_-\frac{1}{E_1 - H_0}V_+ +  V_+\frac{1}{E_0 - H_0}V_-}_\text{renormalization}
\eeq
The renormalization can be written in terms of the matrix elements. In the first term of the renormalization, \(E\) is the energy of the state on which \(V_+\) acts, hence it is the energy of the state \(\ket{0}\), which we call \(E_0\). In the same term, \(H_0\) in the denominator will act on the state \(V_+\ket{0}=\ket{1}\). We will keep only the diagonal part of \(H_0\), which means \(H_0 V_+\ket{0}=H_0 \ket{1}\) will be the energy of the state \(\ket{1}\) which we will call \(E_1\).
\beq
\Delta \ham = V_-\frac{1}{E_0 - E_1}V_+ +  V_+\frac{1}{E_1 - E_0}V_-
\eeq
Since the entire \(\ham_X\) must be Hermitian, we must have \(V_- = V_+^\dagger \equiv V\).
\beq
\Delta \ham = V\frac{1}{E_0 - E_1}V^\dagger +  V^\dagger\frac{1}{E_1 - E_0}V
\eeq
Now take the first term and insert complete bases on both sides of \(V\) and \(V^\dagger\).
\beq
V\frac{1}{E_0 - E_1}V^\dagger = \sum_{ijk}\ket{0_i}\bra{0_i}V\ket{1_j}\bra{1_j}\frac{1}{E_0 - E_1}\ket{1_j}\bra{1_j}V^\dagger\ket{0_k}\bra{0_k} = \sum_{ijk}\ket{0_i}\bra{0_k}\frac{V_{ij}V^\dagger_{kj}}{E^k_0 - E^j_1}
\eeq
where we defined \(\bra{0_i}V\ket{1_j} = V_{ij}\). The second term similarly gives
\beq
V^\dagger\frac{1}{E_1 - E_0}V = \sum_{ijk}\ket{1_i}\bra{1_k}\frac{V^\dagger_{ji}V_{jk}}{E^k_1 - E^j_0}= \sum_{ijk}\ket{1_i}\bra{1_j}\frac{V^\dagger_{ki}V_{kj}}{E^j_1 - E^k_0}
\eeq
The total renormalization becomes
\beq
\Delta \ham = \sum_{ijk}\frac{1}{\Delta_{kj}}\rr{\ket{0_i}\bra{0_k}V_{ij}V^\dagger_{kj} - \ket{1_i}\bra{1_j}V^\dagger_{ki}V_{kj}}
\eeq
where \(\Delta_{kj} = E^k_0 - E^j_1\). This is a general expression that would work irrespective of whether you are decoupling multiple electrons or a single electron. To make a better connection with URG, we next show how the PMS formalism works out for a single-electron decoupling.
...
\pb Keeping in mind this prescription of substituting \(E\) in the denominator with the energy of the initial state, we can write down a more complete expression for the total renormalization brought about by PMS.
\beq
\Delta \ham\bigg\vert_{PMS} &= V_+ \frac{1}{E - V_0 - \ham_0^d} V_- + V_- \frac{1}{E - V_0 - \ham_1^d} V_+ \\
			    &= c_{q\beta}^\dagger T \frac{1}{E - V_0 - \ham_0^d} T^\dagger c_{q\beta} + T^\dagger c_{q\beta} \frac{1}{E - V_0 - \ham_1^d} c_{q\beta}^\dagger T \\
			    &= c_{q\beta}^\dagger T \frac{1}{E_1 - \ham_0^d} T^\dagger c_{q\beta} + T^\dagger c_{q\beta} \frac{1}{E_0 - \ham_1^d} c_{q\beta}^\dagger T \\
			    &= \frac{1}{E_1 - E_0}\rr{ c_{q\beta}^\dagger T T^\dagger c_{q\beta} - T^\dagger c_{q\beta} c_{q\beta}^\dagger T} \\
\eeq
There I used the notation \(V_+ = c^\dagger T = V_-^\dagger\). Also, \(E\) in the first term was replaced by the energy \(E_1\) of the initial state on which \(T^\dagger c\) acts, which must be the state with \(\hat n_{q\beta}=1\). \(\ham_d^0\) on the other hand evaluates according to the intermediate state (\(\hat n_{q\beta}=0\)), hence it was replaced by \(E_0\). The final result is thus
\beq[pmsren]
\Delta \ham\bigg\vert_{PMS} = \frac{1}{E_1 - E_0}\qq{ \hat n_{q\beta} T T^\dagger  - \rr{1 - \hat n_{q\beta}}T^\dagger T} \\
\eeq
We will see later that Schrieffer-Wolff transformation gives the same renormalization.
\begin{figure}
	\centering
...
\beq
S = \sum_{ij}\qq{s\ket{1_i}\bra{0_j} - s^\dagger\ket{0_j}\bra{1_i}}
\eeq
...
\eeq
Multiplying with \(\bra{0_a}\) and \(\ket{1_b}\) from the left and right respectively gives
\beq
-\bra{0_a}V + V^\dagger\ket{1_b} &= \bra{0_a}S H_0 - H_0 S\ket{1_b}\\
\eeq
Since \(V^\dagger\) acts on \(\ket{0}\), it will not affect the LHS. Also, \(\bra{0_a}V\ket{1_b} = V_{ab}\). If we now consider only the diagonal part of \(H_0\), we can write \(H_0 (\ket{0_a},\ket{1_b}) = (E_{0,a}\ket{0_a},E_{1,b}\ket{1_b}))\). We then get
\beq
-V_{ab} &= \bra{0_a}\sum_i\qq{S\ket{1_i}\bra{1_i} H_0 - H_0 \ket{0_i}\bra{0_i}S}\ket{1_b}\\
	&= \sum_i\qq{S_{ai}E_1^i \delta_{bi} - E^i_0 \delta_{ai}S_{ib}}\\
	&= S_{ab}E_1^b  - E^a_0 S_{ab}\\
\implies S_{ab} &= \frac{V_{ab}}{E_0^a - E_1^b}
\eeq
where we defined \(\bra{0_x}S\ket{1_y} = S_{xy}\). The total generator is
\beq
S &= \sum_{ij}\qq{S_{ij}\ket{0_i}\bra{1_j} - S_{ij}^\dagger\ket{1_j}\bra{0_i}}\\
  &= \sum_{ij}\frac{1}{E_0^i - E_1^j}\qq{V_{ij}\ket{0_i}\bra{1_j} - V^\dagger_{ij}\ket{1_j}\bra{0_i}}\\
\eeq
The renormalization is thus
\beq
\Delta \ham &= \hf \qq{S,\ham_X}\\
			      &= \hf \sum_{ij,kl}\qq{\frac{1}{E_0^i - E_1^j}\rr{V_{ij}\ket{0_i}\bra{1_j} - V^\dagger_{ij}\ket{1_j}\bra{0_i}}, V_{kl} \ket{0_k}\bra{1_l}+ V^\dagger_{kl} \ket{1_l}\bra{0_k}}\\
			      &= \hf \sum_{ij,kl}\left[\frac{1}{E_0^i - E_1^j}\left(V_{ij}V^\dagger_{kl}\ket{0_i}\bra{0_k}\delta_{jl} - V^\dagger_{ij}V_{kl}\ket{1_j}\bra{1_l}\delta_{ik}\right.\right.\\
			      &\quad\left.\left.- V^\dagger_{kl}V_{ij}\ket{1_l}\bra{1_j}\delta_{ki} + V_{kl}V^\dagger_{ij}\ket{0_k}\bra{0_i}\delta_{lj}\right)\right]\\
			      &= \hf \sum_{ijk}\left[\frac{1}{E_0^i - E_1^j}\left(V_{ij}V^\dagger_{kj}\ket{0_i}\bra{0_k} - V^\dagger_{ij}V_{ik}\ket{1_j}\bra{1_k}- V^\dagger_{ik}V_{ij}\ket{1_k}\bra{1_j} + V_{kj}V^\dagger_{ij}\ket{0_k}\bra{0_i}\right)\right]\\
			      &= \hf \sum_{ijk}\left[\rr{\frac{1}{E_0^i - E_1^j} + \frac{1}{E_0^k - E_1^j}}V_{ij}V^\dagger_{kj}\ket{0_i}\bra{0_k} - \rr{\frac{1}{E_0^i - E_1^j} + \frac{1}{E_0^i - E_1^k}}V^\dagger_{ij}V_{ik}\ket{1_j}\bra{1_k}\right]\\
			      &= \hf \sum_{ijk}\left[\rr{\frac{1}{\Delta_{ij}} + \frac{1}{\Delta_{kj}}}V_{ij}V^\dagger_{kj}\ket{0_i}\bra{0_k} - \rr{\frac{1}{\Delta_{ij}} + \frac{1}{\Delta_{ik}}}V^\dagger_{ij}V_{ik}\ket{1_j}\bra{1_k}\right]\\
\eeq
where \(\Delta_{ij}= E_0^i - E_1^j\). Comparing with the PMS result eq.~\ref{pmsren}, it is clear that this is not the same result. This expression appears to be more general because of the presence of four terms instead of the two in PMS. If we assume that all the energies in a particular subspace are equal (\(E_0^i = E_0^k\) and \( E_1^j= E_1^k\)), this expression becomes
\beq
\Delta \ham &= \hf \sum_{ijk}\left[\frac{2}{\Delta_{kj}}V_{ij}V^\dagger_{kj}\ket{0_i}\bra{0_k} - \frac{2}{\Delta_{ik}}V^\dagger_{ij}V_{ik}\ket{1_j}\bra{1_k}\right] \\
	    &=\sum_{ijk}\frac{1}{\Delta_{kj}}\left[V_{ij}V^\dagger_{kj}\ket{0_i}\bra{0_k} - V^\dagger_{ki}V_{kj}\ket{1_i}\bra{1_j}\right] \\
\eeq
This is identical to what was obtained from PMS, eq.~\ref{pmsren}. This proves that for the special case in which the energies in the subspace are all equal, SWT reduces to PMS.
...
where 
\beq
\eta &= \sum_{ij}\frac{1}{\omega^j - E_0^i}V_{ij}\ket{0_i}\bra{1_j}\\
\implies \eta^\dagger - \eta &= \sum_{ij}\frac{1}{\omega^j - E_0^i}\rr{V^\dagger_{ij}\ket{1_j}\bra{0_i} - V_{ij}\ket{0_i}\bra{1_j}}
\eeq
Proceeding similarly, we get
\beq
\Delta \ham &= \hf \sum_{ijk}\left[\rr{\frac{1}{\omega^j - E_0^i} + \frac{1}{\omega^k - E_0^i}}V^\dagger_{ij}V_{ik}\ket{1_j}\bra{1_k} - \rr{\frac{1}{\omega^j - E_0^i} + \frac{1}{\omega^j - E_0^k}}V_{ij}V^\dagger_{kj}\ket{0_i}\bra{0_k}\right]\\
\eeq
This is the same as the SWT result after substituting \(\omega\) with the initial state energy \(E_1\).
