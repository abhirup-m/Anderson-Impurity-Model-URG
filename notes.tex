...
\eeq
This renormalizes the \il{\rr{V_q^1}^*} coupling.
\paragraph{3.}
\beq
...
&=\sum_{q\beta k}\rr{1 - \hat n_{q\beta}}\hat n_{d\beta}c^\dagger_{k\ol\beta}c_{d\ol\beta}\fr{V_q V_2}{\omega - \epsilon_q^+ + \epsilon_d}\\
&=\sum_{q\beta k}\hat n_{d\beta}c^\dagger_{k\ol\beta}c_{d\ol\beta}\fr{V^1_q V_2}{\omega - \epsilon_q^+ + \epsilon_d}\\
\eeq
This renormalizes the \il{V_q^1} coupling.
\paragraph{4.}
\il{O(V_2^2)}, ignore for now.
\paragraph{Hole sector}
The intermediate state consists of hole states, obtained by exciting electrons from the lower bandwidth edge (\il{-D}).
...
\beq
		 ...
		 &= \sum_{k\sigma q\beta}V^*_qV_2\hat n_{q\beta}c^\dagger_{d\sigma}c_{k\sigma}\rr{1-\hat n_{d\beta}}\fr{1}{\hat \omega - H_{N-1}^C - \epsilon_d - \rr{\epsilon_d + U}\hat n_{d\ol\beta}} \\
		  ...
		  &= \sum_{kq\beta}\hat n_{q\beta}c^\dagger_{d\beta}c_{k\beta}\fr{V^*_qV_2}{\omega + \epsilon_q^- - \rr{\epsilon_d + U\hat n_{d\ol\beta}}} + \sum_{kq\beta}\hat n_{q\beta}\rr{1-\hat n_{d\beta}}c^\dagger_{d\ol\beta}c_{k\ol\beta}\fr{V^*_qV_2}{\omega + \epsilon_q^- - \epsilon_d - U} \\
		  &= \sum_{kq\beta}V_2c^\dagger_{d\beta}c_{k\beta}\rr{\fr{\rr{V_q^1}^*\hat n_{d\ol\beta}}{\omega + \epsilon_q^- - \epsilon_d - U} + \fr{\rr{V_q^0}^*\rr{1 - \hat n_{d\ol\beta}}}{\omega + \epsilon_q^- - \epsilon_d}} + \sum_{kq\beta}c^\dagger_{d\beta}c_{k\beta}\fr{V_2\rr{V_q^0}^*\rr{1-\hat n_{d\ol\beta}}}{\omega + \epsilon_q^- - \epsilon_d - U} \\
		  &= \sum_{kq\beta}c^\dagger_{d\beta}c_{k\beta}\fr{V_2\rr{V_q^1}^*\hat n_{d\ol\beta}}{\omega + \epsilon_q^- - \epsilon_d - U} + \sum_{kq\beta}c^\dagger_{d\beta}c_{k\beta}\cc{\fr{V_2\rr{V_q^0}^*\rr{1 - \hat n_{d\ol\beta}}}{\omega + \epsilon_q^- - \epsilon_d} + \fr{V_2\rr{V_q^0}^*\rr{1 - \hat n_{d\ol\beta}}}{\omega + \epsilon_q^- - \epsilon_d - U}} \\
\eeq
\subsubsection*{Scaling equations}
\beq
...
\Delta V_q^1 &= \fr{V_2 V_q^1}{\omega - \epsilon_q^+ + \epsilon_d} + \fr{V_2 V_q^1}{\omega + \epsilon_q^- - \epsilon_k^+}\\
\Delta {V_q^1}^* &= \fr{V_2 {V_q^1}^*}{\omega + \epsilon_k^- - \epsilon_q^+} + \fr{V_2 {V_q^1}^*}{\omega + \epsilon_q^- - \epsilon_d - U}\\
\Delta V_q^0 &= \fr{2V_2 V_q^0}{\omega + \epsilon_q^- - \epsilon_k^+}\\
\Delta {V_q^0}^* &= \fr{V_2 {V_q^0}^*}{\omega + \epsilon_q^- - \epsilon_d} + \fr{V_2 {V_q^0}^*}{\omega + \epsilon_q^- - \epsilon_d - U}\\
\eeq
