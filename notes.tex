...
\beq
\Delta \ham &= \sum_{kq\sigma\alpha}V_kc^\dagger_{k\sigma}c_{d\sigma}\frac{1}{\omega - \ham^d}V_q^* c^\dagger_{d\alpha}c_{q\alpha} + \sum_{kq\sigma\alpha}V_q^* c^\dagger_{d\alpha}c_{q\alpha}\frac{1}{\omega - \ham^d}V_k c^\dagger_{k\sigma}c_{d\sigma}
\eeq
In order to connect with SWT, we now need to replace \(\omega\) with the energies of the initial states. Meanwhile, \(\ham^d\) is the energy of the intermediate state. The denominator \(\omega - \ham^d\) then gives the change in energy between the initial and final states.
\comm{
