...
\beq
\qq{\ham^d + c^\dagger_{q\beta}T + T^\dagger c_{q\beta}}\ket{\psi_i(\omega_i)} = \rr{\omega_i^1 - \ham^d_1}\ket{1}\ket{\phi_1^i} + \rr{\omega_i^0 - \ham^d_0}\ket{0}\ket{\phi_0^i}
\eeq
If we now substitue the expansion \ref{matpsi} and gather the terms that result in \(\hat n_{q\beta}=1\), we get
\beq[changea]
c^\dagger_{q\beta}T\ket{0}\ket{\phi_0^i} = \rr{\omega_i^1 - \ham^d_1}\ket{1}\ket{\phi_1^i}
\eeq
Similarly, gathering the terms that result in \(\hat n_{q\beta}=0\) gives
\beq[changeb]
T^\dagger c_{q\beta}\ket{1}\ket{\phi_1^i} = \rr{\omega_i^0 - \ham^d_0}\ket{0}\ket{\phi_0^i}
\eeq
We now define two many-particle transition operators:
\beq
{\eta_1}^\dagger(\omega_i^1) = \fr{1}{\omega_i^1- \ham^d_1}c^\dagger_{q\beta}T \\
\eta_0(\omega_i^0) = \fr{1}{\omega_i^0- \ham^d_0}T^\dagger c_{q\beta}
\eeq
...
\beq[constraint]
\eta_0^+ = \eta_1^\dagger \implies \fr{1}{\omega_i^1- \ham^d_1}c^\dagger_{q\beta}T = c^\dagger_{q\beta}T\fr{1}{\omega_i^0- \ham^d_0}
\eeq
Hereafter, we will assume that this constraint has been imposed, and we will drop the labels \(1\) and \(0\) on the \(\eta\):
\beq[etadagdef]
\eta^\dagger = \fr{1}{\omega_i^1- \ham^d_1}c^\dagger_{q\beta}T \\
\eta = \fr{1}{\omega_i^0- \ham^d_0}T^\dagger c_{q\beta}
\eeq
...
\eeq
and so on. Since \(U_0\) is obtained from \(U_1\) simply by replacing \(\eta\) with \(\eta^\dagger\), we can easily surmise that the unitary transformation that rotates an eigenstate \(\ket{\psi_0}\) to the state \(\ket{0}\) is
\beq
U_0 = \fr{1}{\sqrt 2}\rr{1 + \eta - \eta^\dagger} = \ex{\frac{\pi}{4}\rr{\eta - \eta^\dagger}}
\eeq
...
\eeq
Group 2 consists of purely off-diagonal terms; they amount to 0. To see how, note that terms that have two \il{\eta} or two \il{\eta^\dagger} can only be nonzero if the intervening \il{\ham} has an odd number of creation or destruction operators.
\beq[beats]
...
\beq
 \eta c_q^\dagger  T \eta &= \fr{1}{\omega_i^0 - \ham^d_0}T^\dagger c_q c_q^\dagger  T \eta\\
			      &= T^\dagger c_q \fr{1}{\omega_i^1 - \ham^d_1}c_q^\dagger  T \eta && \qq{\text{eq.~\ref{constraint}}}\\
...
\begin{gather*}
\omega_i^1 - \ham^d_1 = \omega_i^0 - \ham^d_0 = 0
\end{gather*}
We see that a the fixed point, the eigenvalue of \(\hat \omega_i\) matches the eigenvalues of the diagonal blocks. This also leads to the vanishing of the off-diagonal block, because eqs.~\ref{changea} and \ref{changeb} gives
\beq
...
\beq
\Delta_1 \ham = c_{q\beta}^\dagger \text{Tr}\rr{\ham c_{q\beta}}\eta
\eeq
...
\beq
\Delta_0 \ham = -\eta c_{q\beta}^\dagger \text{Tr}\rr{\ham c_{q\beta}}
\eeq
Since the effective Hamiltonian eq.~\ref{final2} was derived with the aim of obtaining the particle sector diagonal part, it makes sense to repeat the calculation but now obtain the unitary from the other equation:
\beq
\ket{\psi_0} &= \rr{1 + {\eta}^\dagger}\ket{0}\ket{\phi_0^i} \implies U_0 = 1 -\eta^\dagger + \eta
\eeq
We can similarly obtain a renormalised Hamiltonian for this unitary; it comes out to be
\beq
\Delta \ham =\eta c_{q\beta}^\dagger \text{Tr}\rr{\ham c_{q\beta}} - c_{q\beta}^\dagger \text{Tr}\rr{\ham c_{q\beta}}\eta
\eeq
The hole sector renormalization then comes out to be
\beq
\Delta_0 \ham = \eta c_{q\beta}^\dagger \text{Tr}\rr{\ham c_{q\beta}}
\eeq
The renormalization due to the entire shell is obtained by summing over all states on the shell. Written out explicitly, they take the forms.
\beq[deltah]
\Delta_0 \ham &= \sum_{q\beta} c_{q\beta}^\dagger \text{Tr}\rr{\ham c_{q\beta}}\fr{1}{\omega_1 - \ham^d_1}\text{Tr}\rr{c^\dagger_{q\beta}\ham }c_{q\beta}\\
\eeq
\beq[deltap]
\Delta_1 \ham &= \sum_{q\beta} \text{Tr}\rr{c^\dagger_{q\beta}\ham }c_{q\beta}\fr{1}{\omega_0 - \ham^d_0}c_{q\beta}^\dagger \text{Tr}\rr{\ham c_{q\beta}}\\
\eeq
