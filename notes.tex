...
The \(\omega\) have to be determined self-consistently or numerically by searching for fixed points where they become identical to the energy eigenvalues. As a simplification, we set \(\omega_e = \omega_h\).
\beq
\Delta E_2 &= 2\sum_{q,\epsilon_q=+D}\fr{|V_q|^2}{\omega - \hf \epsilon_q - \epsilon_d}\\
\Delta E_0 &= 2\sum_{q,\epsilon_q=-D}\fr{|V_q|^2}{\omega - \hf \epsilon_q - \epsilon_d}\\
\Delta E_1 &= \sum_{q,\epsilon_q=-D}\fr{|V_q|^2}{\omega - \hf \epsilon_q - 2\epsilon_d - U} + \sum_{q,\epsilon_q=+D}\fr{|V_q|^2}{\omega - \hf \epsilon_q}\\
\eeq
...
\beq
\Delta \epsilon_d &= \sum_{q,\epsilon_q=-D}\fr{|V_q|^2}{\omega - \hf \epsilon_q - 2\epsilon_d - U} + \sum_{q,\epsilon_q=+D}\fr{|V_q|^2}{\omega - \hf \epsilon_q} - 2\sum_{q,\epsilon_q=-D}\fr{|V_q|^2}{\omega - \hf \epsilon_q - \epsilon_d}\\
\Delta U &= 2\sum_{q,|\epsilon_q|=D}\fr{|V_q|^2}{\omega - \hf \epsilon_q - \epsilon_d} - \sum_{q,\epsilon_q=-D}\fr{|V_q|^2}{\omega - \hf \epsilon_q - 2\epsilon_d - U} - \sum_{q,\epsilon_q=+D}\fr{|V_q|^2}{\omega - \hf \epsilon_q}\\
\eeq
\subsubsection*{Connection to Poor Man's scaling}
To obtain the results of Poor Man's scaling \cite{haldane}\cite{Jefferson},  we can look at various regimes. First, for \(U,\epsilon_d \ll \epsilon_q\), we get
\beq
\Delta \epsilon_d &= \sum_{q,|\epsilon_q|=D}\fr{|V_q|^2}{\omega - \hf \epsilon_q} - 2\sum_{q,\epsilon_q=-D}\fr{|V_q|^2}{\omega - \hf \epsilon_q}\\
\Delta U &= 2\sum_{q,|\epsilon_q|=D}\fr{|V_q|^2}{\omega - \hf \epsilon_q} - \sum_{q,\epsilon_q=-D}\fr{|V_q|^2}{\omega - \hf \epsilon_q} - \sum_{q,\epsilon_q=+D}\fr{|V_q|^2}{\omega - \hf \epsilon_q}\\
\eeq
Assuming the upper and lower band edges are symmetrical such that \(\sum_{-D} = \sum_D\), we get \(\Delta \epsilon_d = \Delta U = 0\). 
\pb In the regime \(U \gg \epsilon_q \gg \epsilon_d\), we get
\beq
\Delta \epsilon_d &= \sum_{q,\epsilon_q=+D}\fr{|V_q|^2}{\omega - \hf \epsilon_q} - 2\sum_{q,\epsilon_q=-D}\fr{|V_q|^2}{\omega - \hf \epsilon_q}\\
\Delta U &= 2\sum_{q,|\epsilon_q|=D}\fr{|V_q|^2}{\omega - \hf \epsilon_q} - \sum_{q,\epsilon_q=+D}\fr{|V_q|^2}{\omega - \hf \epsilon_q}\\
\eeq
Again assuming symmetrical upper and lower edges and \(|\epsilon_q|=D\) and \(\sum_q |V|^2 = \frac{\Delta}{\pi}|\delta D|\), we get
\beq
\Delta U &= 0\\
\delta \epsilon_d &= -\frac{\Delta}{\pi}\fr{1}{\omega - \hf D}
\eeq
There we replaced the change symbol \(\Delta\) for \(\delta\) to avoid confusion with the hybridisation \(\Delta \sim \sum V^2\). If we recall that \(\omega\) was defined as \(\tilde \ham - \ham^i\), we can make a crude estimate of \(\omega\) by ignoring the off-diagonal part and replace it with the renormalized diagonal part of the initial state from which the particle excitations arise (\(\epsilon_q \tau_{q\beta} = -\hf D\)):
\beq
...
\qq{S,\ham_0} = -\ham_X
\eeq
In order to decouple a state \(q\beta\) from the SIAM (\(\epsilon_q > 0\)) , we take an ansatz \(S = (A + B\hat n_{d\ol\beta})(c^\dagger_{q\beta}c_{d\beta} - \text{h.c.})\). Plugging this into the decoupling condition gives
\beq
