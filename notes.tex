...
Friedel\footnote{Friedel 1958}, in 1958, gave a phenomenological theory in which a local impurity developed an effective repulsion which forced the formation of bound states; those bound states where the up and down states became non-degenerate would correspond to the local moment.
Taking inspiration from this, P.W.Anderson\footnote{Anderson 1961} in 1961 designed a model for the formation of local moment in second quantization.
...
In 1964, Jun Kondo\footnote{Kondo 1964} found that a perturbative calculation of the transition probability of electrons from scattering via the impurity, up to second order in the exchange coupling \(J\), revealed a logarithmic dependence of the resistivity on temperature.
...
\section{URG Formalism}\label{section2}
...
\beq
\sum_{k^\prime q\beta k} c^\dagger_{k\beta}c_{k^\prime\beta}u_S^2\qq{ \fr{\hat n_d - 2\hat n_{d\ua}\hat n_{d\da}} {\omega - \epsilon_q^+ + \epsilon_{k^\prime}}}\\
\eeq
\paragraph{7.}
\beq
...
\Delta^+_7 \ham_N &= \sum_{q\beta kk^\prime}u_S(k,q) u_A(q,k^\prime) c^\dagger_{k\beta}c_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}\fr{-1}{\omega - H_1}\\
		  &= \sum_{q\beta kk^\prime}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{-u_S(k,q) u_A(q,k^\prime) }{\omega - \epsilon_{q}^+ + \epsilon_{k^\prime} + u_S(q^+) + u_S(k^\prime)}\\
...
\beq
H_0 = \epsilon_{k^\prime} + \mc{E}_0 + \epsilon_d  = H_1 - \epsilon_q^+ + \epsilon_{k^\prime} - u_S(k^\prime) + u_S(q^+)
\eeq
\beq
\Delta^+_8 \ham_N &=- \sum_{q\beta kk^\prime}u_A(k,q) u_S(q,k^\prime) c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}c_{q\beta}^\dagger c_{k^\prime\beta}\fr{1}{\omega - H_0 - \epsilon_q^+ + \epsilon_{k^\prime}- u_S(k^\prime) + u_S(q^+)}\\
		  &= \sum_{q\beta kk^\prime}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{-u_A(k,q) u_S(q,k^\prime) }{\omega - H_0 - \epsilon_q^+ + \epsilon_{k^\prime}- u_S(k^\prime) + u_S(q^+)}\\
...
\Delta^+_2 \ham_N = -\sum_{q\beta k}u_S V_q^*c^\dagger_{d\beta}c_{q\beta}\hat n_{d\ol\beta}\fr{1}{\omega - \ham_D^+}c_{q\beta}^\dagger c_{k\beta}
\eeq
Intermediate (\il{\hat n_{q\beta}=1, \hat n_{d\ol\beta}=1,\hat n_{d\beta}=0}) energy is
\beq
H_1 = \epsilon_q^+ + u_S\beta m_d + \epsilon_d + \mc{E}_0  = \epsilon_q^+ - u_S + \epsilon_d
\eeq
...
\pb
The initial (\il{\hat n_{q\beta}=0, \hat n_{d\ol\beta}=1,\hat n_{d\beta}=0}) energy is
\beq
H_0 = \mc{E}_0 + \epsilon_d = H_1 - \epsilon_q^+ + u_S
\eeq
\beq
\Delta^+_2 \ham_N &= -\sum_{q\beta k}u_S V_q^*c^\dagger_{d\beta}c_{q\beta}\hat n_{d\ol\beta}c_{q\beta}^\dagger c_{k\beta}\fr{1}{\omega - H_1 - \epsilon_q^+ + u_S}\\
		  &= \sum_{q\beta k}c^\dagger_{d\beta}c_{k\beta}\fr{-\hat n_{d\ol\beta}u_S {V_q^1}^*}{\omega - \epsilon_q^+ + u_S}
\eeq
\paragraph{3.}
\beq
\Delta^+_3 \ham_N = \sum_{q\beta k}u_A V_q^*c^\dagger_{d\beta}c_{q\beta}\fr{1}{\omega - \ham_D^+}c^\dagger_{d\ol\beta}c_{d\beta} c_{q\beta}^\dagger c_{k\ol\beta}
\eeq
Intermediate (\il{\hat n_{d\beta}=0,\hat n_{q\beta}=\hat n_{d\ol\beta}=1}) energy is
\beq
H_1 = \epsilon_q^+ - u_S + \epsilon_d + \mc{E}_0
\eeq
The initial (\il{\hat n_{d\beta}=1,\hat n_{q\beta}=\hat n_{d\ol\beta}=0}) energy is
\beq
H_0 = \epsilon_k + \mc{E}_0 + \epsilon_d = H_1 - \epsilon_q^+ + u_S
\eeq
\beq
\Delta^+_2 \ham_N &= \sum_{q\beta k}u_A V_q^*c^\dagger_{d\beta}c_{q\beta}c^\dagger_{d\ol\beta} c_{d\beta}c_{q\beta}^\dagger c_{k\ol\beta}\fr{1}{\omega - H_1  - \epsilon_q^+ + u_S}\\
		  &= \sum_{q\beta k}-u_A V_q^*\hat n_{d\beta}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\ol\beta}c_{k\ol\beta}\fr{1}{\omega - H_1  - \epsilon_q^+ + u_S}\\
		  &= \sum_{q\beta k}c^\dagger_{d\beta}c_{k\beta}\fr{-u_A {V^1_q}^*\hat n_{d\ol\beta}}{\omega  - \epsilon_q^+ + u_S}
\eeq
\paragraph{4.}
\beq
\Delta^+_4 \ham_N = \sum_{q\beta k\sigma}u_S V_q\rr{\hat n_{d\beta} - \hat n_{d\ol\beta}}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega - \ham_D^+}c_{q\beta}^\dagger c_{d\beta}
...
H_1 = \epsilon_q^+ - u_S + \epsilon_d + \mc{E}_0
\eeq
The initial (\il{\hat n_{q\beta}=0,\hat n_{d\beta}=\hat n_{d\ol\beta}=1}) energy is
\beq
H_0 = 2\epsilon_d + U  + \mc{E}_0 = H_1 + \epsilon_d + U - \epsilon_q^+ + u_S
\eeq
\beq
\Delta^+_4 \ham_N &= -\sum_{q\beta k}u_S V_q \hat n_{d\ol\beta}c^\dagger_{k\beta}c_{q\beta}c_{q\beta}^\dagger c_{d\beta}\fr{1}{\omega - H_0 + \epsilon_d + U - \epsilon_q^+ + u_S}\\
		  &= \sum_{q\beta k}u_S V_q \hat n_{d\ol\beta}\rr{1 - \hat n_{q\beta}}c^\dagger_{k\beta} c_{d\beta}\fr{-1}{\omega + \epsilon_d + U - \epsilon_q^+ + u_S}\\
		  &= \sum_{q\beta k}c^\dagger_{k\beta} c_{d\beta}\fr{-u_S V_q^1 \hat n_{d\ol\beta}}{\omega - \epsilon_q^+ + \epsilon_d + U + u_S}
\eeq
\paragraph{5.}
...
\rr{\hat n_{d\beta} - \hat n_{d\ol\beta}}^2 = \hat n_{d\beta} + \hat n_{d\ol\beta} - 2\hat n_{d\beta}\hat n_{d\ol\beta} = \hat n_d - 2\hat n_{d\beta}\hat n_{d\ol\beta}
\eeq
Intermediate (\il{\hat n_{q\beta}=1}) energy is
\beq
H_1 &= \epsilon_q^+ + u_S(q^+) \rr{\hat n_{d\beta} - \hat n_{d\ol\beta} } + \epsilon_d\hat n_{d} + U\hat n_{d\ua}\hat n_{d\da} + \mc{E}_0
\eeq
The initial (\il{\hat n_{q\beta}=0}) energy is
\beq
H_0 =  \mc{E}_0 + \epsilon_d\hat n_{d} + U\hat n_{d\ua}\hat n_{d\da} = H_1 - \epsilon_q^+ - u_S(q^+)\rr{\hat n_{d\beta} - \hat n_{d\ol\beta}}
\eeq
\beq
\Delta^+_6 \ham_N &= \sum_{k^\prime q\beta k}  \rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}c^\dagger_{k\beta}c_{q\beta} c^\dagger_{q\beta}c_{k^\prime\beta}\fr{u_S(k,q)u_S(q,k^\prime)}{\omega - H_1}\\
		  &= \sum_{k^\prime q\beta k}  \rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}\rr{1  - \hat n_{q\beta}}c^\dagger_{k\beta}c_{k^\prime\beta} \fr{u_S(k,q)u_S(q,k^\prime)}{\omega - H_0 - \epsilon_q^+ - u_S(q^+)\rr{\hat n_{d\beta} - \hat n_{d\ol\beta}}}\\
		  &= \sum_{k^\prime q\beta k} c^\dagger_{k\beta}c_{k^\prime\beta} \fr{u_S(k,q)u_S(q,k^\prime) \rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}}{\omega - \epsilon_q^+ - u_S(q^+)\rr{\hat n_{d\beta} - \hat n_{d\ol\beta}}}\\
		  &= \sum_{k^\prime q\beta k} c^\dagger_{k\beta}c_{k^\prime\beta}\qq{ \fr{u_S(k,q)u_S(q,k^\prime)\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}} {\omega - \epsilon_q^+ - u_S(q^+) } + \fr{u_S(k,q)u_S(q,k^\prime)\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}} {\omega - \epsilon_q^+ + u_S(q^+) }}
\eeq
...
Intermediate (\il{\hat n_{d\beta}=0,\hat n_{q\beta}=\hat n_{d\ol\beta}=1}) energy is
\beq
H_1 = \epsilon_q^+ - u_S(q^+) + \epsilon_d + \mc{E}_0
\eeq
The initial (\il{\hat n_{d\beta}=1,\hat n_{q\beta}=\hat n_{d\ol\beta}=0}) energy is
\beq
H_0 = \mc{E}_0 + \epsilon_d = H_1 - \epsilon_{q}^+ + u_S(q^+)
\eeq
\beq
\Delta^+_7 \ham_N &= \sum_{q\beta kk^\prime}u_S(k,q) u_A(q,k^\prime) c^\dagger_{k\beta}c_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}\fr{-1}{\omega - H_1}\\
		  &= \sum_{q\beta kk^\prime}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{-u_S(k,q) u_A(q,k^\prime) }{\omega - \epsilon_{q}^+  + u_S(q^+)}\\
		  &= \sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{-u_S(k,q) u_A(q,k^\prime) }{\omega - \epsilon_{q}^+ + u_S(q^+)}\\
\eeq
\paragraph{8.}
\beq
\Delta^+_8 \ham_N = \sum_{q\beta kk^\prime}u_A(k,q) u_S(q,k^\prime) c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega - \ham_D^+}\rr{\hat n_{d\beta} - \hat n_{d\ol\beta}}c_{q\beta}^\dagger c_{k^\prime\beta}
...
\beq
H_0 = \mc{E}_0 + \epsilon_d  = H_1 - \epsilon_q^+ + u_S(q^+)
\eeq
\beq
\Delta^+_8 \ham_N &=- \sum_{q\beta kk^\prime}u_A(k,q) u_S(q,k^\prime) c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}c_{q\beta}^\dagger c_{k^\prime\beta}\fr{1}{\omega - H_0 - \epsilon_q^+  + u_S(q^+)}\\
		  &= \sum_{q\beta kk^\prime}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{-u_A(k,q) u_S(q,k^\prime) }{\omega - H_0 - \epsilon_q^+  + u_S(q^+)}\\
		  &= \sum_{q\beta kk^\prime}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{-u_A(k,q) u_S(q,k^\prime) }{\omega - \epsilon_q^+  + u_S(q^+)}\\
\eeq
\paragraph{9.}
\beq
\Delta^+_9 \ham_N = \sum_{q\beta k k\prime}u_A(k,q)u_A(q,k^\prime) c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta} c_{q\beta}\fr{1}{\omega - \ham_D^+}c^\dagger_{d\ol\beta}c_{d\beta}c_{q\beta}^\dagger c_{k^\prime\ol\beta}
...
\eeq
\beq
\Delta^+_9 \ham_N &= \sum_{q\beta k k\prime}u_A(k,q)u_A(q,k^\prime) c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta} c_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c_{q\beta}^\dagger c_{k^\prime\ol\beta}\fr{1}{\omega - H_0 - \epsilon_q^+ +u_S(q^+)}\\
		  &= \sum_{q\beta k k\prime}\rr{1 - \hat n_{q\beta}} \hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}c^\dagger_{k\ol\beta}c_{k^\prime\ol\beta}\fr{u_A(k,q)u_A(q,k^\prime)}{\omega - H_0 - \epsilon_q^+ +u_S(q^+)}\\
		  &= \sum_{q\beta k k\prime}c^\dagger_{k\beta}c_{k^\prime\beta}\fr{u_A(k,q)u_A(q,k^\prime)\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega - \epsilon_q^+ +u_S(q^+)}\\
\eeq
\subsubsection{Hole sector}
The renormalization in the Hamiltonian in the hole sector is
\beq
...
The intermediate (\il{\hat n_{q\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta} + \mc{E}_0 - u_S\rr{1 - \hat n_{d\ol\beta}}
\eeq
The initial state (\il{\hat n_{q\beta}=\hat n_{d\beta}=1}) energy is
\beq
H_0 &= \epsilon^-_{q} + \mc{E}_0 + \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta}\\
    &= H_1 + \epsilon^-_q + u_S\rr{1 - \hat n_{d\ol\beta}}
\eeq
\beq
\Delta_2^- \ham_N &= \sum_{q\beta k}u_S V_q c^\dagger_{q\beta}c_{d\beta}\rr{1 - \hat n_{d\ol\beta}}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega - H_1}\\
&= \sum_{q\beta k}\hat n_{q\beta}\rr{1 - \hat n_{d\ol\beta}}c^\dagger_{k\beta}c_{d\beta}\fr{-u_S V_q}{\omega + \epsilon^-_q + u_S}\\
&= \sum_{q\beta k}c^\dagger_{k\beta}c_{d\beta}\fr{-u_S V_q^0\rr{1 - \hat n_{d\ol\beta}}}{\omega + \epsilon^-_q + u_S}\\
\eeq
\paragraph{3.}
\beq
\Delta_3^- \ham_N &= \sum_{q\beta k}u_A V_q c^\dagger_{q\beta}c_{d\beta}\fr{1}{\hat \omega - \ham_D^-} c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}
...
The intermediate (\il{\hat n_{q\beta}=\hat n_{d\ol\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = \epsilon_d + \mc{E}_0 - u_S
\eeq
The initial state (\il{\hat n_{q\beta}=\hat n_{d\ol\beta}=1,\hat n_{d\beta}=0}) energy is
\beq
H_0 &= \epsilon_{q}^- + \mc{E}_0 + \epsilon_d = H_1 + \epsilon^-_q + u_S
\eeq
\beq
\Delta_3^- \ham_N &= \sum_{q\beta k}u_A V_q c^\dagger_{q\beta}c_{d\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega - H_1}\\
		  &= \sum_{q\beta k}u_A V_q \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}c^\dagger_{k\ol\beta}c_{d\ol\beta}\fr{-1}{\omega + \epsilon^-_q + u_S}\\
		  &= \sum_{q\beta k}c^\dagger_{k\beta}c_{d\beta}\fr{-u_A V_q^0 \rr{1 - \hat n_{d\ol\beta}}}{\omega + \epsilon^-_q + u_S}\\
\eeq
\paragraph{4.}
\beq
\Delta_4^- \ham_N &= \sum_{q\beta k}u_S V^*_q \rr{\hat n_{d\beta} - \hat n_{d\ol\beta}}c^\dagger_{q\beta}c_{k\beta}\fr{1}{\hat \omega - \ham_D^-} c^\dagger_{d\beta}c_{q\beta}
...
\Delta_4^- \ham_N &= \sum_{q\beta k}u_S V^*_q \rr{1 - \hat n_{d\ol\beta}}c^\dagger_{q\beta}c_{k\beta}c^\dagger_{d\beta}c_{q\beta}\fr{1}{\omega - H_0+ \epsilon_{q}^- + u_S  - \epsilon_d - U\hat n_{d\ol\beta}} \\
		  &= \sum_{q\beta k}\hat n_{q\beta}u_S V^*_q \rr{1 - \hat n_{d\ol\beta}}c_{k\beta}c^\dagger_{d\beta}\fr{1}{\omega + \epsilon_{q}^- + u_S  - \epsilon_d - U\hat n_{d\ol\beta}} \\
		  &= \sum_{q\beta k}c^\dagger_{d\beta}c_{k\beta}\fr{-u_S {V^0_q}^* \rr{1 - \hat n_{d\ol\beta}}}{\omega + \epsilon_{q}^- - \epsilon_d+ u_S  } \\
\eeq
\paragraph{5.}
\beq
\Delta_5^- \ham_N &= \sum_{q\beta k}u_A V_q^* c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k\ol\beta}\fr{1}{\hat \omega - \ham_D^-}c^\dagger_{d\beta}c_{q\beta}
...
The initial state (\il{\hat n_{d\beta}=\hat n_{d\ol\beta}=0,\hat n_{q\beta}=1}) energy is
\beq
H_0 &= \mc{E}_0 + \epsilon_{q}^- = H_1 + \epsilon^-_q + u_S - \epsilon_d
\eeq
\beq
\Delta_5^- \ham_N &= \sum_{q\beta k}u_A V_q^* c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k\ol\beta}c^\dagger_{d\beta}c_{q\beta}\fr{1}{\hat \omega - H_0 + \epsilon^-_q + u_S - \epsilon_d}\\
		  &= \sum_{q\beta k}u_A V_q^* \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}c^\dagger_{d\ol\beta}c_{k\ol\beta}\fr{-1}{\omega + \epsilon^-_q + u_S - \epsilon_d}\\
		  &= \sum_{q\beta k}c^\dagger_{d\ol\beta}c_{k\ol\beta}\fr{-u_A {V_q^0}^* \rr{1 - \hat n_{d\beta}}}{\omega + \epsilon^-_q - \epsilon_d+ u_S }
\eeq
\paragraph{6.}
\beq
\Delta_6^- \ham_N &= \sum_{q\beta kk^\prime}u_S(q,k^\prime)u_S(k,q) \rr{\hat n_{d\beta} - \hat n_{d\ol\beta}}c^\dagger_{q\beta}c_{k^\prime\beta}\fr{1}{\hat \omega - \ham_D^-}\rr{\hat n_{d\beta} - \hat n_{d\ol\beta}}c^\dagger_{k\beta}c_{q\beta}
...
\beq
H_1 &= \epsilon_d\hat n_d + U \hat n_{d\ua}\hat n_{d\da} + \mc{E}_0 - u_S(q^-) \rr{\hat n_{d\beta} - \hat n_{d\ol\beta}}
\eeq
The initial state (\il{\hat n_{q\beta}=1}) energy is
\beq
H_0 = \epsilon_{q}^- + \mc{E}_0 + \epsilon_d\hat n_d + U \hat n_{d\ua}\hat n_{d\da} = H_1 + \epsilon_q^- + u_S(q^-) \rr{\hat n_{d\beta} - \hat n_{d\ol\beta}}
\eeq
\beq
\Delta_6^- \ham_N &= \sum_{q\beta kk^\prime}u_S(q,k^\prime)u_S(k,q)\rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}c^\dagger_{q\beta}c_{k^\prime\beta}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega - H_1}\\
		  &= \sum_{q\beta kk^\prime} \hat n_{q\beta}\rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}c_{k^\prime\beta}c^\dagger_{k\beta}\fr{u_S(q,k^\prime)u_S(k,q)}{\omega - H_0 + \epsilon_q^-  + u_S(q^-) \rr{\hat n_{d\beta} - \hat n_{d\ol\beta}}}\\
		  &= -\sum_{q\beta kk^\prime}c^\dagger_{k\beta}c_{k^\prime\beta}\qq{\fr{u_S(q,k^\prime)u_S(k,q)\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega + \epsilon_q^- + u_S(q^-)} + \fr{u_S(q,k^\prime)u_S(k,q)\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega + \epsilon_q^- - u_S(q^-)}} \\
		  &\quad+ \sum_{q\beta k}u_S(q,k^\prime)u_S(k,q)\qq{\fr{\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega + \epsilon_q^- + u_S(q^-)} + \fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega + \epsilon_q^- - u_S(q^-)}}\\
\eeq
\paragraph{7.}
\beq
\Delta_7^- \ham_N &= \sum_{q\beta kk^\prime}u_S(q,k^\prime)u_A(k,q) \rr{\hat n_{d\beta} - \hat n_{d\ol\beta}}c^\dagger_{q\beta}c_{k^\prime\beta}\fr{1}{\hat \omega - \ham_D^-} c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}
...
The intermediate (\il{\hat n_{q\beta}=\hat n_{d\ol\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = \epsilon_d + \mc{E}_0 - u_S(q^-)
\eeq
The initial state (\il{\hat n_{q\beta}=\hat n_{d\ol\beta}=1,\hat n_{d\beta}=0}) energy is
\beq
H_0 &= \epsilon_{q}^- + \mc{E}_0 + \epsilon_d = H_1 + \epsilon^-_q + u_S(q^-)
\eeq
\beq
\Delta_7^- \ham_N &= \sum_{q\beta kk^\prime}u_S(q,k^\prime)u_A(k,q) c^\dagger_{q\beta}c_{k^\prime\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega - H_1} \\
		  &= \sum_{q\beta kk^\prime}u_S(q,k^\prime)u_A(k,q) \hat n_{q\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{-1}{\omega -H_0 + \epsilon^-_q  + u_S(q^-)} \\
		  &= \sum_{q\beta kk^\prime}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{-u_S(q,k^\prime)u_A(k,q) }{\omega + \epsilon^-_q  + u_S(q^-)} \\
		  &= \sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{-u_S(q,k^\prime)u_A(k,q) }{\omega + \epsilon^-_q  + u_S(q^-)}
\eeq
\paragraph{8.}
\beq
\Delta_8^- \ham_N &= \sum_{q\beta kk^\prime}u_A(q,k^\prime)u_S(k,q) c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}\fr{1}{\hat \omega - \ham_D^-}\rr{\hat n_{d\beta} - \hat n_{d\ol\beta}}c^\dagger_{k\beta}c_{q\beta}
...
The intermediate (\il{\hat n_{q\beta}=\hat n_{d\ol\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = \mc{E}_0 - u_S(q^-) + \epsilon_d
\eeq
The initial state (\il{\hat n_{d\ol\beta}=0,\hat n_{q\beta}=\hat n_{d\beta}=1}) energy is
\beq
H_0 &= \epsilon_{q}^- + \mc{E}_0 + \epsilon_d = H_1 + \epsilon^-_q +  u_S(q^-)
\eeq
\beq
\Delta_8^- \ham_N &= \sum_{q\beta kk^\prime}u_A(q,k^\prime)u_S(k,q) c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega - H_1}\\
		  &= \sum_{q\beta kk^\prime}u_A(q,k^\prime)u_S(k,q) \hat n_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{-1}{\omega - H_0 + \epsilon^-_q + u_S(q^-)}\\
		  &= \sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{-u_A(q,k^\prime)u_S(k,q) }{\omega + \epsilon^-_q + u_S(q^-)}\\
\eeq
\paragraph{9.}
\beq
\Delta_9^- \ham_N &= \sum_{q\beta kk^\prime}u_A(q,k^\prime)u_A(k,q)c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}\fr{1}{\hat \omega - \ham_D^-}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}
...
\Delta_9^- \ham_N &= \sum_{q\beta kk^\prime}u_A^2c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}c^\dagger_{d\beta} c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega - H_1}\\
		  &= \sum_{q\beta kk^\prime}u_A^2\hat n_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c_{k^\prime\ol\beta}c^\dagger_{d\beta} c_{d\ol\beta}c^\dagger_{k\ol\beta}\fr{1}{\omega - H_1}\\
		  &= -\sum_{q\beta kk^\prime}u_A^2\hat n_{q\beta}\hat n_{d\ol\beta}c_{d\beta}c_{k^\prime\ol\beta}c^\dagger_{d\beta}c^\dagger_{k\ol\beta}\fr{1}{\omega - H_1}\\
		  &= \sum_{q\beta kk^\prime}u_A^2\hat n_{q\beta}\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}c_{k^\prime\ol\beta}c^\dagger_{k\ol\beta}\fr{1}{\omega + \epsilon_q^-  + u_S(q^-)}\\
		  &= \sum_{q\beta kk^\prime}c^\dagger_{k\beta}c_{k^\prime\beta}\fr{-u_A^2\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega + \epsilon_q^- + u_S(q^-)} + \sum_{q k}\fr{u_A^2\rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}}{\omega + \epsilon_q^- + u_S(q^-)}\\
\eeq
\subsubsection{Scaling equations}
\begin{align*}
	\Delta \epsilon_d &= \sum_q \rr{\frac{|V_q^0|^2}{\omega - \epsilon_q^+ + \epsilon_d } + \fr{|V_q^1|^2}{\omega + \epsilon^-_q - \epsilon_d  - U + u_S} - \fr{2|V_q^0|^2}{\omega + \epsilon^-_q - \epsilon_d} + \sum_k\fr{u_S^2 + u_A^2}{\omega + \epsilon_q^- - \epsilon_k}}\\
...
The Hamiltonian for a single electron \(q\beta\) on the \(N^{th}\) shell is
\beq
\ham_N = H_{N-1} + H_\text{imp} + \rr{\epsilon_q +  \beta J_z S^z_d}\hat n_{q\beta} + V_q c^\dagger_{q\beta}c_{d\beta} + \text{h.c.} + \sum_{k<\Lambda_N}\left[J_z S^z_d \beta \rr{c^\dagger_{k\beta}c_{q\beta} + c^\dagger_{q\beta}c_{k\beta}} \right.\\
\left.+ J_t \rr{c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q \beta} + c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k\ol\beta}}\right]
\eeq
where \il{H_{imp}} is the impurity-diagonal part of the Hamiltonian (\il{\epsilon_d \hat n_d + U\hat n_{d\ua}\hat n_{d\da}}) and 
\beq
...
The \il{\ham_D} is the diagonal part of the Hamiltonian, and the superscript \il{\pm} signifies that its the particle(hole) sector part, with respect to the electron presently being disentangled (\il{q\beta}).
\beq
\ham_D^+ \equiv \text{Tr}_{q\beta}\qq{\ham \hat n_{q\beta}}= \sum_{k<\Lambda_N,\sigma}\rr{\epsilon_k  + \sigma J_z S^z_d}\hat n_{k\sigma} +\rr{\epsilon_q  + \beta J_z S^z_d} + H_{imp}\\
\eeq
The entire renormalization expression has nine terms- one of order \il{|V_q|^2}, four of order \il{V_q u_P} and four of order \il{u_P^2}.
\paragraph{1.}
\beq
...
\pb
The intermediate state is characterized by \il{\hat n_{d\beta} = 0, \hat n_{q\beta} = 1}. Therefore, at the propagator, we have
\beq
H_1 \equiv \ham_D^+ &= \qq{\epsilon_q  + \beta J_z S^z_d} + \epsilon_d \hat n_{d\ol\beta}\\
		    &= \qq{\epsilon_q  - \hf \beta\beta J_t \hat n_{d\ol\beta}} + \epsilon_d \hat n_{d\ol\beta}\\
		    &= \qq{\epsilon_q - \hf J_t \hat n_{d\ol\beta}} + \epsilon_d \hat n_{d\ol\beta}
\eeq
\il{H_1} is the intermediate state Hamiltonian. As a simplification, we replace \il{\hat \omega^+} with its eigenvalue \il{\omega^+}. Since the propagator, in this form, does not depend on \il{q\beta} or \il{d\beta} (they have been resolved inside \il{H_1}), we can move the propagator to the front:
\beq
\Delta_1^+ \ham_N &= \sum_{q\beta}|V_q|^2 c^\dagger_{d\beta}c_{q\beta}c^\dagger_{q\beta}c_{d\beta}\fr{1}{\omega^+ - H_1}\\
...
We will now write the denominator in terms of the initial energy, \il{H_0}. The initial state is characterized by \il{\hat n_{q\beta} = 0, \hat n_{d\beta}=1}:
\beq
H_0 &= \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta}\\
    &= H_1 + \epsilon_d + U\hat n_{d\ol\beta} - \epsilon_q + \fr{J_z}{2} \hat n_{d\ol\beta}
\eeq
If we measure the quantum fluctuation \il{\omega^+} from the initial (diagonal) state energy which does not have any quantum fluctuations, we can set \il{H_0 = 0} in the denominator. Also, since \il{q\beta} is on the upper band edge, we can assume it is unoccupied in the initial state. Then,
\beq
\Delta_1^+ \ham_N &= \sum_{q\beta}|V_q|^2 \hat n_{d\beta}\fr{1}{\omega^+ - \epsilon_q + \epsilon_d + \rr{U + \fr{J_z}{2}}\hat n_{d\ol\beta}}\\
		  &= \sum_{q\beta}\hat n_{d\beta}\qq{\fr{|V^1_q|^2 \hat n_{d\ol\beta}}{\omega^+ - \epsilon_q + \epsilon_d + \rr{U + \fr{J_z}{2}}} + \fr{|V^0_q|^2\rr{1 - \hat n_{d\ol\beta}}}{\omega^+ - \epsilon_q + \epsilon_d}}\\
		  &= \sum_{q\beta}\hat n_{d\beta}\qq{\fr{|V^0_q|^2}{\omega^+ - \epsilon_q + \epsilon_d} + \hat n_{d\ol\beta}\rr{\fr{|V^1_q|^2}{\omega^+ - \epsilon_q + \epsilon_d + \rr{U + \fr{J_z}{2}}} - \fr{|V^0_q|^2}{\omega^+ - \epsilon_q + \epsilon_d}}}\\
\eeq
\comm{
\paragraph{2.}
\beq
...
\Delta^+_6 \ham_N &=\fr{1}{4}J_z^2 \sum_{k^\prime q\beta k}  \rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}c^\dagger_{k\beta}c_{q\beta} c^\dagger_{q\beta}c_{k^\prime\beta}\fr{1}{\omega^+ - H_1}\\
		  &=\fr{1}{4}J_z^2 \sum_{k^\prime q\beta k}  \rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}\rr{1  - \hat n_{q\beta}}c^\dagger_{k\beta}c_{k^\prime\beta} \fr{1}{\omega^+ - H_0 - \epsilon_q - \beta J_z S^z_d}\\
		  &=\fr{1}{4}J_z^2 \sum_{k^\prime q\beta k} c^\dagger_{k\beta}c_{k^\prime\beta} \fr{\rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}}{\omega^+ - \epsilon_q - \beta J_z S^z_d}\\
		  &=\fr{1}{4}J_z^2\sum_{k^\prime q\beta k} c^\dagger_{k\beta}c_{k^\prime\beta}\qq{ \fr{\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}} {\omega^+ - \epsilon_q - \hf J_z} + \fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}} {\omega^+ - \epsilon_q + \hf J_z}}
\eeq
In the last step, we used the fact that \(\hat n_d - 2\hat n_{d\ua}\hat n_{d\da}\) is not zero only in the singly occupied subspace, hence we can expand it into \(\hat n_{\ua}\rr{1 - \hat n_{\da}} + \text{p}\leftrightarrow\text{h}\). 
\paragraph{7.}
\beq
...
\Delta^+_7 \ham_N &= \sum_{q\beta kk^\prime}\hf J_zJ_t c^\dagger_{k\beta}c_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}\fr{-1}{\omega^+ - H_1}\\
		  &= -\hf J_z J_t \sum_{q\beta kk^\prime}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^+ - \epsilon_{q} + J_z}\\
		  &= -\hf J_z J_t \sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^+ - \epsilon_{q} + J_z}\\
\eeq
\paragraph{8.}
\beq
...
\eeq
\beq
\Delta^+_8 \ham_N &=- \sum_{q\beta kk^\prime}\hf J_z J_t c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}c_{q\beta}^\dagger c_{k^\prime\beta}\fr{1}{\omega^+ - \epsilon_q + \hf J_z}\\
		  &=-\hf J_z J_t \sum_{q\beta kk^\prime}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^+- \epsilon_q  + \hf J_z}\\
		  &= -\hf J_z J_t\sum_{q\beta kk^\prime}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^+ - \epsilon_q + \hf J_z}\\
\eeq
\paragraph{9.}
\beq
...
\eeq
\beq
\Delta^+_9 \ham_N &= \sum_{q\beta k k\prime}J_t^2 c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta} c_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c_{q\beta}^\dagger c_{k^\prime\ol\beta}\fr{1}{\omega^+  - \epsilon_q + \hf J_z}\\
		  &= \sum_{q\beta k k\prime}\rr{1 - \hat n_{q\beta}} \hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}c^\dagger_{k\ol\beta}c_{k^\prime\ol\beta}\fr{J_t^2}{\omega^+  - \epsilon_q + \hf J_z}\\
		  &= \sum_{q\beta k k\prime}c^\dagger_{k\beta}c_{k^\prime\beta}\fr{J_t^2\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^+ - \epsilon_q + \hf J_z}\\
\eeq
\subsubsection{Hole sector}
The renormalization in the Hamiltonian in the hole sector is
\beq
...
\beq
\fr{-1}{\hat \omega^- - \ham_D^-} = \fr{1}{\omega^- + \ham_D^-}
\eeq
...
\beq
\ham_D^- = -\epsilon_q -\beta J_zS^z_d + H_\text{imp}
\eeq
\paragraph{1.}
\beq
\Delta_1^- \ham_N &= \sum_{q\beta}|V_q|^2 c^\dagger_{q\beta}c_{d\beta}\fr{1}{\hat \omega^- - \ham_D^-} c^\dagger_{d\beta}c_{q\beta}
...
The intermediate (\il{\hat n_{q\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta} -\epsilon_q -\beta J_zS^z_d = -\epsilon_q - \fr{J_z}{2}\rr{1 - \hat n_{d\ol\beta}} + \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta}
\eeq
\beq
\Delta_1^- \ham_N &= \sum_{q\beta}|V_q|^2 \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}\fr{1}{\omega^- - H_1}
\eeq
...
H_0 &= \epsilon_d \hat n_{d\ol\beta}\\
    &= H_1 + \epsilon_q - \epsilon_d - U\hat n_{d\ol\beta} + \fr{J_z}{2}\rr{1 - \hat n_{d\ol\beta}}
\eeq
As before, we set \(H_0 = 0\) and keep \(H_1 - H_0\) in the denominator.
\beq
&\Delta_1^- \ham_N\\
&= \sum_{q\beta}|V_q|^2 \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}\fr{1}{\omega^- - \epsilon_q + \epsilon_d + U\hat n_{d\ol\beta} - \fr{J_z}{2}\rr{1 - \hat n_{d\ol\beta}}} \\
 &= \sum_{q\beta}\rr{1 - \hat n_{d\beta}}\qq{\fr{|V_q^1|^2 \hat n_{d\ol\beta}}{\omega^- - \epsilon_q + \epsilon_d + U } + \fr{|V_q^0|^2\rr{1 - \hat n_{d\ol\beta}}}{\omega - \epsilon_q + \epsilon_d - \fr{J_z}{2}}}\\
 &= \sum_{q\beta}\left[ \hat n_{d\ol\beta}\rr{\fr{|V_q^1|^2}{\omega - \epsilon_q + \epsilon_d + U} - \fr{2|V_q^0|^2}{\omega - \epsilon_q + \epsilon_d - \fr{J_z}{2}}} + \hat n_{d\ua}\hat n_{d\da}\rr{\fr{|V_q^0|^2}{\omega - \epsilon_q + \epsilon_d - \fr{J_z}{2}} - \fr{|V_q^1|^2}{\omega - \epsilon_q + \epsilon_d + U}}\right]
\eeq
\comm{
\paragraph{2.}
\beq
...
The intermediate (\il{\hat n_{q\beta}=0}) energy is
\beq
H_1 &= H_\text{imp} - \epsilon_q - \beta J_z S^z_d
\eeq
The initial state (\il{\hat n_{q\beta}=1}) energy is
\beq
H_0 = H_\text{imp}= H_1 + \epsilon_q + \beta J_z S^z_d
\eeq
\beq
\Delta_6^- \ham_N &= \sum_{q\beta kk^\prime}\fr{J_z^2}{4}\rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}c^\dagger_{q\beta}c_{k^\prime\beta}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^- - H_1}\\
		  &=\fr{J_z^2}{4} \sum_{q\beta kk^\prime} \hat n_{q\beta}\rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}c_{k^\prime\beta}c^\dagger_{k\beta}\fr{1}{\omega^- - \epsilon_q - \beta J_z S^z_d}\\
		  &= -\fr{J_z^2}{4}\sum_{q\beta kk^\prime}c^\dagger_{k\beta}c_{k^\prime\beta}\qq{\fr{\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q - \fr{J_z}{2}} + \fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^- - \epsilon_q + \fr{J_z}{2}}} \\
		  &\quad+ \fr{J_z^2}{4}\sum_{q\beta k}\qq{\fr{\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q - \fr{J_z}{2}} + \fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^- - \epsilon_q + \fr{J_z}{2}}}\\
\eeq
\paragraph{7.}
\beq
...
\Delta_7^- \ham_N &= \sum_{q\beta kk^\prime}\hf J_z J_t c^\dagger_{q\beta}c_{k^\prime\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^- - H_1} \\
		  &= \sum_{q\beta kk^\prime}\hf J_z J_t  \hat n_{q\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{-1}{\omega^- - \epsilon_q  - \hf J_z} \\
		  &= -\hf J_z J_t \sum_{q\beta kk^\prime}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^- - \epsilon_q  - \hf J_z} \\
		  &= -\hf J_z J_t \sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^- - \epsilon_q  - \hf J_z}
\eeq
\paragraph{8.}
\beq
...
The intermediate (\il{\hat n_{q\beta}=\hat n_{d\ol\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = - \epsilon_q - \fr{J_z}{2} + \epsilon_d
\eeq
The initial state (\il{\hat n_{d\ol\beta}=0,\hat n_{q\beta}=\hat n_{d\beta}=1}) energy is
\beq
H_0 &= \epsilon_d = H_1 + \epsilon_q + \fr{J_z}{2}
\eeq
\beq
\Delta_8^- \ham_N &= \sum_{q\beta kk^\prime}\hf J_z J_t c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^- - H_1}\\
		  &= \sum_{q\beta kk^\prime}\hf J_z J_t \hat n_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{-1}{\omega^-  - \epsilon_q - \fr{J_z}{2}}\\
		  &= -\hf J_z J_t \sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^- - \epsilon_q - \hf J_z}\\
\eeq
\paragraph{9.}
\beq
...
\Delta_9^- \ham_N &= \sum_{q\beta kk^\prime}J_t^2c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}c^\dagger_{d\beta} c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^- - H_1}\\
		  ...
		  &= \sum_{q\beta kk^\prime}J_t^2\hat n_{q\beta}\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}c_{k^\prime\ol\beta}c^\dagger_{k\ol\beta}\fr{1}{\omega^- - \epsilon_q^-  - \hf J_z}\\
		  &=-J_t^2 \sum_{q\beta kk^\prime}c^\dagger_{k\beta}c_{k^\prime\beta}\fr{\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q^- - \hf J_z} + J_t^2\sum_{q k \beta}\fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^- - \epsilon_q^- - \hf J_z}\\
\eeq
...
\beq
2\Delta J_z = 2 \Delta J_t = \Delta J &= - \hf J^2 \sum_q \rr{\fr{1}{\omega^+ - \epsilon_q  + \fr{1}{4}J} + \fr{1}{\omega^- - \epsilon_q  - \fr{1}{4}J}}
\eeq
...
\beq
\Delta J = -\hf J^2 \sum_q \fr{2}{-\epsilon_q}
\eeq
...
\beq
\Delta J = J^2 \rho(D)\fr{|\Delta D|}{D}
\eeq
...
\beq
\Delta \epsilon_d + \hf \Delta U &= \sum_q \left[- \fr{|V_q^0|^2}{\omega^- - \epsilon_q + \epsilon_d - \hf J_z} + \fr{|V_q^1|^2}{\omega^+ - \epsilon_q + \epsilon_d + U + \hf J_z}\right]
\eeq
...
\beq
\Delta \epsilon_d + \hf \Delta U &= \sum_q |V_q|^2 \left[- \fr{1}{\omega - \epsilon_q - \epsilon_d + \hf J_z} + \fr{1}{\omega - \epsilon_q + \epsilon_d + U + \hf J_z}\right]
\eeq
...
u_{\ua\ua}\hat n_{d\ua}c^\dagger_{k\ua}c_{k^\prime\ua} + u_{\da\da}\hat n_{d\da}c^\dagger_{k\da}c_{k^\prime\ua} + u_{\ua\da}\hat n_{d\ua}c^\dagger_{k\da}c_{k^\prime\da} + u_{\da\ua}\hat n_{d\da}c^\dagger_{k\ua}c_{k^\prime\ua}
\eeq
We define \(J_c\), \(J_z\) and \(J_t\) such that the interaction can be written as
\beq
\ham_{I} &= \hf J_c \hat n_d \sum_{kk^\prime}\rr{c^\dagger_{k\ua}c_{k^\prime\ua} + c^\dagger_{k\da}c_{k^\prime\da}}+ J_z \fr{\hat n_{d\ua} - \hat n_{d\da}}{2}\sum_{kk^\prime}\rr{c^\dagger_{k\ua}c_{k^\prime\ua} - c^\dagger_{k\da}c_{k^\prime\da}} + J_t \sum_{kk^\prime}\qq{ c^\dagger_{d\ua}c_{d\da}c^\dagger_{k\da}c_{k^\prime\ua} + c^\dagger_{d\da}c_{d\ua}c^\dagger_{k\ua}c_{k^\prime\da}}\\
	 &= \hf J_c \hat n_d \sum_{kk^\prime}\rr{c^\dagger_{k\ua}c_{k^\prime\ua} + c^\dagger_{k\da}c_{k^\prime\da}} + 2J_z S_d^z s^z + J_t \rr{S_d^+ s^- + S_d^- s^+}
\eeq
The spin-like operators are defined as
\beq
S^z_d &\equiv \hf\rr{\hat n_{d\ua} - \hat n_{d\da}} \quad&S^+_d &\equiv c^\dagger_{d\ua}c_{d\da}\quad &S^-_d \equiv c^\dagger_{d\da}c_{d\ua}\\
...
The Hamiltonian for a single electron \(q\beta\) on the \(N^{th}\) shell is
\beq
\ham_N &= H_{N-1} + H_\text{imp} + \rr{\epsilon_q + \hf J_c \hat n_d + \beta J_z S^z_d}\hat n_{q\beta} + V_q c^\dagger_{q\beta}c_{d\beta} + \text{h.c.} + \\
       &\quad+ \sum_{k<\Lambda_N}\left[\rr{ \hf J_c \hat n_d + J_z S^z_d \beta} \rr{c^\dagger_{k\beta}c_{q\beta} + c^\dagger_{q\beta}c_{k\beta}} + J_t \rr{c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q \beta} + c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k\ol\beta}}\right]
\eeq
where \il{H_{imp}} is the impurity-diagonal part of the Hamiltonian (\il{\epsilon_d \hat n_d + U\hat n_{d\ua}\hat n_{d\da}}) and 
\beq
H_{N-1} = \sum_{k<\Lambda_N,\sigma}\qq{\rr{\epsilon_k + \hf J_c \hat n_d + \sigma J_z S^z_d}\hat n_{k\sigma} + V_k c^\dagger_{k\sigma}c_{d\sigma} + \text{h.c.}} + H_{I,N-1}
\eeq
\subsubsection{Particle sector}
The renormalization in the Hamiltonian in the particle sector is
\beq
\Delta^+ \ham_N = \sum_{q\beta}\qq{V_q^* c^\dagger_{d\beta}c_{q\beta} + \rr{\hf J_c \hat n_d + J_z \beta S^z_d}\sum_kc^\dagger_{k\beta}c_{q\beta} + J_t\sum_{k}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}}\times\fr{1}{\hat \omega^+ - \ham_D^+}\\
\times\qq{V_q c^\dagger_{q\beta}c_{d\beta} + \rr{\hf J_c \hat n_d + J_z \beta S^z_d}\sum_kc^\dagger_{q\beta}c_{k\beta} + J_t\sum_{k}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k\ol\beta}}\\
\eeq
The \il{\ham_D} is the diagonal part of the Hamiltonian, and the superscript \il{\pm} signifies that its the particle(hole) sector part, with respect to the electron presently being disentangled (\il{q\beta}).
\beq
\ham_D^+ \equiv \text{Tr}_{q\beta}\qq{\ham \hat n_{q\beta}}= \sum_{k<\Lambda_N,\sigma}\rr{\epsilon_k  + \hf J_c \hat n_d + \sigma J_z S^z_d}\hat n_{k\sigma} +\rr{\epsilon_q + \hf J_c \hat n_d + \beta J_z S^z_d} + H_{imp}\\
\eeq
The entire renormalization expression has nine terms- one of order \il{|V_q|^2}, four of order \il{V_q J} and four of order \il{J^2}.
\paragraph{1.}
\beq
\Delta_1^+ \ham_N = \sum_{q\beta}|V_q|^2 c^\dagger_{d\beta}c_{q\beta}\fr{1}{\hat \omega^+ - \ham_D^+}c^\dagger_{q\beta}c_{d\beta}
\eeq
...
The intermediate state is characterized by \il{\hat n_{d\beta} = 0, \hat n_{q\beta} = 1}. Therefore, at the propagator, we have
\beq
H_1 \equiv \ham_D^+ &= \qq{\epsilon_q + \hf J_c \hat n_{d} + \beta J_z S^z_d} + \epsilon_d \hat n_{d\ol\beta}\\
		    &= \qq{\epsilon_q + \hf J_c \hat n_{d\ol\beta} - \hf \beta\beta J_z \hat n_{d\ol\beta}} + \epsilon_d \hat n_{d\ol\beta}\\
		    &= \qq{\epsilon_q + \hf J_c \hat n_{d\ol\beta} - \hf J_z \hat n_{d\ol\beta}} + \epsilon_d \hat n_{d\ol\beta}
\eeq
\il{H_1} is the intermediate state Hamiltonian. As a simplification, we replace \il{\hat \omega^+} with its eigenvalue \il{\omega^+}. Since the propagator, in this form, does not depend on \il{q\beta} or \il{d\beta} (they have been resolved inside \il{H_1}), we can move the propagator to the front:
\beq
\Delta_1^+ \ham_N &= \sum_{q\beta}|V_q|^2 c^\dagger_{d\beta}c_{q\beta}c^\dagger_{q\beta}c_{d\beta}\fr{1}{\omega^+ - H_1}\\
...
\beq
H_0 &= \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta}\\
    &= H_1 + \epsilon_d + U\hat n_{d\ol\beta} - \epsilon_q - \hf J_c \hat n_{d\ol\beta} + \fr{J_z}{2} \hat n_{d\ol\beta}
\eeq
If we measure the quantum fluctuation \il{\omega^+} from the initial (diagonal) state energy which does not have any quantum fluctuations, we can set \il{H_0 = 0} in the denominator. Also, since \il{q\beta} is on the upper band edge, we can assume it is unoccupied in the initial state. Then,
\beq
\Delta_1^+ \ham_N &= \sum_{q\beta}|V_q|^2 \hat n_{d\beta}\fr{1}{\omega^+ - \epsilon_q + \epsilon_d + \rr{U - \hf J_c + \hf J_z}\hat n_{d\ol\beta}}\\
		  &= \sum_{q\beta}\hat n_{d\beta}\qq{\fr{|V^1_q|^2 \hat n_{d\ol\beta}}{\omega^+ - \epsilon_q + \epsilon_d + \rr{U - \hf J_c + \hf J_z}} + \fr{|V^0_q|^2\rr{1 - \hat n_{d\ol\beta}}}{\omega^+ - \epsilon_q + \epsilon_d}}\\
		  &= \sum_{q\beta}\hat n_{d\beta}\qq{\fr{|V^0_q|^2}{\omega^+ - \epsilon_q + \epsilon_d} + \hat n_{d\ol\beta}\rr{\fr{|V^1_q|^2}{\omega^+ - \epsilon_q + \epsilon_d + \rr{U - \hf J_c + \hf J_z}} - \fr{|V^0_q|^2}{\omega^+ - \epsilon_q + \epsilon_d}}}\\
\eeq
%\comm{
\paragraph{2.}
\beq
\Delta^+_2 \ham_N = \sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+}\rr{\hf J_c \hat n_d + J_z \beta S^z_d}c_{q\beta}^\dagger c_{k\beta}
\eeq
...
\beq
c^\dagger_{d\beta} \rr{\hf J_c \hat n_d + J_z \beta S^z_d} = c^\dagger_{d\beta}\hf\rr{J_c - J_z}\hat n_{d\ol\beta}
\eeq
The expression simplifies to
\beq
\Delta^+_2 \ham_N = \hf\rr{J_c - J_z}\sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{q\beta}\hat n_{d\ol\beta}\fr{1}{\omega^+ - \ham_D^+}c_{q\beta}^\dagger c_{k\beta}
\eeq
Intermediate (\il{\hat n_{q\beta}=1, \hat n_{d\ol\beta}=1,\hat n_{d\beta}=0}) energy is
\beq
H_1 = \epsilon_q + \rr{\hf J_c \hat n_d + J_z \beta S^z_d} + \epsilon_d = \epsilon_q + \hf\rr{J_c - J_z} + \epsilon_d
\eeq
...
The initial (\il{\hat n_{q\beta}=0, \hat n_{d\ol\beta}=1,\hat n_{d\beta}=0}) energy is
\beq
H_0 = \epsilon_d = H_1 - \epsilon_q - \hf\rr{J_c - J_z}
\eeq
\beq
\Delta^+_2 \ham_N &= \hf\rr{J_c - J_z}\sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{q\beta}\hat n_{d\ol\beta}c_{q\beta}^\dagger c_{k\beta}\fr{1}{\omega^+ - H_1 - \epsilon_q - \hf\rr{J_c - J_z}}\\
		  &= \hf\rr{J_c - J_z}\sum_{q\beta k}c^\dagger_{d\beta}c_{k\beta}\fr{\hat n_{d\ol\beta}{V_q^1}^*}{\omega^+ - \epsilon_q - \hf\rr{J_c - J_z}}
\eeq
\paragraph{3.}
\beq
...
Intermediate (\il{\hat n_{d\beta}=0,\hat n_{q\beta}=\hat n_{d\ol\beta}=1}) energy is
\beq
H_1 = \epsilon_q + \hf\rr{J_c - J_z} + \epsilon_d 
\eeq
The initial (\il{\hat n_{d\beta}=1,\hat n_{q\beta}=\hat n_{d\ol\beta}=0}) energy is
\beq
H_0 = \epsilon_d = H_1 - \epsilon_q - \hf\rr{J_c - J_z}
\eeq
\beq
\Delta^+_3 \ham_N &= \sum_{q\beta k}J_t V_q^*c^\dagger_{d\beta}c_{q\beta}c^\dagger_{d\ol\beta} c_{d\beta}c_{q\beta}^\dagger c_{k\ol\beta}\fr{1}{\omega^+ - H_1  - \epsilon_q - \hf\rr{J_c - J_z}}\\
		  &= \sum_{q\beta k}-J_t V_q^*\hat n_{d\beta}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\ol\beta}c_{k\ol\beta}\fr{1}{\omega^+ - H_1  - \epsilon_q - \hf\rr{J_c - J_z}}\\
		  &= -J_t \sum_{q\beta k}c^\dagger_{d\beta}c_{k\beta}\fr{{V^1_q}^*\hat n_{d\ol\beta}}{\omega^+  - \epsilon_q - \hf\rr{J_c - J_z}}
\eeq
\paragraph{4.}
\beq
\Delta^+_4 \ham_N = \sum_{q\beta k\sigma}\rr{\hf J_c \hat n_d + J_z \beta S^z_d}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+}V_qc_{q\beta}^\dagger c_{d\beta}
\eeq
The first step is a simplification:
\beq
\rr{\hf J_c \hat n_d + J_z \beta S^z_d}c_{d\beta} = \hf\rr{J_c - J_z}\hat n_{d\ol\beta}c_{d\beta}
\eeq
Intermediate (\il{\hat n_{d\beta}=0,\hat n_{q\beta}=\hat n_{d\ol\beta}=1}) energy is
\beq
H_1 = \epsilon_q + \hf\rr{J_c - J_z} + \epsilon_d 
...
The initial (\il{\hat n_{q\beta}=0,\hat n_{d\beta}=\hat n_{d\ol\beta}=1}) energy is
\beq
H_0 = 2\epsilon_d + U = H_1 + \epsilon_d + U - \epsilon_q - \hf\rr{J_c - J_z}
\eeq
\beq
\Delta^+_4 \ham_N &= \sum_{q\beta k}\hf\rr{J_c - J_z} V_q \hat n_{d\ol\beta}c^\dagger_{k\beta}c_{q\beta}c_{q\beta}^\dagger c_{d\beta}\fr{1}{\omega^+ - H_0 + \epsilon_d + U - \epsilon_q - \hf\rr{J_c - J_z}}\\
		  &= \sum_{q\beta k}\hf\rr{J_c - J_z} V_q \hat n_{d\ol\beta}\rr{1 - \hat n_{q\beta}}c^\dagger_{k\beta} c_{d\beta}\fr{1}{\omega^+ + \epsilon_d + U - \epsilon_q - \hf\rr{J_c - J_z}}\\
		  &= \hf\rr{J_c - J_z}\sum_{q\beta k}c^\dagger_{k\beta} c_{d\beta}\fr{V_q^1 \hat n_{d\ol\beta}}{\omega^+ - \epsilon_q + \epsilon_d + U - \hf\rr{J_c - J_z}}
\eeq
\paragraph{5.}
\beq
...
Intermediate (\il{\hat n_{d\beta}=0,\hat n_{q\beta}=\hat n_{d\ol\beta}=1}) energy is
\beq
H_1 = \epsilon_q + \hf\rr{J_c - J_z} + \epsilon_d + \mc{E}_0
\eeq
The initial (\il{\hat n_{q\beta}=0,\hat n_{d\beta}=\hat n_{d\ol\beta}=1}) energy is
\beq
H_0 = 2\epsilon_d + U  + \mc{E}_0 = H_1 + \epsilon_d + U - \epsilon_q - \hf\rr{J_c - J_z}
\eeq
\beq
\Delta^+_5 \ham_N &= \sum_{q\beta k}J_t V_q c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}c_{q\beta}^\dagger c_{d\beta}\fr{1}{\omega^+ - H_0 + \epsilon_d + U - \epsilon_q - \hf\rr{J_c - J_z}}\\
		  &= -\sum_{q\beta k}J_t V_q \rr{1 - \hat n_{q\beta}}\hat n_{d\beta}c^\dagger_{k\ol\beta} c_{d\ol\beta}\fr{1}{\omega^+ + \epsilon_d + U - \epsilon_q - \hf\rr{J_c - J_z}}\\
		  &=-J_t  \sum_{q\beta k}c^\dagger_{k\beta} c_{d\beta}\fr{V_q^1 \hat n_{d\ol\beta}}{\omega^+ - \epsilon_q + \epsilon_d + U - \hf\rr{J_c - J_z}}\\
\eeq
%}
\paragraph{6.}
\beq
\Delta^+_6 \ham_N = \sum_{k^\prime q\beta k}\rr{\hf J_c \hat n_d + J_z S^z_d \beta} c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+}\rr{\hf J_c \hat n_d + J_z S^z_d \beta}c^\dagger_{q\beta}c_{k^\prime\beta}
\eeq
Since the impurity parts are diagonal, we keep them as is for the time-being:
\beq[simpl3]
\hat d^2 =\rr{\hf J_c \hat n_d + J_z S^z_d \beta}^2
\eeq
\beq
\Delta^+_6 \ham_N = \sum_{k^\prime q\beta k}\hat d^2 c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+}c^\dagger_{q\beta}c_{k^\prime\beta}
\eeq
Intermediate (\il{\hat n_{q\beta}=1}) energy is
\beq
...
\eeq
\beq
\Delta^+_6 \ham_N &= \sum_{k^\prime q\beta k}\hat d^2  c^\dagger_{k\beta}c_{q\beta} c^\dagger_{q\beta}c_{k^\prime\beta}\fr{1}{\omega^+ - H_1}\\
		  &= \sum_{k^\prime q\beta k}\hat d^2  \rr{1  - \hat n_{q\beta}}c^\dagger_{k\beta}c_{k^\prime\beta} \fr{1}{\omega^+ - H_0 - \epsilon_q - \hat d}\\
		  &= \sum_{k^\prime q\beta k}\hat d^2 c^\dagger_{k\beta}c_{k^\prime\beta} \fr{1}{\omega^+ - \epsilon_q - \hat d}\\
\eeq
The operator \(\hat d\) takes the values
\beq
\hat d = \begin{cases} 0 & \hat n_{d\beta} = \hat n_{d\ol\beta} = 0\\ \hf\rr{J_c + J_z} &\hat n_{d\beta} = 1-\hat n_{d\ol\beta} = 0\\ \hf\rr{J_c - J_z} & 1 - \hat n_{d\beta} = \hat n_{d\ol\beta} = 0\\ J_c & \hat n_{d\beta} = \hat n_{d\ol\beta} = 1 \end{cases}
\eeq
Expanding in this basis gives
\beq
\Delta_6^+ \ham_N &= \sum_{k^\prime q\beta k}c^\dagger_{k\beta}c_{k^\prime\beta}\qq{\fr{\fr{1}{4}\rr{J_c + J_z}^2\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega^+ - \epsilon_q - \hf\rr{J_c + J_z}} + \fr{\fr{1}{4}\rr{J_c - J_z}^2\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^+ - \epsilon_q - \hf\rr{J_c - J_z}} + \fr{J_c^2\hat n_{d\beta}\hat n_{d\ol\beta}}{\omega^+ - \epsilon_q - J_c}}\\
\eeq
\paragraph{7.}
\beq
\Delta^+_7 \ham_N = \sum_{q\beta kk^\prime}\rr{\hf J_c \hat n_d + J_z S^z_d \beta}  c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+} J_t c^\dagger_{d\ol\beta} c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}
\eeq
The first step is a simplification:
\beq
\rr{\hf J_c \hat n_d + J_z S^z_d \beta} c^\dagger_{d\ol\beta} c_{d\beta} = \hf \rr{J_c - J_z} c^\dagger_{d\ol\beta}c_{d\beta}
\eeq
Intermediate (\il{\hat n_{d\beta}=0,\hat n_{q\beta}=\hat n_{d\ol\beta}=1}) energy is
\beq
H_1 = \epsilon_q + \hf J_c + \beta J_z S^z_d + \epsilon_d = \epsilon_q + \hf\rr{J_c - J_z} + \epsilon_d
\eeq
The initial (\il{\hat n_{d\beta}=1,\hat n_{q\beta}=\hat n_{d\ol\beta}=0}) energy is
\beq
H_0 = \epsilon_d = H_1 - \epsilon_{q} - \hf\rr{J_c - J_z}
\eeq
\beq
\Delta^+_7 \ham_N &= \sum_{q\beta kk^\prime}\hf\rr{J_c - J_z}J_t c^\dagger_{k\beta}c_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^+ - H_1}\\
		  &= \hf\rr{J_c - J_z} J_t \sum_{q\beta kk^\prime}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^+ - \epsilon_{q} - \hf\rr{J_c - J_z}}\\
		  &= \hf\rr{J_c - J_z} J_t \sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^+ - \epsilon_{q} - \hf\rr{J_c - J_z}}\\
\eeq
\paragraph{8.}
\beq
\Delta^+_8 \ham_N = \sum_{q\beta kk^\prime}J_t  c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+}\rr{\hf J_c \hat n_d + J_z S^z_d \beta} c_{q\beta}^\dagger c_{k^\prime\beta}
\eeq
The first step is a simplification:
\beq
c_{d\beta}^\dagger c_{d\ol\beta}\rr{\hf J_c \hat n_d + J_z S^z_d \beta} = \hf \rr{J_c - J_z} c_{d\beta}^\dagger c_{d\ol\beta}
\eeq
Intermediate (\il{\hat n_{d\beta}=0,\hat n_{q\beta}=\hat n_{d\ol\beta}=1}) energy is
\beq
H_1 = \epsilon_q + \rr{\hf J_c + \beta J_z S^z_d} + \epsilon_d = \epsilon_q + \hf\rr{J_c - J_z} + \epsilon_d
\eeq
The initial (\(\hat n_{q\beta}=\hat n_{d\beta}=0,\hat n_{d\ol\beta}=1\)) energy is
\beq
H_0 = \epsilon_d  = H_1 - \epsilon_q - \hf\rr{J_c - J_z}
\eeq
\beq
\Delta^+_8 \ham_N & = \hf \rr{J_c - J_z}J_t \sum_{q\beta kk^\prime}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}c_{q\beta}^\dagger c_{k^\prime\beta}\fr{1}{\omega^+ - \epsilon_q - \hf\rr{J_c - J_z}}\\
		  & = \hf \rr{J_c - J_z}J_t \sum_{q\beta kk^\prime}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^+- \epsilon_q  - \hf\rr{J_c - J_z}}\\
		  & = \hf \rr{J_c - J_z}J_t \sum_{q\beta kk^\prime}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^+ - \epsilon_q - \hf\rr{J_c - J_z}}\\
\eeq
\paragraph{9.}
\beq
\Delta^+_9 \ham_N = \sum_{q\beta k k\prime}J_t c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta} c_{q\beta}\fr{1}{\omega^+ - \ham_D^+} J_t c^\dagger_{d\ol\beta}c_{d\beta}c_{q\beta}^\dagger c_{k^\prime\ol\beta}
...
\eeq
\beq
\Delta^+_9 \ham_N &= \sum_{q\beta k k\prime}J_t^2 c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta} c_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c_{q\beta}^\dagger c_{k^\prime\ol\beta}\fr{1}{\omega^+  - \epsilon_q - \hf\rr{J_c - J_z}}\\
		  &= \sum_{q\beta k k\prime}\rr{1 - \hat n_{q\beta}} \hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}c^\dagger_{k\ol\beta}c_{k^\prime\ol\beta}\fr{J_t^2}{\omega^+  - \epsilon_q - \hf\rr{J_c - J_z}}\\
		  &= \sum_{q\beta k k\prime}c^\dagger_{k\beta}c_{k^\prime\beta}\fr{J_t^2\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^+ - \epsilon_q - \hf\rr{J_c - J_z}}\\
\eeq
\subsubsection{Hole sector}
The renormalization in the Hamiltonian in the hole sector is
\beq
\Delta^- \ham_N = \sum_{q\beta}\qq{V_q c^\dagger_{q\beta}c_{d\beta} + \rr{\hf J_c \hat n_d + J_z \beta S^z_d} \sum_{k\sigma} c_{k\beta} c^\dagger_{q\beta} + J_t\sum_{k\sigma}c^\dagger_{d\ol\beta}c^\dagger_{q\beta}c_{d\beta}c_{k\ol\beta}}\times\fr{-1}{\hat \omega^- - \ham_D^-}\\
\times\qq{V_q^* c^\dagger_{d\beta}c_{q\beta} + \rr{\hf J_c \hat n_d + J_z \beta S^z_d} \sum_{k\sigma} c_{q\beta} c^\dagger_{k\beta} + J_t\sum_{k\sigma}c^\dagger_{d\beta}c^\dagger_{k\ol\beta}c_{d\ol\beta}c_{q\beta}}
\eeq
The propagator can be written as
\beq
\fr{-1}{\hat \omega^- - \ham_D^-} = \fr{1}{\omega^- + \ham_D^-}
...
where we substitute \(\hat \omega^- = 2\omega^-\tau^-  = -\omega^-\). \(\ham_D^-\) is the energy of the hole state. The kinetic energy and spin of this hole will be the negative of those of the particle, due to conservation.
\beq
\ham_D^- = -\epsilon_q - \hf J_c \hat n_d -\beta J_z S^z_d + H_\text{imp}
\eeq
\paragraph{1.}
\beq
\Delta_1^- \ham_N &= \sum_{q\beta}|V_q|^2 c^\dagger_{q\beta}c_{d\beta}\fr{1}{\omega^- + \ham_D^-} c^\dagger_{d\beta}c_{q\beta}
\eeq
The intermediate (\il{\hat n_{q\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 &= \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta} -\epsilon_q -\rr{J_c \hat n_d + \beta J_zS^z_d}\\
    &= -\epsilon_q - \hf\qq{J_c \rr{1 + \hat n_{d\ol\beta}} + J_z\rr{1 - \hat n_{d\ol\beta}}} + \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta}
\eeq
Therefore,
\beq
\Delta_1^- \ham_N &= \sum_{q\beta}|V_q|^2 \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}\fr{1}{\omega^- + H_1}
...
\beq
H_0 &= \epsilon_d \hat n_{d\ol\beta}\\
    &= H_1 + \epsilon_q - \epsilon_d - U\hat n_{d\ol\beta} + \hf\qq{J_c \rr{1 + \hat n_{d\ol\beta}} + J_z\rr{1 - \hat n_{d\ol\beta}}}
\eeq
As before, we set \(H_0 = 0\) and keep \(H_1 - H_0\) in the denominator.
\beq
&\Delta_1^- \ham_N\\
&= \sum_{q\beta}|V_q|^2 \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}\fr{1}{\omega^- - \epsilon_q + \epsilon_d + U\hat n_{d\ol\beta} - \hf\qq{J_c \rr{1 + \hat n_{d\ol\beta}} + J_z\rr{1 - \hat n_{d\ol\beta}}}} \\
&= \sum_{q\beta}\rr{1 - \hat n_{d\beta}}\qq{\fr{|V_q^1|^2 \hat n_{d\ol\beta}}{\omega^- - \epsilon_q + \epsilon_d + U - J_c} + \fr{|V_q^0|^2\rr{1 - \hat n_{d\ol\beta}}}{\omega - \epsilon_q + \epsilon_d - \hf\rr{J_c + J_z}}}\\
 &= \sum_{q\beta}\left[ \hat n_{d\ol\beta}\rr{\fr{|V_q^1|^2}{\omega - \epsilon_q + \epsilon_d + U - J_c} - \fr{2|V_q^0|^2}{\omega - \epsilon_q + \epsilon_d - \hf\rr{J_c + J_z}}} \right.\\
 &\qquad\left.+ \hat n_{d\ua}\hat n_{d\da}\rr{\fr{|V_q^0|^2}{\omega - \epsilon_q + \epsilon_d - \hf\rr{J_c + J_z}} - \fr{|V_q^1|^2}{\omega - \epsilon_q + \epsilon_d + U - J_c}}\right]
\eeq
%\comm{
\paragraph{2.}
\beq
\Delta_2^- \ham_N &= \sum_{q\beta k} V_q c^\dagger_{q\beta}c_{d\beta}\fr{1}{\hat \omega^- - \ham_D^-}\rr{\hf J_c \hat n_d + J_z \beta S^z_d}c^\dagger_{k\beta}c_{q\beta}
\eeq
The first step is a simplification:
\beq
c_{d\beta}\rr{\hf J_c \hat n_d + J_z \beta S^z_d} &=c_{d\beta}\hf\qq{\rr{J_c + J_z} + \rr{J_c - J_z}\hat n_{d\ol\beta}}\\
						  &=c_{d\beta}\hf\qq{2J_c \hat n_{d\ol\beta}+ \rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}}}
\eeq
The intermediate (\il{\hat n_{q\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = -\epsilon_q + \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta} - \hf\qq{2J_c \hat n_{d\ol\beta}+ \rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}}}
\eeq
The initial state (\il{\hat n_{q\beta}=\hat n_{d\beta}=1}) energy is
\beq
H_0 &= \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta}\\
    &= H_1 + \epsilon_q + \hf\qq{2J_c \hat n_{d\ol\beta}+ \rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}}}
\eeq
\beq
\Delta_2^- \ham_N &= \sum_{q\beta k}\hf\qq{2J_c \hat n_{d\ol\beta}+ \rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}}} V_q c^\dagger_{q\beta}c_{d\beta}\rr{1 - \hat n_{d\ol\beta}}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^- + H_1}\\
&= -\sum_{q\beta k}\hat n_{q\beta}c^\dagger_{k\beta}c_{d\beta}\fr{V_q\hf\qq{2J_c \hat n_{d\ol\beta}+ \rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}}}}{\omega^- - \epsilon_q - \hf\qq{2J_c \hat n_{d\ol\beta}+ \rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}}}}\\
&= -\sum_{q\beta k}\hat n_{q\beta}c^\dagger_{k\beta}c_{d\beta}\qq{\fr{J_c V_q^1 \hat n_{d\ol\beta}}{\omega^- - \epsilon_q - J_c} + \fr{\hf \rr{J_c + J_z}V_q^0 \rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q - \hf\rr{J_c + J_z}}}\\
\eeq
\paragraph{3.}
\beq
...
The intermediate (\il{\hat n_{q\beta}=\hat n_{d\ol\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = \epsilon_d - \epsilon_q - \rr{\hf J_c \hat n_d + J_z \beta S^z_d} = \epsilon_d - \epsilon_q - \hf\rr{J_c + J_z}
\eeq
The initial state (\il{\hat n_{q\beta}=\hat n_{d\ol\beta}=1,\hat n_{d\beta}=0}) energy is
\beq
H_0 &= \epsilon_{q} + \epsilon_d = H_1 + \epsilon_q + \hf\rr{J_c + J_z}
\eeq
\beq
\Delta_3^- \ham_N &= \sum_{q\beta k}J_t V_q c^\dagger_{q\beta}c_{d\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^- + H_1}\\
		  &= \sum_{q\beta k}J_t V_q \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}c^\dagger_{k\ol\beta}c_{d\ol\beta}\fr{-1}{\omega^- - \epsilon_q - \hf\rr{J_c + J_z}}\\
		  &= -J_t \sum_{q\beta k}c^\dagger_{k\beta}c_{d\beta}\fr{V_q^0 \rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon^-_q - \hf\rr{J_c + J_z}}\\
\eeq
\paragraph{4.}
\beq
\Delta_4^- \ham_N &= \sum_{q\beta k}\rr{\hf J_c \hat n_d + J_z \beta S^z_d}c^\dagger_{q\beta}c_{k\beta}\fr{1}{\hat \omega^- - \ham_D^-}V^*_q c^\dagger_{d\beta}c_{q\beta}
\eeq
There is a simplification:
\beq
\rr{\hf J_c \hat n_d + J_z \beta S^z_d} c^\dagger_{d\beta} = \hf\qq{2J_c \hat n_{d\ol\beta}+ \rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}}}c^\dagger_{d\beta}
\eeq
The intermediate (\il{\hat n_{q\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = -\epsilon_q + \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta} - \hf\qq{2J_c \hat n_{d\ol\beta}+ \rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}}}
...
\beq
H_0 &= \epsilon_d \hat n_{d\ol\beta}\\
    &= H_1 + \epsilon_{q} + \hf\qq{2J_c \hat n_{d\ol\beta}+ \rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}}} - \epsilon_d - U\hat n_{d\ol\beta}\\
    &=H_1 + \epsilon_{q} + \hf\rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}} - \epsilon_d + \rr{J_c - U}\hat n_{d\ol\beta}\\
\eeq
\beq
\Delta_4^- \ham_N &= \sum_{q\beta k}V^*_q c^\dagger_{q\beta}c_{k\beta}c^\dagger_{d\beta}c_{q\beta}\fr{\hf\qq{2J_c \hat n_{d\ol\beta}+ \rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}}}}{\omega^- - H_0 - \epsilon_{q} - \hf\rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}} + \epsilon_d - \rr{J_c - U}\hat n_{d\ol\beta}} \\
		  &= \sum_{q\beta k}\hat n_{q\beta}V^*_q c_{k\beta}c^\dagger_{d\beta}\fr{\hf\qq{2J_c \hat n_{d\ol\beta}+ \rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}}}}{\omega^- - \epsilon_{q} - \hf\rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}} + \epsilon_d - \rr{J_c - U}\hat n_{d\ol\beta}} \\
		  &= -\sum_{q\beta k} c_{k\beta}c^\dagger_{d\beta}\qq{\fr{{V^1_q}^* J_c \hat n_{d\ol\beta}}{\omega^- - \epsilon_{q} + \epsilon_d + \rr{U- J_c }} + \fr{{V^0_q}^* \hf\rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_{q} - \hf\rr{J_c + J_z}+ \epsilon_d }}\\
\eeq
\paragraph{5.}
\beq
...
The intermediate (\il{\hat n_{q\beta}=\hat n_{d\ol\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = -\epsilon_q + \epsilon_d - \hf\rr{J_c + J_z}
\eeq
The initial state (\il{\hat n_{d\beta}=\hat n_{d\ol\beta}=0,\hat n_{q\beta}=1}) energy is
\beq
H_0 &= 0
\eeq
\beq
\Delta_5^- \ham_N &= \sum_{q\beta k}J_t V_q^* c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k\ol\beta}c^\dagger_{d\beta}c_{q\beta}\fr{1}{\hat \omega^- - \epsilon_q + \epsilon_d- \hf\rr{J_c + J_z}}\\
		  &= -J_t \sum_{q\beta k}V_q^* \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}c^\dagger_{d\ol\beta}c_{k\ol\beta}\fr{1}{\omega^- - \epsilon_q + \epsilon_d- \hf\rr{J_c + J_z}}\\
		  &= -J_t \sum_{q\beta k}c^\dagger_{d\beta}c_{k\beta}\fr{{V_q^0}^* \rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q + \epsilon_d- \hf\rr{J_c + J_z}}\\
\eeq
%}
\paragraph{6.}
\beq
\Delta_6^- \ham_N &= \sum_{q\beta kk^\prime}\rr{\hf J_c \hat n_d + J_z \beta S^z_d} c^\dagger_{q\beta}c_{k^\prime\beta}\fr{1}{\hat \omega^- - \ham_D^-}\rr{\hf J_c \hat n_d + J_z \beta S^z_d} c^\dagger_{k\beta}c_{q\beta}
\eeq
We again label
\beq
\rr{\hf J_c \hat n_d + J_z \beta S^z_d}^2 = \hat d^2
\eeq
The intermediate (\il{\hat n_{q\beta}=0}) energy is
\beq
H_1 &= H_\text{imp} - \epsilon_q - \hat d
\eeq
The initial state (\il{\hat n_{q\beta}=1}) energy is
\beq
H_0 = H_\text{imp}= H_1 + \epsilon_q + \hat d
\eeq
\beq
\Delta_6^- \ham_N &= \sum_{q\beta kk^\prime}\hat d^2 c^\dagger_{q\beta}c_{k^\prime\beta}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^- + H_1}\\
		  &= \sum_{q\beta kk^\prime} \hat n_{q\beta}\hat d^2 c_{k^\prime\beta}c^\dagger_{k\beta}\fr{1}{\omega^- - \epsilon_q - \hat d}\\
		  &= -\sum_{k^\prime q\beta k}c^\dagger_{k\beta}c_{k^\prime\beta}\qq{\fr{\fr{1}{4}\rr{J_c + J_z}^2\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q - \hf\rr{J_c + J_z}} + \fr{\fr{1}{4}\rr{J_c - J_z}^2\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^- - \epsilon_q - \hf\rr{J_c - J_z}} + \fr{J_c^2\hat n_{d\beta}\hat n_{d\ol\beta}}{\omega^- - \epsilon_q - J_c}}\\
		  &\quad+ \sum_{q\beta k}\qq{\fr{\fr{1}{4}\rr{J_c + J_z}^2\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q - \hf\rr{J_c + J_z}} + \fr{\fr{1}{4}\rr{J_c - J_z}^2\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^- - \epsilon_q - \hf\rr{J_c - J_z}} + \fr{J_c^2\hat n_{d\beta}\hat n_{d\ol\beta}}{\omega^- - \epsilon_q - J_c}}\\\\
\eeq
\paragraph{7.}
\beq
\Delta_7^- \ham_N &= \sum_{q\beta kk^\prime}\rr{\hf J_c \hat n_d + J_z \beta S^z_d} c^\dagger_{q\beta}c_{k^\prime\beta}\fr{1}{\hat \omega^- - \ham_D^-} J_t c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}
\eeq
Simplification:
\beq
\rr{\hf J_c \hat n_d + J_z \beta S^z_d} c^\dagger_{d\beta}c_{d\ol\beta} = \hf\rr{J_c + J_z} c^\dagger_{d\beta}c_{d\ol\beta}
\eeq
The intermediate (\il{\hat n_{q\beta}=\hat n_{d\ol\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = \epsilon_d - \epsilon_q - \hf\rr{J_c + J_z}
\eeq
The initial state (\il{\hat n_{q\beta}=\hat n_{d\ol\beta}=1,\hat n_{d\beta}=0}) energy is
\beq
H_0 &= \epsilon_d = H_1 + \epsilon_q + \hf J_z
\eeq
\beq
\Delta_7^- \ham_N &= \hf \rr{J_c + J_z}J_t\sum_{q\beta kk^\prime} c^\dagger_{q\beta}c_{k^\prime\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^- - H_1} \\
		  &= -\hf \rr{J_c + J_z}J_t\sum_{q\beta kk^\prime}  \hat n_{q\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^- - \epsilon_q  - \hf \rr{J_c + J_z}} \\
		  &= -\hf \rr{J_c + J_z}J_t \sum_{q\beta kk^\prime}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^- - \epsilon_q  - \hf \rr{J_c + J_z}} \\
		  &= -\hf \rr{J_c + J_z}J_t \sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^- - \epsilon_q  - \hf \rr{J_c + J_z}}
\eeq
\paragraph{8.}
\beq
\Delta_8^- \ham_N &= \sum_{q\beta kk^\prime}J_t c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}\fr{1}{\hat \omega^- - \ham_D^-}\rr{\hf J_c \hat n_d + J_z \beta S^z_d} c^\dagger_{k\beta}c_{q\beta}
\eeq
Simplification:
\beq
c^\dagger_{d\ol\beta}c_{d\beta} \rr{\hf J_c \hat n_d + J_z \beta S^z_d} = \hf\rr{J_c + J_z} c^\dagger_{d\ol\beta}c_{d\beta}
\eeq
The intermediate (\il{\hat n_{q\beta}=\hat n_{d\ol\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = - \epsilon_q - \hf\rr{J_c + J_z} + \epsilon_d
...
\eeq
\beq
\Delta_8^- \ham_N &= \hf\rr{J_c + J_z}J_t\sum_{q\beta kk^\prime} c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^- - H_1}\\
		  &= \hf\rr{J_c + J_z}J_t\sum_{q\beta kk^\prime} \hat n_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{-1}{\omega^-  - \epsilon_q - \hf \rr{J_c + J_z}}\\
		  &= -\hf\rr{J_c + J_z}J_t\sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^- - \epsilon_q - \hf \rr{J_c + J_z}}\\
\eeq
\paragraph{9.}
\beq
...
\Delta_9^- \ham_N &= \sum_{q\beta kk^\prime}J_t^2c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}c^\dagger_{d\beta} c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^- - H_1}\\
		  &= \sum_{q\beta kk^\prime}J_t^2\hat n_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c_{k^\prime\ol\beta}c^\dagger_{d\beta} c_{d\ol\beta}c^\dagger_{k\ol\beta}\fr{1}{\omega^- - H_1}\\
		  &= -\sum_{q\beta kk^\prime}J_t^2\hat n_{q\beta}\hat n_{d\ol\beta}c_{d\beta}c_{k^\prime\ol\beta}c^\dagger_{d\beta}c^\dagger_{k\ol\beta}\fr{1}{\omega^- - H_1}\\
		  &= \sum_{q\beta kk^\prime}J_t^2\hat n_{q\beta}\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}c_{k^\prime\ol\beta}c^\dagger_{k\ol\beta}\fr{1}{\omega^- - \epsilon_q^-  - \hf\rr{J_c + J_z}}\\
		  &=-J_t^2 \sum_{q\beta kk^\prime}c^\dagger_{k\beta}c_{k^\prime\beta}\fr{\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q^- - \hf\rr{J_c + J_z}} + J_t^2\sum_{q k \beta}\fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^- - \epsilon_q^- - \hf\rr{J_c + J_z}}\\
\eeq
...
We define \(J_c\), \(J_z\) and \(J_t\) such that the interaction can be written as
\beq
\ham_{I} &= \hf J_c \hat n_d \sum_{k\sigma}\hat n_{k\sigma} + J_z \fr{\hat n_{d\ua} - \hat n_{d\da}}{2}\sum_{kk^\prime}\rr{c^\dagger_{k\ua}c_{k^\prime\ua} - c^\dagger_{k\da}c_{k^\prime\da}} + J_t \sum_{kk^\prime}\qq{ c^\dagger_{d\ua}c_{d\da}c^\dagger_{k\da}c_{k^\prime\ua} + c^\dagger_{d\da}c_{d\ua}c^\dagger_{k\ua}c_{k^\prime\da}}\\
	 &= \hf J_c \hat n_d \sum_{k\sigma}\hat n_{k\sigma}  + 2J_z S_d^z s^z + J_t \rr{S_d^+ s^- + S_d^- s^+}
\eeq
The spin-like operators are defined as
\beq
S^z_d &\equiv \hf\rr{\hat n_{d\ua} - \hat n_{d\da}} \quad&S^+_d &\equiv c^\dagger_{d\ua}c_{d\da}\quad &S^-_d \equiv c^\dagger_{d\da}c_{d\ua}\\
...
\beq
\ham_N &= H_{N-1} + H_\text{imp} + \rr{\epsilon_q + \hf J_c \hat n_d + \beta J_z S^z_d}\hat n_{q\beta} + V_q c^\dagger_{q\beta}c_{d\beta} + \text{h.c.} + \\
       &\quad+ \sum_{k<\Lambda_N}\left[J_z S^z_d \beta\rr{c^\dagger_{k\beta}c_{q\beta} + c^\dagger_{q\beta}c_{k\beta}} + J_t \rr{c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q \beta} + c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k\ol\beta}}\right]
\eeq
where \il{H_{imp}} is the impurity-diagonal part of the Hamiltonian (\il{\epsilon_d \hat n_d + U\hat n_{d\ua}\hat n_{d\da}}) and 
\beq
H_{N-1} = \sum_{k<\Lambda_N,\sigma}\qq{\rr{\epsilon_k + \hf J_c \hat n_d + \sigma J_z S^z_d}\hat n_{k\sigma} + V_k c^\dagger_{k\sigma}c_{d\sigma} + \text{h.c.}} + H_{I,N-1}
...
\eeq
\beq
\Delta^+_2 \ham_N &= -\hf J_z\sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{q\beta}\hat n_{d\ol\beta}c_{q\beta}^\dagger c_{k\beta}\fr{1}{\omega^+ - H_1 - \epsilon_q - \hf\rr{J_c - J_z}}\\
		  &= -\hf J_z\sum_{q\beta k}c^\dagger_{d\beta}c_{k\beta}\fr{\hat n_{d\ol\beta}{V_q^1}^*}{\omega^+ - \epsilon_q - \hf\rr{J_c - J_z}}
\eeq
\paragraph{3.}
\beq
\Delta^+_3 \ham_N = \sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+}J_t c^\dagger_{d\ol\beta}c_{d\beta} c_{q\beta}^\dagger c_{k\ol\beta}
...
\eeq
\beq
\Delta^+_4 \ham_N &= \sum_{q\beta k}-\hf J_z V_q \hat n_{d\ol\beta}c^\dagger_{k\beta}c_{q\beta}c_{q\beta}^\dagger c_{d\beta}\fr{1}{\omega^+ - H_0 + \epsilon_d + U - \epsilon_q - \hf\rr{J_c - J_z}}\\
		  &= \sum_{q\beta k}-\hf J_z V_q \hat n_{d\ol\beta}\rr{1 - \hat n_{q\beta}}c^\dagger_{k\beta} c_{d\beta}\fr{1}{\omega^+ + \epsilon_d + U - \epsilon_q - \hf\rr{J_c - J_z}}\\
		  &= -\hf J_z\sum_{q\beta k}c^\dagger_{k\beta} c_{d\beta}\fr{V_q^1 \hat n_{d\ol\beta}}{\omega^+ - \epsilon_q + \epsilon_d + U - \hf\rr{J_c - J_z}}
\eeq
\paragraph{5.}
\beq
\Delta^+_5 \ham_N = \sum_{q\beta k\sigma}J_t c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+}V_q c_{q\beta}^\dagger c_{d\beta}
...
Since the impurity parts are diagonal, we keep them as is for the time-being:
\beq[simpl3]
\hat d^2 =\rr{J_z S^z_d \beta}^2 = J_c^2 \rr{S^z_d}^2
\eeq
\beq
\Delta^+_6 \ham_N = \sum_{k^\prime q\beta k}\hat d^2 c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+}c^\dagger_{q\beta}c_{k^\prime\beta}
\eeq
...
\beq
\Delta^+_6 \ham_N &= \sum_{k^\prime q\beta k}\hat d^2  c^\dagger_{k\beta}c_{q\beta} c^\dagger_{q\beta}c_{k^\prime\beta}\fr{1}{\omega^+ - H_1}\\
		  &= \sum_{k^\prime q\beta k}\hat d^2  \rr{1  - \hat n_{q\beta}}c^\dagger_{k\beta}c_{k^\prime\beta} \fr{1}{\omega^+ - H_0 - \epsilon_q - \beta J_z S^z_d - \hf J_c \hat n_d
}\\
		  &= \sum_{k^\prime q\beta k}\hat d^2 c^\dagger_{k\beta}c_{k^\prime\beta} \fr{1}{\omega^+ - \epsilon_q - \beta J_z S^z_d - \hf J_c \hat n_d}
\eeq
...
Expanding in this basis gives
\beq
\Delta_6^+ \ham_N &= \sum_{k^\prime q\beta k}c^\dagger_{k\beta}c_{k^\prime\beta}\qq{\fr{\fr{1}{4}J_z^2\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega^+ - \epsilon_q - \hf\rr{J_c + J_z}} + \fr{\fr{1}{4}J_z^2\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^+ - \epsilon_q - \hf\rr{J_c - J_z}}}\\
\eeq
\paragraph{7.}
\beq
\Delta^+_7 \ham_N = \sum_{q\beta kk^\prime} J_z S^z_d \beta c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+} J_t c^\dagger_{d\ol\beta} c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}
\eeq
The first step is a simplification:
\beq
...
\eeq
\beq
\Delta^+_7 \ham_N &= \sum_{q\beta kk^\prime}-\hf J_z J_t c^\dagger_{k\beta}c_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^+ - H_1}\\
		  &= -\hf J_z  J_t \sum_{q\beta kk^\prime}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^+ - \epsilon_{q} - \hf\rr{J_c - J_z}}\\
		  &= -\hf J_z  J_t \sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^+ - \epsilon_{q} - \hf\rr{J_c - J_z}}\\
\eeq
\paragraph{8.}
\beq
\Delta^+_8 \ham_N = \sum_{q\beta kk^\prime}J_t  c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+}J_z S^z_d \beta c_{q\beta}^\dagger c_{k^\prime\beta}
\eeq
The first step is a simplification:
\beq
...
\eeq
\beq
\Delta^+_8 \ham_N & = -\hf J_z J_t \sum_{q\beta kk^\prime}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}c_{q\beta}^\dagger c_{k^\prime\beta}\fr{1}{\omega^+ - \epsilon_q - \hf\rr{J_c - J_z}}\\
		  & = -\hf J_z J_t \sum_{q\beta kk^\prime}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^+- \epsilon_q  - \hf\rr{J_c - J_z}}\\
		  & = -\hf J_z J_t \sum_{q\beta kk^\prime}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^+ - \epsilon_q - \hf\rr{J_c - J_z}}\\
\eeq
\paragraph{9.}
\beq
\Delta^+_9 \ham_N = \sum_{q\beta k k\prime}J_t c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta} c_{q\beta}\fr{1}{\omega^+ - \ham_D^+} J_t c^\dagger_{d\ol\beta}c_{d\beta}c_{q\beta}^\dagger c_{k^\prime\ol\beta}
...
The renormalization in the Hamiltonian in the hole sector is
\beq
\Delta^- \ham_N = \sum_{q\beta}\qq{V_q c^\dagger_{q\beta}c_{d\beta} + J_z \beta S^z_d \sum_{k\sigma} c_{k\beta} c^\dagger_{q\beta} + J_t\sum_{k\sigma}c^\dagger_{d\ol\beta}c^\dagger_{q\beta}c_{d\beta}c_{k\ol\beta}}\times\fr{-1}{\hat \omega^- - \ham_D^-}\\
\times\qq{V_q^* c^\dagger_{d\beta}c_{q\beta} + J_z \beta S^z_d \sum_{k\sigma} c_{q\beta} c^\dagger_{k\beta} + J_t\sum_{k\sigma}c^\dagger_{d\beta}c^\dagger_{k\ol\beta}c_{d\ol\beta}c_{q\beta}}
\eeq
The propagator can be written as
\beq
\fr{-1}{\hat \omega^- - \ham_D^-} = \fr{1}{\omega^- + \ham_D^-}
...
The first step is a simplification:
\beq
c_{d\beta}J_z \beta S^z_d &=c_{d\beta}\hf J_z\rr{1 - \hat n_{d\ol\beta}}
\eeq
The intermediate (\il{\hat n_{q\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = -\epsilon_q + \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta} - \hf\qq{2J_c \hat n_{d\ol\beta}+ \rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}}}
...
\eeq
\beq
\Delta_2^- \ham_N &= \sum_{q\beta k}\hf J_z\rr{1 - \hat n_{d\ol\beta}} V_q c^\dagger_{q\beta}c_{d\beta}\rr{1 - \hat n_{d\ol\beta}}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^- + H_1}\\
&= -\sum_{q\beta k}\hat n_{q\beta}c^\dagger_{k\beta}c_{d\beta}\fr{V_q\hf J_z\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q - \hf\qq{2J_c \hat n_{d\ol\beta}+ \rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}}}}\\
&= -\sum_{q\beta k}\hat n_{q\beta}c^\dagger_{k\beta}c_{d\beta}\fr{\hf J_z V_q^0 \rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q - \hf\rr{J_c + J_z}}\\
\eeq
\paragraph{3.}
\beq
\Delta_3^- \ham_N &= \sum_{q\beta k}V_q c^\dagger_{q\beta}c_{d\beta}\fr{1}{\hat \omega^- - \ham_D^-}J_t c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}
...
\paragraph{4.}
\beq
\Delta_4^- \ham_N &= \sum_{q\beta k}\hf J_z \beta S^z_d c^\dagger_{q\beta}c_{k\beta}\fr{1}{\hat \omega^- - \ham_D^-}V^*_q c^\dagger_{d\beta}c_{q\beta}
\eeq
There is a simplification:
\beq
\hf J_z \beta S^z_d c^\dagger_{d\beta} = \hf J_z\rr{1 - \hat n_{d\ol\beta}}c^\dagger_{d\beta}
\eeq
The intermediate (\il{\hat n_{q\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = -\epsilon_q + \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta} - \hf\qq{2J_c \hat n_{d\ol\beta}+ \rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}}}
...
\eeq
\beq
\Delta_4^- \ham_N &= \sum_{q\beta k}V^*_q c^\dagger_{q\beta}c_{k\beta}c^\dagger_{d\beta}c_{q\beta}\fr{\hf J_z\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - H_0 - \epsilon_{q} - \hf\rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}} + \epsilon_d - \rr{J_c - U}\hat n_{d\ol\beta}} \\
		  &= \sum_{q\beta k}\hat n_{q\beta}V^*_q c_{k\beta}c^\dagger_{d\beta}\fr{\hf J_z\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_{q} - \hf\rr{J_c + J_z}\rr{1 - \hat n_{d\ol\beta}} + \epsilon_d - \rr{J_c - U}\hat n_{d\ol\beta}} \\
		  &= -\sum_{q\beta k} c^\dagger_{d\beta}c_{k\beta} \fr{{V^0_q}^* \hf J_z\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_{q} - \hf\rr{J_c + J_z}+ \epsilon_d}\\
\eeq
\paragraph{5.}
\beq
\Delta_5^- \ham_N &= \sum_{q\beta k}J_t  c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k\ol\beta}\fr{1}{\hat \omega^- - \ham_D^-}V_q^* c^\dagger_{d\beta}c_{q\beta}
...
We again label
\beq
\rr{J_z \beta S^z_d}^2 = \hat d^2
\eeq
The intermediate (\il{\hat n_{q\beta}=0}) energy is
\beq
H_1 &= H_\text{imp} - \epsilon_q - \hf J_c \hat n_d - J_z \beta S^z_d
\eeq
The initial state (\il{\hat n_{q\beta}=1}) energy is
\beq
H_0 = H_\text{imp}= H_1 + \epsilon_q + \hf J_c \hat n_d + J_z \beta S^z_d
\eeq
\beq
\Delta_6^- \ham_N &= \sum_{q\beta kk^\prime}\hat d^2 c^\dagger_{q\beta}c_{k^\prime\beta}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^- + H_1}\\
		  &= \sum_{q\beta kk^\prime} \hat n_{q\beta}\hat d^2 c_{k^\prime\beta}c^\dagger_{k\beta}\fr{1}{\omega^- - \epsilon_q - \hat d}\\
		  &= -\fr{1}{4}J_z^2\sum_{k^\prime q\beta k}c^\dagger_{k\beta}c_{k^\prime\beta}\qq{\fr{\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q - \hf\rr{J_c + J_z}} + \fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^- - \epsilon_q - \hf\rr{J_c - J_z}}}\\
		  &\quad+ \fr{1}{4}J_z^2\sum_{q\beta k}\qq{\fr{\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q - \hf\rr{J_c + J_z}} + \fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^- - \epsilon_q - \hf\rr{J_c - J_z}} }\\\\
\eeq
\paragraph{7.}
\beq
\Delta_7^- \ham_N &= \sum_{q\beta kk^\prime} J_z \beta S^z_d c^\dagger_{q\beta}c_{k^\prime\beta}\fr{1}{\hat \omega^- - \ham_D^-} J_t c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}
...
\eeq
\beq
\Delta_7^- \ham_N &= \hf J_z J_t\sum_{q\beta kk^\prime} c^\dagger_{q\beta}c_{k^\prime\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^- - H_1} \\
		  &= -\hf J_z J_t\sum_{q\beta kk^\prime}  \hat n_{q\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^- - \epsilon_q  - \hf \rr{J_c + J_z}} \\
		  &= -\hf J_z J_t \sum_{q\beta kk^\prime}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^- - \epsilon_q  - \hf \rr{J_c + J_z}} \\
		  &= -\hf J_z J_t \sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^- - \epsilon_q  - \hf \rr{J_c + J_z}}
\eeq
\paragraph{8.}
\beq
\Delta_8^- \ham_N &= \sum_{q\beta kk^\prime}J_t c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}\fr{1}{\hat \omega^- - \ham_D^-} J_z \beta S^z_d c^\dagger_{k\beta}c_{q\beta}
...
\eeq
\beq
\Delta_8^- \ham_N &= \hf J_z J_t\sum_{q\beta kk^\prime} c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^- - H_1}\\
		  &= \hf J_z J_t\sum_{q\beta kk^\prime} \hat n_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{-1}{\omega^-  - \epsilon_q - \hf \rr{J_c + J_z}}\\
		  &= -\hf J_z J_t\sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^- - \epsilon_q - \hf \rr{J_c + J_z}}\\
\eeq
\paragraph{9.}
\beq
...
\beq
\hf J_c \sum_{k\sigma\beta}\hat n_{d\beta}\hat n_{k\sigma^\prime} = \hf J_c \sum_{k\sigma\beta}\rr{\hat n_{d\beta} - \hf}\rr{\hat n_{k\sigma^\prime} - \hf} + \fr{1}{4}J_c\sum_{k\sigma\beta}\hat n_{d\beta} + \fr{1}{4}J_c\sum_{k\sigma\beta}\hat n_{k\sigma}
\eeq
The last term can be combined with a chemical potential term:
\beq
\fr{1}{4}J_c\sum_{k\sigma\beta}\hat n_{k\sigma} - \mu \sum_{k\sigma}\hat n_{k\sigma} = -\rr{\mu - \hf J_c} \sum_{k\sigma}\hat n_{k\sigma}
\eeq
...
\beq
\Delta J_c &= -J_t^2 \sum_q \rr{\fr{1}{\omega^+ - \epsilon_q  + \hf  J_z} - \fr{1}{\omega^- - \epsilon_q  + \hf  J_z}} = 0\\
\eeq
...
\beq
\Delta \rr{\epsilon_d + \hf U} = \sum_q \qq{\fr{|V_q^1|^2}{\omega^+ - \epsilon_q + \epsilon_d + U + \hf J_z}- \fr{|V_q^0|^2}{\omega^- - \epsilon_q + \epsilon_d - \hf J_z}}
\eeq
...
\beq
\Delta \rr{\epsilon_d + \hf U} = \sum_q |V_q|^2\qq{\fr{1}{\omega^+ - \epsilon_q + \epsilon_d + U + \hf J_z}- \fr{1}{\omega^- - \epsilon_q - \epsilon_d + \hf J_z}}
\eeq
...
\beq
\Delta V_1 &= -\sum_q V_1(q)\rr{\fr{\hf J_z + J_t}{\omega^+ - \epsilon_q +\epsilon_d + U + \hf J_z}} = \Delta V_1^*
\eeq
...
\beq
\Delta V_0 &= -\sum_q V_0(q)\fr{\hf J_z + J_t}{{\omega^-}* - \epsilon_q + \epsilon_d - \hf J_z} = \Delta V_0^*
\eeq
...
\ham_D^+ \equiv \text{Tr}_{q\beta}\qq{\ham \hat n_{q\beta}}= \sum_{k<\Lambda_N,\sigma}\rr{\epsilon_k  + \sigma J_z S^z_d}\hat n_{k\sigma} +\rr{\epsilon_q  + \beta J_z S^z_d} + H_{imp}\\
\eeq
As a simplification, we will ignore the terms that pertain to the lower electrons (\(k<q\)) in \(\ham_D^+\). The entire renormalization expression has nine terms- one of order \il{|V_q|^2}, four of order \il{V_q u_P} and four of order \il{u_P^2}.
\paragraph{1.}
\beq
\Delta_1^+ \ham_N = \sum_{q\beta}|V_q|^2 c^\dagger_{d\beta}c_{q\beta}\fr{1}{\hat \omega^+ - \ham_D^+}c^\dagger_{q\beta}c_{d\beta}
\eeq
The intermediate state is characterized by \il{\hat n_{d\beta} = 0, \hat n_{q\beta} = 1}. Therefore, at the propagator, we have
\beq
H_1 = \ham_D^+ &= \qq{\epsilon_q  + \beta J_z S^z_d} + \epsilon_d \hat n_{d\ol\beta}\\
		    &= \qq{\epsilon_q  - \hf \beta\beta J_z \hat n_{d\ol\beta}} + \epsilon_d \hat n_{d\ol\beta}\\
		    &= \qq{\epsilon_q - \hf J_z \hat n_{d\ol\beta}} + \epsilon_d \hat n_{d\ol\beta}
\eeq
\il{H_1} is the intermediate state Hamiltonian. As a simplification, we replace \il{\hat \omega^+} with its eigenvalue \il{2\omega^+\tau^+ = \omega^+}.
\beq
\Delta_1^+ \ham_N &= \sum_{q\beta}|V_q|^2 c^\dagger_{d\beta}c_{q\beta}\fr{1}{\hat \omega^+ - H_1}c^\dagger_{q\beta}c_{d\beta}\\
		  &= \sum_{q\beta}|V_q|^2 c^\dagger_{d\beta}c_{q\beta}c^\dagger_{q\beta}c_{d\beta}\fr{1}{\omega^+ - \epsilon_q - \epsilon_d \hat n_{d\ol\beta} + \hf J_z \hat n_{d\ol\beta}}
\eeq
...
\beq
\Delta_1^+ \ham_N &= \sum_{q\beta}|V_q|^2 \hat n_{d\beta}\fr{1}{\omega^+ - \epsilon_q + \rr{\fr{J_z}{2} - \epsilon_d}\hat n_{d\ol\beta}}\\
		  &= \sum_{q\beta}|V(q)|^2\hat n_{d\beta}\qq{\fr{ \hat n_{d\ol\beta}}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z} + \fr{\rr{1 - \hat n_{d\ol\beta}}}{\omega^+ - \epsilon_q}}\\
		  &= \sum_{q\beta}|V(q)|^2\hat n_{d\beta}\qq{\fr{1}{\omega^+ - \epsilon_q} + \hat n_{d\ol\beta}\rr{\fr{1}{\omega^+ - \epsilon_q - \epsilon_d + \hf {J_z}} - \fr{1}{\omega^+ - \epsilon_q}}}\\
\eeq
\paragraph{2.}
\beq
\Delta^+_2 \ham_N = \sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+} J_z \beta S^z_dc_{q\beta}^\dagger c_{k\beta}
...
\Delta^+_2 \ham_N &= -\hf J_z\sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{q\beta}\hat n_{d\ol\beta}c_{q\beta}^\dagger c_{k\beta}\fr{1}{\omega^+ - H_1}\\
		  &= -\hf J_z\sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{k\beta}\fr{\hat n_{d\ol\beta}}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z}
\eeq
\paragraph{3.}
\beq
\Delta^+_3 \ham_N = \sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+}J_t c^\dagger_{d\ol\beta}c_{d\beta} c_{q\beta}^\dagger c_{k\ol\beta}
...
\Delta^+_3 \ham_N &= \sum_{q\beta k}J_t V_q^*c^\dagger_{d\beta}c_{q\beta}c^\dagger_{d\ol\beta} c_{d\beta}c_{q\beta}^\dagger c_{k\ol\beta}\fr{1}{\omega^+ - H_1}\\
		  ...
		  &= -J_t \sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{k\beta}\fr{\hat n_{d\ol\beta}}{\omega^+  - \epsilon_q - \epsilon_d + \hf J_z}
\eeq
\paragraph{4.}
\beq
\Delta^+_4 \ham_N = \sum_{q\beta k\sigma} J_z \beta S^z_dc^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+}V_qc_{q\beta}^\dagger c_{d\beta}
...
\Delta^+_4 \ham_N &= -\hf J_z \sum_{q\beta k}V_q \hat n_{d\ol\beta}c^\dagger_{k\beta}c_{q\beta}c_{q\beta}^\dagger c_{d\beta}\fr{1}{\omega^+ - H_1}\\
		  &= \sum_{q\beta k}-\hf J_z V_q \hat n_{d\ol\beta}\rr{1 - \hat n_{q\beta}}c^\dagger_{k\beta} c_{d\beta}\fr{1}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z}\\
		  &= -\hf J_z\sum_{q\beta k}V_q c^\dagger_{k\beta} c_{d\beta}\fr{\hat n_{d\ol\beta}}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z}
\eeq
\paragraph{5.}
\beq
\Delta^+_5 \ham_N = \sum_{q\beta k\sigma}J_t c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+}V_q c_{q\beta}^\dagger c_{d\beta}
...
\Delta^+_5 \ham_N &= \sum_{q\beta k}J_t V_q c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}c_{q\beta}^\dagger c_{d\beta}\fr{1}{\omega^+ - H_1}\\
		  &= -\sum_{q\beta k}J_t V_q \rr{1 - \hat n_{q\beta}}\hat n_{d\beta}c^\dagger_{k\ol\beta} c_{d\ol\beta}\fr{1}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z}\\
		  &=-J_t  \sum_{q\beta k}V_q c^\dagger_{k\beta} c_{d\beta}\fr{\hat n_{d\ol\beta}}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z}\\
\eeq
%}
\paragraph{6.}
\beq
...
\beq
\Delta^+_6 \ham_N &=\fr{1}{4}J_z^2 \sum_{k^\prime q\beta k}  \rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}c^\dagger_{k\beta}c_{q\beta} c^\dagger_{q\beta}c_{k^\prime\beta}\fr{1}{\omega^+ - H_1}\\
		  &=\fr{1}{4}J_z^2 \sum_{k^\prime q\beta k}  \rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}\rr{1  - \hat n_{q\beta}}c^\dagger_{k\beta}c_{k^\prime\beta} \fr{1}{\omega^+ - \epsilon_q  - H_{imp}- \beta J_z S^z_d}\\
		  &=\fr{1}{4}J_z^2 \sum_{k^\prime q\beta k} c^\dagger_{k\beta}c_{k^\prime\beta} \fr{\rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}}{\omega^+ - \epsilon_q - \epsilon_d - \beta J_z S^z_d}\\
		  &=\fr{1}{4}J_z^2\sum_{k^\prime q\beta k} c^\dagger_{k\beta}c_{k^\prime\beta}\qq{ \fr{\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}} {\omega^+ - \epsilon_q - \epsilon_d - \hf J_z} + \fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}} {\omega^+ - \epsilon_q - \epsilon_d + \hf J_z}}
\eeq
...
\beq
\Delta^+_7 \ham_N &= \sum_{q\beta kk^\prime}\hf J_zJ_t c^\dagger_{k\beta}c_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}\fr{-1}{\omega^+ - H_1}\\
		  &= -\hf J_z J_t \sum_{q\beta kk^\prime}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^+ - \epsilon_{q} - \epsilon_d + \hf J_z}\\
		  &= -\hf J_z J_t \sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^+ - \epsilon_{q} - \epsilon_d + \hf J_z}\\
\eeq
\paragraph{8.}
\beq
\Delta^+_8 \ham_N = \sum_{q\beta kk^\prime}J_t  c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^+ - \ham_D^+}J_z \beta S^z_d c_{q\beta}^\dagger c_{k^\prime\beta}
...
\Delta^+_8 \ham_N &=- \sum_{q\beta kk^\prime}\hf J_z J_t c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}c_{q\beta}^\dagger c_{k^\prime\beta}\fr{1}{\omega^+ - H_1}\\
		  &=-\hf J_z J_t \sum_{q\beta kk^\prime}\rr{1 - \hat n_{q\beta}}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^+- \epsilon_q - \epsilon_d + \hf J_z}\\
		  &= -\hf J_z J_t\sum_{q\beta kk^\prime}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z}\\
\eeq
\paragraph{9.}
\beq
\Delta^+_9 \ham_N = \sum_{q\beta k k\prime}J_t c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta} c_{q\beta}\fr{1}{\omega^+ - \ham_D^+} J_t c^\dagger_{d\ol\beta}c_{d\beta}c_{q\beta}^\dagger c_{k^\prime\ol\beta}
...
\Delta^+_9 \ham_N &= \sum_{q\beta k k\prime}J_t^2 c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta} c_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c_{q\beta}^\dagger c_{k^\prime\ol\beta}\fr{1}{\omega^+  - H_1}\\
		  &= J_t^2\sum_{q\beta k k\prime}\rr{1 - \hat n_{q\beta}} \hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}c^\dagger_{k\ol\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^+  - \epsilon_q - \epsilon_d + \hf J_z}\\
		  &= J_t^2\sum_{q\beta k k\prime}c^\dagger_{k\beta}c_{k^\prime\beta}\fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z}
\eeq
...
\beq
\Delta^+ \epsilon_d &=  \sum_{q}|V(q)|^2\fr{1}{\omega^+ - \epsilon_q}\\
\Delta^+ U &=  \sum_{q}2|V(q)|^2\rr{\fr{1}{\omega^+ - \epsilon_q - \epsilon_d + \hf {J_z}} - \fr{1}{\omega^+ - \epsilon_q}}\\
\Delta^+ V &= -\rr{\hf J_z + J_t}\sum_{q}V_q^*\fr{1}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z}\\
\Delta^+ J_t &= -J_z J_t\sum_{q}\fr{1}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z}\\
\Delta^+ J_z &= -J_t^2 \sum_{q}\fr{1}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z}
\eeq
...
\beq
\Delta_1^- \ham_N &= \sum_{q\beta}|V_q|^2 \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}\fr{1}{\omega^- -\epsilon_q - \fr{J_z}{2}\rr{1 - \hat n_{d\ol\beta}} + \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta}} \\
 &= \sum_{q\beta}\rr{1 - \hat n_{d\beta}}\qq{\fr{|V_q^1|^2 \hat n_{d\ol\beta}}{\omega^- - \epsilon_q + 2\epsilon_d + U } + \fr{|V_q^0|^2\rr{1 - \hat n_{d\ol\beta}}}{\omega - \epsilon_q + \epsilon_d - \hf J_z}}\\
 &= \sum_{q\beta}|V(q)|^2\left[ \hat n_{d\ol\beta}\rr{\fr{1}{\omega - \epsilon_q + 2\epsilon_d + U} - \fr{2}{\omega - \epsilon_q + \epsilon_d - \hf J_z}} \right.\\
 &\quad\quad\left.+ \hat n_{d\ua}\hat n_{d\da}\rr{\fr{1}{\omega - \epsilon_q + \epsilon_d - \hf J_z} - \fr{1}{\omega - \epsilon_q + 2\epsilon_d + U}}\right]
\eeq
\paragraph{2.}
\beq
\Delta_2^- \ham_N &= \sum_{q\beta k} V_q c^\dagger_{q\beta}c_{d\beta}\fr{1}{\hat \omega^- - \ham_D^-} J_z \beta S^z_d c^\dagger_{k\beta}c_{q\beta}
...
The intermediate (\il{\hat n_{q\beta}=0,\hat n_{d\beta}=1}) energy is
\beq
H_1 = -\epsilon_q + \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta} - \hf J_z\rr{1 - \hat n_{d\ol\beta}}
\eeq
\beq
\Delta_2^- \ham_N &= \sum_{q\beta k}\hf J_z\rr{1 - \hat n_{d\ol\beta}} V_q c^\dagger_{q\beta}c_{d\beta}\rr{1 - \hat n_{d\ol\beta}}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^- + H_1}\\
&= -\sum_{q\beta k}\hat n_{q\beta}c^\dagger_{k\beta}c_{d\beta}\fr{V_q\hf J_z\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q + \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta} - \hf J_z\rr{1 - \hat n_{d\ol\beta}}}\\
&= -\hf J_z\sum_{q\beta k} V_q \hat n_{q\beta}c^\dagger_{k\beta}c_{d\beta}\fr{ \rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q + \epsilon_d - \hf J_z}\\
\eeq
\paragraph{3.}
\beq
\Delta_3^- \ham_N &= \sum_{q\beta k}V_q c^\dagger_{q\beta}c_{d\beta}\fr{1}{\hat \omega^- - \ham_D^-}J_t c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}
...
\beq
\Delta_3^- \ham_N &= \sum_{q\beta k}J_t V_q c^\dagger_{q\beta}c_{d\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^- + H_1}\\
		  &= \sum_{q\beta k}J_t V_q \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}c^\dagger_{k\ol\beta}c_{d\ol\beta}\fr{-1}{\omega^- - \epsilon_q + \epsilon_d - \hf J_z}\\
		  &= -J_t \sum_{q\beta k}V_q c^\dagger_{k\beta}c_{d\beta}\fr{1 - \hat n_{d\ol\beta}}{\omega^- - \epsilon_q  + \epsilon_d - \hf J_z}\\
\eeq
\paragraph{4.}
\beq
\Delta_4^- \ham_N &= \sum_{q\beta k}\hf J_z \beta S^z_d c^\dagger_{q\beta}c_{k\beta}\fr{1}{\hat \omega^- - \ham_D^-}V^*_q c^\dagger_{d\beta}c_{q\beta}
...
\eeq
\beq
\Delta_4^- \ham_N &= \sum_{q\beta k}V^*_q c^\dagger_{q\beta}c_{k\beta}c^\dagger_{d\beta}c_{q\beta}\fr{\hf J_z\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - H_1} \\
		  &= \sum_{q\beta k}\hat n_{q\beta}V^*_q c_{k\beta}c^\dagger_{d\beta}\fr{\hf J_z\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_{q} + \epsilon_d + \rr{\epsilon_d + U}\hat n_{d\ol\beta} - \hf J_z\rr{1 - \hat n_{d\ol\beta}}} \\
		  &= -\hf J_z\sum_{q\beta k}V_q^* c^\dagger_{d\beta}c_{k\beta} \fr{1 - \hat n_{d\ol\beta}}{\omega^- - \epsilon_{q} + \epsilon_d - \hf J_z}\\
\eeq
\paragraph{5.}
\beq
\Delta_5^- \ham_N &= \sum_{q\beta k}J_t  c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k\ol\beta}\fr{1}{\hat \omega^- - \ham_D^-}V_q^* c^\dagger_{d\beta}c_{q\beta}
...
\Delta_5^- \ham_N &= \sum_{q\beta k}J_t V_q^* c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k\ol\beta}c^\dagger_{d\beta}c_{q\beta}\fr{1}{\hat \omega^- + H_1}\\
		  &= -J_t \sum_{q\beta k}V_q^* \hat n_{q\beta}\rr{1 - \hat n_{d\beta}}c^\dagger_{d\ol\beta}c_{k\ol\beta}\fr{1}{\omega^- - \epsilon_q + \epsilon_d- \hf J_z}\\
		  &= -J_t \sum_{q\beta k}V_q^*c^\dagger_{d\beta}c_{k\beta}\fr{1 - \hat n_{d\ol\beta}}{\omega^- - \epsilon_q + \epsilon_d- \hf J_z}\\
\eeq
\paragraph{6.}
\beq
\Delta_6^- \ham_N &= \sum_{q\beta kk^\prime}J_z \beta S^z_d c^\dagger_{q\beta}c_{k^\prime\beta}\fr{1}{\hat \omega^- - \ham_D^-}J_z \beta S^z_d c^\dagger_{k\beta}c_{q\beta}
...
\beq
\Delta_6^- \ham_N &= \sum_{q\beta kk^\prime}\fr{J_z^2}{4}\rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}c^\dagger_{q\beta}c_{k^\prime\beta}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^- + H_1}\\
		  &=\fr{J_z^2}{4} \sum_{q\beta kk^\prime} \hat n_{q\beta}\rr{\hat n_{d} - 2\hat n_{d\ua}\hat n_{d\da}}c_{k^\prime\beta}c^\dagger_{k\beta}\fr{1}{\omega^- - \epsilon_q + H_\text{imp} - \beta J_z S^z_d}\\
		  &= -\fr{J_z^2}{4}\sum_{q\beta kk^\prime}c^\dagger_{k\beta}c_{k^\prime\beta}\qq{\fr{\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q + \epsilon_d - \fr{J_z}{2}} + \fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^- - \epsilon_q  + \epsilon_d+ \fr{J_z}{2}}} \\
		  &\quad+ \fr{J_z^2}{4}\sum_{q\beta k}\qq{\fr{\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q  + \epsilon_d- \fr{J_z}{2}} + \fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^- - \epsilon_q  + \epsilon_d + \fr{J_z}{2}}}\\
\eeq
\paragraph{7.}
\beq
\Delta_7^- \ham_N &= \sum_{q\beta kk^\prime}J_z \beta S^z_d c^\dagger_{q\beta}c_{k^\prime\beta}\fr{1}{\hat \omega^- - \ham_D^-} J_t c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}
...
\beq
\Delta_7^- \ham_N &= \sum_{q\beta kk^\prime}\hf J_z J_t c^\dagger_{q\beta}c_{k^\prime\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^- - H_1} \\
		  &= \sum_{q\beta kk^\prime}\hf J_z J_t  \hat n_{q\beta}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{-1}{\omega^- - \epsilon_q + \epsilon_d - \hf J_z} \\
		  &= -\hf J_z J_t \sum_{q\beta kk^\prime}c^\dagger_{d\beta}c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{k^\prime\beta}\fr{1}{\omega^- - \epsilon_q + \epsilon_d - \hf J_z} \\
		  &= -\hf J_z J_t \sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^- - \epsilon_q + \epsilon_d - \hf J_z}
\eeq
\paragraph{8.}
\beq
\Delta_8^- \ham_N &= \sum_{q\beta kk^\prime}J_t c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}\fr{1}{\hat \omega^- - \ham_D^-}J_z \beta S^z_d c^\dagger_{k\beta}c_{q\beta}
...
\beq
\Delta_8^- \ham_N &= \sum_{q\beta kk^\prime}\hf J_z J_t c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}c^\dagger_{k\beta}c_{q\beta}\fr{1}{\omega^- - H_1}\\
		  &= \sum_{q\beta kk^\prime}\hf J_z J_t \hat n_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{-1}{\omega^- - \epsilon_q + \epsilon_d - \fr{J_z}{2}}\\
		  &= -\hf J_z J_t \sum_{q\beta kk^\prime}c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{k\beta}c_{k^\prime\ol\beta}\fr{1}{\omega^- - \epsilon_q + \epsilon_d - \hf J_z}\\
\eeq
\paragraph{9.}
\beq
...
\Delta_9^- \ham_N &= \sum_{q\beta kk^\prime}J_t^2c^\dagger_{d\ol\beta}c_{d\beta}c^\dagger_{q\beta}c_{k^\prime\ol\beta}c^\dagger_{d\beta} c_{d\ol\beta}c^\dagger_{k\ol\beta}c_{q\beta}\fr{1}{\omega^- - H_1}\\
		  &= \sum_{q\beta kk^\prime}J_t^2\hat n_{q\beta}c^\dagger_{d\ol\beta}c_{d\beta}c_{k^\prime\ol\beta}c^\dagger_{d\beta} c_{d\ol\beta}c^\dagger_{k\ol\beta}\fr{1}{\omega^- - H_1}\\
		  &= -\sum_{q\beta kk^\prime}J_t^2\hat n_{q\beta}\hat n_{d\ol\beta}c_{d\beta}c_{k^\prime\ol\beta}c^\dagger_{d\beta}c^\dagger_{k\ol\beta}\fr{1}{\omega^- - H_1}\\
		  &= \sum_{q\beta kk^\prime}J_t^2\hat n_{q\beta}\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}c_{k^\prime\ol\beta}c^\dagger_{k\ol\beta}\fr{1}{\omega^- - \epsilon_q^- + \epsilon_d - \hf J_z}\\
		  &=-J_t^2 \sum_{q\beta kk^\prime}c^\dagger_{k\beta}c_{k^\prime\beta}\fr{\hat n_{d\beta}\rr{1 - \hat n_{d\ol\beta}}}{\omega^- - \epsilon_q^- + \epsilon_d- \hf J_z} + J_t^2\sum_{q k \beta}\fr{\hat n_{d\ol\beta}\rr{1 - \hat n_{d\beta}}}{\omega^- - \epsilon_q^- + \epsilon_d- \hf J_z}\\
\eeq
...
\begin{flalign*}
\Delta^- \epsilon_d &=  \sum_{q}|V(q)|^2\rr{\fr{1}{\omega^- - \epsilon_q + 2\epsilon_d + U} - \fr{2}{\omega^- - \epsilon_q + \epsilon_d - \hf J_z}} + \\
	   &\quad\sum_{q k}\qq{\fr{\fr{1}{4}J_z^2}{\omega^- - \epsilon_q  + \epsilon_d- \fr{J_z}{2}} + \fr{J_t^2 + \fr{1}{4}J_z^2}{\omega^- - \epsilon_q  + \epsilon_d + \fr{J_z}{2}}}\\
\Delta^- U &=  2\sum_{q}|V(q)|^2\rr{\fr{1}{\omega^- - \epsilon_q + \epsilon_d - \hf {J_z}} - \fr{1}{\omega^- - \epsilon_q + 2\epsilon_d + U}} -\\
	   &\quad2\sum_{q k}\qq{\fr{\fr{1}{4}J_z^2}{\omega^- - \epsilon_q  + \epsilon_d- \fr{J_z}{2}} + \fr{J_t^2 + \fr{1}{4}J_z^2}{\omega^- - \epsilon_q  + \epsilon_d + \fr{J_z}{2}}}\\\\
\Delta^- V &= -\rr{\hf J_z + J_t}\sum_{q}V_q^*\fr{1}{\omega^- - \epsilon_q + \epsilon_d - \hf J_z}\\
\Delta^- J_t &= -J_z J_t\sum_{q}\fr{1}{\omega^- - \epsilon_q + \epsilon_d - \hf J_z}\\
\Delta^- J_z &= -J_t^2 \sum_{q}\fr{1}{\omega^- - \epsilon_q + \epsilon_d - \hf J_z}
\end{flalign*}
...
\beq
\Delta^+\eta=\Delta^+\rr{\epsilon_d + \hf U} &= \sum_q |V(q)|^2\fr{1}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z}\\
\Delta^-\eta=\Delta^-\rr{\epsilon_d + \hf U} &= -\sum_q |V(q)|^2\fr{1}{\omega^- - \epsilon_q + \epsilon_d - \hf J_z}
\eeq
...
\subsection{Final scaling equations}
If we apply this equation the scaling equations of the hole sector, we can immediately see that
\beq
\Delta \epsilon_d \equiv& \Delta^+ \epsilon_d  + \Delta^- \epsilon_d = \sum_q |V(q)|^2\qq{\fr{1}{\omega^+ - \epsilon_q} + \fr{1}{\omega^+ - \epsilon_q + U + J_z} -  \fr{2}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z}} \\
		   & \quad\quad\quad\quad\quad\quad\quad+\fr{1}{4}\sum_{q k}\qq{\fr{J_z^2}{\omega^+ - \epsilon_q  - \epsilon_d + \hf J_z} + \fr{4J_t^2 + J_z^2}{\omega^+ - \epsilon_q  - \epsilon_d +\fr{3}{2} J_z}}\\
\Delta U \equiv& \Delta^+ U  + \Delta^- U = 2\sum_q |V(q)|^2\left[\fr{2}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z} - \fr{1}{\omega^+ - \epsilon_q } - \fr{1}{\omega^+ - \epsilon_q + U + J_z}\right]\\
	       &\quad\quad\quad\quad\quad\quad\quad + \hf\sum_{q k}\qq{\fr{J_z^2}{\omega^+ - \epsilon_q  - \epsilon_d + \hf J_z} + \fr{4J_t^2 + J_z^2}{\omega^+ - \epsilon_q  - \epsilon_d +\fr{3}{2} J_z}}\\
\Delta V \equiv& \Delta^+ V + \Delta^- V = 2\Delta^+ V = -2\rr{\hf J_z + J_t}\sum_{q}V_q^*\fr{1}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z}\\
\Delta J_z \equiv& \Delta^+ J_z + \Delta^- J_z = 2\Delta^+ J_z = -2J_t^2 \sum_{q}\fr{1}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z}\\
\Delta J_t \equiv& \Delta^+ J_t + \Delta^- J_t = 2\Delta^+ J_t = -2J_z J_t\sum_{q}\fr{1}{\omega^+ - \epsilon_q - \epsilon_d + \hf J_z}\\
\eeq
...
Setting \(J_z = J_t = \hf J\) makes the interaction \(SU(2)\) symmetric; the last two RG equations can then be written in the common form:
\beq
2\Delta J_z = 2 \Delta J_t = \Delta J &= - J^2 \sum_{q}\fr{1}{\omega^+ - \epsilon_q - \epsilon_d + \fr{1}{4}J}
\eeq
...
\beq[and2kondo]
2\Delta J_z = 2 \Delta J_t = \Delta J &= - J^2 \sum_{q}\fr{1}{\tilde \omega^+ - \epsilon_q + \fr{1}{4}J}
\eeq
...
\beq
\Delta J = -J^2 \sum_q \fr{1}{-\epsilon_q}
\eeq
...
\beq
\Delta J = - J^2 \sum_{q}\fr{1}{\tilde \omega^+ - \epsilon_q + \fr{1}{4}J}
\eeq
For \(\tilde \omega^+ = \hf D\), we get
\beq
\Delta J = 2J^2 \sum_{q}\fr{1}{D - \hf J}
\eeq
...
We first consider the simpler case where \(\epsilon_d + \hf U = 0\) and \(J_z = J_t = \hf J\). The scaling equations become
\beq
\Delta U &= \rr{U + \hf J}\sum_q |V(q)|^2 \fr{1}{\rr{\omega^+ - \epsilon_q}\rr{\omega^+ - \epsilon_q + \hf U + \fr{1}{4}J}\rr{\omega^+ - \epsilon_q + U + \fr{1}{2}J}}\\
	 &+ \fr{1}{8} J^2\sum_{q k}\fr{6\rr{\omega^+ - \epsilon_q} + 3U + 4J}{\rr{\omega^+ - \epsilon_q  + \hf U + \fr{1}{4}J}\rr{\omega^+ - \epsilon_q  + \hf U +\fr{3}{4} J}}\\
\Delta V &= -\fr{3}{2}J\sum_{q}V_q^*\fr{1}{\omega^+ - \epsilon_q + \hf U + \fr{1}{4}J}\\
\Delta J &= -J^2 \sum_{q}\fr{1}{\omega^+ - \epsilon_q + \hf U + \fr{1}{4}J}
\eeq
...
\beq
\Delta U &= U\sum_q |V(q)|^2 \fr{1}{\rr{\omega^+ - \epsilon_q}\rr{\omega^+ - \epsilon_q + \hf U}\rr{\omega^+ - \epsilon_q + U}}
\eeq
To get a feel for this equation, we look at a specific value of \(\omega^+ = 2\epsilon_q - \hf U\). For this value,
\beq
\Delta U &= U\sum_q |V(q)|^2 \fr{1}{\rr{\epsilon_q^2 - \fr{1}{4} U^2}\epsilon_q}
\eeq
If we start with bare values \(U\) and \(\epsilon_q\) such that \(U <  2\epsilon_q\), then \(U\) is relevant, and it will increase. Meanwhile, the bandwidth will decrease as we go to lower energies. The fixed point is achieved when the denominator becomes zero.
\beq
U^* = 2 D^*
\eeq
The opposite thing happens if we start with bare values such that  \(U >  2\epsilon_q\). Then, \(U\) is irrelevant and it will decrease until it reaches the same fixed point condition. This fixed point, characterized by \(U^* = 2 \epsilon_q^*, J=0\), is the local moment fixed point. It is stable along both directions in the \(U-\)axis. It is however unstable to perturbations in \(J\), as we will see later.
\pb
There is another fixed point in this picture, the free orbital fixed point at \(U=V=0\). It is unstable in all directions. The two fixed points and the flows are schematically shown in fig.~\ref{lmflow}.
\begin{figure}
\centering
...
\caption{Plot of a particular flow from free orbital (leftmost) to local moment (rightmost)}
\label{fo2lm}
\end{figure}
\subsubsection*{With the Kondo-type interaction (\(\pmb{J>0})\)}
In this case, all three equations will come into play. Using the value \(\omega^+ = 0\), the equations become
\beq
\Delta U &= \sum_q |V(q)|^2 \fr{-\rr{U + \hf J}}{\rr{\epsilon_q}\rr{\epsilon_q - \hf U - \fr{1}{4}J}\rr{\epsilon_q - U - \fr{1}{2}J}} + \fr{1}{8} J^2\sum_{q k}\fr{-6\epsilon_q + 3U + 4J}{\rr{\epsilon_q  - \hf U - \fr{1}{4}J}\rr{\epsilon_q  - \hf U -\fr{3}{4} J}}\\
\Delta V &= \fr{3}{2}J\sum_{q}V_q^*\fr{1}{\epsilon_q - \hf U - \fr{1}{4}J}\\
\Delta J &= J^2 \sum_{q}\fr{1}{\epsilon_q - \hf U - \fr{1}{4}J}
\eeq
We can now see how the couplings will flow if we start at the local moment fixed point and add a small perturbation \(J\).
\beq
\Delta U &= \sum_q |V(q)|^2 \fr{-\rr{2\epsilon_q + \hf J}}{\rr{\epsilon_q}\rr{- \fr{1}{4}J}\rr{-\epsilon_q - \fr{1}{2}J}} + \fr{1}{8} J^2\sum_{q k}\fr{4J}{\rr{- \fr{1}{4}J}\rr{-\fr{3}{4} J}}\\
\Delta V &= \fr{3}{2}J\sum_{q}V_q^*\fr{1}{\epsilon_q - \hf U - \fr{1}{4}J}\\
\Delta J &= J^2 \sum_{q}\fr{1}{\epsilon_q - \hf U - \fr{1}{4}J}
\eeq
\end{document}
