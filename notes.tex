...
\eeq
This form will be useful later when we make the connection with CUT RG.
\comm{
...
\qq{S,\ham_0} = -\ham_X
\eeq
\pb Avoiding the perturbative route, we can take \(S = \frac{\pi}{4}\rr{\eta^\dagger - \eta}\), where \(\eta\) and its conjugate are non-perturbative and Fermionic - they satisfy \(\eta^2 = {\eta^\dagger}^2 = 0\) and \(\cc{\eta,\eta^\dagger}=1\). We can then write
\beq
...
Here we will show how we can obtain the spin-spin interaction of the Kondo model by performing a one-shot URG on the SIAM. This should justify that the action of performing an SWT is akin to a decoupling the whole band via URG. \pb There are three departures from the conventional way of doing URG.
\begin{itemize}
	...
	\item We will finally have to project on to the singly-occupied subspace in order to obtain a purely spin-spin interaction.
\end{itemize}
Since we are decoupling the whole band, the various parts of the Hamiltonian are
\beq
\ham^I = \sum_{k\sigma}\qq{V_k c^\dagger_{k\sigma}c_{d\sigma}+\text{h.c.}}, &&\ham^d = \sum_{k\sigma}\epsilon_k\hat n_{k\sigma} + \epsilon_d\hat n_d + U\hat n_{d\ua}\hat n_{d\da}
\eeq
