...
\begin{document}
\tableofcontents
\pagebreak\section{Preliminaries}
...
\eeq
where \il{N} is the number of sites. Note that the number of excitations, \il{\avg{n_q}} has to be defined differently for the states above and below the Fermi surface. For excited states above \il{\epsilon_F}, the number of excitations is given usually:
\beq
\avg{n_q^>} = \bra{\psi^>}c^\dagger_k c_k\ket{\psi^>} = n^p_k
\eeq
where \il{n^p_k} stands for the number of particles. For states below \il{\epsilon_F}, however, we need to count the number of holes:
\beq
\avg{n_q^<} = \bra{\psi^<}c^\dagger_k c_k\ket{\psi^<} = -\bra{\psi^<}c_k c^\dagger_k \ket{\psi^<} = -n^h_k
\eeq
where \il{n^h_k} stands for the number of holes. We can thus define a generalized excitation:
\beq
...
\eeq
This is analogous to the Landau quasiparticle energy functional, eq.~\ref{temp_en}, \il{U} acting as the interaction between the quasiparticles. \il{\delta n > 0} acts as the excitations from the ground state.
\pb
...
To calculate the specific heat, \il{C_v = \dv{\avg{E}}{T}}, note that a change in temperature would modify the quasiparticle distribution \il{\delta n_{k\sigma}} and hence the quasiparticle energies \il{\epsilon_{k\sigma}}. This leads to a complicated feedback effect. However, at low temperatures, higher order excitations will be very low and we can approximate by considering only the variation in the distribution:
\beq
...
\eeq
At sufficiently low temperatures, the first term will dominate over the others (\il{T^{-2} \gg T^{-1}}). Hence the low temperature specific heat can be written as
\beq
...
\eeq
The function \il{\fr{e^{\beta\epsilon_{k\sigma}}}{\rr{e^{\beta\epsilon_{k\sigma}} + 1}^2}} is very sharply peaked at the Fermi surface \il{\epsilon_\sigma = 0}. Therefore we can replace the density of states by its value at the Fermi surface.
\beq
...
\eeq
\il{f(\epsilon_\sigma)} is the Fermi-Dirac distribution. Note that
\beq
...
\eeq
The remaining integral gives \il{-\fr{\pi^2}{3}}. For \il{T \ra 0}, quasiparticle excitations will be absent and we can write \il{\rho_\ua = \rho_\da = \rho_d}:
\beq
...
Under a magnetic field \il{B}, \il{ \epsilon_{k\sigma} \ra \epsilon_{k\sigma} + \sigma h}. where \il{h=\fr{1}{2}g B \mu_B}. The magnetisation is
\beq
...
\eeq   
On applying the magnetic field, the Fermi energy of spin \il{-\sigma} decreases as \il{\epsilon_F -\sigma h}. Hence, more number of spin \il{-\sigma} electrons will get excited, the number of such excitations being
\beq
...
\eeq
Close to the Fermi surface, the imaginary part of the self energy goes as \il{\omega^2}. Therefore, up to first order in \il{\omega}, the self energy is completely real close to the Fermi surface:
\beq
\Sigma(\omega) &= \Sigma(0,0) + \omega \Sigma^\prime(0) + O(i\omega^2)\\
	       &\equiv \Sigma(0) + \rr{1 - Z^{-1}}\omega
\eeq
where \il{Z = \rr{1 - \Sigma^\prime}^{-1}}. Substituting this in \il{G_d(\omega)} gives
\beq[impg]
...
G &= G^{(0)} + G^{(0)}VG^{(0)}_dVG^{(0)} + G^{(0)}VG^{(0)}_dVG^{(0)}VG^{(0)}_dVG^{(0)} + ...\\
  ...
  & = G^{(0)} + G^{(0)}V^2 G_d G^{(0)}\\
\eeq
Here, \il{G^{(0)}} are the bare Green functions of the conduction and impurity electron and \il{G_d} is the interaction impurity Green's function. Comparing with
\beq[dyson]
G = G_0 + G_0 T G_0
\eeq
we can write
...
T = V^2 G_d 
\eeq
where \il{T} is the \il{T-}matrix for scattering of conduction electrons off the impurity. From the optical theorem, we know that the \il{S-}matrix (\il{S(\omega) \equiv e^{2i\delta(\omega)}}) is related to the \il{T-}matrix as
\beq
...
Exactly at the strong-coupling fixed point, for particle-hole symmetry, we expect the occupancy of the impurity to be \il{\avg{n_d} = 1}, because the singly-occupied state is below the Fermi level while the doubly occupied state is above. If we now lower the Fermi level by \il{\Delta \mu} while keeping the particle-hole symmetry intact (by suitably shifting the impurity levels), the resonance in the spectral function at the Fermi surface will persist, because the electrons at the Fermi surface will always form a singlet with the impurity and go into a bound state.
\pb
Since the energies are measured relative to the Fermi level, all quasiparticle energies will increase by \il{\Delta \epsilon_{k\sigma} = \Delta \mu}. However, some of the quasiparticles closer to the Fermi surface will now come below it,so that the number of quasiparticles will decrease by \il{\Delta n = -\Delta \mu \rho_d(0)}. The net change in \il{n_\ua} is thus
\beq
...
\eeq
At the Kondo limit, the impurity occupation is fixed at 1 because the resonance in the spectral function of the conduction electrons is pinned at the Fermi energy. This means that even if we shift the Fermi energy, the resonance moves with it, and there should be no change \il{\Delta n_\ua}. Hence,
\beq[nosusc]
1 - U\rho_d(0) = 0 \implies U\rho_d(0) = 1
\eeq
Substituting \il{\avg{n_{d\sigma}} = \fr{1}{2}} and \il{\epsilon_d = -\fr{U}{2}} in the density of states eq.~\ref{dosint} gives \il{\rho_d(0) = \fr{1}{\pi \Delta} = \fr{1}{U}}. This can be substituted in the Wilson ratio to give
\beq
R = 1 + \sin^2 \delta(0)
\eeq
\subsubsection{Connection to the change in Luttinger's volume}
From the Friedel sum rule\footnote{JS Langer and V Ambegaokar. Friedel sum rule for a system of interacting electrons}, we can relate the phase shift \il{\delta(0)} due to scattering (at the Fermi surface) off a local impurity to the number of electrons bound in the potential well produced by that impurity:
\beq
...
\eeq
From the work of Seki and Yunoki\footnote{Topological interpretation of the luttinger theorem. Physical Review B, 96(8):085124, 2017}, we know that this quantity is essentially the winding number of the curve \il{\Gamma(D)} in the complex plane spanned by the real and imaginary parts of \il{D}, and is equal to the change in Luttinger's volume \il{V_L} at \il{T=0}.
\beq
\wl N &= -\int_{\Gamma(D)}\fr{dD}{D} = -\Delta V_L
\eeq
The incoming electrons can have \il{\sigma = \ua,\da}. Since the impurity singlet ground state is rotationally invariant, we have \il{\delta_\ua = \delta_\da = \delta(0)}.
\beq
...
\eeq
A different result is obtained if one is in the regime of \il{\epsilon_d < -D}. This is the situation mentioned at the very beginning of the discussion, fig.~\ref{and1}. Assuming \il{U \ra \infty} and \il{\epsilon_d} outside the conduction band, we can do a SWT and the \il{T_K} obtained is q.~\ref{sol},
\begin{gather}
	J = -\fr{V^2}{\epsilon_d}\\
...
For \il{\epsilon_d^* \gg \Delta}, the renormalization will stop at \il{D \sim \epsilon_d}. Note that we had assumed \il{D \gg \epsilon_d}. That was the starting condition, that is, \il{\epsilon_d} deep inside the Fermi surface. During the renormalization, \il{D} will keep on decreasing and \il{\epsilon_d} will continuously increase. At some value of \il{D}, they will become equal and the impurity level will go outside the Fermi surface. At this point, none of the impurity levels can renormalize any more, because the relevant energy scales are greater than the cutoff. Hence the renormalization stops at this point.
\begin{center} \includegraphics[scale=0.29]{full.png}\captionof{figure}{Renormalization in the energy levels when \il{\epsilon_d^* \gg \Delta}}\end{center}
This point is given by \il{\ol D = a\epsilon_d(\ol D) \equiv \ol \epsilon_d} where \il{a} is a constant of order unity. It satisfies the equation
\beq[eraser]
\ol \epsilon_d = \epsilon_d^* - \fr{\Delta}{\pi}\ln \fr{a\ol \epsilon_d}{\Delta}
\eeq
which is just eq.~\ref{const} with the substitution \il{D =a  \ol \epsilon_d}. In this regime, because \il{\epsilon_d \gg \Delta}, we can do a perturbative expansion of the  bare Hamiltonian in terms of \il{\fr{\Delta}{\epsilon_d}}. The susceptibility is
\beq
\chi_d = \fr{\Delta}{2\pi}\rr{\fr{g \mu_B}{\epsilon_d}}^2\qq{1 + \fr{2\Delta}{\pi \epsilon_d}\ln \fr{\epsilon_d}{D} + ...}
\eeq
From the scaling, we know that \il{D} can be decreased to \il{\ol D}. We can hence substitute \il{D =a \ol \epsilon_d, \epsilon_d = \ol \epsilon_d}. With this in mind, the susceptibility becomes
\beq
...
\eeq
where I used the fact that \il{\ln a } will be of order 1. As we go on decreasing the cutoff, the impurity level will go on moving farther away from the Fermi level, and impurity site will become null occupied: \il{\avg{n_d} \approx 0}. The critical cutoff \il{\ol D} can be associated with a temperature  scale \il{k_b \ol T = \ol D}. At temperatures sufficiently below this temperature (\il{T \ll \ol T}), the susceptibility becomes (again from perturbation theory)
\beq
\chi_d(T) = \fr{\Delta}{2\pi}\rr{\fr{g \mu_B}{\ol \epsilon_d}}^2 + \fr{1}{4T}\qq{1+\fr{1}{2}\ex{\fr{T^*}{T}}}^{-1}
\eeq
...
\beq
T \gg T_{FL} \implies e^\fr{T^*}{T} \gg 1 \implies \qq{1+\fr{1}{2}\ex{\fr{T^*}{T}}}^{-1} \approx 0 
\eeq
The susceptibility in this low temperature range can thus be written as
\beq[sakura]
\chi_d = \fr{\Delta}{2\pi}\rr{\fr{g \mu_B}{\ol \epsilon_d}}^2
\eeq
This is analogous to the  result obtained in eq.~\ref{mimp}, from the mean field version of the Fermi liquid theory, and also obtained from a renormalized perturbation theory of Anderson model. To see how, note that since we are in the limit \il{\avg{n_d} = 0}, the onsite repulsion term \il{U} can be dropped because there is no probability of double occupation. Eq.~\ref{mimp} then becomes
\beq[sasuke]
\chi_d = \fr{g^2 \mu_B^2}{2} \rho_d(0) = \fr{g^2 \mu_B^2}{2} \fr{\Delta}{\pi}\fr{1}{\ol \epsilon_d^2 + \Delta^2}
\eeq
Next note that we had assumed at the beginning that \il{\epsilon_d^* \gg \Delta}. We need to find the relative order difference between \il{\ol \epsilon_d} and \il{\Delta}. From eq.~\ref{eraser}, we can drop the \il{\pi} and \il{a} because they are of order 1.
\beq
...
\eeq
which is the same as eq.~\ref{sakura}. This tells us that scaling all the way down to very low temperatures in regime \il{\epsilon_d^* \gg \Delta} brings us into a Fermi liquid state, characterized by a temperature-independent susceptibility (as is standard in a Fermi liquid). The crossovers can be seen by looking at the variation of the Curie constant \il{\chi T}.\\\\
Since the susceptibility is proportional to the magnetic moment, presence of degeneracy will reduce this moment because the probability of occupying the states will decrease. As a result, the Curie constant is also a measure of the effective degeneracy of the impurity orbital. At very high temperatures \il{T \gg U,\epsilon_d}, all the impurity levels \il{0,\epsilon_d} and \il{2\epsilon_d + U} will become degenerate on energy scales of the order of \il{k_B T}. As a result, the Curie constant is approximately \il{\fr{1}{8}} in this range. The impurity occupancy is \il{n_d = 1}, because there are 4 degenerate states and the average number of electrons on them is 1. At lower temperatures \il{U \gg T \gg T^*}, the degeneracy gets lowered; now, only the vacant and single-occupied states are degenerate. Here the Curie constant is \il{\fr{1}{6}}. In this case, the average occupancy is \il{n_d = \fr{0+1+1}{3} = \fr{2}{3}}. At still lower temperatures, we saw that the impurity becomes vacant and \il{n_d = 0}. The Curie constant becomes linear in temperature, going down to 0. More formally,
\beq[suscep]
m = \fr{1}{\beta}\pd{\ln Z}{B} \implies \chi = \lim_{B \to 0}\pd{m}{B} = \lim_{B \to 0}\fr{1}{Z^2\beta}\qq{Z\fr{\partial^2 Z}{\partial B^2} - \rr{\pd{Z}{B}}^2}
\eeq
For the case of four-fold degeneracy, all the states can be assumed to be at zero energy. Then, under a magnetic field B (\il{h=\fr{g\mu_B}{2}B}), the partition function is
\begin{gather}
...
Similarly, for the triplet case (\il{\epsilon_d} and 0 are degenerate while \il{U \gg T}), the doubly occupied case is essentially cut off from the available states, so \il{Z = 1 + 2\cosh \beta h}. The proof again goes through similarly. But this time, we have \il{Z(h=0) = 1 + 2 = 3 = \mc{D}}.\\\\
For \il{\epsilon_d = k_B T^* > k_B T} such that \il{k_B T^* \gg \Delta}, we can find the magnetic moment in a perturbative fashion. At the zeroth order, we can neglect the hybridisation \il{\Delta}. Then,
\beq
...
\eeq
Since \il{\Delta \ll T^*}, we can expand the arctan in a Taylor series. Up to first order, we get
\beq
...
\end{gather}
We can now visualize the various phases as the temperature is changed. For \il{T \gg U,\epsilon_d}, all the four states states \il{\ket{0},\ket{\ua},\ket{\da},\ket{2}} are degenerate (\il{\mc D = 4}), the average occupancy is \il{\avg{n_d} = \fr{0+1+1+2}{4}=1} and the effective Curie constant is \il{\fr{1}{2\mc D} = \fr{1}{8}}. At lower temperatures \il{U \gg T \gg T^*}, the level \il{\ket{2}} is disconnected from the conduction band and the three remaining states are now degenerate (\il{\mc D = 3}). The average occupancy becomes \il{\fr{0+1+1}{3} = \fr{2}{3}} and the effective Curie constant is now \il{\fr{1}{2\times 3} = \fr{1}{6}}. At still lower temperatures \il{T^* \gg T}, the singly-occupied levels become disconnected and thee impurity occupancy becomes 0. The effective Curie constant in this regime is linear in \il{T}.
\begin{center}
\begin{minipage}{50pt}
...
\begin{center} \includegraphics[scale=0.29]{mixed.png}\captionof{figure}{Renormalization in energy levels when \il{|\epsilon_d^*| \lesssim \Delta}} \end{center}
Since we have \il{|\epsilon_d^*| \lesssim \Delta}, as we renormalize, the decreasing cutoff will first match \il{\Delta} or \il{k_B T}, whichever is greater. From eq.~\ref{mirai2}, we know that if \il{D} comes close to \il{\Delta}, our analysis will break down because we can no longer ignore that term. Since that term represents the broadening of the impurity level, this same broadening can also be brought about by the thermal fluctuations which are of the scale \il{k_B T}. This means that real valence fluctuations will now renormalize the potential \il{V_{kd}}. Hence, our analysis will stop at \il{D = \text{max}\cc{\Delta, k_B T}}. For the simpler situation in which \il{T = 0}, the renormalization will stop at \il{D = \Delta}. From eq.~\ref{const}, putting \il{D = \Delta}, we get
\beq
...
The crossovers in the mixed valence regime are as follows. Similar to the previous case, at high and intermediate temperatures, we have \il{n_d = 1} and \il{n_d = \fr{2}{3}} respectively. However, while the triplet degeneracy lasted upto \il{T \sim T^*} in the previous case, here it continues up to \il{T \sim \Delta} because that is where the scaling breaks down. That is, \il{T = \Delta} is the point where we can no longer ignore the renormalization in \il{V} and it begins to increase with scaling. Beyond this point, the impurity occupation remains fractional and not much else can be said.
\begin{center}
\begin{minipage}{50pt}
...
\end{minipage}
\end{center}
For \il{\epsilon_d^* \ll -\Delta}, the scaling will stop when the impurity level again goes out of the Fermi surface. But this time, it goes out from below. This again decouples the singly-occupied state from the conduction band and the scaling stops. This happens at say \il{\wl D = -\wl {\epsilon_d} = \wl {T}}. Since the singly-occupied impurity level is now well below \il{-D}, we have \il{\avg{n_d} = 1} and we are comfortably in the Kondo limit and the SWT and a consequent poor man's scaling can be performed, which will give eqs.~\ref{historia} through \ref{jean2}. The resulf of the Schrieffer-Wolff transformation is a Hamiltonian that couples the impurity to the conduction electrons only through their spins; their is no charge fluctuation. At high temperatures \il{T \gg T_K}, the impurity is essentially decoupled and we get a susceptibility of the form eq.~\ref{suscdeg}, but with a degeneracy of 2. To go to lower temperatures, we can do a Poor Man's scaling which suggests that the Hamiltonian at \il{T\ll T_K} is one with a large coupling between the impurity and the conduction electrons.
...
\end{center}
\begin{center}
	\includegraphics[scale=0.29]{empty.png}\captionof{figure}{Renormalization in energy levels when \il{\epsilon_d^* \ll -\Delta}}
\end{center}
...
\eeq
Before scaling, \il{V^+ = V^- = V}. The Schrodinger equation we want to solve is
\beq
H \psi = E \psi
\eeq
We know the eigenstates \il{\psi_0} of \il{H_0}. They are the states \il{\{\ket{n_{k_i\sigma},n_{d\sigma^\prime}}\}}. These states of course span the entire Hilbert space. A subset of these states form the model subspace. We call these states \il{\phi}. For our case, that is the subspace with all conduction electrons inside \il{D - \delta D}. The projection operator for this subspace is 
\beq
...
P \{\psi\}_d = \{\phi\}
\eeq
This is of course true in the non-interacting limit. There, the \il{\psi_0} are the exact eigenstates, and the action of \il{P} is basically
\beq
P \psi_0\bigg\vert_{|k|<D-\delta D} = \psi_0\bigg\vert_{|k|<D-\delta D}
\eeq
Now, as we turn on the interactions adiabatically, it is safe to assume that these \il{d} non-interacting eigenstates flow into \il{d} interacting eigenstates. This means that we can define an inverse for the \il{P} operator which takes a non-interacting eigenstate from the model subspace into the interacting eigenstate:
\beq
\Omega \{\phi\} = \{\psi\}
\eeq
...
\end{gather}
In the first equation, I used \il{P \phi = \phi} because the projection of \il{\phi} into the model subspace is \il{\phi} itself. Together these two identities give
\beq[hikki]
\Omega P = \Omega
\eeq
...
\eeq
Since we want to write down an equation only in the model subspace, the equation should operate only on the \il{\phi}. To remove the \il{\Omega} on the right side, operate on this equation with \il{P} from the left. This gives
\beq
...
\eeq
Now, since \il{P} is made up of the eigenstates of \il{H_0}, those two will commute: \il{\qq{H_0, P} = 0}. The equation then becomes
\beq
...
\eeq
This is the main equation. To make progress, we expand the operator \il{\Omega} in powers of the interaction \il{V}:
\beq
...
\end{gather}
Let \il{\ket{l}} and \il{\ket{h}} belong to the model subspace and its orthogonal subspace respectively. Then, taking matrix element between \il{\bra{h}} and \il{\ket{l}} of the general form equation gives
\beq
...
\end{gather}
We can now expand the effective Hamiltonian in powers of \il{V}. From eq.~\ref{yuigahama}, the interacting part of the effective Hamiltonian becomes
\beq
...
\eeq
where I used \il{P P_l = \sum_{l^\prime} \ket{l^\prime}\bra{l^\prime} \ket{l}\bra{l} = \sum_{l^\prime}\ket{l^\prime}\bra{l^\prime}\delta_{ll^\prime} = P_l}. The third term becomes
\beq
...
\eeq
In the high energy subspaces, \il{n_q^+ = 1-n_q^- = 0}. Therefore,
\beq
PV \sum_l G_l V P_l  &= {\sum_q}^+\fr{|V_q|^2 n_{d\sigma}}{\epsilon_d - \epsilon_q} + {\sum_q}^-\fr{|V_q|^2\rr{1-n_{d\sigma}}}{\epsilon_q - \epsilon_d} \\
		     ...
		     &= n_d \delta \epsilon_d
\eeq
The third term is zero in our case. The part \il{G_l V G_l V} will do the following. 
\beq
...
\begin{itemize}
	...
	\item Since we truncated at third order, we need \il{D - |\epsilon_d| \gg v^\pm}. This gives us \il{D \gg |\epsilon_d|}.
\end{itemize}
...
n_k \wl \ham(1-n_k) =0
\eeq
To find the form for \il{U_k}, we define a few things. First,
\beq
...
\eeq
\il{P_k} is hence those degrees of freedom in which the given Hamiltonian \il{\ham} is diagonal. Hence it is natural that they are obtained by rotating the \il{n_k}. If we were to go to the eigenbasis of \ham, the form for \il{\wl \ham} will be 
\beq
...
\ol \ham  = P_k \ham P_k
\eeq
\il{\ol \ham} is a matrix that, in the eigenbasis of \ham, has only the upper diagonal block of \il{\ham}.  That is, you first form a matrix by keeping only the upper diagonal block of \il{\wl \ham} and then rotating to the basis of \il{n_k}. Finally, let \il{\psi} be a wavefunction such that
\beq
P\psi = \psi
\eeq
...
\ham \psi = \ol \ham \psi
\eeq
This is the equation we will use to find \il{U_k}. But first we will write \il{\psi} in the following fashion:
\beq
\ket{\psi} = a_1 \ket{1,\psi_1} + a_0\ket{0,\psi_0}
\eeq
The first entry signifies whether the degree of freedom \il{k} is occupied or not, and the second entry is the wavefunction of the remaining degrees of freedom. This is just a resolution of the total wavefunction in the two-dimensional Hilbert space of \il{k}.\\\\
We will also write the Hamiltonian \il{\ham} as
\beq[ham]
\ham = \ham^D + \ham^I + \ham^i 
\eeq
\il{\ham^D} is the diagonal part of the Hamiltonian, something of the form \il{\epsilon_k n_k}. \il{\ham^I} is the interaction between the current degree of freedom \il{k} and the remaining degrees of freedom. It will consist of terms like \il{c^\dagger_q c_k} or \il{c^\dagger_k c_q}. The third term \il{\ham^i} has interactions between the remaining degrees of freedom. This term will also be diagonal in \il{n_k} because it doesn't involve scattering of \il{k} states. It will involve terms like \il{c^\dagger_{q_1}c_{q_2}}. \\\\
\comm{
The Hamiltonian \il{\ol\ham} can also be decomposed in a similar form. Note however that since it is already diagonal in \il{n_k}, it won't have any scattering in \il{k}, and hence won't have a term like \il{\ham^I}.
\beq
...
\eeq
To get expressions from this, note that on the left hand side, \il{\ol \ham} does not scatter \il{k}, so it will not change left entry in the kets; it can only change the right entries. Similarly, on the right hand side, \il{\ham^D} and \il{\ham^i} will not change the occupation of the \il{k} degree of freedom. \il{\ham^I} however \it{will} change it. Matching the states with \il{\ket{0}} gives
\beq
...
\end{gather}
We now need to find the unitary operation \il{U_k} that disentangles the state \il{\ket{1,\psi_1}} from the state \il{\ket{psi}}. Using eq.~\ref{eta},
\beq
...
\eeq
and \il{\mc{C}} is a real scalar. To find \il{\mc{O}}, note that we can write \il{U_k^{-1}} as 
\beq
U_k^{-1} = \mc{C} \ex{\eta_k + \mc{O}}
\eeq
This is because \il{\eta_k^2 = 0} and since \il{\mc{O}} is made up of \il{\eta_k} and \il{\eta_k^\dagger} is has the same property. This exponential form allows us to write
\beq[u1]
...
\beq[roth]
 \wl\ham &= U_k \ham U_k^\dagger\\
	 &= \fr{1}{2}\rr{1+\eta_k^\dagger - \eta_k}\ham\rr{1 + \eta_k - \eta^\dagger_k}\\
				&= \fr{1}{2}\rr{1+\eta_k^\dagger - \eta_k}\rr{\ham + \ham\eta - \ham\eta_k^\dagger}\\
				&=\fr{1}{2}\rr{\ham+ \ham\eta - \ham\eta_k^\dagger + \eta_k^\dagger \ham + \eta_k^\dagger\ham\eta_k - \eta_k^\dagger \ham\eta_k^\dagger - \eta_k\ham - \eta_k \ham \eta_k + \eta_k \ham \eta_k^\dagger}\\
&=\fr{1}{2}\rr{\ham^D + \ham^i + \ham^I + \ham\eta - \ham\eta_k^\dagger + \eta_k^\dagger \ham + \eta_k^\dagger\ham\eta_k - \eta_k^\dagger \ham\eta_k^\dagger - \eta_k\ham - \eta_k \ham \eta_k + \eta_k \ham \eta_k^\dagger}\\
\eeq
In the last step I split \il{\ham} using eq.~\ref{ham}. At this stage, note that terms that have two \il{\eta_k} or two \il{\eta_k^\dagger} can only be nonzero if the intervening \il{\ham} has a creation or destruction operator. If we resolve the Hamiltonian in the basis of \il{k} in the following form:
\beq[hisoka]
...
\beq
 \hat \omega &= \ol \ham - \ham^i \\
	     &= P_k \ham P_k - \qq{H_e \hat n + H_h(1-\hat n) - \text{Tr }\qq{\ham \hat n_1 \hat n_2 ... \hat n_N}\hat n_1 \hat n_2 ... \hat n_N}
\eeq
...
\beq
\wl \ham = \fr{1}{2}\rr{\ham^D + \ham^i + \ham\eta - \ham\eta_k^\dagger + \eta_k^\dagger \ham + \eta_k^\dagger\ham\eta_k - \eta_k\ham  + \eta_k \ham \eta_k^\dagger}
\eeq
...
\beq
 \wl \ham &= \fr{1}{2}\rr{\ham^D + \ham^i + \ham\eta - \ham\eta_k^\dagger + \eta_k^\dagger \ham + \eta_k^\dagger\ham\eta_k - \eta_k\ham  + \eta_k \ham \eta_k^\dagger}\\
	  &=\fr{1}{2}\rr{\ham^D + \ham^i} + \qq{c_k^\dagger T,\eta_k} + \fr{1}{2}\qq{H_e \hat n_k + H_h(1-\hat n_k)}
\eeq
...
\beq
 \text{Tr}\qq{\ham\hat n_k}\hat n_k + \text{Tr}\qq{\ham(1- \hat n_k)}(1-\hat n_k) &= \text{Tr}\qq{\ham\hat n_k}\hat n_k + \text{Tr}\qq{\ham(\hat n_k -1)}(\hat n_k -1)\\
										  ...
										  &=\text{Tr}\qq{\ham\rr{\hat n_k - \fr{1}{2}}}2\hat n_k -\text{Tr}\qq{\ham(\hat n_k -\fr{1}{2})} + \fr{1}{2}\text{Tr}\qq{\ham} \\
										  &= \text{Tr}\qq{\ham\rr{\hat n_k - \fr{1}{2}}}\rr{2\hat n_k - 1} + \fr{1}{2}\text{Tr}\qq{\ham}\\
										  &=\text{Tr}\qq{\ham\tau_k}2\tau_k + \fr{1}{2}\text{Tr}\qq{\ham}\\
\eeq
...
\eeq
I used \il{\hat n_k c^\dagger_k = c^\dagger_k} and \il{\hat n_k \eta_k = 0}. Then,
\beq
...
\bbx
\beq
\wl \ham = U_k \ham U_k^\dagger = \fr{1}{2}\text{Tr}\qq{\ham}+  2\tau_k\text{Tr}\qq{\ham\tau_k} + 2\tau_k \cc{c_k^\dagger \text{Tr}\rr{\ham c_k},\eta_k}
\eeq
\ebx
...
\beq
 \qq{\wl \ham, \hat n_k} &= \qq{\qq{c_k^\dagger T,\eta_k},\hat n_k}\\
			 ...
			 &=c^\dagger_k T \eta_k \hat n_k - \hat n_kc^\dagger_k T \eta_k &&\qq{\text{2\uu{nd} [ . ] is 0, }\because c_k^\dagger \hat n_k = \hat n_k \eta_k=0}\\
...
\beq
\ham_{j,1} = \fr{1}{2}\text{Tr}_1\qq{\ham_j}+  \tau_1\text{Tr}_1\qq{\ham_j\tau_1} + \tau_1 \cc{c_1^\dagger \text{Tr}_1\rr{\ham_j c_1},\eta_1}
\eeq
The second term is of order \il{\tau^2}, so we drop it because they will vanish in thermodynamic limit:
\beq
\ham_{j,1} = \fr{1}{2}\text{Tr}_1\qq{\ham_j} + \tau_1 \cc{c_1^\dagger \text{Tr}_1\rr{\ham_j c_1},\eta_1}\eeq
We will keep dropping terms of second order from now on. Also, we will assume at each  The second run gives
\beq
 \ham_{j,2} &= \fr{1}{2}\text{Tr}_2\qq{\ham_{j,1}} + \tau_2 \cc{c_2^\dagger \text{Tr}_2\rr{\ham_{j,1} c_2},\eta_2}\\
 \fr{1}{2}\text{Tr}_2\qq{\ham_{j,1}} &= \fr{1}{2}\text{Tr}_2\qq{\fr{1}{2}\text{Tr}_1\qq{\ham_j} + \tau_1 \cc{c_1^\dagger \text{Tr}_1\rr{\ham_j c_1},\eta_1}}\\
				     &=\fr{1}{2^2}\text{Tr}_{1,2}\qq{\ham_j} +  \fr{1}{2}\tau_1\text{Tr}_{2}\cc{c_1^\dagger \text{Tr}_1\rr{\ham_j c_1},\eta_1}\\
\text{Tr}_2\qq{\ham_{j,1}c_2} &= \text{Tr}_2\qq{\fr{1}{2}\text{Tr}_1\qq{\ham_j} c_2+ \tau_1 \cc{c_1^\dagger \text{Tr}_1\rr{\ham_j c_1},\eta_1}c_2}\\
			      &=\fr{1}{2}\text{Tr}_2\qq{\rr{\text{Tr}_{1}\ham_j}c_2} +  \tau_1\text{Tr}_2\qq{ \cc{c_1^\dagger \text{Tr}_1\rr{\ham_j c_1},\eta_1}c_2}\\
\implies \tau_2 \cc{c_2^\dagger \text{Tr}_2\rr{\ham_{j,1} c_2},\eta_2} &= \fr{1}{2}\tau_2 \cc{c_2^\dagger \text{Tr}_2\qq{\rr{\text{Tr}_{1}\ham_j}c_2},\eta_2} 
\eeq
Therefore,
\beq
 \ham_{j,2} &=\fr{1}{2^2}\text{Tr}_{1,2}\qq{\ham_j} +  \fr{1}{2}\tau_1\text{Tr}_{2}\cc{c_1^\dagger \text{Tr}_1\rr{\ham_j c_1},\eta_1} + \fr{1}{2}\tau_2 \cc{c_2^\dagger \text{Tr}_2\qq{\rr{\text{Tr}_{1}\ham_j}c_2},\eta_2}
\eeq
As part of keeping only the first order quantum fluctuations, we consider only one electron on the shell at a time. That is, in the second term, \il{\ham_j} will have only 1 electron on the j\uu{th} shell and that electron will have the index 1. Similarly, in the third term, the only electron on the shell will have index 2. That means that the Tr\dd{2} and Tr\dd{1} are redundant in second and third terms respectively. These redundant traces will cancel the half factors in front, giving us
\beq
...
\eeq
Here, \il{l} and the indices \il{1,2,...N} refer to fermionic degrees of freedom on the shell \il{\Lambda_j}. That is, they are a placeholder for \il{(\mathbf q, \beta)} where \il{|\mathbf q| = \Lambda_j} and \il{\beta \in \{\ua,\da\}}. More explicitly, the index 1 designates (\il{\mathbf k_{1,\ua}}),  the index 2 means (\il{\mathbf k_{1,\da}}),  the index 3 means (\il{\mathbf k_{2,\ua}}) and so on.\\\\
Let \il{j = j_\text{max}}. That is, this is the first decoupling. Then, \il{\ham_j = \ham}. The first term in eq.~\ref{dec} is
\beq
\fr{1}{2^{N_j}}\text{Tr}_{1,2,...N_j}\ham
\eeq
As discussed before, in order to consider just the leading order quantum fluctuations, we will make the following simplification: While evaluating  the first trace, say Tr\dd{1}, we will assume the only electron on the j\uu{th} shell has that index. Therefore,
\beq
\fr{1}{2} \text{Tr}_{k_1\sigma}\ham &= \fr{1}{2}\text{Tr}_{k_1\sigma}\qq{\sum_{k<\Lambda_j\atop{\alpha}}\epsilon_{k}\hat n_{k\alpha} + \fr{J}{2}\sum_{k,k^\prime<\Lambda_j\atop{\alpha,\alpha^\prime}} \mathbf{S}\cdot \mathbf{\sigma}_{\alpha\alpha^\prime} c^\dagger_{k\alpha}c_{k^\prime\alpha^\prime} + \epsilon_{k_1} \hat n_{k_1\sigma} + \fr{J}{2}S^z \sigma^z_{\sigma} \hat n_{k_1\sigma}}\\
...
\eeq
The \il{H_{j-1}} part will not change. To take the next partial trace \il{k_1\ol \sigma}, we add an electron in this state to the previous Hamiltonian:
\beq
\fr{1}{2} \text{Tr}_{k_1\ol \sigma}\qq{\fr{1}{2} \text{Tr}_{k_1\sigma}\ham} &=\fr{1}{2} \text{Tr}_{k_1\ol \sigma}\qq{H_{j-1} + \fr{\epsilon_{k_1}}{2} + \fr{J}{2}S^z \fr{\sigma}{2} + \epsilon_{k_1} \hat n_{k_1\ol\sigma} + \fr{J}{2}S^z \sigma^z_{\ol\sigma} \hat n_{k_1\ol\sigma}}\\
									    &= H_{j-1} + \fr{\epsilon_{k_1}}{2} + \fr{J}{2}S^z \fr{\sigma}{2}  + \fr{1}{2} \text{Tr}_{k_1\ol \sigma}\qq{\epsilon_{k_1} \hat n_{k_1\ol\sigma} + \fr{J}{2}S^z \sigma^z_{\ol\sigma} \hat n_{k_1\ol\sigma}}\\
...
\eeq
\comm{
	\il{n_l} refers to the number of fermions in the state \il{\ket{N}}. If I take a single inner product first,
\beq
...
\eeq
The spin-spin interaction term eq.~\ref{ss} will lose the terms with \il{\sigma_{\beta\beta}} because \il{\beta} will be summed over and \il{\sum_\beta \mathbf{S}\cdot\mathbf{\sigma}_{\beta\beta} = \sum_\beta S^z\sigma^z_{\beta\beta} = 0}. Therefore, only the first term survives
\beq
...
\beq[fir]
 2^{-N}\text{Tr}_{1,2,...N}\ham_j &= \sum_{|k|<\Lambda_j,\alpha}\epsilon_k \hat n_{k\alpha} + \sum_{|q|=\Lambda_j}\epsilon_{q} + \fr{J}{2}\sum_{|k|,|k^\prime|<\Lambda_j\atop{\alpha,\alpha^\prime}} \mathbf{S}\cdot \mathbf{\sigma}_{\alpha\alpha^\prime}c^\dagger_{k\alpha}c_{k^\prime\alpha^\prime}\\
				  &={\sum_{k,k^\prime\atop{\alpha,\alpha^\prime}}}^<\rr{\epsilon_k \hat n_{k\alpha} + \fr{J}{2}\mathbf{S}\cdot \mathbf{\sigma}_{\alpha\alpha^\prime}c^\dagger_{k\alpha}c_{k^\prime\alpha^\prime}} + \fr{1}{2}{\sum_{q,\alpha}}^=\epsilon_q
\eeq
...
Now I need to compute the second term. That term involves another trace, \il{\text{Tr}_l\qq{\ham c_l}}. The kinetic energy term will not survive this. The only terms that will survive are those that have exactly one \il{c_{q\beta}^\dagger}. 
\beq
...
\eeq
Define \il{\mc{X}_{q\beta} = \sum_{k<\Lambda^*,\alpha} \mathbf{S}\cdot \mathbf{\sigma}_{\alpha\beta}c^\dagger_{k\alpha} c_{q\beta}}. The second term in the effective Hamiltonian \il{\ham_{j-1}} can then be written as
\beq
...
Since we will replace \il{\hat \omega} with some function of \il{\tau_l}, the fraction with \il{\omega} will commute with all the operators. Therefore,
...
\eeq
where \il{m\equiv k\alpha, m^\prime \equiv k^\prime\alpha^\prime} and \il{\mc{C}_{lmm^\prime}} is the anticommutator \il{\left\{\mathbf{S}\cdot \mathbf{\sigma}_{\beta\alpha}c^\dagger_{q\beta} c_{k\alpha},\mathbf{S}\cdot \mathbf{\sigma}_{\alpha^\prime\beta}c^\dagger_{k^\prime\alpha^\prime} c_{q\beta}\right\}}. Denote the prefactor in eq.~\ref{kura} as \il{\Gamma_{q\beta}}. The sum \il{\sum_{m\neq l\neq m^\prime}} is over all points \il{m,m^\prime} except the point \il{m=m^\prime=l}. Split the sum in the following way: 
\beq
...
\eeq
We drop the first term because it does not have  \il{S^z} and hence does not renormalize the Ising term. Joining with the \il{\Gamma_{q\beta}} prefactor, we get
\beq
...
\eeq
We replace \il{\hat{\ol \omega}} with \il{2\omega \tau_{q\beta}}. This gives
\beq
...
\beq
\sum_{\beta}\fr{\tau_{q\beta}}{\rr{2\omega\tau_{q\beta} - \xi_{q\beta}\tau_{q\beta}}} = \fr{1}{2}\qq{\fr{1}{\omega - \fr{\epsilon_q}{2} - \fr{1}{2}\fr{J}{2}S^z \sigma^z_{\ua\ua}} - \fr{1}{-\omega + \fr{\epsilon_q}{2} + \fr{1}{2}\fr{J}{2}S^z \sigma^z_{\da\da}}} = \fr{\omega - \fr{\epsilon_q}{2}}{\rr{\omega - \fr{\epsilon_q}{2}}^2 - \fr{J^2}{64}}
\eeq
In total, we get
\beq[aluka]
...
The RG starts from \il{j=N} and runs down to \il{j^*<N}. As a result, integrating out the \il{j^\text{th}} Hamiltonian gives the \il{(j-1)^\text{th}} Hamiltonian. \il{\Lambda_N} is the highest momentum being integrated out and is thus the Brillouin zone edge. From eq.~\ref{rgeqt}, we can write
\beq[diffeq]
...
\beq
-\fr{\Delta J_j}{\Delta\Lambda_j} = \fr{n_j J_j^2 \rr{\omega - \fr{\hbar v_F}{2}\Lambda_j}}{\rr{\omega - \fr{\hbar v_F}{2}\Lambda_j}^2 - \fr{1}{16}{J_j}^2}
\eeq
...
\eeq
where \il{\tilde \Lambda = \fr{\hbar v_F}{2}\Lambda}. Define a dimensionless coupling 
\beq
...
\eeq
where \il{n(0)} is the DOS on FS. There are two fixed points, \il{K^*=0} and \il{K^*=4}. At both these points, \il{\beta(K)} changes sign. At \il{K=0}, it changes between \il{0^+} and \il{0^-}, while at \il{K=4}, it switches between \il{\pm \infty}.
\pb
...
\implies \Lambda^* &=  \Lambda_N \exp\qq{-\fr{1}{n(0)}\rr{\fr{1}{K_N} + \fr{K_N}{16} - \fr{1}{2}}}\\
\implies T_K &=  \fr{E(\Lambda^*)}{k_B} =\fr{\hbar v_F \Lambda^*}{k_B} = \fr{\hbar v_F \Lambda_N}{k_B} \exp\qq{-\fr{1}{n(0)}\rr{\fr{1}{K_N} + \fr{K_N}{16} - \fr{1}{2}}}\\
\implies \xi_K &= \fr{2\pi}{\Lambda^*} = \fr{2\pi}{\Lambda_N} \exp\qq{\fr{1}{n(0)}\rr{\fr{1}{K_N} + \fr{K_N}{16} - \fr{1}{2}}}\\
\eeq
...
\eeq
\il{\ham^*_0} is the kinetic energy of the Kondo cloud (electrons lying inside \il{\Lambda^*}). The zero-mode Hamiltonian is
\beq
...
\eeq
on the singlet state (\il{S_\text{tot} = 0}), \il{\mb{S}_\text{tot}} being \il{\mb{S} + \mb{s}}. The lowest value of the energy occurs when \il{\mb{q}} is placed on the shell \il{\Lambda^*}, because \il{J^q} falls rapidly with distance from the Kondo cloud. Therefore,
\beq
E_g = -\fr{7J^*}{4}
\eeq
...
\eeq
\il{\ket{\ua_k}} represents \il{\sum_{k<\Lambda^*}\ket{\ua_k}}. We can label the states of the combined Hilbert space of Kondo cloud and impurity as \il{1,2,3} and 4, which represent \il{\ket{\ua\ua},\ket{\ua\da},\ket{\da\ua},\ket{\da\da}} respectively. The ground state, in this notation, is \il{\ket{\Psi^*} = \fr{1}{\sqrt 2}\rr{\ket{2}\ket{\da} - \ket{3}\ket{\ua}}}. First we compute the density matrix for this state:
\beq
\rho_\text{all} &= \ket{\Psi^*}\bra{\Psi^*}\\
		&= \fr{1}{2}\qq{\ket{2}\bra{2}\otimes\ket{\da}\bra{\da} - \ket{2}\bra{3}\otimes\ket{\da}\bra{\ua} - \ket{3}\bra{2}\otimes\ket{\ua}\bra{\da} + \ket{3}\bra{3}\otimes\ket{\ua}\bra{\ua}}
\eeq
...
\rho_\text{LFL} &= \text{Tr}_\text{imp+cloud}\qq{\rho_\text{all}}\\
		...
		&= \fr{1}{2}\rr{\ket{\ua}\bra{\ua} + \ket{\da}\bra{\da}}
\eeq
This is just \fr{1}{2} times identity matrix in two dimensions, so the eigenvalues are both \fr{1}{2}. The von-Neumann entropy for a matrix \il{\rho} is defined as \il{S =- \sum_\lambda \lambda \ln \lambda}, where the sum is over the eigenvalues of the matrix. 
\beq
S_\text{LFL} = -2\times\fr{1}{2}\ln \fr{1}{2} = \ln 2
\eeq
Since von-Neumann entropy is independent of which partial system I trace over, it will be same if I had instead traced over the imp+cloud system. Still I will calculate the reduced density matrix because I will need it to trace over the cloud or the impurity.
\beq
\rho_\text{imp+cloud} &= \text{Tr}_\text{LFL}\qq{\rho_\text{all}}\\
		      &= \sum_{m=\pm \ua,\da} \bra{m}\rho_\text{all}\ket{m}\\
		      &= \fr{1}{2}\rr{\ket{2}\bra{2} +\ket{3}\bra{3}}\\
		      &= \fr{1}{2}\begin{pmatrix} 0 &&& \\ &1&&\\&&1&\\&&&0 \end{pmatrix}
\eeq
As mentioned just before, the non-zero eigenvalues are again both \il{\fr{1}{2}} so the von-Neumann entropy is the same as \il{S_\text{LFL}}:
\beq
S_\text{imp+cloud} = -2\times\fr{1}{2}\ln \fr{1}{2} = \ln 2
\eeq
...
\rho_\text{imp} &= \text{Tr}_\text{cloud}\qq{\rho_\text{imp+cloud}}\\
		&= \text{Tr}_\text{cloud}\qq{\fr{1}{2}\rr{\ket{\ua}\bra{\ua}\otimes\ket{\da}\bra{\da} + \ket{\da}\bra{\da}\otimes\ket{\ua}\bra{\ua}}}\\
		&= \fr{1}{2}\rr{\ket{\ua}\bra{\ua} +\ket{\da}\bra{\da}}\\
		&= \fr{1}{2}\begin{pmatrix} 1 &\\&1\end{pmatrix}\\
S_\text{imp} &= \ln 2
\eeq
which also means
\beq
S_\text{cloud} &= \ln 2
\eeq
We thus have the conclusion that all partial systems (imp, cloud,LFL, imp+cloud, cloud+LFL, imp+LFL) have an entropy of \il{\ln 2} and are thus maximally entangled with their partners. We can also compute the mutual information between the LFL and imp+cloud systems. It is defined as \il{I(A,B) = S_A + S_B - S_{AB}}.
\beq
...
\eeq
where \il{\ham_0} is the kinetic energy of the disentangled electrons and \il{h = \fr{1}{2}\sum_{k>\Lambda^*} J_k s^z_k}.
\beq
...
\eeq
where \il{A = \sqrt{h^2 + \rr{\fr{J^*}{2}}^2}}. We can now take the trace by exponentiating this matrix:
\beq
...
\eeq
In the limit of low temperature (\il{\beta \to \infty}), we have \il{e^{-\beta\fr{J^*}{4}} \to 0} and \il{\cosh A\beta \to e^{A\beta}}. We can then write
\beq
...
\eeq
LFL stands for local Fermi liquid; it is the Hamiltonian closest to the cloud.  We now consider quasiparticle excitations \il{\delta n_{k\sigma}}. The ground state energy is
\beq
...
\eeq
We thus see that as a result of the scattering, the momentum eigenvalues change by \il{\Delta k = -\fr{1}{R}\delta_{k\sigma}}. This will change \il{E_k} by
\beq
...
\eeq
Close to the Fermi surface, \il{\rr{\sin k_x + \sin k_y} \approx k_{Fx} + k_{Fy}}. Therfore,
\beq
...
\eeq
where \il{B = J^*\sum_k s^z_k} and \il{h} is an external field.  \il{B^\prime = B+h} is the total field on the impurity spin. We can write the expression for susceptibility in terms of this total field:
\beq
...
\eeq
\beq[zsq]
	\rr{\pd{Z_\text{imp}}{B^\prime}}^2 &= \rr{\fr{\beta}{2}}^2\left\{\sinh^2 \fr{\beta B^\prime}{2} + \fr{{B^\prime}^2}{{B^\prime}^2 + J^2} \sinh^2\rr{\fr{\beta}{2}\sqrt{{{B^\prime}^2 + J^2}}}\right. \\&\quad+ \left.2\sinh \fr{\beta B^\prime}{2}\fr{B^\prime}{\sqrt{{B^\prime}^2 + J^2}} \sinh\rr{\fr{\beta}{2}\sqrt{{{B^\prime}^2 + J^2}}}\right\}
\eeq
\beq
...
\end{gather}
In the absence of LFL magnetization, we can set \il{B=0}. Then eq.~\ref{zsq} vanishes and what remains is
\begin{gather}
...
\end{gather}
The Wilson ratio is defined as \il{W \equiv 4T_K \chi(T=0)}. It turns out to be
\beq
...
\beq
	\fr{1}{J^*}\rr{E_0 - \ham^*_0}\ket{\Psi} &= \rr{S^z s^z + \fr{1}{2}S^+s^- + \fr{1}{2}S^- s^+}\rr{a_\da \ket{\ua_\text{imp}}\ket{\Downarrow} + a_\ua\ket{\da_\text{imp}}\ket{\Uparrow}}\\
...
\beq[effeig]
	\fr{1}{J^*}\rr{E_0-\ham^*_0}\ket{\Uparrow} &= -\fr{s^z}{2}\ket{\Uparrow} + \fr{1}{2}s^+ \qq{\fr{1}{J^*}\rr{E_0-\ham^*_0}-\fr{s^z}{2}}^{-1} s^-\ket{\Uparrow}\\
...
\eeq
Now we can simplify the effective Hamiltonian for the Kondo cloud. First put \il{E_0=0} and \il{s^z=\fr{1}{2}}. The latter is obvious from the fact that this is the effective Hamiltonian for the up-spin state. The justification for the former will be given later. Then,
\beq
...
\eeq
We can calculate the commutator by looking at the expression of \il{s^-} from eq.~\ref{sminus}: \il{s^- = \sum_{k_1,k_2}c^\dagger_{k_1\da}c_{k_2\ua}}. Using the identities
\beq
...
\pb
We can also evaluate the effective Hamiltonian for the orthogonal state \il{\ket{\Downarrow}}. First we solve the coupled set of eqs.~\ref{coupled} to eliminate \il{\ket{\Uparrow}}:
\beq
...
\eeq
Again substituting \il{E=0} and \il{s^z = -\fr{1}{2}} gives
\beq
H_\text{eff} &= -\fr{J^*}{4} - \rr{\fr{J^*}{2}}^2s^- \fr{1}{\ham^*_0 + \fr{J^*}{4}}s^+\\
	     ...
	     &= -\fr{J^*}{4} - J^*\rr{\fr{1}{2}-s^z}  + 4s^-\ham^*_0s^+ - \fr{16}{J^*}s^- {\ham_0^*}^2 s^+ \\
...
We want to look at the excitations of the Kondo model in the ground state of the cloud+impurity composite system. If we assume that a single localized impurity cannot destabilize the Fermi surface of the Kondo cloud electrons, the total energy of composite system must be greater than zero. Otherwise, the total system will come below the Fermi surface to form a bound system and the Fermi surface will get destabilized. Consequently, the minimum energy of the composite system has to be \il{E_0 = 0}.
\pb
The requirement that the total energy of the composite system be zero constrains the kinetic energy of the Kondo cloud. This is understood by noting that from eq.~\ref{fixham}, the Hamiltonian of Kondo+impurity part is \il{\ham_0^* + \mb{S}\cdot\mb{s}}. This should give an eigenvalue of 0 in the ground state. The second term in the Hamiltonian acting on the singlet state gives \il{-\fr{3J^*}{2}}. Therefore, we get
\begin{gather}
\ham_0^* -\fr{3J^*}{2} = E_0 = 0 \\
\implies \ham_0^s = \fr{3J^*}{2}
\end{gather}
This is the minimum value of the kinetic energy for the Kondo cloud Hamiltonian; anything lower than this would lead to a total energy \il{E_0} less than 0 and the destabilization of the Fermi surface. This tells us that just as one electron comes below the Fermi surface to form the singlet with the impurity spin, other electrons have to excite suitably above the Fermi surface (that is, become more energetic) in order to constrain the ground state energy at 0. Excitations of the cloud then involve higher values of \il{\ham_0^* > \fr{3J^*}{2}} and lead to excitations of the composite system as a whole (\il{E_0 > 0}). The spectrum of \il{\ham^*_0} can then be thought to be gapped; the kinetic energy must start from a non-zero value. It is not possible to have a kinetic energy infinitesimally close to the Fermi surface.

\pagebreak\section{Anderson Model URG}
\beq
\ham = \sum_{k\sigma}\epsilon_k \hat n_{k\sigma} + \sum_{k\sigma} \rr{V_{k} c^\dagger_{k\sigma} c_{d\sigma} + h.c.} + \sum_\sigma \epsilon_{d\sigma} \hat n_{d\sigma} +  U \hat n_{d\ua} \hat n_{d\da}
\eeq
...
\eeq
\il{n_j} is the number of states on the shell \il{\Lambda_j}. We take the full Hamiltonian as our \il{\ham_j}. Since this is the first step of the RG, the shell being decoupled is the highest one, which we call \il{\Lambda_N}. 
\paragraph{Calculation of first term}
The first term, the initial trace, is a sequential trace over all the states on the shell being disentangled. At each trace, we consider only electrons on the current degree of freedom and on shells below the current shell:
\beq
\fr{1}{2}\text{Tr}_{q\ua} \ham_j &= \sum_{k<\Lambda_N,\sigma}\epsilon_k \hat n_{k\sigma} + \sum_\sigma \epsilon_{d\sigma} \hat n_{d\sigma} +  U \hat n_{d\ua} \hat n_{d\da} + \fr{1}{2}\text{Tr}_{q\ua}\cc{\epsilon_k \hat n_{q\ua}}\\
&= \sum_{k<\Lambda_N,\sigma}\epsilon_k \hat n_{k\sigma} + \sum_\sigma \epsilon_{d\sigma} \hat n_{d\sigma} +  U \hat n_{d\ua} \hat n_{d\da} + \fr{1}{2}\epsilon_q\\
\fr{1}{2}\text{Tr}_{q\da} \fr{1}{2}\text{Tr}_{q\ua}\ham_j &= \sum_{k<\Lambda_N,\sigma}\epsilon_k \hat n_{k\sigma} + \sum_\sigma \epsilon_{d\sigma} \hat n_{d\sigma} +  U \hat n_{d\ua} \hat n_{d\da} + \epsilon_q\\
\eeq
\beq[term1]
\implies 2^{-n_j}\text{Tr}_{1,2,...,n_j}\ham_j &= \sum_{k<\Lambda_N,\sigma}\epsilon_k \hat n_{k\sigma} + \sum_\sigma \epsilon_{d\sigma} \hat n_{d\sigma} +  U \hat n_{d\ua} \hat n_{d\da} + \sum_{|q|=\Lambda_N}\epsilon_q\\
\eeq
...
\eeq
\beq
\ham^D &= \sum_{k\sigma}\epsilon_k \hat n_{k\sigma} + \sum_\sigma \epsilon_{d\sigma} \hat n_{d\sigma} +  U \hat n_{d\ua} \hat n_{d\da}\\
\text{Tr}_{q\beta}\rr{\ham^D \hat n_{q\beta}} &= \sum_{k<\Lambda_N,\sigma}\epsilon_k \hat n_{k\sigma} + \sum_\sigma \epsilon_{d\sigma} \hat n_{d\sigma} +  U \hat n_{d\ua} \hat n_{d\da} + \epsilon_q\\
\eta_{q\beta} &= \text{Tr}_{q\beta}\rr{c^\dagger_{q\beta}\ham}c_{q\beta}\fr{1}{\hat \omega - \text{Tr}_{q\beta}\rr{\ham^D \hat n_{q\beta}}\hat n_{q\beta}}\\
	      &= V^*_q c^\dagger_{d\beta}c_{q\beta}\fr{1}{\hat \omega - \rr{\sum_{k<\Lambda_N,\sigma}\epsilon_k \hat n_{k\sigma} + \sum_\sigma \epsilon_{d\sigma} \hat n_{d\sigma} +  U \hat n_{d\ua} \hat n_{d\da}}\hat n_{q\beta} - \epsilon_q\hat n_{q\beta}}\\
	      &= V^*_q c^\dagger_{d\beta}c_{q\beta}\fr{1}{\omega\hat n_{q\beta} - \rr{\sum_\sigma \epsilon_{d\sigma}\hat n_{d\sigma} + U \hat n_{d\ua}\hat n_{d\da} + \epsilon_q}\hat n_{q\beta}}\\
\eeq
At the last step, \textbf{\textcolor{red}{I replaced \il{\pmb{\hat \omega - \sum_{k<\Lambda_N,\sigma}\epsilon_k \hat n_{k\sigma}\hat n_{q\beta} }} with \il{\pmb{\omega\hat n_{q\beta}}}}}. Note that since this term has a \il{c^\dagger_{d\beta}}, it will not vanish only when acting on a state with \il{\hat n_{d\beta} = 0}. Hence we can drop the terms \il{\hat n_{d\ua}\hat n_{d\da}} and \il{\epsilon_{d\beta}\hat n_{d\beta}} in the denominator. Also, since it has a \il{c_{q\beta}}, we can set the \il{\hat n_{q\beta}} in the denominator to 1. Putting together the individual pieces, we can now write the second term:
\beq
\sum_{q\beta}\tau_{q\beta}\cc{c^\dagger_{q\beta}\text{Tr}_{q\beta}\rr{\ham c_{q\beta}},\eta_{q\beta}} &= \sum_{q\beta}\tau_{q\beta}\cc{V_q c^\dagger_{q\beta}c_{d\beta}, V^*_q c^\dagger_{d\beta}c_{q\beta}\fr{1}{\omega - \epsilon_q - \epsilon_{d\ol\beta}\hat n_{d\ol\beta}}}
\eeq
We now note that the factor with \il{\omega} can be written as follows:
\beq
\fr{1}{\omega - \epsilon_q - \epsilon_{d\ol\beta}\hat n_{d\ol\beta}} = \fr{\hat n_{d\ol\beta}}{\omega - \epsilon_q - \epsilon_{d\ol\beta}} + \fr{1 -\hat n_{d\ol\beta}}{\omega - \epsilon_q }
\eeq
...
\begin{gather}
	\tau_{q\beta}\rr{1-\hat n_{q\beta}} = \fr{1}{2}\rr{\hat n_{q\beta} - 1}\\
	\tau_{q\beta}\hat n_{q\beta} = \fr{1}{2}\hat n_{q\beta}
\end{gather}
The total thing becomes
\beq
\sum_{q\beta}\fr{1}{2}|V_q|^2\qq{\hat n_{d\beta}\rr{\hat n_{q\beta} - 1}+ \hat n_{q\beta}\rr{1-\hat n_{d\beta}}}\qq{\fr{\hat n_{d\ol\beta}}{\omega - \epsilon_q - \epsilon_{d\ol\beta}} + \fr{1 -\hat n_{d\ol\beta}}{\omega - \epsilon_q }}\\
=\sum_{q\beta}\fr{1}{2}|V_q|^2\qq{\hat n_{q\beta}-\hat n_{d\beta}}\qq{\fr{\hat n_{d\ol\beta}}{\omega - \epsilon_q - \epsilon_{d\ol\beta}} + \fr{1 -\hat n_{d\ol\beta}}{\omega - \epsilon_q }}\\ 
\eeq
\textbf{\textcolor{red}{Putting \il{\hat n_{q\beta} = 1}}}, and dropping the non-operator terms, we get
\beq[term2]
\sum_{\beta}\fr{1}{2}\hat n_{d\beta}\sum_q |V_q|^2\fr{\epsilon_q - \omega + 2\epsilon_{d\beta}}{\rr{\omega - \epsilon_q}\rr{\omega - \epsilon_q - \epsilon_{d\beta}}} -  \hat n_{d\ua}\hat n_{d\da}\sum_{q\beta}\fr{1}{2}|V_q|^2\fr{\epsilon_{d\beta}}{\rr{\omega - \epsilon_q}\rr{\omega -\epsilon_q - \epsilon_{d\beta}}}
\eeq
...
\beq[impren]
\Delta \epsilon_{d\beta}^{N} \equiv \epsilon_{d\beta}\big\vert_{N-1} - \epsilon_{d\beta}\big\vert_{N} = \fr{1}{2}\sum_q |V_q|^2\fr{\epsilon_q - \omega + 2\epsilon_{d\beta}}{\rr{\omega - \epsilon_q}\rr{\omega - \epsilon_q - \epsilon_{d\beta}}} 
\eeq
Since the equation is same for \il{\beta = \ua,\da}, we can drop the \il{\beta} index. According  to Hewson eq.~3.62 (page 68),
\beq[hewson]
...
\beq
\delta\epsilon_{d}(D) = \fr{1}{2}|V|^2\rho(D)|\delta D| \fr{D - \omega + 2\epsilon_{d}}{\rr{\omega - D}\rr{\omega - D - \epsilon_{d}}} 
\eeq
where \il{\rho(D)} is the number of single-spin states on the shell D. This can be compared to eq.~\ref{hewson}. In two dimensions, the energy density of states is independent of energy. \textbf{\textcolor{red}{Setting \il{\pmb{\omega = 0}}}}, we get 
\beq
\delta\epsilon_{d}(D) &=\fr{1}{2}|V|^2\rho(D)\fr{|\delta D|}{D} \fr{D + 2\epsilon_{d}}{D + \epsilon_{d}}\\
		      &= -\fr{1}{2}|V|^2\rho(D)\delta \rr{\ln D }\fr{D + 2\epsilon_{d}}{D + \epsilon_{d}}
\eeq
I used \il{\delta D = -|\delta D|}. Changing to continuum equation,
\beq
\dv{\epsilon_d}{\ln D} &= -\fr{\Delta}{2\pi}\fr{D + 2\epsilon_{d}}{D + \epsilon_{d}}
\eeq
In the regime where the single-occupied impurity level is comfortably inside the conduction band (\il{D \gg |\epsilon_d|}), we can approximate both the numerator and denominator as simply \il{D}. Then,
\begin{gather}
\dv{\epsilon_d}{\ln D} =- \fr{\Delta}{2\pi} \\
\implies \epsilon_d + \fr{\Delta}{2\pi}\log D = \text{constant}
\end{gather}
\textcolor{red}{This differs from eq.~3.65 in Hewson by a factor \il{2\pi} vs \il{\pi}.}
\pb
...
\beq
\dv{\epsilon_d}{\ln D} = \fr{1}{2}|V|^2n(D) \fr{\omega - D - 2\epsilon_{d}}{\rr{\omega - D}\rr{\omega - D - \epsilon_{d}}} 
\eeq
\il{n(D)} is not the density of states, but the total number of states on the shell at energy \il{D}. Similarly, the renormalization in \il{U} is
\beq
\delta U &= -\sum_{q\beta}\fr{1}{2}|V_q|^2\fr{\epsilon_{d\beta}}{\rr{\omega - \epsilon_q}\rr{\omega -\epsilon_q - \epsilon_{d\beta}}}\\
		       &= -\fr{1}{2}|V|^2n(D) \sum_\beta\fr{\epsilon_{d\beta}}{\rr{\omega - D}\rr{\omega -D - \epsilon_{d\beta}}}\\
		       &= -|V|^2n(D) \fr{\epsilon_{d}}{\rr{\omega - D}\rr{\omega -D - \epsilon_{d}}}\\
\implies \dv{U}{\ln D} &= |V|^2n(D) \fr{\epsilon_{d}}{\rr{\omega - D}\rr{\omega -D - \epsilon_{d}}}
\eeq
In the last step, I used the fact that since the onsite energy for either spin is same, the summation just returns a factor of 2.

\subsection*{Alternate}
We will write the Hamiltonian in terms of \il{\tau = \hat n -\fr{1}{2}}:
\beq
\ham &= \sum_{k\sigma}\epsilon_k \hat n_{k\sigma} + \sum_{k\sigma} \rr{V_{k} c^\dagger_{k\sigma} c_{d\sigma} + h.c.} + \sum_\sigma \epsilon_{d\sigma} \hat n_{d\sigma} +  U \hat n_{d\ua} \hat n_{d\da}\\
     &= \sum_{k\sigma}\epsilon_k \rr{\tau_{k\sigma} + \fr{1}{2}} + \sum_{k\sigma} \rr{V_{k} c^\dagger_{k\sigma} c_{d\sigma} + h.c.} + \sum_\sigma \epsilon_{d\sigma}\rr{ \tau_{d\sigma} + \fr{1}{2}} \\
     &\quad\quad+  U \rr{\tau_{d\ua} + \fr{1}{2}} \rr{\tau_{d\da} + \fr{1}{2}}\\
     &= \fr{1}{2} \sum_{k\sigma}\epsilon_k + \fr{1}{2}\sum_\sigma \epsilon_{d\sigma} + \fr{1}{4}U + \sum_{k\sigma}\epsilon_k \tau_{k\sigma} + \sum_{k\sigma} \rr{V_{k} c^\dagger_{k\sigma} c_{d\sigma} + h.c.} + \sum_\sigma \epsilon_{d\sigma}\tau_{d\sigma} \\
     &\quad\quad+  U \tau_{d\ua} \tau_{d\da}+ \fr{1}{2}U\sum_\sigma \tau_{d\sigma}\\
     &= \xi_0 + \sum_{k\sigma}\epsilon_k \tau_{k\sigma} + \sum_{k\sigma} \rr{V_{k} c^\dagger_{k\sigma} c_{d\sigma} + h.c.} + \sum_\sigma \rr{\epsilon_{d\sigma} + \fr{U}{2}}\tau_{d\sigma} +  U \tau_{d\ua} \tau_{d\da}\\
     &= \xi_0 + \sum_{k\sigma}\epsilon_k \tau_{k\sigma} + \sum_{k\sigma} \rr{V_{k} c^\dagger_{k\sigma} c_{d\sigma} + h.c.} + \sum_\sigma \xi_{d\sigma}\tau_{d\sigma} +  U \tau_{d\ua} \tau_{d\da}\\
\eeq
\il{\xi_0} is the constant background energy \il{\fr{1}{2} \sum_{k\sigma}\epsilon_k + \fr{1}{2}\sum_\sigma \epsilon_{d\sigma} + \fr{1}{4}U} which we will drop henceforth. \il{\xi_{d\sigma} =\epsilon_{d\sigma} + \fr{U}{2}} is a modified impurity-site energy. \il{U} remains unchanged in the process. The renormalized Hamiltonian (up to first order in \il{\tau}) after disentangling the electron at \il{q\beta} is
\beq
\ham_{q\beta} = \fr{1}{2}\text{Tr}_{q\beta}\ham_N + \tau_{q\beta} \cc{c^\dagger_{q\beta} \text{Tr}_{q\beta}\rr{\ham_N c_{q\beta}},\eta_{q\beta}}
\eeq
The calculations follow. As a first approximation, we will consider just one electron on the outermost shell \il{\Lambda_N}.
\beq
\ham_N = \sum^<_{k\sigma}\rr{\epsilon_k \tau_{k\sigma} + V_{k} c^\dagger_{k\sigma} c_{d\sigma} + h.c.} + \sum_\sigma \xi_{d\sigma}\tau_{d\sigma} +  U \tau_{d\ua} \tau_{d\da} + \epsilon_q \tau_{q\beta} + V_{q} c^\dagger_{q\beta} c_{d\beta} + h.c.
\eeq
The sum \il{\sum^<} runs over all shells below \il{\Lambda_N}. The first term gives
\beq
\fr{1}{2}\text{Tr}_{q\beta}\ham_N = \fr{1}{2}\text{Tr}_{q\beta}\qq{\sum_{k\sigma}^<\rr{\epsilon_k \tau_{k\sigma} + V_{k} c^\dagger_{k\sigma} c_{d\sigma} + h.c.} + \sum_\sigma \xi_{d\sigma}\tau_{d\sigma} +  U \tau_{d\ua} \tau_{d\da}} \\
+ \fr{1}{2}\text{Tr}_{q\beta}\rr{\epsilon_q\tau_{q\beta} + V_{q} c^\dagger_{q\beta} c_{d\beta} + h.c.}
\eeq
The first trace, involving terms not having \il{q\beta}, simply give a factor of 2 which is canceled by the half in front. Tracing over \il{\epsilon_q \tau_{q\beta}} gives zero because the eigenvalues of \il{\tau_{q\beta}} are \il{\pm \fr{1}{2}}. Therefore,
\beq
\fr{1}{2}\text{Tr}_{q\beta}\ham_N = \sum_{k\sigma}^<\rr{\epsilon_k \tau_{k\sigma} + V_{k} c^\dagger_{k\sigma} c_{d\sigma} + h.c.} + \sum_\sigma \xi_{d\sigma}\tau_{d\sigma} +  U \tau_{d\ua} \tau_{d\da}
\eeq
To calculate the second term, we note
\begin{gather}
c^\dagger_{q\beta}\text{Tr}_{q\beta}\rr{\ham_N c_{q\beta}} = V_q c^\dagger_{q\beta}c_{d\beta}\\
\text{Tr}_{q\beta}\rr{ c_{q\beta}^\dagger \ham_N} c_{q\beta}= V^*_q c^\dagger_{d\beta}c_{q\beta}\\
\text{Tr}_{q\beta}\rr{\ham^D_N \hat n_{q\beta}}\hat n_{q\beta} = \qq{\sum^<_{k\sigma}\rr{\epsilon_k \tau_{k\sigma} + V_{k} c^\dagger_{k\sigma} c_{d\sigma} + h.c.} + \sum_\sigma \xi_{d\sigma}\tau_{d\sigma} +  U \tau_{d\ua} \tau_{d\da} + \fr{1}{2}\epsilon_q}\hat n_{q\beta}
\end{gather}
\beq
\eta_{q\beta} &= \text{Tr}_{q\beta}\rr{ c_{q\beta}^\dagger \ham_N} c_{q\beta}\fr{1}{\hat \omega - \text{Tr}_{q\beta}\rr{\ham^D_N \hat n_{q\beta}}\hat n_{q\beta}}\\
	      &= V^*_q c^\dagger_{d\beta}c_{q\beta}\fr{1}{\hat \omega - \rr{\sum_\sigma \xi_{d\sigma}\tau_{d\sigma} +  U \tau_{d\ua} \tau_{d\da} + \fr{1}{2}\epsilon_q}\hat n_{q\beta}}
\eeq
\textbf{\textcolor{red}{where I absorbed the part below the shell into the \il{\hat \omega}. We can also absorb a \\\il{\fr{1}{2}\times \rr{\sum_\sigma \xi_{d\sigma}\tau_{d\sigma} +  U \tau_{d\ua} \tau_{d\da} + \fr{1}{2}\epsilon_q}\hat n_{q\beta}} into \il{\hat \omega} to convert the \il{\hat n_{q\beta}} into \il{\tau_{q\beta}}. Then we replace \il{\hat \omega} with \il{\omega \tau_{q\beta}}}}:
\beq
\eta_{q\beta} &= V^*_q c^\dagger_{d\beta}c_{q\beta}\fr{1}{\omega \tau_{q\beta} - \rr{\sum_\sigma \xi_{d\sigma}\tau_{d\sigma} +  U \tau_{d\ua} \tau_{d\da} + \fr{1}{2}\epsilon_q}\tau_{q\beta}}
\eeq
Since this term has a \il{c_{q\beta}}, it will survive only when acting on \il{\ket{\tau_{q\beta} = \fr{1}{2}}}, so we can replace the \il{\tau_{q\beta}} in the denominator with \il{\fr{1}{2}}. Also, since there is a \il{c^\dagger_{d\beta}}, we can similarly replace \il{\tau_{d\beta} = -\fr{1}{2}}.

\beq
\eta_{q\beta} &= V^*_q c^\dagger_{d\beta}c_{q\beta}\fr{2}{\omega  + \fr{1}{2}\xi_{d\beta} -  \xi_{d\ol\beta}\tau_{d\ol\beta} + \fr{1}{2}U \tau_{d\ol\beta} - \fr{1}{2}\epsilon_q}\\
	      &= V^*_q c^\dagger_{d\beta}c_{q\beta}\qq{\fr{ \fr{1}{2}+\tau_{d\ol\beta} }{\omega  + \fr{1}{2}\xi_{d\beta} -  \fr{1}{2}\xi_{d\ol\beta} + \fr{1}{4}U - \fr{1}{2}\epsilon_q} + \fr{ \fr{1}{2}- \tau_{d\ol\beta}}{\omega  + \fr{1}{2}\xi_{d\beta} + \fr{1}{2}\xi_{d\ol\beta} - \fr{1}{4}U - \fr{1}{2}\epsilon_q}}\\
	      &= V^*_q c^\dagger_{d\beta}c_{q\beta}\fr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q - \rr{ \fr{1}{2}U - \xi_{d\ol\beta}}\tau_{q\ol\beta}}{\rr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d\ol\beta}}^2}
\eeq
Then,
\beq
\tau_{q\beta} \cc{c^\dagger_{q\beta} \text{Tr}_{q\beta}\rr{\ham_N c_{q\beta}},\eta_{q\beta}} &= \tau_{q\beta}|V_q|^2\fr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q - \rr{ \fr{1}{2}U - \xi_{d\ol\beta}}\tau_{q\ol\beta}}{\rr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d\ol\beta}}^2}\cc{c^\dagger_{q\beta}c_{d\beta},c^\dagger_{d\beta}c_{q\beta}}\\
											     &= \tau_{q\beta}|V_q|^2\fr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q - \rr{ \fr{1}{2}U - \xi_{d\ol\beta}}\tau_{q\ol\beta}}{\rr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d\ol\beta}}^2}\rr{\hat n_{q\beta} - \hat n_{d\beta}}\\
											     &= \tau_{q\beta}|V_q|^2\fr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q - \rr{ \fr{1}{2}U - \xi_{d\ol\beta}}\tau_{q\ol\beta}}{\rr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d\ol\beta}}^2}\rr{\tau_{q\beta} - \tau_{d\beta}}\\
											     &= |V_q|^2\fr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q - \rr{ \fr{1}{2}U - \xi_{d\ol\beta}}\tau_{q\ol\beta}}{\rr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d\ol\beta}}^2}\rr{\fr{1}{4} - \tau_{q\beta}\tau_{d\beta}}\\
\eeq
\textbf{\textcolor{red}{Putting \il{\tau_{q\beta} = \fr{1}{2}}}},
\beq
\tau_{q\beta} \cc{c^\dagger_{q\beta} \text{Tr}_{q\beta}\rr{\ham_N c_{q\beta}},\eta_{q\beta}} &=\fr{1}{2} |V_q|^2\fr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q - \rr{ \fr{1}{2}U - \xi_{d\ol\beta}}\tau_{q\ol\beta}}{\rr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d\ol\beta}}^2}\rr{\fr{1}{2} - \tau_{d\beta}}\\
&=\fr{1}{4} |V_q|^2\fr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q }{\rr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d\ol\beta}}^2} \\
&\quad-\fr{1}{4} |V_q|^2\fr{\fr{1}{2}U - \xi_{d\ol\beta}}{\rr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d\ol\beta}}^2}\tau_{q\ol\beta}\\
&\quad-\fr{1}{2} |V_q|^2\fr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q }{\rr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d\ol\beta}}^2} \tau_{q\beta}\\
&\quad+\fr{1}{2} |V_q|^2\fr{\fr{1}{2}U - \xi_{d\ol\beta}}{\rr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d\ol\beta}}^2} \tau_{q\beta}\tau_{q\ol\beta}\\
&= \Delta \xi_q + \Delta \xi_{d\ol\beta} \tau_{q\ol\beta}+ \Delta \xi_{d\beta} \tau_{q\beta}+ \Delta U\tau_{q\beta}\tau_{q\ol\beta}
\eeq
The first term is a renormalization in the kinetic energy of the disentangled electrons. The rest of the terms are self-explanatory.
\pb
These are the changes after disentangling just one electron, \il{q\beta}: \begin{gather}
\xi_{d\beta} \ra \xi_{d\beta}^\prime = \xi_{d\beta} + \Delta \xi_{d\beta}\\
\xi_{d\ol\beta} \ra \xi_{d\ol\beta}^\prime = \xi_{d\ol\beta} + \Delta \xi_{d\ol\beta}
\end{gather}
If we now disentangle the electron \il{q\ol\beta}, the changes will get exchanged:
\beq
\xi_{d\beta}^\prime \ra \xi_{d\beta}^{\prime\prime} = \xi_{d\beta}^\prime + \Delta \xi_{d\ol\beta} = \xi_{d\beta} + \Delta \xi_{d\beta}+ \Delta \xi_{d\ol\beta} \equiv \Delta \xi_{dq}\\
\xi_{d\ol\beta}^\prime \ra \xi_{d\ol\beta}^{\prime\prime} = \xi_{d\ol\beta}^\prime + \Delta \xi_{d\beta} = \xi_{d\beta} + \Delta \xi_{d\ol\beta}+ \Delta \xi_{d\beta}\equiv \Delta \xi_{dq}
\eeq
where
\beq
\Delta \xi_{dq} &= \fr{1}{2} |V_q|^2\qq{\fr{\fr{1}{2}U - \xi_{d\ol\beta}}{\rr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d\ol\beta}}^2} -\fr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q }{\rr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d\ol\beta}}^2}}\\
		&=\fr{1}{2} |V_q|^2\fr{\fr{1}{2}U - \xi_{d\ol\beta} - \omega  - \fr{1}{2}\xi_{d\beta} + \fr{1}{2}\epsilon_q }{\rr{\omega  + \fr{1}{2}\xi_{d\beta}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d\ol\beta}}^2}\\
\eeq
Since, initially, \il{\xi_{d\beta} = \xi_{d\ol\beta} \equiv \xi_d}, this simplifies to
\beq
\Delta \xi_{dq} &= \fr{1}{2} |V_q|^2\fr{\fr{1}{2}U - \fr{3}{2}\xi_{d} - \omega  + \fr{1}{2}\epsilon_q }{\rr{\omega  + \fr{1}{2}\xi_{d}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d}}^2}\\
\eeq
Similarly, after two disentanglings, \il{U} changes by
\beq
\Delta U = |V_q|^2\fr{\fr{1}{2}U - \xi_{d}}{\rr{\omega  + \fr{1}{2}\xi_{d}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d}}^2}
\eeq
Disentangling the entire shell (all \il{q}) gives
\beq
\Delta \xi_{d} &= \fr{1}{2} \sum_q |V_q|^2\fr{\fr{1}{2}U - \fr{3}{2}\xi_{d} - \omega  + \fr{1}{2}\epsilon_q }{\rr{\omega  + \fr{1}{2}\xi_{d}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d}}^2}\\
\Delta U &= \sum_q |V_q|^2\fr{\fr{1}{2}U - \xi_{d}}{\rr{\omega  + \fr{1}{2}\xi_{d}- \fr{1}{2}\epsilon_q}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d}}^2}
\eeq
Assuming spherically-symmetric scattering, we can take out \il{|V_q|^2 = |V|^2}. For a spherical shell, we can set \il{\epsilon_q = D}, and the summation will produce a factor \il{n(D)}, the number of states on the shell.
\beq
\Delta \xi_{d} &= \fr{1}{2} |V|^2 n(D) \fr{\fr{1}{2}U - \fr{3}{2}\xi_{d} - \omega  + \fr{1}{2}D }{\rr{\omega  + \fr{1}{2}\xi_{d}- \fr{1}{2}D}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d}}^2}\\
\Delta U &= |V|^2 n(D) \fr{\fr{1}{2}U - \xi_{d}}{\rr{\omega  + \fr{1}{2}\xi_{d}- \fr{1}{2}D}^2 - \rr{ \fr{1}{4}U - \fr{1}{2}\xi_{d}}^2}
\eeq

\end{document}

\subsection*{All electrons on shell}
The total expression for the renormalized Hamiltonian after disentangling \il{q\beta} is
\beq
\ham_{q\beta} = \text{Tr}_{q\beta}\rr{\ham \hat n_{q\beta}}\hat n_{q\beta} + \text{Tr}_{q\beta}\qq{\ham \rr{1 - \hat n_{q\beta}}}\rr{1 - \hat n_{q\beta} } + \tau_{q\beta}\cc{c^\dagger_{q\beta}\text{Tr}_{q\beta}\rr{\ham c_{q\beta}},\eta_{q\beta}}
\eeq
The Hamiltonian I am working with can be written in a partitioned fashion:
\beq
\ham &= \sum_{k<\Lambda_N\atop{\sigma}} \rr{\epsilon_k \hat n_{k\sigma} + V_k c^\dagger_{k\sigma}c_{d\sigma} + h.c.} + \sum_{\sigma} \epsilon_{d\sigma}\hat n_{d\sigma} + U\hat n_{d\ua}\hat n_{d\da} +\\
     &\quad\sum_{q=\Lambda_N\atop{\beta}} \rr{\epsilon_q \hat n_{q\beta} + V_q c^\dagger_{q\beta}c_{d\beta} + h.c.}\\
     &= \ham_0 + \sum_{q=\Lambda_N\atop{\beta}} \rr{\epsilon_q \hat n_{q\beta} + V_q c^\dagger_{q\beta}c_{d\beta} + h.c.}
\eeq
where \il{\ham_0} is the part that has no electron on the shell \il{\Lambda_N} and hence will be unchanged by the trace. The terms will now be evaluated individually:
\beq
\text{Tr}_{q\beta}\rr{\ham \hat n_{q\beta}} &= \ham_0 +   \sum_{k\sigma\neq q\beta} \rr{\epsilon_k \hat n_{k\sigma} + V_k c^\dagger_{k\sigma}c_{d\sigma} + h.c.} + \epsilon_q\\
\text{Tr}_{q\beta}\qq{\ham \rr{1 - \hat n_{q\beta}}} &= \ham_0 +   \sum_{k\sigma\neq q\beta} \rr{\epsilon_k \hat n_{k\sigma} + V_k c^\dagger_{k\sigma}c_{d\sigma} + h.c.}
\eeq
Therefore,
\beq
\text{Tr}_{q\beta}\rr{\ham \hat n_{q\beta}}\hat n_{q\beta} + \text{Tr}_{q\beta}\qq{\ham \rr{1 - \hat n_{q\beta}}}\rr{1 - \hat n_{q\beta}} = \ham_0 +   \sum_{k\sigma\neq q\beta} \rr{\epsilon_k \hat n_{k\sigma} + V_k c^\dagger_{k\sigma}c_{d\sigma} + h.c.} \\
+ \epsilon_q\hat n_{q\beta}\quad
\eeq
\begin{gather}
\text{Tr}_{q\beta}\rr{\ham c_{q\beta}} = V_q c_{d\beta}
\end{gather}
\beq
\text{Tr}_{q\beta}\rr{\ham^D \hat n_{q\beta}} &= \sum_{k\sigma \neq q\beta} \epsilon_k \hat n_{k\sigma} + \sum_{\sigma} \epsilon_{d\sigma}\hat n_{d\sigma} + U\hat n_{d\ua}\hat n_{d\da} + \epsilon_q\\
					      &= \sum_{k\sigma} \epsilon_k \hat n_{k\sigma} + \sum_{\sigma} \epsilon_{d\sigma}\hat n_{d\sigma} + U\hat n_{d\ua}\hat n_{d\da} + \epsilon_q\rr{1 - \hat n_{q\beta}}\\
					      &= \ham^D + \epsilon_q\rr{1 - \hat n_{q\beta}}\\
\implies \text{Tr}_{q\beta}\rr{\ham^D \hat n_{q\beta}} \hat n_{q\beta}&= \ham^D \hat n_{q\beta}
\eeq
where \il{\ham^D = \sum_{k\sigma} \epsilon_k \hat n_{k\sigma} + \sum_{\sigma} \epsilon_{d\sigma}\hat n_{d\sigma} + U\hat n_{d\ua}\hat n_{d\da}} is the diagonal part of the Hamiltonian.
\beq
\implies \eta_{q\beta} &= V_q^* c^\dagger_{d\beta} c_{q\beta} \fr{1}{\hat \omega - \ham^D \hat n_{q\beta}}\\
\implies \cc{c^\dagger_{q\beta}\text{Tr}_{q\beta}\rr{\ham c_{q\beta}},\eta_{q\beta}}&=|V_q|^2 \cc{c^\dagger_{q\beta}c_{d\beta}, c^\dagger_{d\beta}c_{q\beta}\fr{1}{\hat \omega - \ham^D \hat n_{q\beta}}}
\eeq
