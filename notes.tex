...
\begin{itemize}
	...
	\item In practise, PMS replaces \(E-V_0\) with the diagonal part of the initial state at the current step of the RG. There are several points to make here. Firstly, we are talking about the energy of the initial state, not the intermediate state. This is because, from eq.~\ref{problem}, \(E\) is the energy of the initial state on which \(V_\pm\) act. Secondly, the ideal solution would have been to substitute the exact energy, but since we do not know it, we use our current best guess (renormalised diagonal part). And thirdly, in doing so, we essentially ignore the \(V_0\) in the denominator, since it is not possible to compute off-diagonal terms.
\end{itemize}
\begin{figure}
	\centering
