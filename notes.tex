...
We are given a Hamiltonian \il{\ham} which is not completely diagonal in the occupation number basis of the electrons, \il{\hat n_k}: \il{\qq{\ham,n_k} \neq 0}. \(k\) labels any set of quantum numbers depending on the system. For spinless Fermions it can be the momentum of the particle, while for spin-full Fermions it can be the set of momentum and spin. There are terms that scatter electrons from one quantum number \(k\) to another quantum number \(k^\prime\).
...
&= \fr{1}{16} J_z^2 \sum_{kk^\prime q} c^\dagger_{k\ua}c_{q\ua}c^\dagger_{q\ua}c_{k^\prime\ua} \qq{\fr{\hf + S^z_d }{\omega_a - \epsilon_q - \fr{1}{4} J_z } + \fr{\hf - S^z_d}{\omega_b - \epsilon_q + \fr{1}{4} J_z}} \\
\eeq
The \(\omega_a\) and \(\omega_b\) refer to the two different \(\omega\) for the terms. To equate them, we need to subtract the corresponding initial energies from them. In this case the initial energies are 0, because both the terms in the diagonal Hamiltonian multiple with \(\hat n_{q\ua}\) and that is zero in the initial state.
\beq
\Delta^+_1 &= \fr{1}{32} J_z^2 \sum_{kk^\prime q} c^\dagger_{k\ua}c_{k^\prime\ua} \qq{\fr{1}{\omega - \epsilon_q - \fr{1}{4} J_z } + \fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z}} \\
...
\beq
\Delta^-_1 \ham &= \sum_{kk^\prime q} S_d^zc^\dagger_{q\ua}c_{k^\prime\ua}\fr{1}{4} J_z^2 \fr{1}{\omega_a - E_0 + E_0} S_d^z c^\dagger_{k\ua}c_{q\ua}\\
...
\beq
\Delta J = -J^2\sum_{q}\fr{1}{\omega - \epsilon_q + \fr{1}{4} J_z}
\eeq
...
\eeq
The superscript \(N-1\) implies it has been renormalized.
\comm{
