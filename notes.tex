...
\eeq
\il{\mathcal{N}(0) = \pd{n}{E}\bigg |_{\epsilon_F}} is the density of states at the Fermi energy and the \il{\fr{1}{2}} is because we are counting electrons of a particular spin only.
Therefore,
\beq
...
\eeq
Substituting \il{\avg{n_{d\sigma}} = \fr{1}{2}} and \il{\epsilon_d = -\fr{U}{2}} in the density of states eq.~\ref{dosint} gives \il{\rho_d(0) = \fr{1}{\pi \Delta} = \fr{1}{U}}.
...
\eeq
This is just \fr{1}{2} times identity matrix in two dimensions, so the eigenvalues are both \fr{1}{2}.
...
First put \il{E_0=0} and \il{s^z=\fr{1}{2}}.
...
\eeq
At the last step, \textbf{\textcolor{red}{I replaced \il{\pmb{\hat \omega - \sum_{k<\Lambda_N,\sigma}\epsilon_k \hat n_{k\sigma}\hat n_{q\beta} }} with \il{\pmb{\omega\hat n_{q\beta}}}}}.
...
Also, since it has a \il{c_{q\beta}}, we can set the \il{\hat n_{q\beta}} in the denominator to 1.
...
\eeq
\il{\xi_0} is the constant background energy \il{\fr{1}{2} \sum_{k\sigma}\epsilon_k + \fr{1}{2}\sum_\sigma \epsilon_{d\sigma} + \fr{1}{4}U} which we will drop henceforth.
\il{\xi_{d\sigma} =\epsilon_{d\sigma} + \fr{U}{2}} is a modified impurity-site energy.
\il{U} remains unchanged in the process.
The renormalized Hamiltonian (up to first order in \il{\tau}) after disentangling the electron at \il{q\beta} is
\beq
...
As a first approximation, we will consider just one electron on the outermost shell \il{\Lambda_N}.
\beq
...
\eeq
The sum \il{\sum^<} runs over all shells below \il{\Lambda_N}.
The first term gives
\beq
...
\eeq
The first trace, involving terms not having \il{q\beta}, simply give a factor of 2 which is canceled by the half in front.
Tracing over \il{\epsilon_q \tau_{q\beta}} gives zero because the eigenvalues of \il{\tau_{q\beta}} are \il{\pm \fr{1}{2}}.
Therefore,
\beq
...
\eeq
\textbf{\textcolor{red}{where I absorbed the part below the shell into the \il{\hat \omega}.
We can replace \il{\hat \omega} with \il{\fr{\omega}{2} \hat n_{q\beta}}}}:
\beq
...
\eeq
Since this term has a \il{c_{q\beta}}, it will survive only when acting on \il{\ket{\hat n_{q\beta} = 1}}, so we can replace the \il{\hat n_{q\beta}} in the denominator with \il{1}.
Also, since there is a \il{c^\dagger_{d\beta}}, we can similarly replace \il{\tau_{d\beta} = -\fr{1}{2}}.
...
\eeq
Since \il{A} and \il{B} are independent of the index \il{\beta}, we can change \il{\ol\beta} to \il{\beta} because they are being summed over.
Dropping the operator-free term,
\beq
...
\eeq
The first term is a renormalization in the onsite energy of the impurity \il{ \xi_d}.
The second term renormalizes the impurity-impurity repulsion energy \il{ U}.
\beq
...
\eeq
At the last step, \textbf{\textcolor{red}{I replaced \il{\pmb{\hat \omega - \sum_{k<\Lambda_N,\sigma}\epsilon_k \hat n_{k\sigma}\hat n_{q\beta} - \fr{1}{2}\rr{\epsilon_{d}\sum_\sigma \hat n_{d\sigma} + U \hat n_{d\ua}\hat n_{d\da} + \epsilon_q} }} with \il{\pmb{\omega\tau_{q\beta}}}}}.
Note that since this term has a \il{c^\dagger_{d\beta}}, it will not vanish only when acting on a state with \il{\hat n_{d\beta} = 0}.
Hence we can drop the terms \il{\hat n_{d\ua}\hat n_{d\da}} and \il{\epsilon_{d\beta}\hat n_{d\beta}} in the denominator.
...
\eeq
where \il{\ham_0} is the part that has no electron on the shell \il{\Lambda_N} and hence will be unchanged by the trace.
The terms will now be evaluated individually:
\beq
